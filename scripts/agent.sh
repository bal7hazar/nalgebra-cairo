...
#
# claude: `--model` is claude-opus-5-5 | sonnet (never Fable: owner rule 2026-09-23); codex: `<model>` may carry an effort suffix, e.g.
...
- Definition of done, in the FOREGROUND (never a background command followed by the end of your turn): crate-scoped checks only (`scarb fmt --check`, `python3 scripts/api_parity.py --check`, `python3 tools/shapegen/shapegen.py --check`, `scarb build -p` / `scarb lint -p <pkg> --deny-warnings` / unfiltered `snforge test -p <pkg> | python3 scripts/gas_report.py --update gas/` on the packages you touched); NEVER `./scripts/check.sh` nor any `--workspace` command locally: the pull-request CI is the full gate; conventional commits ending with the line `Co-Authored-By: Claude <noreply@anthropic.com>`; `git push -u origin <branch>`; `gh pr create` following .github/PULL_REQUEST_TEMPLATE.md; `gh pr checks --watch` until green (fix and push again if red); NEVER merge; write REPORT.md at the worktree root with: summary, API, gas table, deviations, deferred items, requested re-exports, escalations, PR URL. Never switch branches, never stash or reset, never touch other worktrees.'
