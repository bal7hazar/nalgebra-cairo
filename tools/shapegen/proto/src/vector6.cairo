...
//! other operations are methods of `Vector6Trait`, products between shapes are
//! `MatrixMul::mul_mat`.
...
/// Upstream alias `Matrix6x1`: the same type as `Vector6`.
pub type Matrix6x1<T> = Vector6<T>;

/// Methods of `Vector6<T>` for any `Real` scalar.
#[generate_trait]
pub impl Vector6Impl<
    ...
    +PartialOrd<T>,
> of Vector6Trait<T> {
    /// The 6-dimensional column vector with the given components. Upstream: `Vector6::new`.
    #[inline(always)]
    fn new(x: T, y: T, z: T, w: T, a: T, b: T) -> Vector6<T> {
        Vector6 { x, y, z, w, a, b }
    }

    /// The zero 6-dimensional column vector. Upstream: `zeros`.
    #[inline(always)]
    fn zeros() -> Vector6<T> {
        Vector6 { x: R::ZERO, y: R::ZERO, z: R::ZERO, w: R::ZERO, a: R::ZERO, b: R::ZERO }
    }

    /// The 6-dimensional column vector whose components all equal `elem`. Upstream: `from_element`.
    #[inline(always)]
    fn from_element(elem: T) -> Vector6<T> {
        Vector6 { x: elem, y: elem, z: elem, w: elem, a: elem, b: elem }
    }

    /// The 6-dimensional column vector whose components all equal `elem`. Alias of `from_element`.
    /// Upstream: `repeat`.
    #[inline(always)]
...
pub impl Vector6IndexLinear<T, +Copy<T>, +Drop<T>> of core::ops::IndexView<Vector6<T>, usize> {
    type Target = T;
    #[inline(always)]
...
/// bounds'` when `i >= 6` or `j >= 1`. Upstream: `Index<(usize, usize)>`.
///
/// A `match` on the column then on the row: the linear form (`i + j * 6` then one `match`, with a
/// row check) is dearer (`bench_*_index2__alt_linear`).
pub impl Vector6Index<T, +Copy<T>, +Drop<T>> of core::ops::IndexView<Vector6<T>, (usize, usize)> {
    type Target = T;
    #[inline(always)]
