...
FEATURES = {"statistics": "statistics", "blas": "blas", "dynamic": "dynamic"}
# WP 8.5-P13: `base/dynamic.cairo` is hand-written (its sub-module `dynamic/shapes.cairo` is
# generated by `dynamic.py`); its `mod` line and re-exports are generated here like the shared
# modules', behind the `dynamic` feature.
# WP 8.6-P18: plus the vector convolutions (`Convolution`, hand-written in
# `base/dynamic/convolution.cairo`).
SHARED_EXPORTS["dynamic"] = sorted(D.EXPORTS + ["Convolution"])
HAND_MODULES = {"dynamic"}


def cfg(module: str) -> str:
    return f"#[cfg(feature: '{FEATURES[module]}')]\n" if module in FEATURES else ""
...
def base_block() -> list[str]:
    ...
    mods = sorted(list(SHARED_MODULES) + list(HAND_MODULES) + [s.module for s in ALL_SHAPES])
    lines = [cfg(m) + (f"mod {m};" if m in PRIVATE_MODULES else f"pub mod {m};") for m in mods]
    uses = [(m, cfg(m) + (f"pub use {m}::{names[0]};" if len(names) == 1 else
                          f"pub use {m}::{{{', '.join(names)}}};"))
            for m, names in SHARED_EXPORTS.items()]
...
def lib_block() -> list[str]:
    """The re-exports of `lib.cairo`: one list for the ungated names, then one per feature."""
    gated = {n for m in FEATURES for n in SHARED_EXPORTS[m]}
    names = sorted([n for ns in SHARED_EXPORTS.values() for n in ns if n not in gated]
                   + [n for s in ALL_SHAPES for n in exported(s)])
    out = [f"pub use base::{{{', '.join(names)}}};"]
    for m in sorted(FEATURES, key=lambda m: SHARED_EXPORTS[m][0]):
        out.append(f"{cfg(m)}pub use base::{{{', '.join(sorted(SHARED_EXPORTS[m]))}}};")
    return out
...
def owned_modules() -> set[str]:
    return set(SHARED_MODULES) | HAND_MODULES | {s.module for s in ALL_SHAPES}
