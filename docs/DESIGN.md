...
- **Loops** are allowed (runtime sizes); inner loops walk contiguous runs with
  ...
  and 58 370 indexed); dot products of up to 6 terms are straight-line (6 870 vs 14 240 for 6).
  The product transposes the left factor once so that every output is ONE fused sum of products
  of two contiguous runs, accumulated in `Real::Wide` and floored once (bit-identical to the static
  kernels).
- **Partially dynamic aliases**: `MatrixXx1` IS `DVector` and `Matrix1xX` IS `RowDVector` (upstream
  ...
  `Matrix3xX::zeros(n)`).
- **Static shapes**: the conversions `Into<MatrixRxC, DMatrix>` (and `VectorN -> DVector`,
  `RowVectorN -> RowDVector`) and upstream's edition forms of the static shapes
  (`insert_columns`, `remove_rows_at`, `resize_vertically`, `from_vec`, `len`...) are generated by
  `tools/shapegen` into `base/dynamic/shapes.cairo`, gated with the rest.
