...
pub trait UnitComplexTrait<T> {
    ...
    /// floored division per component), so the result is unit within about `1 + 1/|c|` ulp
    /// whatever the drift. Panics with `simba: division by zero` on the zero pair. Upstream:
