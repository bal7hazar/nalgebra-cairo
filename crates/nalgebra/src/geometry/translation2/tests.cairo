...
fn t2(t: (i64, i64)) -> Translation2<Fixed> {
    Translation2 { vector: v2(t) }
}
...
fn test_new_from_vector_and_accessor() {
    ...
    assert!(t == t2((3 * ONE_RAW, -5 * ONE_RAW)));
    assert!(Translation2Trait::from_vector(v2((3 * ONE_RAW, -5 * ONE_RAW))) == t);
    assert!(t.vector() == v2((3 * ONE_RAW, -5 * ONE_RAW)));
...
fn test_inverse_negates_and_is_involutive() {
    let t = t2((0x123456789, -0x9876543));
    assert!(t.inverse() == t2((-0x123456789, 0x9876543)));
...
fn test_mul_is_the_sum_and_commutes_bit_for_bit() {
    let (a, b) = (t2((0x140000000, -0x60000000)), t2((-0x40000000, 0x280000000)));
    assert!(a * b == t2((0x100000000, 0x220000000)));
    assert!(a * b == b * a);
...
fn test_transform_point_is_the_exact_sum() {
    let t = t2((0x140000000, -0x60000000));
    assert!(t.transform_point(p2((0x40000000, 0x40000000))) == p2((0x180000000, -0x20000000)));
    assert!(id().transform_point(p2((7, -9))) == p2((7, -9)));
}
...
fn test_abs_diff_eq_counts_raw_units() {
    let t = t2((0x140000000, -0x60000000));
    assert!(t.abs_diff_eq(t2((0x140000003, -0x60000002)), 3));
    assert!(!t.abs_diff_eq(t2((0x140000004, -0x60000000)), 3));
    assert!(t.abs_diff_eq(t, 0));
}

#[test]
fn test_into_conversions_round_trip() {
    let v = v2((0x140000000, -0x60000000));
    let t: Translation2<Fixed> = v.into();
    assert!(t == t2((0x140000000, -0x60000000)));
...
fn test_inverse_oracle() {
    ...
    while let Some(case) = cases.pop_front() {
        ...
        assert!(tol == 0);
        assert!(t2(t).inverse() == t2(expected));
    }
}
...
fn test_transform_point_oracle() {
    ...
    while let Some(case) = cases.pop_front() {
        ...
        assert!(tol == 0);
        assert!(t2(t).transform_point(p2(p)) == p2(expected));
    }
}
...
fn test_inverse_transform_point_oracle() {
    ...
    while let Some(case) = cases.pop_front() {
        let (t, p, expected, tol) = *case;
        assert!(tol == 0);
        assert!(t2(t).inverse_transform_point(p2(p)) == p2(expected));
    }
}
...
fn test_to_homogeneous_oracle_consistency() {
    ...
    while let Some(case) = cases.pop_front() {
        ...
        let (ex, ey) = expected;
        let h = t2(t).to_homogeneous().mul_vec(p2(p).to_homogeneous());
