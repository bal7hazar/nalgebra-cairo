...
fn q(t: (i64, i64, i64, i64)) -> Quaternion<Fixed> {
    let (w, i, j, k) = t;
    Quaternion { i: fx(i), j: fx(j), k: fx(k), w: fx(w) }
}

/// Quaternion from integers, in the `(w, i, j, k)` order.
fn qi(w: i64, i: i64, j: i64, k: i64) -> Quaternion<Fixed> {
...
fn test_add_overflow_panics() {
    let big = q((0x7fffffffffffffff, 0, 0, 0));
    let _ = black_box(big) + black_box(q((1, 0, 0, 0)));
}

#[test]
#[should_panic(expected: 'simba: overflow')]
fn test_neg_min_panics() {
    let _ = -black_box(q((MIN, 0, 0, 0)));
}
...
fn test_mul_preserves_the_norm() {
    ...
    assert!(u * v == q((-1574821342, 1861152495, 1861152494, -3006477107)));
}
...
fn test_conjugate_oracle() {
    ...
    while let Some(case) = cases.pop_front() {
        ...
        assert!(tol == 0);
        assert!(q(rq).conjugate() == q(expected));
    }
}
...
fn test_norm_oracle() {
    ...
    while let Some(case) = cases.pop_front() {
        let (rq, expected, tol) = *case;
        assert!(Real::abs_diff_eq(q(rq).norm(), fx(expected), tol));
    }
}
...
fn test_norm_huge_does_not_overflow() {
    assert!(q((0x186a000000000, 0, 0, 0)).norm() == int(100000));
}

#[test]
#[should_panic(expected: 'simba: overflow')]
fn test_norm_squared_overflow_panics() {
    let _ = black_box(q((0x186a000000000, 0, 0, 0))).norm_squared();
}
...
fn test_dot_is_fused() {
    ...
    let tiny = q((46341, 46341, 46341, 0));
...
fn test_scale_and_unscale() {
    ...
    assert!(a().unscale(int(2)) == q((ONE_RAW / 2, ONE_RAW, -3 * ONE_RAW / 2, 2 * ONE_RAW)));
    // Floor division: -1 / 2 = -0.5 exactly, -1 / 3 rounds down.
    assert!(qi(-1, 0, 0, 0).unscale(int(3)) == q((-1431655766, 0, 0, 0)));
}
...
fn test_normalize_exact_and_oracle() {
    ...
    assert!(qi(0, 3, 4, 0).normalize() == q((0, 2576980377, 3435973836, 0)));
    assert!(qi(1, 1, 1, 1).normalize() == q((0x80000000, 0x80000000, 0x80000000, 0x80000000)));
    let mut cases = oracle::quaternion_normalize_cases();
    while let Some(case) = cases.pop_front() {
        let (rq, expected, tol) = *case;
        assert!(q(rq).normalize().abs_diff_eq(q(expected), tol));
        assert!(q(rq).normalize().norm().abs_diff_eq(Real::ONE, 4));
    }
}
...
fn test_try_inverse_product_is_identity() {
    ...
    while let Some(case) = cases.pop_front() {
        let (rq, expected, tol) = *case;
        let inv = q(rq).try_inverse().unwrap();
        assert!(inv.abs_diff_eq(q(expected), tol));
        // q · q⁻¹ = 1 within the rounding of the inverse times the magnitude of q.
        let one = q(rq) * inv;
        let scale: u64 = 4 + q(rq).norm().abs().raw.try_into().unwrap() / 0x100000000;
...
fn test_try_inverse_none_on_zero() {
    ...
    assert!(q((1, -1, 0, 0)).try_inverse() == None);
}
...
fn test_lerp_endpoints_are_exact() {
    ...
    assert!(a().lerp(b(), Real::HALF) == q((-0x80000000, 0x180000000, 0x100000000, 0x180000000)));
...
fn test_abs_diff_eq_counts_ulps() {
    ...
    let drifted = q((int(1).raw + 2, int(2).raw - 2, int(-3).raw + 1, int(4).raw));
