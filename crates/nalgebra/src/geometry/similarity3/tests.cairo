...
fn sim(t: ((i64, i64, i64), (i64, i64, i64, i64), i64)) -> Similarity3<Fixed> {
    ...
    Similarity3 {
        isometry: Isometry3 { rotation: uq(rot), translation: Translation3 { vector: v3(tr) } },
        scaling: fx(scaling),
    }
}
...
fn b() -> Similarity3<Fixed> {
    Similarity3AngleTrait::new(
        v3((-0xc0000000, 0x80000000, 0x140000000)),
        v3((-0x80000000, 0x60000000, 0xe0000000)),
        fx(0x180000000),
    )
}
...
fn test_identity_and_constructors_are_exact() {
    ...
    let r = UnitQuaternionAngleTrait::<
        Fixed,
    >::from_scaled_axis(v3((0x40000000, -0x30000000, 0x20000000)));
...
fn test_pure_scaling_and_half_turn_are_exact() {
    ...
    assert!(
        s
            ...
            ) == p3((3 * ONE_RAW, -6 * ONE_RAW, 9 * ONE_RAW)),
    );
    ...
    assert!(
        q
            .transform_point(
                p3((ONE_RAW, 2 * ONE_RAW, 3 * ONE_RAW)),
            ) == p3((-ONE_RAW, 4 * ONE_RAW, -6 * ONE_RAW)),
    );
}
...
fn test_inverse_composes_to_identity_and_undoes_points() {
    ...
    let v = v3((-0x280000000, 0x3c0000000, 0xc0000000));
...
fn test_scaling_append_and_prepend_match_upstream_order() {
    ...
    assert!(
        x
            .prepend_scaling(s)
            .transform_point(p3((ONE_RAW, 0, 0)))
            .abs_diff_eq(x.transform_point(p3((0x180000000, 0, 0))), 4),
    );
    let p = p3((ONE_RAW, 0, 0));
...
fn test_append_rotation_wrt_point_and_center() {
    let (x, r, p) = (a(), half_turn_y(), p3((0x180000000, -0x80000000, 0x40000000)));
...
fn test_transform_point_oracle() {
    ...
    while let Some(case) = cases.pop_front() {
        ...
        assert!(sim(x).transform_point(p3(p)).abs_diff_eq(p3(expected), tol));
    }
}
...
fn test_transform_vector_oracle() {
    ...
    while let Some(case) = cases.pop_front() {
        ...
        assert!(sim(x).transform_vector(v3(v)).abs_diff_eq(v3(expected), tol));
    }
}
...
fn test_inverse_transform_point_oracle() {
    ...
    while let Some(case) = cases.pop_front() {
        let (x, p, expected, tol) = *case;
        assert!(sim(x).inverse_transform_point(p3(p)).abs_diff_eq(p3(expected), tol));
    }
}
