...
/// A 3D transformation of category `TGeneral`: the homogeneous 4x4 matrix `matrix`
...
/// Operations of `Transform3<T>` over a `Real` scalar. By value, unrolled, no loop.
#[generate_trait]
pub impl Transform3Impl<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
...
> of Transform3Trait<T> {
    ...
    #[inline(always)]
    fn from_matrix_unchecked(matrix: Matrix4<T>) -> Transform3<T> {
        Transform3 { matrix }
    }

    /// The identity transform. Exact. Upstream: `Transform::identity`.
    #[inline(always)]
    fn identity() -> Transform3<T> {
        Transform3 { matrix: Matrix4Trait::identity() }
    }

    /// The homogeneous matrix. Exact. Upstream: `into_inner`.
    #[inline(always)]
    fn into_inner(self: Transform3<T>) -> Matrix4<T> {
        self.matrix
    }
    ...
    #[inline(always)]
    fn unwrap(self: Transform3<T>) -> Matrix4<T> {
        self.matrix
    }
    ...
    /// `matrix`.
    #[inline(always)]
    fn matrix(self: Transform3<T>) -> Matrix4<T> {
        self.matrix
    }

    /// The homogeneous matrix. Exact. Upstream: `to_homogeneous`.
    #[inline(always)]
    fn to_homogeneous(self: Transform3<T>) -> Matrix4<T> {
        self.matrix
    }
    ...
    #[inline(always)]
    fn try_inverse(self: Transform3<T>) -> Option<Transform3<T>> {
        match Matrix4Trait::try_inverse(self.matrix) {
            Option::Some(matrix) => Option::Some(Transform3 { matrix }),
    ...
    #[inline(always)]
    fn try_inverse_mut(ref self: Transform3<T>) -> bool {
    ...
    #[inline(always)]
    fn transform_point(self: Transform3<T>, pt: Point3<T>) -> Point3<T> {
    ...
    #[inline(always)]
    fn transform_vector(self: Transform3<T>, v: Vector3<T>) -> Vector3<T> {
        Matrix4CgTrait::transform_vector(self.matrix, v)
    }

    /// `self * r`: `m * [[r, 0], [0, 1]]`, the first 3 columns one fused sum of products per
    /// entry, the last one copied (bit for bit the homogeneous product); a `Transform3` (upstream's
    /// `TCategoryMul<TAffine>` keeps the category). Upstream: `Mul<Rotation> for Transform`.
    #[inline(always)]
    fn mul_rotation(self: Transform3<T>, r: Rotation3<T>) -> Transform3<T> {
        Transform3 { matrix: TransformKernels::mul_rotation3(self.matrix, r.matrix) }
    }

    /// `self / r = self * r⁻¹` (the inverse rotation is the exact transpose), rounded like
    /// `mul_rotation`. Upstream: `Div<Rotation> for Transform`.
    #[inline(always)]
    fn div_rotation(self: Transform3<T>, r: Rotation3<T>) -> Transform3<T> {
        Transform3 { matrix: TransformKernels::mul_rotation3(self.matrix, r.inverse().matrix) }
    }
    ...
    #[inline(always)]
    fn mul_unit_quaternion(self: Transform3<T>, q: UnitQuaternion<T>) -> Transform3<T> {
        Transform3 {
    ...
    #[inline(always)]
    fn div_unit_quaternion(self: Transform3<T>, q: UnitQuaternion<T>) -> Transform3<T> {
        Transform3 {
    ...
    #[inline(always)]
    fn mul_translation(self: Transform3<T>, tr: Translation3<T>) -> Transform3<T> {
        Transform3 { matrix: Matrix4CgTrait::prepend_translation(self.matrix, tr.vector) }
    }

    /// `self / tr = self * tr⁻¹` (the negated translation, exact), rounded like
    /// `mul_translation`. Upstream: `Div<Translation> for Transform`.
    #[inline(always)]
    fn div_translation(self: Transform3<T>, tr: Translation3<T>) -> Transform3<T> {
        Transform3 { matrix: Matrix4CgTrait::prepend_translation(self.matrix, tr.inverse().vector) }
    }
    ...
    #[inline(always)]
    fn mul_isometry(self: Transform3<T>, iso: Isometry3<T>) -> Transform3<T> {
        Transform3 { matrix: TransformKernels::mul_affine3(self.matrix, iso.to_homogeneous()) }
    }
    ...
    #[inline(always)]
    fn mul_similarity(self: Transform3<T>, sim: Similarity3<T>) -> Transform3<T> {
        Transform3 { matrix: TransformKernels::mul_affine3(self.matrix, sim.to_homogeneous()) }
    }

    /// `true` when every entry of the matrices is within `ulps` raw units. Upstream:
    /// `approx::AbsDiffEq::abs_diff_eq` (DESIGN D3).
    #[inline(always)]
    fn abs_diff_eq(self: Transform3<T>, other: Transform3<T>, ulps: u64) -> bool {
    ...
    #[inline(always)]
    fn relative_eq(
        self: Transform3<T>, other: Transform3<T>, epsilon: u64, max_relative: T,
    ) -> bool {
    ...
    #[inline(always)]
    fn ulps_eq(self: Transform3<T>, other: Transform3<T>, epsilon: u64, max_ulps: u32) -> bool {
...
/// a `Transform3` (upstream's `TCategoryMul<TGeneral> for TGeneral`). The other category pairs are
/// `TransformMul::mul_transform`. Upstream: `Mul<Transform> for Transform`.
pub impl Transform3Mul<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
...
> of Mul<Transform3<T>> {
    #[inline(always)]
    fn mul(lhs: Transform3<T>, rhs: Transform3<T>) -> Transform3<T> {
        Transform3 { matrix: lhs.matrix * rhs.matrix }
    }
}

/// `Default::default()`: the identity. Upstream: `Default for Transform`.
pub impl Transform3Default<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of Default<Transform3<T>> {
    #[inline(always)]
    fn default() -> Transform3<T> {
        Transform3Trait::identity()
    }
}

/// `One::one()`: the identity; `is_one` compares with it exactly. Upstream: `num::One for
/// Transform`.
pub impl Transform3One<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
...
> of One<Transform3<T>> {
    ...
    #[inline(always)]
    fn is_one(self: @Transform3<T>) -> bool {
        *self == Transform3Trait::identity()
    }

    #[inline(always)]
    fn is_non_one(self: @Transform3<T>) -> bool {
        !Self::is_one(self)
    }
}
...
/// for Transform`.
pub impl Transform3Index<T, +Copy<T>, +Drop<T>> of Index<Transform3<T>, (usize, usize)> {
    type Target = T;

    #[inline(always)]
    fn index(ref self: Transform3<T>, index: (usize, usize)) -> T {
        let m = self.matrix;
        m[index]
    }
}
...
/// Matrix4` (`OMatrix`).
pub impl Matrix4FromTransform3<T> of Into<Transform3<T>, Matrix4<T>> {
    #[inline(always)]
    fn into(self: Transform3<T>) -> Matrix4<T> {
        self.matrix
    }
}
...
/// `SubsetOf<Matrix4> for Transform3` (`nalgebra::try_convert`, `from_superset`).
pub impl Transform3TryFromMatrix4<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of TryInto<Matrix4<T>, Transform3<T>> {
    fn try_into(self: Matrix4<T>) -> Option<Transform3<T>> {
        let m = self;
        if true {
            Option::Some(Transform3 { matrix: m })
        } else {
            Option::None
        }
    }
...
/// `x.into()`: the same matrix as a `Transform3` (`TGeneral` is a super-category of `TProjective`,
/// no check). Exact. Upstream: `SubsetOf<Transform3> for Projective3` (`nalgebra::convert`; also
/// `set_category`).
pub impl Transform3FromProjective3<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of Into<Projective3<T>, Transform3<T>> {
    #[inline(always)]
    fn into(self: Projective3<T>) -> Transform3<T> {
        Transform3 { matrix: self.into_inner() }
    }
}

/// `x.into()`: the same matrix as a `Transform3` (`TGeneral` is a super-category of `TAffine`,
/// no check). Exact. Upstream: `SubsetOf<Transform3> for Affine3` (`nalgebra::convert`; also
/// `set_category`).
pub impl Transform3FromAffine3<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of Into<Affine3<T>, Transform3<T>> {
    #[inline(always)]
    fn into(self: Affine3<T>) -> Transform3<T> {
        Transform3 { matrix: self.into_inner() }
    }
}
...
pub impl Transform3FromRotation3<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of Into<Rotation3<T>, Transform3<T>> {
    #[inline(always)]
    fn into(self: Rotation3<T>) -> Transform3<T> {
        Transform3 { matrix: self.to_homogeneous() }
    }
}

/// `g.into()`: the transform of homogeneous matrix `g.to_homogeneous()` (its rounding). Upstream:
/// `SubsetOf<Transform> for UnitQuaternion` (`nalgebra::convert`, any category).
pub impl Transform3FromUnitQuaternion<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of Into<UnitQuaternion<T>, Transform3<T>> {
    #[inline(always)]
    fn into(self: UnitQuaternion<T>) -> Transform3<T> {
        Transform3 { matrix: self.to_homogeneous() }
    }
}
...
pub impl Transform3FromTranslation3<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of Into<Translation3<T>, Transform3<T>> {
    #[inline(always)]
    fn into(self: Translation3<T>) -> Transform3<T> {
        Transform3 { matrix: self.to_homogeneous() }
    }
}
...
pub impl Transform3FromIsometry3<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of Into<Isometry3<T>, Transform3<T>> {
    #[inline(always)]
    fn into(self: Isometry3<T>) -> Transform3<T> {
        Transform3 { matrix: self.to_homogeneous() }
    }
}
...
pub impl Transform3FromIsometryMatrix3<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of Into<IsometryMatrix3<T>, Transform3<T>> {
    #[inline(always)]
    fn into(self: IsometryMatrix3<T>) -> Transform3<T> {
        Transform3 { matrix: self.to_homogeneous() }
    }
}
...
pub impl Transform3FromSimilarity3<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of Into<Similarity3<T>, Transform3<T>> {
    #[inline(always)]
    fn into(self: Similarity3<T>) -> Transform3<T> {
        Transform3 { matrix: self.to_homogeneous() }
    }
}
...
pub impl Transform3FromSimilarityMatrix3<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of Into<SimilarityMatrix3<T>, Transform3<T>> {
    #[inline(always)]
    fn into(self: SimilarityMatrix3<T>) -> Transform3<T> {
        Transform3 { matrix: self.to_homogeneous() }
    }
}
...
pub impl Transform3FromScale3<
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of Into<Scale3<T>, Transform3<T>> {
    #[inline(always)]
    fn into(self: Scale3<T>) -> Transform3<T> {
        Transform3 { matrix: self.to_homogeneous() }
    }
}
...
pub impl Transform3FromUnitDualQuaternion<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of Into<UnitDualQuaternion<T>, Transform3<T>> {
    #[inline(always)]
    fn into(self: UnitDualQuaternion<T>) -> Transform3<T> {
        Transform3 { matrix: self.to_homogeneous() }
    }
}
