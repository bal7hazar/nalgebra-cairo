...
//! `reflect_rows_with_sign`: `L` is any of the six shapes with 4 columns and `W` the scratch
...
#[derive(Drop)]
pub struct Reflection4<T> {
    axis: Vector4<T>,
    bias: T,
}

impl Reflection4Copy<T, +Copy<T>> of Copy<Reflection4<T>>;

/// Constructors and accessors of `Reflection4<T>` over a `Real` scalar.
#[generate_trait]
pub impl Reflection4Impl<T, impl R: Real<T>, +Copy<T>, +Drop<T>> of Reflection4Trait<T> {
    ...
    #[inline(always)]
    fn new(axis: Unit<Vector4<T>>, bias: T) -> Reflection4<T> {
        Reflection4 { axis: axis.value, bias }
    }
    ...
    #[inline(always)]
    fn new_containing_point(axis: Unit<Vector4<T>>, pt: Point4<T>) -> Reflection4<T> {
        let a = axis.value;
        let w = R::wide_zero();
        ...
        Reflection4 { axis: a, bias: R::wide_rescale(w) }
    }

    /// The reflection axis. Upstream: `Reflection::axis`.
    #[inline(always)]
    fn axis(self: Reflection4<T>) -> Vector4<T> {
        self.axis
    }
    ...
    /// `Reflection::bias`.
    #[inline(always)]
    fn bias(self: Reflection4<T>) -> T {
        self.bias
    }
}

/// `reflect` / `reflect_with_sign` of `Reflection4` on a matrix `M` with 4 rows, updated in place.
pub trait Reflection4Columns<M, T> {
    ...
    fn reflect(self: Reflection4<T>, ref rhs: M);
    ...
    fn reflect_with_sign(self: Reflection4<T>, ref rhs: M, sign: T);
}

/// `Reflection4Columns` on `Vector4`.
pub impl Reflection4ColumnsVector4<
    T, impl R: Real<T>, +Copy<T>, +Drop<T>, +Add<T>, +Neg<T>,
> of Reflection4Columns<Vector4<T>, T> {
    fn reflect(self: Reflection4<T>, ref rhs: Vector4<T>) {
    ...
    fn reflect_with_sign(self: Reflection4<T>, ref rhs: Vector4<T>, sign: T) {
        let (a, b, m) = (self.axis, self.bias, rhs);
        let m_two = -(sign + sign);
        let w = R::wide_zero();
        let w = R::wide_add_prod(w, a.x, m.x);
...
/// `Reflection4Columns` on `Matrix4x2`.
pub impl Reflection4ColumnsMatrix4x2<
    T, impl R: Real<T>, +Copy<T>, +Drop<T>, +Add<T>, +Neg<T>,
> of Reflection4Columns<Matrix4x2<T>, T> {
    fn reflect(self: Reflection4<T>, ref rhs: Matrix4x2<T>) {
        let (a, b, m) = (self.axis, self.bias, rhs);
        let m_two = -R::TWO;
        let w = R::wide_zero();
    ...
    fn reflect_with_sign(self: Reflection4<T>, ref rhs: Matrix4x2<T>, sign: T) {
        let (a, b, m) = (self.axis, self.bias, rhs);
        let m_two = -(sign + sign);
        let w = R::wide_zero();
        let w = R::wide_add_prod(w, a.x, m.m11);
...
/// `Reflection4Columns` on `Matrix4x3`.
pub impl Reflection4ColumnsMatrix4x3<
    T, impl R: Real<T>, +Copy<T>, +Drop<T>, +Add<T>, +Neg<T>,
> of Reflection4Columns<Matrix4x3<T>, T> {
    fn reflect(self: Reflection4<T>, ref rhs: Matrix4x3<T>) {
        let (a, b, m) = (self.axis, self.bias, rhs);
        let m_two = -R::TWO;
        let w = R::wide_zero();
        let w = R::wide_add_prod(w, a.x, m.m11);
    ...
    fn reflect_with_sign(self: Reflection4<T>, ref rhs: Matrix4x3<T>, sign: T) {
        let (a, b, m) = (self.axis, self.bias, rhs);
        let m_two = -(sign + sign);
        let w = R::wide_zero();
        let w = R::wide_add_prod(w, a.x, m.m11);
...
/// `Reflection4Columns` on `Matrix4`.
pub impl Reflection4ColumnsMatrix4<
    T, impl R: Real<T>, +Copy<T>, +Drop<T>, +Add<T>, +Neg<T>,
> of Reflection4Columns<Matrix4<T>, T> {
    fn reflect(self: Reflection4<T>, ref rhs: Matrix4<T>) {
        let (a, b, m) = (self.axis, self.bias, rhs);
        let m_two = -R::TWO;
        let w = R::wide_zero();
        let w = R::wide_add_prod(w, a.x, m.m11);
    ...
    fn reflect_with_sign(self: Reflection4<T>, ref rhs: Matrix4<T>, sign: T) {
        let (a, b, m) = (self.axis, self.bias, rhs);
        let m_two = -(sign + sign);
        let w = R::wide_zero();
        let w = R::wide_add_prod(w, a.x, m.m11);
...
/// `Reflection4Columns` on `Matrix4x5`.
pub impl Reflection4ColumnsMatrix4x5<
    T, impl R: Real<T>, +Copy<T>, +Drop<T>, +Add<T>, +Neg<T>,
> of Reflection4Columns<Matrix4x5<T>, T> {
    fn reflect(self: Reflection4<T>, ref rhs: Matrix4x5<T>) {
        let (a, b, m) = (self.axis, self.bias, rhs);
        let m_two = -R::TWO;
        let w = R::wide_zero();
        let w = R::wide_add_prod(w, a.x, m.m11);
    ...
    fn reflect_with_sign(self: Reflection4<T>, ref rhs: Matrix4x5<T>, sign: T) {
        let (a, b, m) = (self.axis, self.bias, rhs);
        let m_two = -(sign + sign);
        let w = R::wide_zero();
        let w = R::wide_add_prod(w, a.x, m.m11);
...
/// `Reflection4Columns` on `Matrix4x6`.
pub impl Reflection4ColumnsMatrix4x6<
    T, impl R: Real<T>, +Copy<T>, +Drop<T>, +Add<T>, +Neg<T>,
> of Reflection4Columns<Matrix4x6<T>, T> {
    fn reflect(self: Reflection4<T>, ref rhs: Matrix4x6<T>) {
        let (a, b, m) = (self.axis, self.bias, rhs);
        let m_two = -R::TWO;
        let w = R::wide_zero();
        let w = R::wide_add_prod(w, a.x, m.m11);
    ...
    fn reflect_with_sign(self: Reflection4<T>, ref rhs: Matrix4x6<T>, sign: T) {
        let (a, b, m) = (self.axis, self.bias, rhs);
        let m_two = -(sign + sign);
        let w = R::wide_zero();
        let w = R::wide_add_prod(w, a.x, m.m11);
...
/// `reflect_rows` / `reflect_rows_with_sign` of `Reflection4` on a matrix `L` with 4 columns and
/// its scratch vector `W`, updated in place.
pub trait Reflection4Rows<L, W, T> {
    ...
    fn reflect_rows(self: Reflection4<T>, ref lhs: L, ref work: W);
    ...
    fn reflect_rows_with_sign(self: Reflection4<T>, ref lhs: L, ref work: W, sign: T);
}

/// `Reflection4Rows` on `RowVector4` with the scratch vector `Matrix1`.
pub impl Reflection4RowsRowVector4<
    T, impl R: Real<T>, +Copy<T>, +Drop<T>, +Add<T>, +Mul<T>, +Neg<T>,
> of Reflection4Rows<RowVector4<T>, Matrix1<T>, T> {
    fn reflect_rows(self: Reflection4<T>, ref lhs: RowVector4<T>, ref work: Matrix1<T>) {
    ...
    fn reflect_rows_with_sign(
        self: Reflection4<T>, ref lhs: RowVector4<T>, ref work: Matrix1<T>, sign: T,
    ) {
        let (a, b, m) = (self.axis, self.bias, lhs);
        let m_two = -(sign + sign);
        let w = R::wide_zero();
...
/// `Reflection4Rows` on `Matrix2x4` with the scratch vector `Vector2`.
pub impl Reflection4RowsMatrix2x4<
    T, impl R: Real<T>, +Copy<T>, +Drop<T>, +Add<T>, +Mul<T>, +Neg<T>,
> of Reflection4Rows<Matrix2x4<T>, Vector2<T>, T> {
    fn reflect_rows(self: Reflection4<T>, ref lhs: Matrix2x4<T>, ref work: Vector2<T>) {
        let (a, b, m) = (self.axis, self.bias, lhs);
        let w = R::wide_zero();
    ...
    fn reflect_rows_with_sign(
        self: Reflection4<T>, ref lhs: Matrix2x4<T>, ref work: Vector2<T>, sign: T,
    ) {
        let (a, b, m) = (self.axis, self.bias, lhs);
        let m_two = -(sign + sign);
        let w = R::wide_zero();
...
/// `Reflection4Rows` on `Matrix3x4` with the scratch vector `Vector3`.
pub impl Reflection4RowsMatrix3x4<
    T, impl R: Real<T>, +Copy<T>, +Drop<T>, +Add<T>, +Mul<T>, +Neg<T>,
> of Reflection4Rows<Matrix3x4<T>, Vector3<T>, T> {
    fn reflect_rows(self: Reflection4<T>, ref lhs: Matrix3x4<T>, ref work: Vector3<T>) {
        let (a, b, m) = (self.axis, self.bias, lhs);
        let w = R::wide_zero();
        let w = R::wide_add_prod(w, m.m11, a.x);
        let w = R::wide_add_prod(w, m.m12, a.y);
    ...
    fn reflect_rows_with_sign(
        self: Reflection4<T>, ref lhs: Matrix3x4<T>, ref work: Vector3<T>, sign: T,
    ) {
        let (a, b, m) = (self.axis, self.bias, lhs);
        let m_two = -(sign + sign);
        let w = R::wide_zero();
...
/// `Reflection4Rows` on `Matrix4` with the scratch vector `Vector4`.
pub impl Reflection4RowsMatrix4<
    T, impl R: Real<T>, +Copy<T>, +Drop<T>, +Add<T>, +Mul<T>, +Neg<T>,
> of Reflection4Rows<Matrix4<T>, Vector4<T>, T> {
    fn reflect_rows(self: Reflection4<T>, ref lhs: Matrix4<T>, ref work: Vector4<T>) {
        let (a, b, m) = (self.axis, self.bias, lhs);
        let w = R::wide_zero();
        let w = R::wide_add_prod(w, m.m11, a.x);
        let w = R::wide_add_prod(w, m.m12, a.y);
    ...
    fn reflect_rows_with_sign(
        self: Reflection4<T>, ref lhs: Matrix4<T>, ref work: Vector4<T>, sign: T,
    ) {
        let (a, b, m) = (self.axis, self.bias, lhs);
        let m_two = -(sign + sign);
        let w = R::wide_zero();
...
/// `Reflection4Rows` on `Matrix5x4` with the scratch vector `Vector5`.
pub impl Reflection4RowsMatrix5x4<
    T, impl R: Real<T>, +Copy<T>, +Drop<T>, +Add<T>, +Mul<T>, +Neg<T>,
> of Reflection4Rows<Matrix5x4<T>, Vector5<T>, T> {
    fn reflect_rows(self: Reflection4<T>, ref lhs: Matrix5x4<T>, ref work: Vector5<T>) {
        let (a, b, m) = (self.axis, self.bias, lhs);
        let w = R::wide_zero();
        let w = R::wide_add_prod(w, m.m11, a.x);
        let w = R::wide_add_prod(w, m.m12, a.y);
    ...
    fn reflect_rows_with_sign(
        self: Reflection4<T>, ref lhs: Matrix5x4<T>, ref work: Vector5<T>, sign: T,
    ) {
        let (a, b, m) = (self.axis, self.bias, lhs);
        let m_two = -(sign + sign);
        let w = R::wide_zero();
