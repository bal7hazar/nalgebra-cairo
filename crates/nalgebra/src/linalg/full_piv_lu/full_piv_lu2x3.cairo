...
//! `FullPivLu2x3`: the LU factorisation with full pivoting of a `Matrix2x3` (upstream
...
/// the compact sequences `Perm2` / `Perm3` — the permutations of the rows / columns of the
...
/// `(i, i)`).
///
/// Non-square: `m = min(2, 3) = 2` elimination steps; `L` is 2x2, `U` 2x3. Built by
/// `FullPivLu2x3Trait::new` or `Matrix2x3FullPivLuTrait::full_piv_lu`.
/// Upstream: `nalgebra::linalg::FullPivLU`.
#[derive(Copy, Drop, Serde, Debug)]
pub struct FullPivLu2x3<T> {
...
/// Methods of `FullPivLu2x3<T>` for any `Real` scalar.
#[generate_trait]
pub impl FullPivLu2x3Impl<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
...
> of FullPivLu2x3Trait<T> {
    ...
    fn new(matrix: Matrix2x3<T>) -> FullPivLu2x3<T> {
        revoke_ap_tracking();
        ...
        // step 1: the first largest |a_ij| of rows 1..2, columns 1..3 (column-major scan)
        ...
        // step 2: the first largest |a_ij| of rows 2..2, columns 2..3 (column-major scan)
    ...
    #[inline(always)]
    fn lu_internal(self: FullPivLu2x3<T>) -> Matrix2x3<T> {
        self.lu
    }

    /// The unit lower triangular factor `L` (2x2). Exact: moves. Upstream: `FullPivLU::l`.
    #[inline(always)]
    fn l(self: FullPivLu2x3<T>) -> Matrix2<T> {
        Matrix2 { m11: R::one(), m21: self.lu.m21, m12: R::zero(), m22: R::one() }
    }

    /// The upper triangular factor `U` (2x3). Exact: moves. Upstream: `FullPivLU::u`.
    #[inline(always)]
    fn u(self: FullPivLu2x3<T>) -> Matrix2x3<T> {
        Matrix2x3 {
            m11: self.lu.m11,
            m21: R::zero(),
            m12: self.lu.m12,
    ...
    #[inline(always)]
    fn p(self: FullPivLu2x3<T>) -> Perm2 {
        self.p
    }

    /// The column permutation `Q`. Upstream: `FullPivLU::q`.
    #[inline(always)]
    fn q(self: FullPivLu2x3<T>) -> Perm3 {
        self.q
    }

    /// `(P, L, U, Q)`: `(p(), l(), u(), q())`, exact. Upstream: `FullPivLU::unpack`.
    #[inline(always)]
    fn unpack(self: FullPivLu2x3<T>) -> (Perm2, Matrix2<T>, Matrix2x3<T>, Perm3) {
        (self.p, Self::l(self), Self::u(self), self.q)
    }
}

/// `Matrix2x3` methods that go through the full-pivot LU factorisation. Import
/// `Matrix2x3FullPivLuTrait`.
#[generate_trait]
pub impl Matrix2x3FullPivLuImpl<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of Matrix2x3FullPivLuTrait<T> {
    /// The LU factorisation with full pivoting. Upstream: `Matrix::full_piv_lu`.
    #[inline(always)]
    fn full_piv_lu(self: Matrix2x3<T>) -> FullPivLu2x3<T> {
        FullPivLu2x3Trait::new(self)
    }
}
