...
//! `FullPivLu6x5`: the LU factorisation with full pivoting of a `Matrix6x5` (upstream
//! `nalgebra::linalg::FullPivLU<T, U6, U5>`), fully unrolled (WP 8.5-P15).
...
/// The LU factorisation with full (row and column) pivoting of a `Matrix6x5<T>`: `P A Q = L U`.
///
...
/// the compact sequences `Perm6` / `Perm5` — the permutations of the rows / columns of the
...
/// `(i, i)`).
///
/// Non-square: `m = min(6, 5) = 5` elimination steps; `L` is 6x5, `U` 5x5. Built by
/// `FullPivLu6x5Trait::new` or `Matrix6x5FullPivLuTrait::full_piv_lu`.
/// Upstream: `nalgebra::linalg::FullPivLU`.
#[derive(Copy, Drop, Serde, Debug)]
pub struct FullPivLu6x5<T> {
...
/// Methods of `FullPivLu6x5<T>` for any `Real` scalar.
#[generate_trait]
pub impl FullPivLu6x5Impl<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
...
> of FullPivLu6x5Trait<T> {
    ...
    fn new(matrix: Matrix6x5<T>) -> FullPivLu6x5<T> {
        revoke_ap_tracking();
        ...
        // step 1: the first largest |a_ij| of rows 1..6, columns 1..5 (column-major scan)
        ...
        // step 2: the first largest |a_ij| of rows 2..6, columns 2..5 (column-major scan)
        ...
        // step 3: the first largest |a_ij| of rows 3..6, columns 3..5 (column-major scan)
        ...
        // step 4: the first largest |a_ij| of rows 4..6, columns 4..5 (column-major scan)
        ...
        // step 5: the first largest |a_ij| of rows 5..6, columns 5..5 (column-major scan)
    ...
    #[inline(always)]
    fn lu_internal(self: FullPivLu6x5<T>) -> Matrix6x5<T> {
        self.lu
    }

    /// The unit lower triangular factor `L` (6x5). Exact: moves. Upstream: `FullPivLU::l`.
    #[inline(always)]
    fn l(self: FullPivLu6x5<T>) -> Matrix6x5<T> {
        Matrix6x5 {
            m11: R::one(),
            m21: self.lu.m21,
            m31: self.lu.m31,
    ...
    #[inline(always)]
    fn u(self: FullPivLu6x5<T>) -> Matrix5<T> {
        Matrix5 {
            m11: self.lu.m11,
            m21: R::zero(),
            m31: R::zero(),
    ...
    #[inline(always)]
    fn p(self: FullPivLu6x5<T>) -> Perm6 {
        self.p
    }

    /// The column permutation `Q`. Upstream: `FullPivLU::q`.
    #[inline(always)]
    fn q(self: FullPivLu6x5<T>) -> Perm5 {
        self.q
    }

    /// `(P, L, U, Q)`: `(p(), l(), u(), q())`, exact. Upstream: `FullPivLU::unpack`.
    #[inline(always)]
    fn unpack(self: FullPivLu6x5<T>) -> (Perm6, Matrix6x5<T>, Matrix5<T>, Perm5) {
        (self.p, Self::l(self), Self::u(self), self.q)
    }
}

/// `Matrix6x5` methods that go through the full-pivot LU factorisation. Import
/// `Matrix6x5FullPivLuTrait`.
#[generate_trait]
pub impl Matrix6x5FullPivLuImpl<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of Matrix6x5FullPivLuTrait<T> {
    /// The LU factorisation with full pivoting. Upstream: `Matrix::full_piv_lu`.
    #[inline(always)]
    fn full_piv_lu(self: Matrix6x5<T>) -> FullPivLu6x5<T> {
        FullPivLu6x5Trait::new(self)
    }
}
