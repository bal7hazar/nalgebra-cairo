...
/// `Svd5x3InternalTrait`).
#[derive(Copy, Drop)]
pub(crate) struct SortedSvd5x3<T> {
    s0: T,
    s1: T,
    s2: T,
    w0: Vector5<T>,
...
/// Crate-internal kernels of `Svd5x3<T>`: the Gram matrix, the right singular vectors, the sorted
...
> of Svd5x3InternalTrait<T> {
    ...
    fn normalised(m: Matrix5x3<T>) -> Matrix5x3<T> {
        let mut a = m.m11.abs();
        if m.m21.abs() > a {
            a = m.m21.abs();
        }
        ...
        let (q12, q13, q14) = R::div3(m.m33, m.m43, m.m53, a);
        Matrix5x3 {
            m11: q0,
            m21: q1,
            m31: q2,
    ...
    /// `MᵀM` (6 fused sums of 5 products, one rounding each).
    #[inline(always)]
    ...
    fn sorted(m: Matrix5x3<T>, v: Matrix3<T>) -> SortedSvd5x3<T> {
        revoke_ap_tracking();
        ...
        SortedSvd5x3 { s0, s1, s2, w0, w1, w2, v0, v1, v2 }
    }
