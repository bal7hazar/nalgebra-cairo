...
use super::kernels::{SvdComplete2Impl, SvdRightImpl};
...
> of Svd2x1Trait<T> {
    ...
    fn recompose(self: Svd2x1<T>) -> Vector2<T> {
        revoke_ap_tracking();
        Vector2 { x: self.u.x * self.singular_values.x, y: self.u.y * self.singular_values.x }
            .mul_mat(self.v_t)
    }
    ...
    fn pseudo_inverse(self: Svd2x1<T>, eps: T) -> Option<RowVector2<T>> {
        ...
        Some(Matrix1 { x: self.v_t.x * p0 }.mul_mat(RowVector2 { x: self.u.x, y: self.u.y }))
    }
...
/// `Svd2x1InternalTrait`).
#[derive(Copy, Drop)]
pub(crate) struct SortedSvd2x1<T> {
    s0: T,
    w0: Vector2<T>,
    v0: Matrix1<T>,
}

/// Crate-internal kernels of `Svd2x1<T>`: the Gram matrix, the right singular vectors, the sorted
...
> of Svd2x1InternalTrait<T> {
    ...
    fn sorted(m: Vector2<T>, v: Matrix1<T>) -> SortedSvd2x1<T> {
        revoke_ap_tracking();
        let v0 = Matrix1 { x: v.x };
        let mv = m.mul_mat(v);
        let w0 = Vector2 { x: mv.x, y: mv.y };
        let s0 = R::norm2(w0.x, w0.y);
        let (mut s0, mut w0, mut v0) = (s0, w0, v0);

        SortedSvd2x1 { s0, w0, v0 }
    }

    /// The decomposition from the right singular vectors `v` (columns): `sorted`, then the left
    /// vectors by classical Gram-Schmidt run TWICE ("twice is enough", `SvdComplete2::gs*`):
    ...
    fn from_right(m: Vector2<T>, v: Matrix1<T>) -> Svd2x1<T> {
        ...
        let u0 = SvdComplete2Impl::<T>::first(w0, s0);
        Svd2x1 {
