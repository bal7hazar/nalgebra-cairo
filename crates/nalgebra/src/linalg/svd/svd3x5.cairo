...
//! `Svd3x5`: the singular value decomposition `M = U · Σ · Vᵀ` of a `Matrix3x5` (upstream
//! `nalgebra::linalg::SVD<T, U3, U5>`), its pseudo-inverse, least-squares solve, rank and polar
...
/// The singular value decomposition `M = u · diag(singular_values) · v_t` of a `Matrix3x5<T>`:
/// `u` is 3x3 with orthonormal columns, `v_t` is 3x5 with orthonormal rows, the 3
...
> of Svd3x5Trait<T> {
    ...
    /// TRANSPOSE (`Svd5x3`): `Mᵀ = U' Σ V'ᵀ` gives `M = V' Σ U'ᵀ`, so `u = V'` and `v_t =
    /// U'ᵀ`
    /// (moves only). The Gram matrix is then `M Mᵀ` (3x3), the smaller one. Same algorithm,
    /// accuracy and conventions as `Svd5x3Trait::new`. Upstream: `SVD::new(matrix, true, true)`
    /// (both factors always computed).
    fn new(matrix: Matrix3x5<T>) -> Svd3x5<T> {
        let t = Svd5x3Trait::new(
            Matrix5x3 {
                m11: matrix.m11,
                m21: matrix.m12,
                m31: matrix.m13,
    ...
    /// see `Svd5x3Trait::try_new`. `max_niter` is ignored (constant budget). Upstream:
    /// `SVD::try_new(matrix, true, true, eps, max_niter)`.
    fn try_new(matrix: Matrix3x5<T>, eps: T, max_niter: usize) -> Option<Svd3x5<T>> {
        match Svd5x3Trait::try_new(
            Matrix5x3 {
                m11: matrix.m11,
                m21: matrix.m12,
                m31: matrix.m13,
    ...
    #[inline(always)]
    fn new_unordered(matrix: Matrix3x5<T>) -> Svd3x5<T> {
        Self::new(matrix)
    }

    /// `try_new`, see `new_unordered`. Upstream: `SVD::try_new_unordered`.
    #[inline(always)]
    fn try_new_unordered(matrix: Matrix3x5<T>, eps: T, max_niter: usize) -> Option<Svd3x5<T>> {
    ...
    fn rank(self: Svd3x5<T>, eps: T) -> usize {
        let mut n: usize = 0;
    ...
    /// here).
    fn recompose(self: Svd3x5<T>) -> Matrix3x5<T> {
        revoke_ap_tracking();
        Matrix3 {
    ...
    /// The Moore-Penrose pseudo-inverse `V · diag(σ⁺) · Uᵀ` (5x3), `σ⁺_i = 1 / σ_i` when
    ...
    fn pseudo_inverse(self: Svd3x5<T>, eps: T) -> Option<Matrix5x3<T>> {
        revoke_ap_tracking();
    ...
    fn solve(self: Svd3x5<T>, b: Vector3<T>, eps: T) -> Option<Vector5<T>> {
        revoke_ap_tracking();
        if eps.is_sign_negative() {
            return None;
        }
    ...
    /// and `U = u · v_t` (3x5). Two roundings per entry of `P`, one per entry of `U`. Always
    /// `Some` (upstream returns `None` only when `u` or `v_t` was not computed). Panics on
    /// overflow. Upstream: `SVD::to_polar`.
    fn to_polar(self: Svd3x5<T>) -> Option<(Matrix3<T>, Matrix3x5<T>)> {
        revoke_ap_tracking();
    ...
    fn sort_by_singular_values(ref self: Svd3x5<T>) {
        revoke_ap_tracking();
...
/// Upstream's `Matrix` methods that go through the SVD, on `Matrix3x5`. Import `Matrix3x5SvdTrait`
/// to use them.
#[generate_trait]
pub impl Matrix3x5SvdImpl<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of Matrix3x5SvdTrait<T> {
    /// The singular value decomposition, see `Svd3x5Trait::new`. Upstream: `Matrix::svd(true,
    /// true)`.
    #[inline(always)]
    fn svd(self: Matrix3x5<T>) -> Svd3x5<T> {
        Svd3x5Trait::new(self)
    }

    /// `svd` (already sorted, see `Svd3x5Trait::new_unordered`). Upstream: `Matrix::svd_unordered`.
    #[inline(always)]
    fn svd_unordered(self: Matrix3x5<T>) -> Svd3x5<T> {
        Svd3x5Trait::new(self)
    }

    /// See `Svd3x5Trait::try_new`. Upstream: `Matrix::try_svd(true, true, eps, max_niter)`.
    #[inline(always)]
    fn try_svd(self: Matrix3x5<T>, eps: T, max_niter: usize) -> Option<Svd3x5<T>> {
        Svd3x5Trait::try_new(self, eps, max_niter)
    }

    /// See `Svd3x5Trait::try_new_unordered`. Upstream: `Matrix::try_svd_unordered`.
    #[inline(always)]
    fn try_svd_unordered(self: Matrix3x5<T>, eps: T, max_niter: usize) -> Option<Svd3x5<T>> {
        Svd3x5Trait::try_new(self, eps, max_niter)
    }
    ...
    fn singular_values(self: Matrix3x5<T>) -> Vector3<T> {
        {
            let t = Matrix5x3 {
                m11: self.m11,
                m21: self.m12,
            ...
            Svd5x3InternalTrait::values_from_right(t, Svd5x3InternalTrait::right(t))
        }
    }

    /// `singular_values` (sorted; see `Svd3x5Trait::new_unordered`). Upstream:
    /// `Matrix::singular_values_unordered`.
    #[inline(always)]
    fn singular_values_unordered(self: Matrix3x5<T>) -> Vector3<T> {
    ...
    /// here).
    fn rank(self: Matrix3x5<T>, eps: T) -> usize {
    ...
    /// The Moore-Penrose pseudo-inverse, see `Svd3x5Trait::pseudo_inverse`. Upstream:
    /// `Matrix::pseudo_inverse` (`Err` on a negative `eps`; `None` here).
    #[inline(always)]
    fn pseudo_inverse(self: Matrix3x5<T>, eps: T) -> Option<Matrix5x3<T>> {
        Svd3x5Trait::new(self).pseudo_inverse(eps)
    }

    /// The left polar decomposition `M = P · U`, see `Svd3x5Trait::to_polar`. Upstream:
    /// `Matrix::polar`.
    fn polar(self: Matrix3x5<T>) -> (Matrix3<T>, Matrix3x5<T>) {
        Svd3x5Trait::new(self).to_polar().unwrap()
    }

    /// `polar`, or `None` when the decomposition did not converge within `eps`, see
    /// `Svd3x5Trait::try_new`. Upstream: `Matrix::try_polar`.
    fn try_polar(
        self: Matrix3x5<T>, eps: T, max_niter: usize,
    ) -> Option<(Matrix3<T>, Matrix3x5<T>)> {
        match Svd3x5Trait::try_new(self, eps, max_niter) {
            Some(d) => d.to_polar(),
            None => None,
        }
    }
