...
/// `Svd4x3InternalTrait`).
#[derive(Copy, Drop)]
pub(crate) struct SortedSvd4x3<T> {
    s0: T,
    s1: T,
    s2: T,
    w0: Vector4<T>,
...
/// Crate-internal kernels of `Svd4x3<T>`: the Gram matrix, the right singular vectors, the sorted
...
> of Svd4x3InternalTrait<T> {
    ...
    fn normalised(m: Matrix4x3<T>) -> Matrix4x3<T> {
        let mut a = m.m11.abs();
        if m.m21.abs() > a {
            a = m.m21.abs();
        }
    ...
    /// `MᵀM` (6 fused sums of 4 products, one rounding each).
    #[inline(always)]
    ...
    fn sorted(m: Matrix4x3<T>, v: Matrix3<T>) -> SortedSvd4x3<T> {
        revoke_ap_tracking();
