...
//! `Svd2x5`: the singular value decomposition `M = U · Σ · Vᵀ` of a `Matrix2x5` (upstream
//! `nalgebra::linalg::SVD<T, U2, U5>`), its pseudo-inverse, least-squares solve, rank and polar
...
/// `u` is 2x2 with orthonormal columns, `v_t` is 2x5 with orthonormal rows, the 2
...
> of Svd2x5Trait<T> {
    ...
    /// TRANSPOSE (`Svd5x2`): `Mᵀ = U' Σ V'ᵀ` gives `M = V' Σ U'ᵀ`, so `u = V'` and `v_t =
    /// U'ᵀ`
    /// (moves only). The Gram matrix is then `M Mᵀ` (2x2), the smaller one. Same algorithm,
    /// accuracy and conventions as `Svd5x2Trait::new`. Upstream: `SVD::new(matrix, true, true)`
    /// (both factors always computed).
    fn new(matrix: Matrix2x5<T>) -> Svd2x5<T> {
        let t = Svd5x2Trait::new(
            Matrix5x2 {
                m11: matrix.m11,
                m21: matrix.m12,
                m31: matrix.m13,
    ...
    /// see `Svd5x2Trait::try_new`. `max_niter` is ignored (constant budget). Upstream:
    /// `SVD::try_new(matrix, true, true, eps, max_niter)`.
    fn try_new(matrix: Matrix2x5<T>, eps: T, max_niter: usize) -> Option<Svd2x5<T>> {
        match Svd5x2Trait::try_new(
            Matrix5x2 {
                m11: matrix.m11,
                m21: matrix.m12,
                m31: matrix.m13,
    ...
    #[inline(always)]
    fn new_unordered(matrix: Matrix2x5<T>) -> Svd2x5<T> {
        Self::new(matrix)
    }

    /// `try_new`, see `new_unordered`. Upstream: `SVD::try_new_unordered`.
    #[inline(always)]
    fn try_new_unordered(matrix: Matrix2x5<T>, eps: T, max_niter: usize) -> Option<Svd2x5<T>> {
    ...
    fn rank(self: Svd2x5<T>, eps: T) -> usize {
        let mut n: usize = 0;
    ...
    fn recompose(self: Svd2x5<T>) -> Matrix2x5<T> {
    ...
    /// The Moore-Penrose pseudo-inverse `V · diag(σ⁺) · Uᵀ` (5x2), `σ⁺_i = 1 / σ_i` when
    ...
    fn pseudo_inverse(self: Svd2x5<T>, eps: T) -> Option<Matrix5x2<T>> {
    ...
    fn solve(self: Svd2x5<T>, b: Vector2<T>, eps: T) -> Option<Vector5<T>> {
        if eps.is_sign_negative() {
            return None;
        }
    ...
    /// and `U = u · v_t` (2x5). Two roundings per entry of `P`, one per entry of `U`. Always
    /// `Some` (upstream returns `None` only when `u` or `v_t` was not computed). Panics on
    /// overflow. Upstream: `SVD::to_polar`.
    fn to_polar(self: Svd2x5<T>) -> Option<(Matrix2<T>, Matrix2x5<T>)> {
        let a0_0 = self.u.m11 * self.singular_values.x;
        let a0_1 = self.u.m12 * self.singular_values.y;
        let a1_0 = self.u.m21 * self.singular_values.x;
        let a1_1 = self.u.m22 * self.singular_values.y;
    ...
    fn sort_by_singular_values(ref self: Svd2x5<T>) {
...
/// Upstream's `Matrix` methods that go through the SVD, on `Matrix2x5`. Import `Matrix2x5SvdTrait`
/// to use them.
#[generate_trait]
pub impl Matrix2x5SvdImpl<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of Matrix2x5SvdTrait<T> {
    /// The singular value decomposition, see `Svd2x5Trait::new`. Upstream: `Matrix::svd(true,
    /// true)`.
    #[inline(always)]
    fn svd(self: Matrix2x5<T>) -> Svd2x5<T> {
        Svd2x5Trait::new(self)
    }

    /// `svd` (already sorted, see `Svd2x5Trait::new_unordered`). Upstream: `Matrix::svd_unordered`.
    #[inline(always)]
    fn svd_unordered(self: Matrix2x5<T>) -> Svd2x5<T> {
        Svd2x5Trait::new(self)
    }

    /// See `Svd2x5Trait::try_new`. Upstream: `Matrix::try_svd(true, true, eps, max_niter)`.
    #[inline(always)]
    fn try_svd(self: Matrix2x5<T>, eps: T, max_niter: usize) -> Option<Svd2x5<T>> {
        Svd2x5Trait::try_new(self, eps, max_niter)
    }

    /// See `Svd2x5Trait::try_new_unordered`. Upstream: `Matrix::try_svd_unordered`.
    #[inline(always)]
    fn try_svd_unordered(self: Matrix2x5<T>, eps: T, max_niter: usize) -> Option<Svd2x5<T>> {
        Svd2x5Trait::try_new(self, eps, max_niter)
    }
    ...
    fn singular_values(self: Matrix2x5<T>) -> Vector2<T> {
        {
            let t = Matrix5x2 {
                m11: self.m11,
                m21: self.m12,
            ...
            Svd5x2InternalTrait::values_from_right(t, Svd5x2InternalTrait::right(t))
        }
    }

    /// `singular_values` (sorted; see `Svd2x5Trait::new_unordered`). Upstream:
    /// `Matrix::singular_values_unordered`.
    #[inline(always)]
    fn singular_values_unordered(self: Matrix2x5<T>) -> Vector2<T> {
    ...
    /// here).
    fn rank(self: Matrix2x5<T>, eps: T) -> usize {
    ...
    /// The Moore-Penrose pseudo-inverse, see `Svd2x5Trait::pseudo_inverse`. Upstream:
    /// `Matrix::pseudo_inverse` (`Err` on a negative `eps`; `None` here).
    #[inline(always)]
    fn pseudo_inverse(self: Matrix2x5<T>, eps: T) -> Option<Matrix5x2<T>> {
        Svd2x5Trait::new(self).pseudo_inverse(eps)
    }

    /// The left polar decomposition `M = P · U`, see `Svd2x5Trait::to_polar`. Upstream:
    /// `Matrix::polar`.
    fn polar(self: Matrix2x5<T>) -> (Matrix2<T>, Matrix2x5<T>) {
        Svd2x5Trait::new(self).to_polar().unwrap()
    }

    /// `polar`, or `None` when the decomposition did not converge within `eps`, see
    /// `Svd2x5Trait::try_new`. Upstream: `Matrix::try_polar`.
    fn try_polar(
        self: Matrix2x5<T>, eps: T, max_niter: usize,
    ) -> Option<(Matrix2<T>, Matrix2x5<T>)> {
        match Svd2x5Trait::try_new(self, eps, max_niter) {
            Some(d) => d.to_polar(),
            None => None,
        }
    }
