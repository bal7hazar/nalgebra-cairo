...
//! `Svd1x4`: the singular value decomposition `M = U · Σ · Vᵀ` of a `RowVector4` (upstream
...
/// `u` is 1x1 with orthonormal columns, `v_t` is 1x4 with orthonormal rows, the 1
...
pub struct Svd1x4<T> {
    ...
    /// The TRANSPOSE of the right singular vectors (1x4): `v_i` is row `i`.
    pub v_t: RowVector4<T>,
}

/// Test-only field-wise equality (upstream `SVD` has no `PartialEq`).
#[cfg(test)]
impl Svd1x4PartialEq<T, +PartialEq<T>> of PartialEq<Svd1x4<T>> {
    fn eq(lhs: @Svd1x4<T>, rhs: @Svd1x4<T>) -> bool {
        lhs.u == rhs.u && lhs.singular_values == rhs.singular_values && lhs.v_t == rhs.v_t
    }
}

/// Methods of `Svd1x4<T>` for any `Real` scalar.
#[generate_trait]
pub impl Svd1x4Impl<
    T,
    impl R: Real<T>,
...
> of Svd1x4Trait<T> {
    ...
    /// TRANSPOSE (`Svd4x1`): `Mᵀ = U' Σ V'ᵀ` gives `M = V' Σ U'ᵀ`, so `u = V'` and `v_t =
    /// U'ᵀ`
    /// (moves only). The Gram matrix is then `M Mᵀ` (1x1), the smaller one. Same algorithm,
    /// accuracy and conventions as `Svd4x1Trait::new`. Upstream: `SVD::new(matrix, true, true)`
    /// (both factors always computed).
    fn new(matrix: RowVector4<T>) -> Svd1x4<T> {
        let t = Svd4x1Trait::new(Vector4 { x: matrix.x, y: matrix.y, z: matrix.z, w: matrix.w });
        Svd1x4 {
            u: Matrix1 { x: t.v_t.x },
            singular_values: t.singular_values,
            v_t: RowVector4 { x: t.u.x, y: t.u.y, z: t.u.z, w: t.u.w },
        }
    }

    /// `new`, or `None` when the eigen decomposition of `M Mᵀ` did not converge within `eps`,
    /// see `Svd4x1Trait::try_new`. `max_niter` is ignored (constant budget). Upstream:
    /// `SVD::try_new(matrix, true, true, eps, max_niter)`.
    fn try_new(matrix: RowVector4<T>, eps: T, max_niter: usize) -> Option<Svd1x4<T>> {
        match Svd4x1Trait::try_new(
            Vector4 { x: matrix.x, y: matrix.y, z: matrix.z, w: matrix.w }, eps, max_niter,
        ) {
            Some(t) => Some(
                Svd1x4 {
                    u: Matrix1 { x: t.v_t.x },
    ...
    #[inline(always)]
    fn new_unordered(matrix: RowVector4<T>) -> Svd1x4<T> {
        Self::new(matrix)
    }

    /// `try_new`, see `new_unordered`. Upstream: `SVD::try_new_unordered`.
    #[inline(always)]
    fn try_new_unordered(matrix: RowVector4<T>, eps: T, max_niter: usize) -> Option<Svd1x4<T>> {
    ...
    fn rank(self: Svd1x4<T>, eps: T) -> usize {
        let mut n: usize = 0;
    ...
    /// and `U = u · v_t` (1x4). Two roundings per entry of `P`, one per entry of `U`. Always
    ...
    fn sort_by_singular_values(ref self: Svd1x4<T>) {
        ...
        self =
            Svd1x4 {
                ...
                v_t: RowVector4 { x: vr0.x, y: vr0.y, z: vr0.z, w: vr0.w },
            };
    }
}


/// Upstream's `Matrix` methods that go through the SVD, on `RowVector4`. Import
/// `RowVector4SvdTrait` to use them.
#[generate_trait]
pub impl RowVector4SvdImpl<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of RowVector4SvdTrait<T> {
    /// The singular value decomposition, see `Svd1x4Trait::new`. Upstream: `Matrix::svd(true,
    /// true)`.
    #[inline(always)]
    fn svd(self: RowVector4<T>) -> Svd1x4<T> {
        Svd1x4Trait::new(self)
    }

    /// `svd` (already sorted, see `Svd1x4Trait::new_unordered`). Upstream: `Matrix::svd_unordered`.
    #[inline(always)]
    fn svd_unordered(self: RowVector4<T>) -> Svd1x4<T> {
        Svd1x4Trait::new(self)
    }

    /// See `Svd1x4Trait::try_new`. Upstream: `Matrix::try_svd(true, true, eps, max_niter)`.
    #[inline(always)]
    fn try_svd(self: RowVector4<T>, eps: T, max_niter: usize) -> Option<Svd1x4<T>> {
        Svd1x4Trait::try_new(self, eps, max_niter)
    }

    /// See `Svd1x4Trait::try_new_unordered`. Upstream: `Matrix::try_svd_unordered`.
    #[inline(always)]
    fn try_svd_unordered(self: RowVector4<T>, eps: T, max_niter: usize) -> Option<Svd1x4<T>> {
        Svd1x4Trait::try_new(self, eps, max_niter)
    }
    ...
    fn singular_values(self: RowVector4<T>) -> Matrix1<T> {
        {
            let t = Vector4 { x: self.x, y: self.y, z: self.z, w: self.w };
            Svd4x1InternalTrait::values_from_right(t, Svd4x1InternalTrait::right(t))
        }
    }

    /// `singular_values` (sorted; see `Svd1x4Trait::new_unordered`). Upstream:
    /// `Matrix::singular_values_unordered`.
    #[inline(always)]
    fn singular_values_unordered(self: RowVector4<T>) -> Matrix1<T> {
    ...
    /// here).
    fn rank(self: RowVector4<T>, eps: T) -> usize {
    ...
    /// The Moore-Penrose pseudo-inverse, see `Svd1x4Trait::pseudo_inverse`. Upstream:
    /// `Matrix::pseudo_inverse` (`Err` on a negative `eps`; `None` here).
    #[inline(always)]
    fn pseudo_inverse(self: RowVector4<T>, eps: T) -> Option<Vector4<T>> {
        Svd1x4Trait::new(self).pseudo_inverse(eps)
    }

    /// The left polar decomposition `M = P · U`, see `Svd1x4Trait::to_polar`. Upstream:
    /// `Matrix::polar`.
    fn polar(self: RowVector4<T>) -> (Matrix1<T>, RowVector4<T>) {
        Svd1x4Trait::new(self).to_polar().unwrap()
    }

    /// `polar`, or `None` when the decomposition did not converge within `eps`, see
    /// `Svd1x4Trait::try_new`. Upstream: `Matrix::try_polar`.
    fn try_polar(
        self: RowVector4<T>, eps: T, max_niter: usize,
    ) -> Option<(Matrix1<T>, RowVector4<T>)> {
        match Svd1x4Trait::try_new(self, eps, max_niter) {
            Some(d) => d.to_polar(),
            None => None,
        }
    }
