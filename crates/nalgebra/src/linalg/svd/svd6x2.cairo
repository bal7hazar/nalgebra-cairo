...
/// `Svd6x2InternalTrait`).
#[derive(Copy, Drop)]
pub(crate) struct SortedSvd6x2<T> {
    s0: T,
    s1: T,
    w0: Vector6<T>,
    w1: Vector6<T>,
...
/// Crate-internal kernels of `Svd6x2<T>`: the Gram matrix, the right singular vectors, the sorted
...
> of Svd6x2InternalTrait<T> {
    ...
    fn normalised(m: Matrix6x2<T>) -> Matrix6x2<T> {
        let mut a = m.m11.abs();
        if m.m21.abs() > a {
            a = m.m21.abs();
        }
    ...
    /// `MᵀM` (3 fused sums of 6 products, one rounding each).
    #[inline(always)]
    ...
    fn sorted(m: Matrix6x2<T>, v: Matrix2<T>) -> SortedSvd6x2<T> {
        revoke_ap_tracking();
        ...
        SortedSvd6x2 { s0, s1, w0, w1, v0, v1 }
    }
