...
/// `Svd5x2InternalTrait`).
#[derive(Copy, Drop)]
pub(crate) struct SortedSvd5x2<T> {
    s0: T,
    s1: T,
    w0: Vector5<T>,
    w1: Vector5<T>,
...
/// Crate-internal kernels of `Svd5x2<T>`: the Gram matrix, the right singular vectors, the sorted
...
> of Svd5x2InternalTrait<T> {
    ...
    fn normalised(m: Matrix5x2<T>) -> Matrix5x2<T> {
        let mut a = m.m11.abs();
        if m.m21.abs() > a {
            a = m.m21.abs();
        }
        ...
        let (q6, q7, q8, q9) = R::div4(m.m22, m.m32, m.m42, m.m52, a);
        Matrix5x2 {
            m11: q0,
            m21: q1,
            m31: q2,
    ...
    /// `MᵀM` (3 fused sums of 5 products, one rounding each).
    #[inline(always)]
    ...
    fn sorted(m: Matrix5x2<T>, v: Matrix2<T>) -> SortedSvd5x2<T> {
        revoke_ap_tracking();
        ...
        SortedSvd5x2 { s0, s1, w0, w1, v0, v1 }
    }
