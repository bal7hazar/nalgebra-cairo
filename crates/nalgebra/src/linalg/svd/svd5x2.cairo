...
//! `Svd5x2`: the singular value decomposition `M = U · Σ · Vᵀ` of a `Matrix5x2` (upstream
//! `nalgebra::linalg::SVD<T, U5, U2>`), its pseudo-inverse, least-squares solve, rank and polar
...
pub struct Svd5x2<T> {
    /// The left singular vectors, as columns (5x2).
    pub u: Matrix5x2<T>,
...
> of Svd5x2Trait<T> {
    ...
    fn new(matrix: Matrix5x2<T>) -> Svd5x2<T> {
        Svd5x2InternalTrait::from_right(matrix, Svd5x2InternalTrait::right(matrix))
    }
    ...
    fn try_new(matrix: Matrix5x2<T>, eps: T, max_niter: usize) -> Option<Svd5x2<T>> {
        let _ = max_niter;
        match Svd5x2InternalTrait::try_right(matrix, eps) {
            Some(v) => Some(Svd5x2InternalTrait::from_right(matrix, v)),
            None => None,
        }
    }
    ...
    #[inline(always)]
    fn new_unordered(matrix: Matrix5x2<T>) -> Svd5x2<T> {
        Self::new(matrix)
    }

    /// `try_new`, see `new_unordered`. Upstream: `SVD::try_new_unordered`.
    #[inline(always)]
    fn try_new_unordered(matrix: Matrix5x2<T>, eps: T, max_niter: usize) -> Option<Svd5x2<T>> {
    ...
    fn rank(self: Svd5x2<T>, eps: T) -> usize {
        let mut n: usize = 0;
    ...
    fn recompose(self: Svd5x2<T>) -> Matrix5x2<T> {
    ...
    /// The Moore-Penrose pseudo-inverse `V · diag(σ⁺) · Uᵀ` (2x5), `σ⁺_i = 1 / σ_i` when
    ...
    fn pseudo_inverse(self: Svd5x2<T>, eps: T) -> Option<Matrix2x5<T>> {
    ...
    fn solve(self: Svd5x2<T>, b: Vector5<T>, eps: T) -> Option<Vector2<T>> {
        if eps.is_sign_negative() {
            return None;
        }
    ...
    /// and `U = u · v_t` (5x2). Two roundings per entry of `P`, one per entry of `U`. Always
    /// `Some` (upstream returns `None` only when `u` or `v_t` was not computed). Panics on
    /// overflow. Upstream: `SVD::to_polar`.
    fn to_polar(self: Svd5x2<T>) -> Option<(Matrix5<T>, Matrix5x2<T>)> {
        let a0_0 = self.u.m11 * self.singular_values.x;
        let a0_1 = self.u.m12 * self.singular_values.y;
        let a1_0 = self.u.m21 * self.singular_values.x;
        let a1_1 = self.u.m22 * self.singular_values.y;
    ...
    fn sort_by_singular_values(ref self: Svd5x2<T>) {
...
/// Crate-internal kernels of `Svd5x2<T>`: the Gram matrix, the decomposition from the right
/// singular vectors, the singular values alone.
#[generate_trait]
pub(crate) impl Svd5x2InternalImpl<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of Svd5x2InternalTrait<T> {
    /// `MᵀM` (3 fused sums of 5 products, one rounding each). Panics on overflow.
    #[inline(always)]
    fn gram(m: Matrix5x2<T>) -> SymMatrix2<T> {
        SymMatrix2 {
            m11: R::wide_rescale(
                R::wide_add_prod(
                    R::wide_add_prod(
    ...
    fn from_right(m: Matrix5x2<T>, v: Matrix2<T>) -> Svd5x2<T> {
        revoke_ap_tracking();
    ...
    fn values_from_right(m: Matrix5x2<T>, v: Matrix2<T>) -> Vector2<T> {
        revoke_ap_tracking();
        let v0 = Vector2 { x: v.m11, y: v.m21 };
        let v1 = Vector2 { x: v.m12, y: v.m22 };
        let w0 = Vector5 {
...
/// Upstream's `Matrix` methods that go through the SVD, on `Matrix5x2`. Import `Matrix5x2SvdTrait`
/// to use them.
#[generate_trait]
pub impl Matrix5x2SvdImpl<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of Matrix5x2SvdTrait<T> {
    /// The singular value decomposition, see `Svd5x2Trait::new`. Upstream: `Matrix::svd(true,
    /// true)`.
    #[inline(always)]
    fn svd(self: Matrix5x2<T>) -> Svd5x2<T> {
        Svd5x2Trait::new(self)
    }

    /// `svd` (already sorted, see `Svd5x2Trait::new_unordered`). Upstream: `Matrix::svd_unordered`.
    #[inline(always)]
    fn svd_unordered(self: Matrix5x2<T>) -> Svd5x2<T> {
        Svd5x2Trait::new(self)
    }

    /// See `Svd5x2Trait::try_new`. Upstream: `Matrix::try_svd(true, true, eps, max_niter)`.
    #[inline(always)]
    fn try_svd(self: Matrix5x2<T>, eps: T, max_niter: usize) -> Option<Svd5x2<T>> {
        Svd5x2Trait::try_new(self, eps, max_niter)
    }

    /// See `Svd5x2Trait::try_new_unordered`. Upstream: `Matrix::try_svd_unordered`.
    #[inline(always)]
    fn try_svd_unordered(self: Matrix5x2<T>, eps: T, max_niter: usize) -> Option<Svd5x2<T>> {
        Svd5x2Trait::try_new(self, eps, max_niter)
    }
    ...
    fn singular_values(self: Matrix5x2<T>) -> Vector2<T> {
        Svd5x2InternalTrait::values_from_right(self, Svd5x2InternalTrait::right(self))
    }

    /// `singular_values` (sorted; see `Svd5x2Trait::new_unordered`). Upstream:
    /// `Matrix::singular_values_unordered`.
    #[inline(always)]
    fn singular_values_unordered(self: Matrix5x2<T>) -> Vector2<T> {
    ...
    /// here).
    fn rank(self: Matrix5x2<T>, eps: T) -> usize {
    ...
    /// The Moore-Penrose pseudo-inverse, see `Svd5x2Trait::pseudo_inverse`. Upstream:
    /// `Matrix::pseudo_inverse` (`Err` on a negative `eps`; `None` here).
    #[inline(always)]
    fn pseudo_inverse(self: Matrix5x2<T>, eps: T) -> Option<Matrix2x5<T>> {
        Svd5x2Trait::new(self).pseudo_inverse(eps)
    }

    /// The left polar decomposition `M = P · U`, see `Svd5x2Trait::to_polar`. Upstream:
    /// `Matrix::polar`.
    fn polar(self: Matrix5x2<T>) -> (Matrix5<T>, Matrix5x2<T>) {
        Svd5x2Trait::new(self).to_polar().unwrap()
    }

    /// `polar`, or `None` when the decomposition did not converge within `eps`, see
    /// `Svd5x2Trait::try_new`. Upstream: `Matrix::try_polar`.
    fn try_polar(
        self: Matrix5x2<T>, eps: T, max_niter: usize,
    ) -> Option<(Matrix5<T>, Matrix5x2<T>)> {
        match Svd5x2Trait::try_new(self, eps, max_niter) {
            Some(d) => d.to_polar(),
            None => None,
        }
    }
