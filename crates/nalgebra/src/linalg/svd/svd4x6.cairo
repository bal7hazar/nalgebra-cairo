...
//! `Svd4x6`: the singular value decomposition `M = U · Σ · Vᵀ` of a `Matrix4x6` (upstream
//! `nalgebra::linalg::SVD<T, U4, U6>`), its pseudo-inverse, least-squares solve, rank and polar
...
/// `u` is 4x4 with orthonormal columns, `v_t` is 4x6 with orthonormal rows, the 4
...
pub struct Svd4x6<T> {
    ...
    /// The TRANSPOSE of the right singular vectors (4x6): `v_i` is row `i`.
    pub v_t: Matrix4x6<T>,
}

/// Test-only field-wise equality (upstream `SVD` has no `PartialEq`).
#[cfg(test)]
impl Svd4x6PartialEq<T, +PartialEq<T>> of PartialEq<Svd4x6<T>> {
    fn eq(lhs: @Svd4x6<T>, rhs: @Svd4x6<T>) -> bool {
        lhs.u == rhs.u && lhs.singular_values == rhs.singular_values && lhs.v_t == rhs.v_t
    }
}

/// Methods of `Svd4x6<T>` for any `Real` scalar.
#[generate_trait]
pub impl Svd4x6Impl<
    T,
    impl R: Real<T>,
...
> of Svd4x6Trait<T> {
    ...
    /// TRANSPOSE (`Svd6x4`): `Mᵀ = U' Σ V'ᵀ` gives `M = V' Σ U'ᵀ`, so `u = V'` and `v_t =
    /// U'ᵀ`
    /// (moves only). The Gram matrix is then `M Mᵀ` (4x4), the smaller one. Same algorithm,
    /// accuracy and conventions as `Svd6x4Trait::new`. Upstream: `SVD::new(matrix, true, true)`
    /// (both factors always computed).
    fn new(matrix: Matrix4x6<T>) -> Svd4x6<T> {
        let t = Svd6x4Trait::new(
            Matrix6x4 {
                m11: matrix.m11,
                m21: matrix.m12,
                m31: matrix.m13,
    ...
    /// see `Svd6x4Trait::try_new`. `max_niter` is ignored (constant budget). Upstream:
    /// `SVD::try_new(matrix, true, true, eps, max_niter)`.
    fn try_new(matrix: Matrix4x6<T>, eps: T, max_niter: usize) -> Option<Svd4x6<T>> {
        match Svd6x4Trait::try_new(
            Matrix6x4 {
                m11: matrix.m11,
                m21: matrix.m12,
                m31: matrix.m13,
    ...
    #[inline(always)]
    fn new_unordered(matrix: Matrix4x6<T>) -> Svd4x6<T> {
        Self::new(matrix)
    }

    /// `try_new`, see `new_unordered`. Upstream: `SVD::try_new_unordered`.
    #[inline(always)]
    fn try_new_unordered(matrix: Matrix4x6<T>, eps: T, max_niter: usize) -> Option<Svd4x6<T>> {
    ...
    fn rank(self: Svd4x6<T>, eps: T) -> usize {
        let mut n: usize = 0;
    ...
    fn recompose(self: Svd4x6<T>) -> Matrix4x6<T> {
        ...
        Matrix4x6 {
            ...
            m16: R::sum_prod4(
                a0_0, self.v_t.m16, a0_1, self.v_t.m26, a0_2, self.v_t.m36, a0_3, self.v_t.m46,
            ),
            m26: R::sum_prod4(
                a1_0, self.v_t.m16, a1_1, self.v_t.m26, a1_2, self.v_t.m36, a1_3, self.v_t.m46,
            ),
            m36: R::sum_prod4(
                a2_0, self.v_t.m16, a2_1, self.v_t.m26, a2_2, self.v_t.m36, a2_3, self.v_t.m46,
            ),
            m46: R::sum_prod4(
                a3_0, self.v_t.m16, a3_1, self.v_t.m26, a3_2, self.v_t.m36, a3_3, self.v_t.m46,
            ),
        }
    }

    /// The Moore-Penrose pseudo-inverse `V · diag(σ⁺) · Uᵀ` (6x4), `σ⁺_i = 1 / σ_i` when
    ...
    fn pseudo_inverse(self: Svd4x6<T>, eps: T) -> Option<Matrix6x4<T>> {
        ...
        Some(
            Matrix6x4 {
                ...
                m61: R::sum_prod4(
                    b5_0, self.u.m11, b5_1, self.u.m12, b5_2, self.u.m13, b5_3, self.u.m14,
                ),
                m12: R::sum_prod4(
                ...
                m62: R::sum_prod4(
                    b5_0, self.u.m21, b5_1, self.u.m22, b5_2, self.u.m23, b5_3, self.u.m24,
                ),
                m13: R::sum_prod4(
                ...
                m63: R::sum_prod4(
                    b5_0, self.u.m31, b5_1, self.u.m32, b5_2, self.u.m33, b5_3, self.u.m34,
                ),
                m14: R::sum_prod4(
                ...
                m64: R::sum_prod4(
                    b5_0, self.u.m41, b5_1, self.u.m42, b5_2, self.u.m43, b5_3, self.u.m44,
                ),
            },
        )
    }
    ...
    fn solve(self: Svd4x6<T>, b: Vector4<T>, eps: T) -> Option<Vector6<T>> {
        if eps.is_sign_negative() {
            return None;
        }
        ...
        Some(
            Vector6 {
                ...
                b: R::sum_prod4(
                    self.v_t.m16, z0, self.v_t.m26, z1, self.v_t.m36, z2, self.v_t.m46, z3,
                ),
            },
        )
    }
    ...
    /// and `U = u · v_t` (4x6). Two roundings per entry of `P`, one per entry of `U`. Always
    /// `Some` (upstream returns `None` only when `u` or `v_t` was not computed). Panics on
    /// overflow. Upstream: `SVD::to_polar`.
    fn to_polar(self: Svd4x6<T>) -> Option<(Matrix4<T>, Matrix4x6<T>)> {
        let a0_0 = self.u.m11 * self.singular_values.x;
        let a0_1 = self.u.m12 * self.singular_values.y;
        let a0_2 = self.u.m13 * self.singular_values.z;
        let a0_3 = self.u.m14 * self.singular_values.w;
    ...
    fn sort_by_singular_values(ref self: Svd4x6<T>) {
...
/// Upstream's `Matrix` methods that go through the SVD, on `Matrix4x6`. Import `Matrix4x6SvdTrait`
/// to use them.
#[generate_trait]
pub impl Matrix4x6SvdImpl<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of Matrix4x6SvdTrait<T> {
    /// The singular value decomposition, see `Svd4x6Trait::new`. Upstream: `Matrix::svd(true,
    /// true)`.
    #[inline(always)]
    fn svd(self: Matrix4x6<T>) -> Svd4x6<T> {
        Svd4x6Trait::new(self)
    }

    /// `svd` (already sorted, see `Svd4x6Trait::new_unordered`). Upstream: `Matrix::svd_unordered`.
    #[inline(always)]
    fn svd_unordered(self: Matrix4x6<T>) -> Svd4x6<T> {
        Svd4x6Trait::new(self)
    }

    /// See `Svd4x6Trait::try_new`. Upstream: `Matrix::try_svd(true, true, eps, max_niter)`.
    #[inline(always)]
    fn try_svd(self: Matrix4x6<T>, eps: T, max_niter: usize) -> Option<Svd4x6<T>> {
        Svd4x6Trait::try_new(self, eps, max_niter)
    }

    /// See `Svd4x6Trait::try_new_unordered`. Upstream: `Matrix::try_svd_unordered`.
    #[inline(always)]
    fn try_svd_unordered(self: Matrix4x6<T>, eps: T, max_niter: usize) -> Option<Svd4x6<T>> {
        Svd4x6Trait::try_new(self, eps, max_niter)
    }
    ...
    fn singular_values(self: Matrix4x6<T>) -> Vector4<T> {
        {
            let t = Matrix6x4 {
                m11: self.m11,
                m21: self.m12,
            ...
            Svd6x4InternalTrait::values_from_right(t, Svd6x4InternalTrait::right(t))
        }
    }

    /// `singular_values` (sorted; see `Svd4x6Trait::new_unordered`). Upstream:
    /// `Matrix::singular_values_unordered`.
    #[inline(always)]
    fn singular_values_unordered(self: Matrix4x6<T>) -> Vector4<T> {
    ...
    /// here).
    fn rank(self: Matrix4x6<T>, eps: T) -> usize {
    ...
    /// The Moore-Penrose pseudo-inverse, see `Svd4x6Trait::pseudo_inverse`. Upstream:
    /// `Matrix::pseudo_inverse` (`Err` on a negative `eps`; `None` here).
    #[inline(always)]
    fn pseudo_inverse(self: Matrix4x6<T>, eps: T) -> Option<Matrix6x4<T>> {
        Svd4x6Trait::new(self).pseudo_inverse(eps)
    }

    /// The left polar decomposition `M = P · U`, see `Svd4x6Trait::to_polar`. Upstream:
    /// `Matrix::polar`.
    fn polar(self: Matrix4x6<T>) -> (Matrix4<T>, Matrix4x6<T>) {
        Svd4x6Trait::new(self).to_polar().unwrap()
    }

    /// `polar`, or `None` when the decomposition did not converge within `eps`, see
    /// `Svd4x6Trait::try_new`. Upstream: `Matrix::try_polar`.
    fn try_polar(
        self: Matrix4x6<T>, eps: T, max_niter: usize,
    ) -> Option<(Matrix4<T>, Matrix4x6<T>)> {
        match Svd4x6Trait::try_new(self, eps, max_niter) {
            Some(d) => d.to_polar(),
            None => None,
        }
    }
