...
/// `Svd6x4InternalTrait`).
#[derive(Copy, Drop)]
pub(crate) struct SortedSvd6x4<T> {
    s0: T,
    s1: T,
    s2: T,
    s3: T,
...
/// Crate-internal kernels of `Svd6x4<T>`: the Gram matrix, the right singular vectors, the sorted
...
> of Svd6x4InternalTrait<T> {
    ...
    fn normalised(m: Matrix6x4<T>) -> Matrix6x4<T> {
        let mut a = m.m11.abs();
        if m.m21.abs() > a {
            a = m.m21.abs();
        }
    ...
    /// `MᵀM` (10 fused sums of 6 products, one rounding each).
    #[inline(always)]
    ...
    fn sorted(m: Matrix6x4<T>, v: Matrix4<T>) -> SortedSvd6x4<T> {
        revoke_ap_tracking();
        ...
        SortedSvd6x4 { s0, s1, s2, s3, w0, w1, w2, w3, v0, v1, v2, v3 }
    }
