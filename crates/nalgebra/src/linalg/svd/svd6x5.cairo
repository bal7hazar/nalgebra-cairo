...
/// `Svd6x5InternalTrait`).
#[derive(Copy, Drop)]
pub(crate) struct SortedSvd6x5<T> {
    s0: T,
    s1: T,
    s2: T,
    s3: T,
...
/// Crate-internal kernels of `Svd6x5<T>`: the Gram matrix, the right singular vectors, the sorted
...
> of Svd6x5InternalTrait<T> {
    ...
    fn normalised(m: Matrix6x5<T>) -> Matrix6x5<T> {
        let mut a = m.m11.abs();
        if m.m21.abs() > a {
            a = m.m21.abs();
        }
    ...
    /// `MᵀM` (15 fused sums of 6 products, one rounding each).
    #[inline(always)]
    ...
    fn sorted(m: Matrix6x5<T>, v: Matrix5<T>) -> SortedSvd6x5<T> {
        revoke_ap_tracking();
