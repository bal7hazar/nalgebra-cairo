...
//! `Svd6x5`: the singular value decomposition `M = U · Σ · Vᵀ` of a `Matrix6x5` (upstream
...
#[cfg(test)]
impl Svd6x5PartialEq<T, +PartialEq<T>> of PartialEq<Svd6x5<T>> {
    fn eq(lhs: @Svd6x5<T>, rhs: @Svd6x5<T>) -> bool {
        lhs.u == rhs.u && lhs.singular_values == rhs.singular_values && lhs.v_t == rhs.v_t
    }
}

/// Methods of `Svd6x5<T>` for any `Real` scalar.
#[generate_trait]
pub impl Svd6x5Impl<
    T,
    impl R: Real<T>,
...
> of Svd6x5Trait<T> {
    ...
    fn rank(self: Svd6x5<T>, eps: T) -> usize {
        let mut n: usize = 0;
                    ...
                    m61: R::wide_rescale(
                        R::wide_add_prod(
                            R::wide_add_prod(
                                R::wide_add_prod(
                                    R::wide_add_prod(
                                        R::wide_add_prod(R::wide_zero(), self.u.m61, self.v_t.m11),
                                        self.u.m62,
                                        self.v_t.m21,
                                    ),
                                    self.u.m63,
                    ...
                    m62: R::wide_rescale(
                        R::wide_add_prod(
                            R::wide_add_prod(
                                R::wide_add_prod(
                                    R::wide_add_prod(
                                        R::wide_add_prod(R::wide_zero(), self.u.m61, self.v_t.m12),
                                        self.u.m62,
                                        self.v_t.m22,
                                    ),
                                    self.u.m63,
                    ...
                    m63: R::wide_rescale(
                        R::wide_add_prod(
                            R::wide_add_prod(
                                R::wide_add_prod(
                                    R::wide_add_prod(
                                        R::wide_add_prod(R::wide_zero(), self.u.m61, self.v_t.m13),
                                        self.u.m62,
                                        self.v_t.m23,
                                    ),
                                    self.u.m63,
                    ...
                    m64: R::wide_rescale(
                        R::wide_add_prod(
                            R::wide_add_prod(
                                R::wide_add_prod(
                                    R::wide_add_prod(
                                        R::wide_add_prod(R::wide_zero(), self.u.m61, self.v_t.m14),
                                        self.u.m62,
                                        self.v_t.m24,
                                    ),
                                    self.u.m63,
                    ...
                    m65: R::wide_rescale(
                        R::wide_add_prod(
                            R::wide_add_prod(
                                R::wide_add_prod(
                                    R::wide_add_prod(
                                        R::wide_add_prod(R::wide_zero(), self.u.m61, self.v_t.m15),
                                        self.u.m62,
                                        self.v_t.m25,
                                    ),
                                    self.u.m63,
...
/// Crate-internal kernels of `Svd6x5<T>`: the Gram matrix, the decomposition from the right
/// singular vectors, the singular values alone.
#[generate_trait]
pub(crate) impl Svd6x5InternalImpl<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of Svd6x5InternalTrait<T> {
    /// `MᵀM` (15 fused sums of 6 products, one rounding each). Panics on overflow.
    #[inline(always)]
    fn gram(m: Matrix6x5<T>) -> Sym5<T> {
        Sym5 {
            m11: R::wide_rescale(
                R::wide_add_prod(
                    R::wide_add_prod(
    ...
    fn values_from_right(m: Matrix6x5<T>, v: Matrix5<T>) -> Vector5<T> {
        revoke_ap_tracking();
        let v0 = Vector5 { x: v.m11, y: v.m21, z: v.m31, w: v.m41, a: v.m51 };
        let v1 = Vector5 { x: v.m12, y: v.m22, z: v.m32, w: v.m42, a: v.m52 };
        let v2 = Vector5 { x: v.m13, y: v.m23, z: v.m33, w: v.m43, a: v.m53 };
...
/// Upstream's `Matrix` methods that go through the SVD, on `Matrix6x5`. Import `Matrix6x5SvdTrait`
/// to use them.
#[generate_trait]
pub impl Matrix6x5SvdImpl<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
...
> of Matrix6x5SvdTrait<T> {
    ...
    /// `singular_values` (sorted; see `Svd6x5Trait::new_unordered`). Upstream:
    /// `Matrix::singular_values_unordered`.
    #[inline(always)]
    fn singular_values_unordered(self: Matrix6x5<T>) -> Vector5<T> {
    ...
    /// here).
    fn rank(self: Matrix6x5<T>, eps: T) -> usize {
    ...
    /// The Moore-Penrose pseudo-inverse, see `Svd6x5Trait::pseudo_inverse`. Upstream:
    ...
    /// `Svd6x5Trait::try_new`. Upstream: `Matrix::try_polar`.
    fn try_polar(
