...
//! `Svd5x1`: the singular value decomposition `M = U · Σ · Vᵀ` of a `Vector5` (upstream
//! `nalgebra::linalg::SVD<T, U5, U1>`), its pseudo-inverse, least-squares solve, rank and polar
...
/// The singular value decomposition `M = u · diag(singular_values) · v_t` of a `Vector5<T>`:
/// `u` is 5x1 with orthonormal columns, `v_t` is 1x1 with orthonormal rows, the 1
...
pub struct Svd5x1<T> {
    /// The left singular vectors, as columns (5x1).
    pub u: Vector5<T>,
...
> of Svd5x1Trait<T> {
    ...
    fn new(matrix: Vector5<T>) -> Svd5x1<T> {
        Svd5x1InternalTrait::from_right(matrix, Svd5x1InternalTrait::right(matrix))
    }
    ...
    fn try_new(matrix: Vector5<T>, eps: T, max_niter: usize) -> Option<Svd5x1<T>> {
        let _ = max_niter;
        match Svd5x1InternalTrait::try_right(matrix, eps) {
            Some(v) => Some(Svd5x1InternalTrait::from_right(matrix, v)),
            None => None,
        }
    }
    ...
    #[inline(always)]
    fn new_unordered(matrix: Vector5<T>) -> Svd5x1<T> {
        Self::new(matrix)
    }

    /// `try_new`, see `new_unordered`. Upstream: `SVD::try_new_unordered`.
    #[inline(always)]
    fn try_new_unordered(matrix: Vector5<T>, eps: T, max_niter: usize) -> Option<Svd5x1<T>> {
    ...
    fn rank(self: Svd5x1<T>, eps: T) -> usize {
        let mut n: usize = 0;
    ...
    /// here).
    fn recompose(self: Svd5x1<T>) -> Vector5<T> {
        revoke_ap_tracking();
        Vector5 {
    ...
    /// The Moore-Penrose pseudo-inverse `V · diag(σ⁺) · Uᵀ` (1x5), `σ⁺_i = 1 / σ_i` when
    ...
    fn pseudo_inverse(self: Svd5x1<T>, eps: T) -> Option<RowVector5<T>> {
        revoke_ap_tracking();
        ...
        Some(
            Matrix1 { x: self.v_t.x * p0 }
                .mul_mat(
                    RowVector5 { x: self.u.x, y: self.u.y, z: self.u.z, w: self.u.w, a: self.u.a },
                ),
        )
    }
    ...
    fn solve(self: Svd5x1<T>, b: Vector5<T>, eps: T) -> Option<Matrix1<T>> {
        revoke_ap_tracking();
        if eps.is_sign_negative() {
            return None;
        }
    ...
    /// and `U = u · v_t` (5x1). Two roundings per entry of `P`, one per entry of `U`. Always
    /// `Some` (upstream returns `None` only when `u` or `v_t` was not computed). Panics on
    /// overflow. Upstream: `SVD::to_polar`.
    fn to_polar(self: Svd5x1<T>) -> Option<(Matrix5<T>, Vector5<T>)> {
        revoke_ap_tracking();
    ...
    fn sort_by_singular_values(ref self: Svd5x1<T>) {
        revoke_ap_tracking();
        let mut s0 = self.singular_values.x;
        let mut uc0 = Vector5 { x: self.u.x, y: self.u.y, z: self.u.z, w: self.u.w, a: self.u.a };
        let mut vr0 = Matrix1 { x: self.v_t.x };

        self =
            Svd5x1 {
                u: Vector5 { x: uc0.x, y: uc0.y, z: uc0.z, w: uc0.w, a: uc0.a },
...
/// `Svd5x1InternalTrait`).
#[derive(Copy, Drop)]
pub(crate) struct SortedSvd5x1<T> {
    s0: T,
    w0: Vector5<T>,
    v0: Matrix1<T>,
}

/// Crate-internal kernels of `Svd5x1<T>`: the Gram matrix, the right singular vectors, the sorted
/// columns `M v_i`, the decomposition.
#[generate_trait]
pub(crate) impl Svd5x1InternalImpl<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
...
> of Svd5x1InternalTrait<T> {
    ...
    fn sorted(m: Vector5<T>, v: Matrix1<T>) -> SortedSvd5x1<T> {
        revoke_ap_tracking();
        let v0 = Matrix1 { x: v.x };
        let mv = m.mul_mat(v);
        let w0 = Vector5 { x: mv.x, y: mv.y, z: mv.z, w: mv.w, a: mv.a };
        let s0 = R::wide_sqrt(
            R::wide_add_prod(
                R::wide_add_prod(
                    R::wide_add_prod(
    ...
    fn from_right(m: Vector5<T>, v: Matrix1<T>) -> Svd5x1<T> {
        let t = Self::sorted(m, v);
        let (s0, w0, v0) = (t.s0, t.w0, t.v0);
        let u0 = SvdComplete5Impl::<T>::first(w0, s0);
        Svd5x1 {
            u: Vector5 { x: u0.x, y: u0.y, z: u0.z, w: u0.w, a: u0.a },
            singular_values: Matrix1 { x: s0 },
            v_t: Matrix1 { x: v0.x },
        }
    }
    ...
    fn values_from_right(m: Vector5<T>, v: Matrix1<T>) -> Matrix1<T> {
        let t = Self::sorted(m, v);
        Matrix1 { x: t.s0 }
    }
}

/// Upstream's `Matrix` methods that go through the SVD, on `Vector5`. Import `Vector5SvdTrait` to
/// use them.
#[generate_trait]
pub impl Vector5SvdImpl<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of Vector5SvdTrait<T> {
    /// The singular value decomposition, see `Svd5x1Trait::new`. Upstream: `Matrix::svd(true,
    /// true)`.
    #[inline(always)]
    fn svd(self: Vector5<T>) -> Svd5x1<T> {
        Svd5x1Trait::new(self)
    }

    /// `svd` (already sorted, see `Svd5x1Trait::new_unordered`). Upstream: `Matrix::svd_unordered`.
    #[inline(always)]
    fn svd_unordered(self: Vector5<T>) -> Svd5x1<T> {
        Svd5x1Trait::new(self)
    }

    /// See `Svd5x1Trait::try_new`. Upstream: `Matrix::try_svd(true, true, eps, max_niter)`.
    #[inline(always)]
    fn try_svd(self: Vector5<T>, eps: T, max_niter: usize) -> Option<Svd5x1<T>> {
        Svd5x1Trait::try_new(self, eps, max_niter)
    }

    /// See `Svd5x1Trait::try_new_unordered`. Upstream: `Matrix::try_svd_unordered`.
    #[inline(always)]
    fn try_svd_unordered(self: Vector5<T>, eps: T, max_niter: usize) -> Option<Svd5x1<T>> {
        Svd5x1Trait::try_new(self, eps, max_niter)
    }
    ...
    fn singular_values(self: Vector5<T>) -> Matrix1<T> {
        Svd5x1InternalTrait::values_from_right(self, Svd5x1InternalTrait::right(self))
    }

    /// `singular_values` (sorted; see `Svd5x1Trait::new_unordered`). Upstream:
    /// `Matrix::singular_values_unordered`.
    #[inline(always)]
    fn singular_values_unordered(self: Vector5<T>) -> Matrix1<T> {
    ...
    /// here).
    fn rank(self: Vector5<T>, eps: T) -> usize {
    ...
    /// The Moore-Penrose pseudo-inverse, see `Svd5x1Trait::pseudo_inverse`. Upstream:
    /// `Matrix::pseudo_inverse` (`Err` on a negative `eps`; `None` here).
    #[inline(always)]
    fn pseudo_inverse(self: Vector5<T>, eps: T) -> Option<RowVector5<T>> {
        Svd5x1Trait::new(self).pseudo_inverse(eps)
    }

    /// The left polar decomposition `M = P · U`, see `Svd5x1Trait::to_polar`. Upstream:
    /// `Matrix::polar`.
    fn polar(self: Vector5<T>) -> (Matrix5<T>, Vector5<T>) {
        Svd5x1Trait::new(self).to_polar().unwrap()
    }

    /// `polar`, or `None` when the decomposition did not converge within `eps`, see
    /// `Svd5x1Trait::try_new`. Upstream: `Matrix::try_polar`.
    fn try_polar(self: Vector5<T>, eps: T, max_niter: usize) -> Option<(Matrix5<T>, Vector5<T>)> {
        match Svd5x1Trait::try_new(self, eps, max_niter) {
            Some(d) => d.to_polar(),
            None => None,
        }
    }
