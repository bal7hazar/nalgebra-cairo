...
> of Lu4Trait<T> {
    ...
    /// once, so after each of the 3 steps an entry of `U` is within about `k * (1 + |a_kj|)` raw
    ...
    ///
    /// The 4 truncated divisions are kept rather than 4 reciprocals and 4 multiplications. That
    ...
    /// twice where `x / u` rounds once, which is the rule DESIGN D2 and `Vector4::unscale` already
...
mod tests {
    ...
    /// `solve` with ONE reciprocal per pivot and 4 multiplications instead of 4 truncated
    /// divisions.
