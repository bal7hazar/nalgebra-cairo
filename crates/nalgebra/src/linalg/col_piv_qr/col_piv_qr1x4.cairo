...
//! `ColPivQr1x4`: the QR factorisation with column pivoting of a `RowVector4` (upstream
//! `nalgebra::linalg::ColPivQR<T, U1, U4>`) by Householder reflections, fully unrolled
//! (WP 8.5-P15).
...
/// `ColPivQr1x4Trait::new` or `RowVector4ColPivQrTrait::col_piv_qr`. Upstream:
/// `nalgebra::linalg::ColPivQR`.
#[derive(Copy, Drop, Serde, Debug)]
pub struct ColPivQr1x4<T> {
    /// Householder axes (lower part) and strict upper triangle of `R`, packed.
    pub col_piv_qr: RowVector4<T>,
...
/// Methods of `ColPivQr1x4<T>` for any `Real` scalar.
#[generate_trait]
pub impl ColPivQr1x4Impl<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
...
> of ColPivQr1x4Trait<T> {
    ...
    fn new(matrix: RowVector4<T>) -> ColPivQr1x4<T> {
        revoke_ap_tracking();
        ...
        ColPivQr1x4 {
            col_piv_qr: RowVector4 { x: a00, y: a01, z: a02, w: a03 },
            p: Perm4 { p1: cp0, p2: 2, p3: 3 },
            diag: Matrix1 { x: d0 },
        }
    }

    /// The upper trapezoidal factor `R` (1x4): the strict upper triangle as stored and
    /// `|diag|` on the diagonal. Exact: moves and absolute values. Upstream: `ColPivQR::r`.
    #[inline(always)]
    fn r(self: ColPivQr1x4<T>) -> RowVector4<T> {
        RowVector4 {
            x: R::abs(self.diag.x),
            y: self.col_piv_qr.y,
            z: self.col_piv_qr.z,
    ...
    #[inline(always)]
    fn unpack_r(self: ColPivQr1x4<T>) -> RowVector4<T> {
        Self::r(self)
    }
    ...
    fn q(self: ColPivQr1x4<T>) -> Matrix1<T> {
        revoke_ap_tracking();
    ...
    #[inline(always)]
    fn p(self: ColPivQr1x4<T>) -> Perm4 {
        self.p
    }

    /// `(Q, R, P)`: `(q(), r(), p())`. Upstream: `ColPivQR::unpack`.
    #[inline(always)]
    fn unpack(self: ColPivQr1x4<T>) -> (Matrix1<T>, RowVector4<T>, Perm4) {
    ...
    #[inline(always)]
    fn col_piv_qr_internal(self: ColPivQr1x4<T>) -> RowVector4<T> {
        self.col_piv_qr
    }
    ...
    fn q_tr_mul<B, impl K: SolveKernel<Matrix1<T>, B>, +Drop<B>>(self: ColPivQr1x4<T>, ref rhs: B) {
        rhs = K::tr_mul_rhs(Self::q(self), rhs);
    }
}

/// `RowVector4` methods that go through the column-pivot QR factorisation. Import
/// `RowVector4ColPivQrTrait`.
#[generate_trait]
pub impl RowVector4ColPivQrImpl<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of RowVector4ColPivQrTrait<T> {
    /// The QR factorisation with column pivoting. Upstream: `Matrix::col_piv_qr`.
    #[inline(always)]
    fn col_piv_qr(self: RowVector4<T>) -> ColPivQr1x4<T> {
        ColPivQr1x4Trait::new(self)
    }
}
