...
//! `ColPivQr1x3`: the QR factorisation with column pivoting of a `RowVector3` (upstream
//! `nalgebra::linalg::ColPivQR<T, U1, U3>`) by Householder reflections, fully unrolled
//! (WP 8.5-P15).
...
/// `ColPivQr1x3Trait::new` or `RowVector3ColPivQrTrait::col_piv_qr`. Upstream:
/// `nalgebra::linalg::ColPivQR`.
#[derive(Copy, Drop, Serde, Debug)]
pub struct ColPivQr1x3<T> {
    /// Householder axes (lower part) and strict upper triangle of `R`, packed.
    pub col_piv_qr: RowVector3<T>,
...
/// Methods of `ColPivQr1x3<T>` for any `Real` scalar.
#[generate_trait]
pub impl ColPivQr1x3Impl<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
...
> of ColPivQr1x3Trait<T> {
    ...
    fn new(matrix: RowVector3<T>) -> ColPivQr1x3<T> {
        revoke_ap_tracking();
        ...
        ColPivQr1x3 {
            col_piv_qr: RowVector3 { x: a00, y: a01, z: a02 },
            p: Perm3 { p1: cp0, p2: 2 },
            diag: Matrix1 { x: d0 },
        }
    }

    /// The upper trapezoidal factor `R` (1x3): the strict upper triangle as stored and
    /// `|diag|` on the diagonal. Exact: moves and absolute values. Upstream: `ColPivQR::r`.
    #[inline(always)]
    fn r(self: ColPivQr1x3<T>) -> RowVector3<T> {
        RowVector3 { x: R::abs(self.diag.x), y: self.col_piv_qr.y, z: self.col_piv_qr.z }
    }

    /// `R`, consuming the factorisation: `r()`. Upstream: `ColPivQR::unpack_r`.
    #[inline(always)]
    fn unpack_r(self: ColPivQr1x3<T>) -> RowVector3<T> {
        Self::r(self)
    }
    ...
    fn q(self: ColPivQr1x3<T>) -> Matrix1<T> {
        revoke_ap_tracking();
    ...
    #[inline(always)]
    fn p(self: ColPivQr1x3<T>) -> Perm3 {
        self.p
    }

    /// `(Q, R, P)`: `(q(), r(), p())`. Upstream: `ColPivQR::unpack`.
    #[inline(always)]
    fn unpack(self: ColPivQr1x3<T>) -> (Matrix1<T>, RowVector3<T>, Perm3) {
    ...
    #[inline(always)]
    fn col_piv_qr_internal(self: ColPivQr1x3<T>) -> RowVector3<T> {
        self.col_piv_qr
    }
    ...
    fn q_tr_mul<B, impl K: SolveKernel<Matrix1<T>, B>, +Drop<B>>(self: ColPivQr1x3<T>, ref rhs: B) {
        rhs = K::tr_mul_rhs(Self::q(self), rhs);
    }
}

/// `RowVector3` methods that go through the column-pivot QR factorisation. Import
/// `RowVector3ColPivQrTrait`.
#[generate_trait]
pub impl RowVector3ColPivQrImpl<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
    ...
    +PartialEq<T>,
    +PartialOrd<T>,
> of RowVector3ColPivQrTrait<T> {
    /// The QR factorisation with column pivoting. Upstream: `Matrix::col_piv_qr`.
    #[inline(always)]
    fn col_piv_qr(self: RowVector3<T>) -> ColPivQr1x3<T> {
        ColPivQr1x3Trait::new(self)
    }
}
