"""The same iteration in f64 (upstream nalgebra's arithmetic, `eps = f64::EPSILON`, threshold `eps²`,
no Q32.32 deviation flags): the reference pass counts of the WP 8.5-P16 report."""
import math, random
ONE = 1.0
...
def fused(prods, adds=()):  # prods: list of (a,b) (sign folded), adds raw
    return sum(a * b for a, b in prods) + sum(adds)
def mul_add(a, b, c): return a * b + c
def div(a, b): return a / b
def isqrt(w): return math.sqrt(w)
...
def basis2(t, i):
    ...
    e = eig2(h00, h01, h10, h11)
    if e is None: return None
    x1, x2 = e[0] - h11, e[1] - h11
    x = x1 if abs(x1) > abs(x2) else x2
    return givens_new(x, h10)[0]
