...
> of Matrix2x5Trait<T> {
    ...
    /// The 2x5 matrix whose components all equal `elem`. Upstream: `Matrix2x5::repeat`.
    #[inline(always)]
    fn repeat(elem: T) -> Matrix2x5<T> {
        Matrix2x5 {
            m11: elem,
            m21: elem,
            m12: elem,
    ...
    /// Alias of `repeat`. Upstream: `Matrix2x5::from_element`.
    #[inline(always)]
    fn from_element(elem: T) -> Matrix2x5<T> {
        Matrix2x5 {
            m11: elem,
            m21: elem,
            m12: elem,
    ...
    /// The 2x5 matrix whose component `(i, j)` (row, column, 0-based) is `f(i, j)`, `f` being
    ...
    /// The 2x5 matrix whose first `data.len()` diagonal components are `data`, every other
    /// component zero. Panics with `nalgebra: diagonal too long` when `data.len() > 2`. Upstream:
    /// `Matrix2x5::from_partial_diagonal` (`&[T]`).
    #[inline(always)]
    fn from_partial_diagonal(data: Span<T>) -> Matrix2x5<T> {
        let len = data.len();
        if len > 2 {
    ...
    #[inline(always)]
    fn is_zero(self: Matrix2x5<T>) -> bool {
        self.m11 == R::zero()
            && self.m21 == R::zero()
            && self.m12 == R::zero()
            && self.m22 == R::zero()
    ...
    #[inline(always)]
    fn component_mul(self: Matrix2x5<T>, rhs: Matrix2x5<T>) -> Matrix2x5<T> {
        Matrix2x5 {
    ...
    #[inline(always)]
    fn component_mul_assign(ref self: Matrix2x5<T>, rhs: Matrix2x5<T>) {
        self =
            Matrix2x5 {
                m11: self.m11 * rhs.m11,
                m21: self.m21 * rhs.m21,
    ...
    #[inline(always)]
    fn component_div(self: Matrix2x5<T>, rhs: Matrix2x5<T>) -> Matrix2x5<T> {
        Matrix2x5 {
    ...
    #[inline(always)]
    fn component_div_assign(ref self: Matrix2x5<T>, rhs: Matrix2x5<T>) {
        self =
            Matrix2x5 {
                m11: R::div(self.m11, rhs.m11),
                m21: R::div(self.m21, rhs.m21),
    ...
    #[inline(always)]
    fn inf(self: Matrix2x5<T>, other: Matrix2x5<T>) -> Matrix2x5<T> {
        Matrix2x5 {
    ...
    #[inline(always)]
    fn sup(self: Matrix2x5<T>, other: Matrix2x5<T>) -> Matrix2x5<T> {
        Matrix2x5 {
    ...
    #[inline(always)]
    fn inf_sup(self: Matrix2x5<T>, other: Matrix2x5<T>) -> (Matrix2x5<T>, Matrix2x5<T>) {
        (Self::inf(self, other), Self::sup(self, other))
    }

    /// `self + k` added to every component. Exact; panics on overflow. Upstream: `add_scalar`.
    #[inline(always)]
    fn add_scalar(self: Matrix2x5<T>, k: T) -> Matrix2x5<T> {
        Matrix2x5 {
            m11: self.m11 + k,
            m21: self.m21 + k,
            m12: self.m12 + k,
    ...
    #[inline(always)]
    fn cmpy(ref self: Matrix2x5<T>, alpha: T, a: Matrix2x5<T>, b: Matrix2x5<T>, beta: T) {
        self =
            Matrix2x5 {
    ...
    #[inline(always)]
    fn cdpy(ref self: Matrix2x5<T>, alpha: T, a: Matrix2x5<T>, b: Matrix2x5<T>, beta: T) {
        self =
            Matrix2x5 {
    ...
    fn iamax_full(self: Matrix2x5<T>) -> (usize, usize) {
    ...
    #[inline(always)]
    fn icamax_full(self: Matrix2x5<T>) -> (usize, usize) {
    ...
    fn dot(self: Matrix2x5<T>, rhs: Matrix2x5<T>) -> T {
    ...
    /// `norm_squared`.
    fn norm_squared(self: Matrix2x5<T>) -> T {
        ...
        R::wide_rescale(R::wide_add_prod(w, self.m25, self.m25))
    }
    ...
    fn norm(self: Matrix2x5<T>) -> T {
        ...
        let w = R::wide_add_prod(w, self.m24, self.m24);
        let w = R::wide_add_prod(w, self.m15, self.m15);
        R::wide_sqrt(R::wide_add_prod(w, self.m25, self.m25))
    }

    /// Alias of `norm_squared`. Upstream: `magnitude_squared`.
    #[inline(always)]
    fn magnitude_squared(self: Matrix2x5<T>) -> T {
        Self::norm_squared(self)
    }

    /// Alias of `norm`. Upstream: `magnitude`.
    #[inline(always)]
    fn magnitude(self: Matrix2x5<T>) -> T {
        Self::norm(self)
    }
    ...
    fn metric_distance(self: Matrix2x5<T>, rhs: Matrix2x5<T>) -> T {
        ...
        R::wide_sqrt(R::wide_add_prod(w, self.m25 - rhs.m25, self.m25 - rhs.m25))
    }
    ...
    fn unscale(self: Matrix2x5<T>, k: T) -> Matrix2x5<T> {
    ...
    fn normalize(self: Matrix2x5<T>) -> Matrix2x5<T> {
    ...
    fn try_normalize(self: Matrix2x5<T>, min_norm: T) -> Option<Matrix2x5<T>> {
    ...
    /// `cap_magnitude`.
    fn cap_magnitude(self: Matrix2x5<T>, max: T) -> Matrix2x5<T> {
        let n = Self::norm(self);
        if n <= max {
            self
        } else {
    ...
    fn try_set_magnitude(ref self: Matrix2x5<T>, magnitude: T, min_magnitude: T) {
        let n = Self::norm(self);
        if n > min_magnitude {
    ...
    #[inline(always)]
    fn one_norm(self: Matrix2x5<T>) -> T {
        R::max(
            R::max(
                R::max(
                    R::max(
                        R::abs(self.m11) + R::abs(self.m21), R::abs(self.m12) + R::abs(self.m22),
                    ),
    ...
    /// The conjugate transpose, a `Matrix5x2`: the transpose for a real scalar. Exact. Upstream:
    /// `adjoint`.
    #[inline(always)]
    fn adjoint(self: Matrix2x5<T>) -> Matrix5x2<T> {
        Self::transpose(self)
    }

    /// Alias of `adjoint` (deprecated upstream). Upstream: `conjugate_transpose`.
    #[inline(always)]
    fn conjugate_transpose(self: Matrix2x5<T>) -> Matrix5x2<T> {
        Self::transpose(self)
    }

    /// The component-wise conjugate: `self` for a real scalar. Upstream: `conjugate`.
    #[inline(always)]
    fn conjugate(self: Matrix2x5<T>) -> Matrix2x5<T> {
        self
    }
    ...
    fn cast<U, +Into<T, U>, +Drop<U>>(self: Matrix2x5<T>) -> Matrix2x5<U> {
        Matrix2x5 {
            m11: self.m11.into(),
            m21: self.m21.into(),
            m12: self.m12.into(),
    ...
    /// (DESIGN D3).
    #[inline(always)]
    fn relative_eq(self: Matrix2x5<T>, other: Matrix2x5<T>, epsilon: u64, max_relative: T) -> bool {
    ...
    #[inline(always)]
    fn ulps_eq(self: Matrix2x5<T>, other: Matrix2x5<T>, epsilon: u64, max_ulps: u32) -> bool {
    ...
    /// The 2x5 matrix of `f(x)` for every component `x` (called in column-major order). `f` is any
    ...
    /// The 2x5 matrix of `f(i, j, x)` for every component `x` at row `i`, column `j` (0-based,
    ...
    /// The 2x5 matrix of `f(a, b)` for the components `a` of `self` and `b` of `rhs` at the same
    ...
    fn zip_map<
        ...
        +Drop<Func::Output>,
    >(
        self: Matrix2x5<T>, rhs: Matrix2x5<T2>, f: F,
    ) -> Matrix2x5<Func::Output> {
        Matrix2x5 {
    ...
    /// The 2x5 matrix of `f(a, b, c)` for the components of `self`, `b` and `c` at the same
    /// position. `f` is any closure or `Fn` value; Cairo closures take their arguments by value and
    ...
    fn zip_zip_map<
        ...
        +Drop<Func::Output>,
    >(
        self: Matrix2x5<T>, b: Matrix2x5<T2>, c: Matrix2x5<T3>, f: F,
    ) -> Matrix2x5<Func::Output> {
        Matrix2x5 {
    ...
    >(
        self: Matrix2x5<T>, rhs: Matrix2x5<T2>, init: Acc, f: F,
    ) -> Acc {
    ...
    fn zip_apply<
        ...
        +Into<Func::Output, T>,
    >(
        ref self: Matrix2x5<T>, rhs: Matrix2x5<T2>, f: F,
    ) {
        self =
            Matrix2x5 {
    ...
    fn zip_zip_apply<
        ...
        +Into<Func::Output, T>,
    >(
        ref self: Matrix2x5<T>, b: Matrix2x5<T2>, c: Matrix2x5<T3>, f: F,
    ) {
        self =
            Matrix2x5 {
    ...
    #[inline(always)]
    fn fill(ref self: Matrix2x5<T>, val: T) {
        self =
            Matrix2x5 {
                m11: val,
                m21: val,
    ...
    #[inline(always)]
    fn fill_diagonal(ref self: Matrix2x5<T>, val: T) {
        self =
            Matrix2x5 {
                m11: val,
                m21: self.m21,
    ...
    #[inline(always)]
    fn fill_with_identity(ref self: Matrix2x5<T>) {
        self =
            Matrix2x5 {
                m11: R::one(),
                m21: R::zero(),
    ...
    #[inline(always)]
    fn fill_row(ref self: Matrix2x5<T>, i: usize, val: T) {
        self = match i {
            0 => Matrix2x5 {
                m11: val,
                m21: self.m21,
    ...
    #[inline(always)]
    fn fill_column(ref self: Matrix2x5<T>, j: usize, val: T) {
        self = match j {
            0 => Matrix2x5 {
                m11: val,
                m21: val,
    ...
    #[inline(always)]
    fn fill_lower_triangle(ref self: Matrix2x5<T>, val: T, shift: usize) {
        self = match shift {
            0 => Matrix2x5 {
                m11: val,
                m21: val,
    ...
    #[inline(always)]
    fn fill_upper_triangle(ref self: Matrix2x5<T>, val: T, shift: usize) {
        self = match shift {
            0 => Matrix2x5 {
                m11: val,
                m21: self.m21,
    ...
    #[inline(always)]
    fn set_row(ref self: Matrix2x5<T>, i: usize, row: RowVector5<T>) {
        self = match i {
            0 => Matrix2x5 {
                m11: row.x,
                m21: self.m21,
    ...
    #[inline(always)]
    fn set_column(ref self: Matrix2x5<T>, j: usize, column: Vector2<T>) {
        self = match j {
            0 => Matrix2x5 {
                m11: column.x,
                m21: column.y,
    ...
    #[inline(always)]
    fn set_diagonal(ref self: Matrix2x5<T>, diag: Vector2<T>) {
        self =
            Matrix2x5 {
                m11: diag.x,
                m21: self.m21,
    ...
    #[inline(always)]
    fn set_partial_diagonal(ref self: Matrix2x5<T>, diag: Span<T>) {
        let len = diag.len();
        self =
            Matrix2x5 {
                m11: if len > 0 {
    ...
    /// Sets `self` to `other` (a `Matrix2x5`: upstream requires the same shape). Upstream:
    /// `copy_from`.
    #[inline(always)]
    fn copy_from(ref self: Matrix2x5<T>, other: Matrix2x5<T>) {
        self = other;
    }
    ...
    /// (`&[T]`).
    #[inline(always)]
    fn copy_from_slice(ref self: Matrix2x5<T>, slice: Span<T>) {
        self = Self::from_column_slice(slice);
    }

    /// Sets `self` to the transpose of `other`, a `Matrix5x2`. Exact. Upstream: `tr_copy_from`.
    #[inline(always)]
    fn tr_copy_from(ref self: Matrix2x5<T>, other: Matrix5x2<T>) {
        self =
            Matrix2x5 {
                m11: other.m11,
                m21: other.m12,
    ...
    #[inline]
    fn swap(ref self: Matrix2x5<T>, row_cols1: (usize, usize), row_cols2: (usize, usize)) {
        let a = MatrixIndex::index(self, row_cols1);
        let b = MatrixIndex::index(self, row_cols2);
        self =
            Matrix2x5EditTrait::replace(
                Matrix2x5EditTrait::replace(self, row_cols1, b), row_cols2, a,
            );
    }
    ...
    /// benchmark.
    #[inline]
    fn swap_rows(ref self: Matrix2x5<T>, irow1: usize, irow2: usize) {
        let a = Matrix2x5EditTrait::row_at(self, irow1);
        let b = Matrix2x5EditTrait::row_at(self, irow2);
    ...
    #[inline]
    fn swap_columns(ref self: Matrix2x5<T>, icol1: usize, icol2: usize) {
        let a = Matrix2x5EditTrait::column_at(self, icol1);
        let b = Matrix2x5EditTrait::column_at(self, icol2);
    ...
    #[inline(always)]
    fn neg_mut(ref self: Matrix2x5<T>) {
        self = -self;
    }
    ...
    /// `scale_mut`.
    #[inline(always)]
    fn scale_mut(ref self: Matrix2x5<T>, k: T) {
    ...
    #[inline(always)]
    fn unscale_mut(ref self: Matrix2x5<T>, k: T) {
    ...
    #[inline(always)]
    fn normalize_mut(ref self: Matrix2x5<T>) -> T {
        let n = Self::norm(self);
    ...
    #[inline(always)]
    fn try_normalize_mut(ref self: Matrix2x5<T>, min_norm: T) -> Option<T> {
        let n = Self::norm(self);
        if n <= min_norm {
            return None;
        }
    ...
    #[inline(always)]
    fn set_magnitude(ref self: Matrix2x5<T>, magnitude: T) {
        let n = Self::norm(self);
        self = Self::scale(self, R::div(magnitude, n));
    }
    ...
    /// `conjugate_mut`.
    #[inline(always)]
    fn conjugate_mut(ref self: Matrix2x5<T>) {
        self = Self::conjugate(self);
    }

    /// Writes the transpose of `self` into `out`, a `Matrix5x2`. Exact. Upstream: `transpose_to`.
    #[inline(always)]
    fn transpose_to(self: Matrix2x5<T>, ref out: Matrix5x2<T>) {
        out = Self::transpose(self);
    }

    /// Writes the adjoint of `self` (its transpose for a real scalar) into `out`, a `Matrix5x2`.
    /// Exact. Upstream: `adjoint_to`.
    #[inline(always)]
    fn adjoint_to(self: Matrix2x5<T>, ref out: Matrix5x2<T>) {
        out = Self::transpose(self);
    }

    /// Writes the adjoint of `self` (deprecated upstream alias of `adjoint_to`) into `out`, a
    /// `Matrix5x2`. Exact. Upstream: `conjugate_transpose_to`.
    #[inline(always)]
    fn conjugate_transpose_to(self: Matrix2x5<T>, ref out: Matrix5x2<T>) {
        out = Self::transpose(self);
    }

    /// Writes the sum `self + rhs` into `out`. Exact; panics on overflow. Upstream: `add_to`.
    #[inline(always)]
    fn add_to(self: Matrix2x5<T>, rhs: Matrix2x5<T>, ref out: Matrix2x5<T>) {
        out = self + rhs;
    }
    ...
    /// `sub_to`.
    #[inline(always)]
    fn sub_to(self: Matrix2x5<T>, rhs: Matrix2x5<T>, ref out: Matrix2x5<T>) {
        out = self - rhs;
    }
    ...
    #[inline]
    fn apply_norm<N, +Drop<N>, impl Nm: Norm<N, Matrix2x5<T>, T>>(
        self: Matrix2x5<T>, norm: N,
    ) -> T {
        Nm::norm(@norm, self)
    }
    ...
    #[inline]
    fn apply_metric_distance<N, +Drop<N>, impl Nm: Norm<N, Matrix2x5<T>, T>>(
        self: Matrix2x5<T>, rhs: Matrix2x5<T>, norm: N,
    ) -> T {
...
> of Matrix2x5AngleTrait<T> {
    ...
    fn angle(self: Matrix2x5<T>, other: Matrix2x5<T>) -> T {
        let n1 = Matrix2x5Trait::norm(self);
        let n2 = Matrix2x5Trait::norm(other);
        if n1 == R::zero() || n2 == R::zero() {
            return R::zero();
        }
        let u = Matrix2x5Trait::unscale(self, n1);
        let v = Matrix2x5Trait::unscale(other, n2);
        let half = Tr::atan2(Matrix2x5Trait::metric_distance(u, v), Matrix2x5Trait::norm(u + v));
        half + half
    }
    ...
    /// Upstream: `lp_norm`.
    fn lp_norm(self: Matrix2x5<T>, p: i32) -> T {
        if p < 1 {
        ...
        if p == 2 {
            return Matrix2x5Trait::norm(self);
        }
        let m = Matrix2x5Trait::amax(self);
        if m == R::zero() {
            return R::zero();
        }
        let r = Matrix2x5Trait::unscale(Matrix2x5Trait::abs(self), m);
...
/// `selfᵀ * rhs`, a `Vector5`: `mul_mat` of the transposed components, so one `sum_prod2` per
...
/// `selfᵀ * rhs`, a `Matrix5x2`: `mul_mat` of the transposed components, so one `sum_prod2` per
/// component (one rounding each); bit-identical to `self.transpose().mul_mat(rhs)`, at the same gas
/// (the transpose only relabels values). Panics on overflow. Upstream: `tr_mul`.
...
/// `selfᵀ * rhs`, a `Matrix5x3`: `mul_mat` of the transposed components, so one `sum_prod2` per
/// component (one rounding each); bit-identical to `self.transpose().mul_mat(rhs)`, at the same gas
/// (the transpose only relabels values). Panics on overflow. Upstream: `tr_mul`.
...
/// `selfᵀ * rhs`, a `Matrix5x4`: `mul_mat` of the transposed components, so one `sum_prod2` per
/// component (one rounding each); bit-identical to `self.transpose().mul_mat(rhs)`, at the same gas
/// (the transpose only relabels values). Panics on overflow. Upstream: `tr_mul`.
...
/// `selfᵀ * rhs`, a `Matrix5`: `mul_mat` of the transposed components, so one `sum_prod2` per
/// component (one rounding each); bit-identical to `self.transpose().mul_mat(rhs)`, at the same gas
/// (the transpose only relabels values). Panics on overflow. Upstream: `tr_mul`.
...
pub impl Matrix2x5MatrixIndexLinear<T, +Copy<T>, +Drop<T>> of MatrixIndex<Matrix2x5<T>, usize> {
    type Output = T;
    #[inline(always)]
    fn get(self: Matrix2x5<T>, index: usize) -> Option<T> {
        match index {
    ...
    #[inline(always)]
    fn index(self: Matrix2x5<T>, index: usize) -> T {
        match index {
            0 => self.m11,
            1 => self.m21,
            2 => self.m12,
...
pub impl Matrix2x5IndexLinear<T, +Copy<T>, +Drop<T>> of IndexView<Matrix2x5<T>, usize> {
    type Target = T;
    #[inline(always)]
    fn index(self: @Matrix2x5<T>, index: usize) -> T {
        match index {
...
pub impl Matrix2x5MatrixIndexPair<
    T, +Copy<T>, +Drop<T>,
> of MatrixIndex<Matrix2x5<T>, (usize, usize)> {
    type Output = T;
    #[inline(always)]
    fn get(self: Matrix2x5<T>, index: (usize, usize)) -> Option<T> {
        let (i, j) = index;
        match j {
            0 => match i {
                0 => Option::Some(self.m11),
    ...
    #[inline(always)]
    fn index(self: Matrix2x5<T>, index: (usize, usize)) -> T {
        let (i, j) = index;
        match j {
            0 => match i {
                0 => self.m11,
...
pub impl Matrix2x5IndexPair<T, +Copy<T>, +Drop<T>> of IndexView<Matrix2x5<T>, (usize, usize)> {
    type Target = T;
    #[inline(always)]
    fn index(self: @Matrix2x5<T>, index: (usize, usize)) -> T {
        let (i, j) = index;
...
pub impl Matrix2x5PartialOrd<T, +PartialOrd<T>, +Copy<T>, +Drop<T>> of PartialOrd<Matrix2x5<T>> {
    #[inline(always)]
    fn lt(lhs: Matrix2x5<T>, rhs: Matrix2x5<T>) -> bool {
        lhs.m11 < rhs.m11
            && lhs.m21 < rhs.m21
            && lhs.m12 < rhs.m12
            && lhs.m22 < rhs.m22
    ...
    #[inline(always)]
    fn le(lhs: Matrix2x5<T>, rhs: Matrix2x5<T>) -> bool {
        lhs.m11 <= rhs.m11
            && lhs.m21 <= rhs.m21
            && lhs.m12 <= rhs.m12
            && lhs.m22 <= rhs.m22
    ...
    #[inline(always)]
    fn gt(lhs: Matrix2x5<T>, rhs: Matrix2x5<T>) -> bool {
        lhs.m11 > rhs.m11
            && lhs.m21 > rhs.m21
            && lhs.m12 > rhs.m12
            && lhs.m22 > rhs.m22
    ...
    #[inline(always)]
    fn ge(lhs: Matrix2x5<T>, rhs: Matrix2x5<T>) -> bool {
        lhs.m11 >= rhs.m11
            && lhs.m21 >= rhs.m21
            && lhs.m12 >= rhs.m12
            && lhs.m22 >= rhs.m22
...
/// for Matrix`.
pub impl Matrix2x5Bounded<T, +Bounded<T>, +Drop<T>> of Bounded<Matrix2x5<T>> {
    const MIN: Matrix2x5<T> = Matrix2x5 {
        m11: Bounded::<T>::MIN,
        m21: Bounded::<T>::MIN,
        m12: Bounded::<T>::MIN,
        m22: Bounded::<T>::MIN,
    ...
    const MAX: Matrix2x5<T> = Matrix2x5 {
        m11: Bounded::<T>::MAX,
        m21: Bounded::<T>::MAX,
        m12: Bounded::<T>::MAX,
        m22: Bounded::<T>::MAX,
...
/// The 2x5 matrix of the given COLUMNS (`[[m11, m21, ..], [m12, ..], ..]`). Upstream: `From<[[T;
/// R]; C]>`.
pub impl Matrix2x5FromColumnArrays<T, +Drop<T>> of Into<[[T; 2]; 5], Matrix2x5<T>> {
    #[inline(always)]
    fn into(self: [[T; 2]; 5]) -> Matrix2x5<T> {
...
/// The columns of the 2x5 matrix as nested arrays (`[[m11, m21, ..], [m12, ..], ..]`). Upstream:
/// `Into<[[T; R]; C]>`.
pub impl Matrix2x5IntoColumnArrays<T, +Drop<T>> of Into<Matrix2x5<T>, [[T; 2]; 5]> {
    #[inline(always)]
    fn into(self: Matrix2x5<T>) -> [[T; 2]; 5] {
        let Matrix2x5 { m11, m21, m12, m22, m13, m23, m14, m24, m15, m25 } = self;
        [[m11, m21], [m12, m22], [m13, m23], [m14, m24], [m15, m25]]
    }
}

/// `self *= k` for a scalar `k`: `scale` in place, each component floored once. Panics on overflow.
/// Upstream: `MulAssign<T>`.
pub impl Matrix2x5MulAssignScalar<T, +Mul<T>, +Copy<T>, +Drop<T>> of MulAssign<Matrix2x5<T>, T> {
    #[inline(always)]
    fn mul_assign(ref self: Matrix2x5<T>, rhs: T) {
        self =
            Matrix2x5 {
                m11: self.m11 * rhs,
                m21: self.m21 * rhs,
...
pub impl Matrix2x5DivAssignScalar<
    T, impl R: Real<T>, +Copy<T>, +Drop<T>,
> of DivAssign<Matrix2x5<T>, T> {
    fn div_assign(ref self: Matrix2x5<T>, rhs: T) {
        let (m11, m21, m12, m22, m13, m23, m14, m24, m15) = R::div9(
            self.m11,
            self.m21,
            self.m12,
        ...
        let m25 = R::div(self.m25, rhs);
        self = Matrix2x5 { m11, m21, m12, m22, m13, m23, m14, m24, m15, m25 };
    }
}
...
#[generate_trait]
impl Matrix2x5EditImpl<T, +Copy<T>, +Drop<T>> of Matrix2x5EditTrait<T> {
    /// `self` with the component at `index` (`(row, column)`) replaced by `v`; panics out of
    /// bounds.
    #[inline(always)]
    fn replace(self: Matrix2x5<T>, index: (usize, usize), v: T) -> Matrix2x5<T> {
        let (i, j) = index;
        match j {
            0 => match i {
                0 => Matrix2x5 {
    ...
    #[inline(always)]
    fn row_at(self: Matrix2x5<T>, i: usize) -> RowVector5<T> {
        match i {
    ...
    #[inline(always)]
    fn column_at(self: Matrix2x5<T>, j: usize) -> Vector2<T> {
        match j {
...
/// `iter.sum()` of an iterator of `Matrix2x5`s: the first item plus the others, in order (exact;
/// panics on overflow); the zero 2x5 matrix when empty. Upstream: `Sum for Matrix` (a fold
/// from `zero()`: the same result, one addition more).
pub impl Matrix2x5Sum<
    T, impl R: Real<T>, +Add<T>, +Copy<T>, +Drop<T>,
> of core::iter::Sum<Matrix2x5<T>> {
    fn sum<I, +Iterator<I>[Item: Matrix2x5<T>], +Destruct<I>, +Destruct<Matrix2x5<T>>>(
        mut iter: I,
    ) -> Matrix2x5<T> {
...
/// `*iter.sum()` of an iterator of snapshots `@Matrix2x5` (`span.into_iter()`): a snapshot of the
/// sum of the items, like `Sum<Matrix2x5>`. Upstream: `Sum<&Matrix> for Matrix` (references).
pub impl Matrix2x5SumSnapshot<
    T, impl R: Real<T>, +Add<T>, +Copy<T>, +Drop<T>,
> of core::iter::Sum<@Matrix2x5<T>> {
    fn sum<I, +Iterator<I>[Item: @Matrix2x5<T>], +Destruct<I>, +Destruct<@Matrix2x5<T>>>(
        mut iter: I,
