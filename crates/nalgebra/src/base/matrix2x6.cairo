...
> of Matrix2x6Trait<T> {
    ...
    /// The 2x6 matrix whose components all equal `elem`. Upstream: `Matrix2x6::repeat`.
    #[inline(always)]
    fn repeat(elem: T) -> Matrix2x6<T> {
        Matrix2x6 {
            m11: elem,
            m21: elem,
            m12: elem,
    ...
    /// Alias of `repeat`. Upstream: `Matrix2x6::from_element`.
    #[inline(always)]
    fn from_element(elem: T) -> Matrix2x6<T> {
        Matrix2x6 {
            m11: elem,
            m21: elem,
            m12: elem,
    ...
    /// The 2x6 matrix whose component `(i, j)` (row, column, 0-based) is `f(i, j)`, `f` being
    ...
    /// The 2x6 matrix of the 12 values of `data`, in row-major order. Panics with `nalgebra: wrong
    /// slice length` unless `data.len() == 12`. Upstream: `Matrix2x6::from_row_slice` (`&[T]`).
    ///
    ...
    #[inline(always)]
    fn from_row_slice(data: Span<T>) -> Matrix2x6<T> {
    ...
    /// wrong slice length` unless `data.len() == 12`. Upstream: `Matrix2x6::from_column_slice`
    /// (`&[T]`).
    ///
    /// `data` is read as ONE fixed-size array (`Span -> @Box<[T; 12]>`, one length check): measured
    /// about 5 times cheaper than a bounds-checked `*data[k]` per component
    /// (`bench_matrix3_from_row_slice__alt_span_index`).
    #[inline(always)]
    fn from_column_slice(data: Span<T>) -> Matrix2x6<T> {
        let boxed: @Box<[T; 12]> = data.try_into().expect(errors::SLICE_LENGTH);
        let [v0, v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11] = boxed.unbox();
        Matrix2x6 {
            m11: v0,
    ...
    /// The 2x6 matrix whose first `data.len()` diagonal components are `data`, every other
    /// component zero. Panics with `nalgebra: diagonal too long` when `data.len() > 2`. Upstream:
    /// `Matrix2x6::from_partial_diagonal` (`&[T]`).
    #[inline(always)]
    fn from_partial_diagonal(data: Span<T>) -> Matrix2x6<T> {
        let len = data.len();
        if len > 2 {
    ...
    #[inline(always)]
    fn is_zero(self: Matrix2x6<T>) -> bool {
        self.m11 == R::zero()
            && self.m21 == R::zero()
            && self.m12 == R::zero()
            && self.m22 == R::zero()
    ...
    #[inline(always)]
    fn component_mul(self: Matrix2x6<T>, rhs: Matrix2x6<T>) -> Matrix2x6<T> {
        Matrix2x6 {
    ...
    #[inline(always)]
    fn component_mul_assign(ref self: Matrix2x6<T>, rhs: Matrix2x6<T>) {
        self =
            Matrix2x6 {
                m11: self.m11 * rhs.m11,
                m21: self.m21 * rhs.m21,
    ...
    #[inline(always)]
    fn component_div(self: Matrix2x6<T>, rhs: Matrix2x6<T>) -> Matrix2x6<T> {
        Matrix2x6 {
    ...
    #[inline(always)]
    fn component_div_assign(ref self: Matrix2x6<T>, rhs: Matrix2x6<T>) {
        self =
            Matrix2x6 {
                m11: R::div(self.m11, rhs.m11),
                m21: R::div(self.m21, rhs.m21),
    ...
    #[inline(always)]
    fn inf(self: Matrix2x6<T>, other: Matrix2x6<T>) -> Matrix2x6<T> {
        Matrix2x6 {
    ...
    #[inline(always)]
    fn sup(self: Matrix2x6<T>, other: Matrix2x6<T>) -> Matrix2x6<T> {
        Matrix2x6 {
    ...
    #[inline(always)]
    fn inf_sup(self: Matrix2x6<T>, other: Matrix2x6<T>) -> (Matrix2x6<T>, Matrix2x6<T>) {
        (Self::inf(self, other), Self::sup(self, other))
    }

    /// `self + k` added to every component. Exact; panics on overflow. Upstream: `add_scalar`.
    #[inline(always)]
    fn add_scalar(self: Matrix2x6<T>, k: T) -> Matrix2x6<T> {
        Matrix2x6 {
            m11: self.m11 + k,
            m21: self.m21 + k,
            m12: self.m12 + k,
    ...
    #[inline(always)]
    fn cmpy(ref self: Matrix2x6<T>, alpha: T, a: Matrix2x6<T>, b: Matrix2x6<T>, beta: T) {
        self =
            Matrix2x6 {
    ...
    #[inline(always)]
    fn cdpy(ref self: Matrix2x6<T>, alpha: T, a: Matrix2x6<T>, b: Matrix2x6<T>, beta: T) {
        self =
            Matrix2x6 {
                        ...
                        R::min(
                            R::min(
                                R::min(
                                    R::min(
                                        R::min(
                                            R::min(R::min(self.m11, self.m21), self.m12), self.m22,
                                        ),
                                        self.m13,
                                    ),
                                    self.m23,
                        ...
                        R::max(
                            R::max(
                                R::max(
                                    R::max(
                                        R::max(
                                            R::max(R::max(self.m11, self.m21), self.m12), self.m22,
                                        ),
                                        self.m13,
                                    ),
                                    self.m23,
    ...
    fn iamax_full(self: Matrix2x6<T>) -> (usize, usize) {
    ...
    #[inline(always)]
    fn icamax_full(self: Matrix2x6<T>) -> (usize, usize) {
    ...
    fn dot(self: Matrix2x6<T>, rhs: Matrix2x6<T>) -> T {
    ...
    /// `norm_squared`.
    fn norm_squared(self: Matrix2x6<T>) -> T {
        ...
        R::wide_rescale(R::wide_add_prod(w, self.m26, self.m26))
    }
    ...
    fn norm(self: Matrix2x6<T>) -> T {
        ...
        let w = R::wide_add_prod(w, self.m25, self.m25);
        let w = R::wide_add_prod(w, self.m16, self.m16);
        R::wide_sqrt(R::wide_add_prod(w, self.m26, self.m26))
    }

    /// Alias of `norm_squared`. Upstream: `magnitude_squared`.
    #[inline(always)]
    fn magnitude_squared(self: Matrix2x6<T>) -> T {
        Self::norm_squared(self)
    }

    /// Alias of `norm`. Upstream: `magnitude`.
    #[inline(always)]
    fn magnitude(self: Matrix2x6<T>) -> T {
        Self::norm(self)
    }
    ...
    fn metric_distance(self: Matrix2x6<T>, rhs: Matrix2x6<T>) -> T {
        ...
        R::wide_sqrt(R::wide_add_prod(w, self.m26 - rhs.m26, self.m26 - rhs.m26))
    }
    ...
    fn unscale(self: Matrix2x6<T>, k: T) -> Matrix2x6<T> {
        ...
        let (m25, m16, m26) = R::div3(self.m25, self.m16, self.m26, k);
        Matrix2x6 { m11, m21, m12, m22, m13, m23, m14, m24, m15, m25, m16, m26 }
    }
    ...
    fn normalize(self: Matrix2x6<T>) -> Matrix2x6<T> {
    ...
    fn try_normalize(self: Matrix2x6<T>, min_norm: T) -> Option<Matrix2x6<T>> {
    ...
    /// `cap_magnitude`.
    fn cap_magnitude(self: Matrix2x6<T>, max: T) -> Matrix2x6<T> {
        let n = Self::norm(self);
        if n <= max {
            self
        } else {
    ...
    fn try_set_magnitude(ref self: Matrix2x6<T>, magnitude: T, min_magnitude: T) {
        let n = Self::norm(self);
        if n > min_magnitude {
    ...
    #[inline(always)]
    fn one_norm(self: Matrix2x6<T>) -> T {
        R::max(
            R::max(
                R::max(
                    R::max(
                        R::max(
                            ...
                            R::abs(self.m12) + R::abs(self.m22),
                        ),
                        R::abs(self.m13) + R::abs(self.m23),
                    ),
                    R::abs(self.m14) + R::abs(self.m24),
                ),
                R::abs(self.m15) + R::abs(self.m25),
            ),
            R::abs(self.m16) + R::abs(self.m26),
        )
    }

    /// The conjugate transpose, a `Matrix6x2`: the transpose for a real scalar. Exact. Upstream:
    /// `adjoint`.
    #[inline(always)]
    fn adjoint(self: Matrix2x6<T>) -> Matrix6x2<T> {
        Self::transpose(self)
    }

    /// Alias of `adjoint` (deprecated upstream). Upstream: `conjugate_transpose`.
    #[inline(always)]
    fn conjugate_transpose(self: Matrix2x6<T>) -> Matrix6x2<T> {
        Self::transpose(self)
    }

    /// The component-wise conjugate: `self` for a real scalar. Upstream: `conjugate`.
    #[inline(always)]
    fn conjugate(self: Matrix2x6<T>) -> Matrix2x6<T> {
        self
    }
    ...
    fn cast<U, +Into<T, U>, +Drop<U>>(self: Matrix2x6<T>) -> Matrix2x6<U> {
        Matrix2x6 {
            m11: self.m11.into(),
            m21: self.m21.into(),
            m12: self.m12.into(),
    ...
    fn try_cast<U, +TryInto<T, U>, +Drop<U>>(self: Matrix2x6<T>) -> Option<Matrix2x6<U>> {
        ...
        Option::Some(Matrix2x6 { m11, m21, m12, m22, m13, m23, m14, m24, m15, m25, m16, m26 })
    }
    ...
    /// (DESIGN D3).
    #[inline(always)]
    fn relative_eq(self: Matrix2x6<T>, other: Matrix2x6<T>, epsilon: u64, max_relative: T) -> bool {
    ...
    #[inline(always)]
    fn ulps_eq(self: Matrix2x6<T>, other: Matrix2x6<T>, epsilon: u64, max_ulps: u32) -> bool {
    ...
    /// The 2x6 matrix of `f(x)` for every component `x` (called in column-major order). `f` is any
    ...
    /// The 2x6 matrix of `f(i, j, x)` for every component `x` at row `i`, column `j` (0-based,
    ...
    /// The 2x6 matrix of `f(a, b)` for the components `a` of `self` and `b` of `rhs` at the same
    ...
    fn zip_map<
        ...
        +Drop<Func::Output>,
    >(
        self: Matrix2x6<T>, rhs: Matrix2x6<T2>, f: F,
    ) -> Matrix2x6<Func::Output> {
        Matrix2x6 {
    ...
    /// The 2x6 matrix of `f(a, b, c)` for the components of `self`, `b` and `c` at the same
    /// position. `f` is any closure or `Fn` value; Cairo closures take their arguments by value and
    ...
    fn zip_zip_map<
        ...
        +Drop<Func::Output>,
    >(
        self: Matrix2x6<T>, b: Matrix2x6<T2>, c: Matrix2x6<T3>, f: F,
    ) -> Matrix2x6<Func::Output> {
        Matrix2x6 {
    ...
    >(
        self: Matrix2x6<T>, rhs: Matrix2x6<T2>, init: Acc, f: F,
    ) -> Acc {
    ...
    fn zip_apply<
        ...
        +Into<Func::Output, T>,
    >(
        ref self: Matrix2x6<T>, rhs: Matrix2x6<T2>, f: F,
    ) {
        self =
            Matrix2x6 {
    ...
    fn zip_zip_apply<
        ...
        +Into<Func::Output, T>,
    >(
        ref self: Matrix2x6<T>, b: Matrix2x6<T2>, c: Matrix2x6<T3>, f: F,
    ) {
        self =
            Matrix2x6 {
    ...
    #[inline(always)]
    fn fill(ref self: Matrix2x6<T>, val: T) {
        self =
            Matrix2x6 {
                m11: val,
                m21: val,
    ...
    #[inline(always)]
    fn fill_diagonal(ref self: Matrix2x6<T>, val: T) {
        self =
            Matrix2x6 {
                m11: val,
                m21: self.m21,
    ...
    #[inline(always)]
    fn fill_with_identity(ref self: Matrix2x6<T>) {
        self =
            Matrix2x6 {
                m11: R::one(),
                m21: R::zero(),
    ...
    #[inline(always)]
    fn fill_row(ref self: Matrix2x6<T>, i: usize, val: T) {
        self = match i {
            0 => Matrix2x6 {
                m11: val,
                m21: self.m21,
    ...
    #[inline(always)]
    fn fill_column(ref self: Matrix2x6<T>, j: usize, val: T) {
        self = match j {
            0 => Matrix2x6 {
                m11: val,
                m21: val,
    ...
    #[inline(always)]
    fn fill_lower_triangle(ref self: Matrix2x6<T>, val: T, shift: usize) {
        self = match shift {
            0 => Matrix2x6 {
                m11: val,
                m21: val,
    ...
    #[inline(always)]
    fn fill_upper_triangle(ref self: Matrix2x6<T>, val: T, shift: usize) {
        self = match shift {
            0 => Matrix2x6 {
                m11: val,
                m21: self.m21,
    ...
    #[inline(always)]
    fn set_row(ref self: Matrix2x6<T>, i: usize, row: RowVector6<T>) {
        self = match i {
            0 => Matrix2x6 {
                m11: row.x,
                m21: self.m21,
    ...
    #[inline(always)]
    fn set_column(ref self: Matrix2x6<T>, j: usize, column: Vector2<T>) {
        self = match j {
            0 => Matrix2x6 {
                m11: column.x,
                m21: column.y,
    ...
    #[inline(always)]
    fn set_diagonal(ref self: Matrix2x6<T>, diag: Vector2<T>) {
        self =
            Matrix2x6 {
                m11: diag.x,
                m21: self.m21,
    ...
    #[inline(always)]
    fn set_partial_diagonal(ref self: Matrix2x6<T>, diag: Span<T>) {
        let len = diag.len();
        self =
            Matrix2x6 {
                m11: if len > 0 {
    ...
    /// Sets `self` to `other` (a `Matrix2x6`: upstream requires the same shape). Upstream:
    /// `copy_from`.
    #[inline(always)]
    fn copy_from(ref self: Matrix2x6<T>, other: Matrix2x6<T>) {
        self = other;
    }
    ...
    /// (`&[T]`).
    #[inline(always)]
    fn copy_from_slice(ref self: Matrix2x6<T>, slice: Span<T>) {
        self = Self::from_column_slice(slice);
    }

    /// Sets `self` to the transpose of `other`, a `Matrix6x2`. Exact. Upstream: `tr_copy_from`.
    #[inline(always)]
    fn tr_copy_from(ref self: Matrix2x6<T>, other: Matrix6x2<T>) {
        self =
            Matrix2x6 {
                m11: other.m11,
                m21: other.m12,
    ...
    #[inline]
    fn swap(ref self: Matrix2x6<T>, row_cols1: (usize, usize), row_cols2: (usize, usize)) {
        let a = MatrixIndex::index(self, row_cols1);
        let b = MatrixIndex::index(self, row_cols2);
        self =
            Matrix2x6EditTrait::replace(
                Matrix2x6EditTrait::replace(self, row_cols1, b), row_cols2, a,
            );
    }
    ...
    /// benchmark.
    #[inline]
    fn swap_rows(ref self: Matrix2x6<T>, irow1: usize, irow2: usize) {
        let a = Matrix2x6EditTrait::row_at(self, irow1);
        let b = Matrix2x6EditTrait::row_at(self, irow2);
    ...
    #[inline]
    fn swap_columns(ref self: Matrix2x6<T>, icol1: usize, icol2: usize) {
        let a = Matrix2x6EditTrait::column_at(self, icol1);
        let b = Matrix2x6EditTrait::column_at(self, icol2);
    ...
    #[inline(always)]
    fn neg_mut(ref self: Matrix2x6<T>) {
        self = -self;
    }
    ...
    /// `scale_mut`.
    #[inline(always)]
    fn scale_mut(ref self: Matrix2x6<T>, k: T) {
    ...
    #[inline(always)]
    fn unscale_mut(ref self: Matrix2x6<T>, k: T) {
    ...
    #[inline(always)]
    fn normalize_mut(ref self: Matrix2x6<T>) -> T {
        let n = Self::norm(self);
    ...
    #[inline(always)]
    fn try_normalize_mut(ref self: Matrix2x6<T>, min_norm: T) -> Option<T> {
        let n = Self::norm(self);
        if n <= min_norm {
            return None;
        }
    ...
    #[inline(always)]
    fn set_magnitude(ref self: Matrix2x6<T>, magnitude: T) {
        let n = Self::norm(self);
        self = Self::scale(self, R::div(magnitude, n));
    }
    ...
    /// `conjugate_mut`.
    #[inline(always)]
    fn conjugate_mut(ref self: Matrix2x6<T>) {
        self = Self::conjugate(self);
    }

    /// Writes the transpose of `self` into `out`, a `Matrix6x2`. Exact. Upstream: `transpose_to`.
    #[inline(always)]
    fn transpose_to(self: Matrix2x6<T>, ref out: Matrix6x2<T>) {
        out = Self::transpose(self);
    }

    /// Writes the adjoint of `self` (its transpose for a real scalar) into `out`, a `Matrix6x2`.
    /// Exact. Upstream: `adjoint_to`.
    #[inline(always)]
    fn adjoint_to(self: Matrix2x6<T>, ref out: Matrix6x2<T>) {
        out = Self::transpose(self);
    }

    /// Writes the adjoint of `self` (deprecated upstream alias of `adjoint_to`) into `out`, a
    /// `Matrix6x2`. Exact. Upstream: `conjugate_transpose_to`.
    #[inline(always)]
    fn conjugate_transpose_to(self: Matrix2x6<T>, ref out: Matrix6x2<T>) {
        out = Self::transpose(self);
    }

    /// Writes the sum `self + rhs` into `out`. Exact; panics on overflow. Upstream: `add_to`.
    #[inline(always)]
    fn add_to(self: Matrix2x6<T>, rhs: Matrix2x6<T>, ref out: Matrix2x6<T>) {
        out = self + rhs;
    }
    ...
    /// `sub_to`.
    #[inline(always)]
    fn sub_to(self: Matrix2x6<T>, rhs: Matrix2x6<T>, ref out: Matrix2x6<T>) {
        out = self - rhs;
    }
    ...
    #[inline]
    fn apply_norm<N, +Drop<N>, impl Nm: Norm<N, Matrix2x6<T>, T>>(
        self: Matrix2x6<T>, norm: N,
    ) -> T {
        Nm::norm(@norm, self)
    }
    ...
    #[inline]
    fn apply_metric_distance<N, +Drop<N>, impl Nm: Norm<N, Matrix2x6<T>, T>>(
        self: Matrix2x6<T>, rhs: Matrix2x6<T>, norm: N,
    ) -> T {
    ...
    #[inline(always)]
    fn nrows(self: Matrix2x6<T>) -> usize {
        2
    }

    /// The number of columns, 6. Upstream: `ncols`.
    #[inline(always)]
    fn ncols(self: Matrix2x6<T>) -> usize {
        6
    }

    /// `(nrows, ncols)`, `(2, 6)`. Upstream: `shape`.
    #[inline(always)]
    fn shape(self: Matrix2x6<T>) -> (usize, usize) {
        (2, 6)
    }

    /// Whether the shape is square: `false`. Upstream: `is_square`.
    #[inline(always)]
    fn is_square(self: Matrix2x6<T>) -> bool {
        false
    }
    ...
    #[inline(always)]
    fn vector_to_matrix_index(self: Matrix2x6<T>, i: usize) -> (usize, usize) {
    ...
    #[inline(always)]
    fn row(self: Matrix2x6<T>, i: usize) -> RowVector6<T> {
        Matrix2x6EditTrait::row_at(self, i)
    }
    ...
    #[inline(always)]
    fn column(self: Matrix2x6<T>, j: usize) -> Vector2<T> {
        Matrix2x6EditTrait::column_at(self, j)
    }
    ...
    #[inline(always)]
    fn upper_triangle(self: Matrix2x6<T>) -> Matrix2x6<T> {
        Matrix2x6 {
            m11: self.m11,
            m21: R::zero(),
            m12: self.m12,
    ...
    #[inline(always)]
    fn lower_triangle(self: Matrix2x6<T>) -> Matrix2x6<T> {
        Matrix2x6 {
            m11: self.m11,
            m21: self.m21,
            m12: R::zero(),
    ...
    /// The 2x6 matrix whose 2 rows are the given vectors, each a `Vector6` of the row's 6
    ...
    #[inline(always)]
    fn from_rows(r1: Vector6<T>, r2: Vector6<T>) -> Matrix2x6<T> {
        Matrix2x6 {
            m11: r1.x,
            m21: r2.x,
            m12: r1.y,
    ...
    /// The 2x6 matrix whose 6 columns are the given `Vector2`s. Exact. Upstream: `from_columns` (a
    ...
    #[inline(always)]
    fn is_orthogonal(self: Matrix2x6<T>, ulps: u64) -> bool {
        Matrix6Trait::is_identity(MatrixTrMul::tr_mul(self, self), ulps)
    }

    /// `self` with a row of `val` inserted at index `i` (`i <= 2`), a `Matrix3x6`. Panics with
    /// `nalgebra: index out of bounds` for `i > 2`. Upstream: `insert_row`.
    fn insert_row(self: Matrix2x6<T>, i: usize, val: T) -> Matrix3x6<T> {
        match i {
            0 => Matrix3x6 {
                m11: val,
                m21: self.m11,
    ...
    /// `self` without its row `i`, a `RowVector6`. Panics with `nalgebra: index out of bounds` for
    /// `i >= 2`. Upstream: `remove_row`.
    #[inline(always)]
    fn remove_row(self: Matrix2x6<T>, i: usize) -> RowVector6<T> {
        match i {
            0 => RowVector6 {
    ...
    /// `self` without its column `i`, a `Matrix2x5`. Panics with `nalgebra: index out of bounds`
    /// for `i >= 6`. Upstream: `remove_column`.
    #[inline(always)]
    fn remove_column(self: Matrix2x6<T>, i: usize) -> Matrix2x5<T> {
        match i {
            0 => Matrix2x5 {
                m11: self.m12,
                m21: self.m22,
...
/// The operations of `Matrix2x6<T>` that need `Transcendental` (inverse trigonometry, `exp`, `ln`):
/// a scalar may implement `Real` only.
#[generate_trait]
pub impl Matrix2x6AngleImpl<
    T,
    impl R: Real<T>,
...
> of Matrix2x6AngleTrait<T> {
    ...
    fn angle(self: Matrix2x6<T>, other: Matrix2x6<T>) -> T {
        let n1 = Matrix2x6Trait::norm(self);
        let n2 = Matrix2x6Trait::norm(other);
        if n1 == R::zero() || n2 == R::zero() {
            return R::zero();
        }
        let u = Matrix2x6Trait::unscale(self, n1);
        let v = Matrix2x6Trait::unscale(other, n2);
        let half = Tr::atan2(Matrix2x6Trait::metric_distance(u, v), Matrix2x6Trait::norm(u + v));
        half + half
    }
    ...
    /// Upstream: `lp_norm`.
    fn lp_norm(self: Matrix2x6<T>, p: i32) -> T {
        if p < 1 {
        ...
        if p == 2 {
            return Matrix2x6Trait::norm(self);
        }
        let m = Matrix2x6Trait::amax(self);
        if m == R::zero() {
            return R::zero();
        }
        let r = Matrix2x6Trait::unscale(Matrix2x6Trait::abs(self), m);
...
/// `selfᵀ * rhs`, a `Vector6`: `mul_mat` of the transposed components, so one `sum_prod2` per
...
/// `selfᵀ * rhs`, a `Matrix6x2`: `mul_mat` of the transposed components, so one `sum_prod2` per
/// component (one rounding each); bit-identical to `self.transpose().mul_mat(rhs)`, at the same gas
/// (the transpose only relabels values). Panics on overflow. Upstream: `tr_mul`.
...
/// `selfᵀ * rhs`, a `Matrix6x3`: `mul_mat` of the transposed components, so one `sum_prod2` per
/// component (one rounding each); bit-identical to `self.transpose().mul_mat(rhs)`, at the same gas
/// (the transpose only relabels values). Panics on overflow. Upstream: `tr_mul`.
...
/// `selfᵀ * rhs`, a `Matrix6x4`: `mul_mat` of the transposed components, so one `sum_prod2` per
/// component (one rounding each); bit-identical to `self.transpose().mul_mat(rhs)`, at the same gas
/// (the transpose only relabels values). Panics on overflow. Upstream: `tr_mul`.
...
/// `selfᵀ * rhs`, a `Matrix6x5`: `mul_mat` of the transposed components, so one `sum_prod2` per
/// component (one rounding each); bit-identical to `self.transpose().mul_mat(rhs)`, at the same gas
/// (the transpose only relabels values). Panics on overflow. Upstream: `tr_mul`.
...
/// `selfᵀ * rhs`, a `Matrix6`: `mul_mat` of the transposed components, so one `sum_prod2` per
/// component (one rounding each); bit-identical to `self.transpose().mul_mat(rhs)`, at the same gas
/// (the transpose only relabels values). Panics on overflow. Upstream: `tr_mul`.
...
pub impl Matrix2x6MatrixIndexLinear<T, +Copy<T>, +Drop<T>> of MatrixIndex<Matrix2x6<T>, usize> {
    type Output = T;
    #[inline(always)]
    fn get(self: Matrix2x6<T>, index: usize) -> Option<T> {
        match index {
            ...
            6 => Option::Some(self.m14),
            7 => Option::Some(self.m24),
            8 => Option::Some(self.m15),
            9 => Option::Some(self.m25),
            10 => Option::Some(self.m16),
            11 => Option::Some(self.m26),
            _ => Option::None,
        }
    }
    #[inline(always)]
    fn index(self: Matrix2x6<T>, index: usize) -> T {
        match index {
            0 => self.m11,
            1 => self.m21,
            2 => self.m12,
...
pub impl Matrix2x6IndexLinear<T, +Copy<T>, +Drop<T>> of IndexView<Matrix2x6<T>, usize> {
    type Target = T;
    #[inline(always)]
    fn index(self: @Matrix2x6<T>, index: usize) -> T {
        match index {
...
pub impl Matrix2x6MatrixIndexPair<
    T, +Copy<T>, +Drop<T>,
> of MatrixIndex<Matrix2x6<T>, (usize, usize)> {
    type Output = T;
    #[inline(always)]
    fn get(self: Matrix2x6<T>, index: (usize, usize)) -> Option<T> {
        let (i, j) = index;
        match j {
            0 => match i {
                0 => Option::Some(self.m11),
    ...
    #[inline(always)]
    fn index(self: Matrix2x6<T>, index: (usize, usize)) -> T {
        let (i, j) = index;
        match j {
            0 => match i {
                0 => self.m11,
...
pub impl Matrix2x6IndexPair<T, +Copy<T>, +Drop<T>> of IndexView<Matrix2x6<T>, (usize, usize)> {
    type Target = T;
    #[inline(always)]
    fn index(self: @Matrix2x6<T>, index: (usize, usize)) -> T {
        let (i, j) = index;
...
pub impl Matrix2x6PartialOrd<T, +PartialOrd<T>, +Copy<T>, +Drop<T>> of PartialOrd<Matrix2x6<T>> {
    #[inline(always)]
    fn lt(lhs: Matrix2x6<T>, rhs: Matrix2x6<T>) -> bool {
        lhs.m11 < rhs.m11
            && lhs.m21 < rhs.m21
            && lhs.m12 < rhs.m12
            && lhs.m22 < rhs.m22
    ...
    #[inline(always)]
    fn le(lhs: Matrix2x6<T>, rhs: Matrix2x6<T>) -> bool {
        lhs.m11 <= rhs.m11
            && lhs.m21 <= rhs.m21
            && lhs.m12 <= rhs.m12
            && lhs.m22 <= rhs.m22
    ...
    #[inline(always)]
    fn gt(lhs: Matrix2x6<T>, rhs: Matrix2x6<T>) -> bool {
        lhs.m11 > rhs.m11
            && lhs.m21 > rhs.m21
            && lhs.m12 > rhs.m12
            && lhs.m22 > rhs.m22
    ...
    #[inline(always)]
    fn ge(lhs: Matrix2x6<T>, rhs: Matrix2x6<T>) -> bool {
        lhs.m11 >= rhs.m11
            && lhs.m21 >= rhs.m21
            && lhs.m12 >= rhs.m12
            && lhs.m22 >= rhs.m22
...
/// for Matrix`.
pub impl Matrix2x6Bounded<T, +Bounded<T>, +Drop<T>> of Bounded<Matrix2x6<T>> {
    const MIN: Matrix2x6<T> = Matrix2x6 {
        m11: Bounded::<T>::MIN,
        m21: Bounded::<T>::MIN,
        m12: Bounded::<T>::MIN,
        m22: Bounded::<T>::MIN,
    ...
    const MAX: Matrix2x6<T> = Matrix2x6 {
        m11: Bounded::<T>::MAX,
        m21: Bounded::<T>::MAX,
        m12: Bounded::<T>::MAX,
        m22: Bounded::<T>::MAX,
...
/// The 2x6 matrix of the given COLUMNS (`[[m11, m21, ..], [m12, ..], ..]`). Upstream: `From<[[T;
/// R]; C]>`.
pub impl Matrix2x6FromColumnArrays<T, +Drop<T>> of Into<[[T; 2]; 6], Matrix2x6<T>> {
    #[inline(always)]
    fn into(self: [[T; 2]; 6]) -> Matrix2x6<T> {
...
/// The columns of the 2x6 matrix as nested arrays (`[[m11, m21, ..], [m12, ..], ..]`). Upstream:
/// `Into<[[T; R]; C]>`.
pub impl Matrix2x6IntoColumnArrays<T, +Drop<T>> of Into<Matrix2x6<T>, [[T; 2]; 6]> {
    #[inline(always)]
...
#[generate_trait]
impl Matrix2x6EditImpl<T, +Copy<T>, +Drop<T>> of Matrix2x6EditTrait<T> {
...
/// The 1 consecutive rows of a `Matrix2x6` as a `RowVector6`. Upstream: `fixed_rows::<1>`, `rows(i,
/// 1)`, `rows_range`, `select_rows`.
pub impl Matrix2x6FixedRowsRowVector6<
    T, +Copy<T>, +Drop<T>,
> of FixedRows<Matrix2x6<T>, RowVector6<T>> {
    #[inline(always)]
    fn fixed_rows(self: Matrix2x6<T>, i: usize) -> RowVector6<T> {
        match i {
            0 => RowVector6 {
                x: self.m11, y: self.m12, z: self.m13, w: self.m14, a: self.m15, b: self.m16,
            },
    ...
    #[inline(always)]
    fn rows(self: Matrix2x6<T>, first_row: usize, nrows: usize) -> RowVector6<T> {
        if nrows != 1 {
    ...
    #[inline(always)]
    fn rows_range(self: Matrix2x6<T>, rows: Range<usize>) -> RowVector6<T> {
    ...
    #[inline(always)]
    fn select_rows(self: Matrix2x6<T>, irows: Span<usize>) -> RowVector6<T> {
        if irows.len() != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let r0 = Matrix2x6EditTrait::row_at(self, *irows[0]);
        RowVector6 { x: r0.x, y: r0.y, z: r0.z, w: r0.w, a: r0.a, b: r0.b }
    }
}

/// The 2 consecutive rows of a `Matrix2x6` as a `Matrix2x6`. Upstream: `fixed_rows::<2>`, `rows(i,
/// 2)`, `rows_range`, `select_rows`.
pub impl Matrix2x6FixedRowsMatrix2x6<
    T, +Copy<T>, +Drop<T>,
> of FixedRows<Matrix2x6<T>, Matrix2x6<T>> {
    #[inline(always)]
    fn fixed_rows(self: Matrix2x6<T>, i: usize) -> Matrix2x6<T> {
        match i {
            0 => Matrix2x6 {
                m11: self.m11,
                m21: self.m21,
    ...
    #[inline(always)]
    fn rows(self: Matrix2x6<T>, first_row: usize, nrows: usize) -> Matrix2x6<T> {
        if nrows != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_rows(self, first_row)
    ...
    #[inline(always)]
    fn rows_range(self: Matrix2x6<T>, rows: Range<usize>) -> Matrix2x6<T> {
        let Range { start, end } = rows;
        if end != start + 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn select_rows(self: Matrix2x6<T>, irows: Span<usize>) -> Matrix2x6<T> {
        if irows.len() != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let r0 = Matrix2x6EditTrait::row_at(self, *irows[0]);
        let r1 = Matrix2x6EditTrait::row_at(self, *irows[1]);
        Matrix2x6 {
            m11: r0.x,
            m21: r1.x,
            m12: r0.y,
...
/// The 1 consecutive columns of a `Matrix2x6` as a `Vector2`. Upstream: `fixed_columns::<1>`,
/// `columns(j, 1)`, `columns_range`, `select_columns`.
pub impl Matrix2x6FixedColumnsVector2<
    T, +Copy<T>, +Drop<T>,
> of FixedColumns<Matrix2x6<T>, Vector2<T>> {
    #[inline(always)]
    fn fixed_columns(self: Matrix2x6<T>, i: usize) -> Vector2<T> {
        match i {
            0 => Vector2 { x: self.m11, y: self.m21 },
            1 => Vector2 { x: self.m12, y: self.m22 },
            2 => Vector2 { x: self.m13, y: self.m23 },
    ...
    #[inline(always)]
    fn columns(self: Matrix2x6<T>, first_col: usize, ncols: usize) -> Vector2<T> {
        if ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn columns_range(self: Matrix2x6<T>, cols: Range<usize>) -> Vector2<T> {
    ...
    #[inline(always)]
    fn select_columns(self: Matrix2x6<T>, icols: Span<usize>) -> Vector2<T> {
        if icols.len() != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let c0 = Matrix2x6EditTrait::column_at(self, *icols[0]);
        Vector2 { x: c0.x, y: c0.y }
    }
}

/// The 2 consecutive columns of a `Matrix2x6` as a `Matrix2`. Upstream: `fixed_columns::<2>`,
/// `columns(j, 2)`, `columns_range`, `select_columns`.
pub impl Matrix2x6FixedColumnsMatrix2<
    T, +Copy<T>, +Drop<T>,
> of FixedColumns<Matrix2x6<T>, Matrix2<T>> {
    #[inline(always)]
    fn fixed_columns(self: Matrix2x6<T>, i: usize) -> Matrix2<T> {
        match i {
            ...
            4 => Matrix2 { m11: self.m15, m21: self.m25, m12: self.m16, m22: self.m26 },
            _ => core::panic_with_felt252(errors::INDEX_OUT_OF_BOUNDS),
        }
    }

    #[inline(always)]
    fn columns(self: Matrix2x6<T>, first_col: usize, ncols: usize) -> Matrix2<T> {
        if ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix2x6<T>, cols: Range<usize>) -> Matrix2<T> {
        let Range { start, end } = cols;
        if end != start + 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn select_columns(self: Matrix2x6<T>, icols: Span<usize>) -> Matrix2<T> {
        if icols.len() != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let c0 = Matrix2x6EditTrait::column_at(self, *icols[0]);
        let c1 = Matrix2x6EditTrait::column_at(self, *icols[1]);
        Matrix2 { m11: c0.x, m21: c0.y, m12: c1.x, m22: c1.y }
    }
}

/// The 3 consecutive columns of a `Matrix2x6` as a `Matrix2x3`. Upstream: `fixed_columns::<3>`,
/// `columns(j, 3)`, `columns_range`, `select_columns`.
pub impl Matrix2x6FixedColumnsMatrix2x3<
    T, +Copy<T>, +Drop<T>,
> of FixedColumns<Matrix2x6<T>, Matrix2x3<T>> {
    #[inline(always)]
    fn fixed_columns(self: Matrix2x6<T>, i: usize) -> Matrix2x3<T> {
        match i {
            0 => Matrix2x3 {
                m11: self.m11,
                m21: self.m21,
    ...
    #[inline(always)]
    fn columns(self: Matrix2x6<T>, first_col: usize, ncols: usize) -> Matrix2x3<T> {
        if ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix2x6<T>, cols: Range<usize>) -> Matrix2x3<T> {
        let Range { start, end } = cols;
        if end != start + 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn select_columns(self: Matrix2x6<T>, icols: Span<usize>) -> Matrix2x3<T> {
        if icols.len() != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let c0 = Matrix2x6EditTrait::column_at(self, *icols[0]);
        let c1 = Matrix2x6EditTrait::column_at(self, *icols[1]);
        let c2 = Matrix2x6EditTrait::column_at(self, *icols[2]);
        Matrix2x3 { m11: c0.x, m21: c0.y, m12: c1.x, m22: c1.y, m13: c2.x, m23: c2.y }
    }
}

/// The 4 consecutive columns of a `Matrix2x6` as a `Matrix2x4`. Upstream: `fixed_columns::<4>`,
/// `columns(j, 4)`, `columns_range`, `select_columns`.
pub impl Matrix2x6FixedColumnsMatrix2x4<
    T, +Copy<T>, +Drop<T>,
> of FixedColumns<Matrix2x6<T>, Matrix2x4<T>> {
    #[inline(always)]
    fn fixed_columns(self: Matrix2x6<T>, i: usize) -> Matrix2x4<T> {
        match i {
            0 => Matrix2x4 {
                m11: self.m11,
                m21: self.m21,
    ...
    #[inline(always)]
    fn columns(self: Matrix2x6<T>, first_col: usize, ncols: usize) -> Matrix2x4<T> {
        if ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix2x6<T>, cols: Range<usize>) -> Matrix2x4<T> {
        let Range { start, end } = cols;
        if end != start + 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn select_columns(self: Matrix2x6<T>, icols: Span<usize>) -> Matrix2x4<T> {
        if icols.len() != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let c0 = Matrix2x6EditTrait::column_at(self, *icols[0]);
        let c1 = Matrix2x6EditTrait::column_at(self, *icols[1]);
        let c2 = Matrix2x6EditTrait::column_at(self, *icols[2]);
        let c3 = Matrix2x6EditTrait::column_at(self, *icols[3]);
        Matrix2x4 {
...
/// The 5 consecutive columns of a `Matrix2x6` as a `Matrix2x5`. Upstream: `fixed_columns::<5>`,
/// `columns(j, 5)`, `columns_range`, `select_columns`.
pub impl Matrix2x6FixedColumnsMatrix2x5<
    T, +Copy<T>, +Drop<T>,
> of FixedColumns<Matrix2x6<T>, Matrix2x5<T>> {
    #[inline(always)]
    fn fixed_columns(self: Matrix2x6<T>, i: usize) -> Matrix2x5<T> {
        match i {
            0 => Matrix2x5 {
                m11: self.m11,
                m21: self.m21,
    ...
    #[inline(always)]
    fn columns(self: Matrix2x6<T>, first_col: usize, ncols: usize) -> Matrix2x5<T> {
        if ncols != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix2x6<T>, cols: Range<usize>) -> Matrix2x5<T> {
        let Range { start, end } = cols;
        if end != start + 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn select_columns(self: Matrix2x6<T>, icols: Span<usize>) -> Matrix2x5<T> {
        if icols.len() != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let c0 = Matrix2x6EditTrait::column_at(self, *icols[0]);
        ...
        let c2 = Matrix2x6EditTrait::column_at(self, *icols[2]);
        let c3 = Matrix2x6EditTrait::column_at(self, *icols[3]);
        let c4 = Matrix2x6EditTrait::column_at(self, *icols[4]);
        Matrix2x5 {
            m11: c0.x,
            m21: c0.y,
            m12: c1.x,
...
/// The 6 consecutive columns of a `Matrix2x6` as a `Matrix2x6`. Upstream: `fixed_columns::<6>`,
/// `columns(j, 6)`, `columns_range`, `select_columns`.
pub impl Matrix2x6FixedColumnsMatrix2x6<
    T, +Copy<T>, +Drop<T>,
> of FixedColumns<Matrix2x6<T>, Matrix2x6<T>> {
    #[inline(always)]
    fn fixed_columns(self: Matrix2x6<T>, i: usize) -> Matrix2x6<T> {
        match i {
            0 => Matrix2x6 {
                m11: self.m11,
                m21: self.m21,
    ...
    #[inline(always)]
    fn columns(self: Matrix2x6<T>, first_col: usize, ncols: usize) -> Matrix2x6<T> {
        if ncols != 6 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix2x6<T>, cols: Range<usize>) -> Matrix2x6<T> {
        let Range { start, end } = cols;
        if end != start + 6 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn select_columns(self: Matrix2x6<T>, icols: Span<usize>) -> Matrix2x6<T> {
        if icols.len() != 6 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let c0 = Matrix2x6EditTrait::column_at(self, *icols[0]);
        ...
        let c3 = Matrix2x6EditTrait::column_at(self, *icols[3]);
        let c4 = Matrix2x6EditTrait::column_at(self, *icols[4]);
        let c5 = Matrix2x6EditTrait::column_at(self, *icols[5]);
        Matrix2x6 {
            m11: c0.x,
            m21: c0.y,
            m12: c1.x,
...
/// The 1x1 blocks of a `Matrix2x6` as a `Matrix1`. Upstream: `fixed_view::<1, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix2x6FixedViewMatrix1<T, +Copy<T>, +Drop<T>> of FixedView<Matrix2x6<T>, Matrix1<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix2x6<T>, irow: usize, icol: usize) -> Matrix1<T> {
        match icol {
            0 => match irow {
    ...
    #[inline(always)]
    fn view(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix1<T> {
        let (irow, icol) = start;
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix2x6<T>, irow: usize, icol: usize) -> Matrix1<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix1<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x2 blocks of a `Matrix2x6` as a `RowVector2`. Upstream: `fixed_view::<1, 2>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix2x6FixedViewRowVector2<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix2x6<T>, RowVector2<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix2x6<T>, irow: usize, icol: usize) -> RowVector2<T> {
        match icol {
            0 => match irow {
    ...
    #[inline(always)]
    fn view(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix2x6<T>, irow: usize, icol: usize) -> RowVector2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector2<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x3 blocks of a `Matrix2x6` as a `RowVector3`. Upstream: `fixed_view::<1, 3>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix2x6FixedViewRowVector3<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix2x6<T>, RowVector3<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix2x6<T>, irow: usize, icol: usize) -> RowVector3<T> {
        match icol {
            0 => match irow {
    ...
    #[inline(always)]
    fn view(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix2x6<T>, irow: usize, icol: usize) -> RowVector3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector3<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x4 blocks of a `Matrix2x6` as a `RowVector4`. Upstream: `fixed_view::<1, 4>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix2x6FixedViewRowVector4<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix2x6<T>, RowVector4<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix2x6<T>, irow: usize, icol: usize) -> RowVector4<T> {
        match icol {
            0 => match irow {
    ...
    #[inline(always)]
    fn view(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix2x6<T>, irow: usize, icol: usize) -> RowVector4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector4<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x5 blocks of a `Matrix2x6` as a `RowVector5`. Upstream: `fixed_view::<1, 5>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix2x6FixedViewRowVector5<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix2x6<T>, RowVector5<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix2x6<T>, irow: usize, icol: usize) -> RowVector5<T> {
        match icol {
            0 => match irow {
    ...
    #[inline(always)]
    fn view(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector5<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix2x6<T>, irow: usize, icol: usize) -> RowVector5<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector5<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x6 blocks of a `Matrix2x6` as a `RowVector6`. Upstream: `fixed_view::<1, 6>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix2x6FixedViewRowVector6<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix2x6<T>, RowVector6<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix2x6<T>, irow: usize, icol: usize) -> RowVector6<T> {
        match icol {
            0 => match irow {
                0 => RowVector6 {
                    x: self.m11, y: self.m12, z: self.m13, w: self.m14, a: self.m15, b: self.m16,
    ...
    #[inline(always)]
    fn view(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector6<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 6 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix2x6<T>, irow: usize, icol: usize) -> RowVector6<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector6<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x1 blocks of a `Matrix2x6` as a `Vector2`. Upstream: `fixed_view::<2, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix2x6FixedViewVector2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix2x6<T>, Vector2<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix2x6<T>, irow: usize, icol: usize) -> Vector2<T> {
        match icol {
            0 => match irow {
                0 => Vector2 { x: self.m11, y: self.m21 },
                _ => core::panic_with_felt252(errors::INDEX_OUT_OF_BOUNDS),
    ...
    #[inline(always)]
    fn view(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> Vector2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix2x6<T>, irow: usize, icol: usize) -> Vector2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> Vector2<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x2 blocks of a `Matrix2x6` as a `Matrix2`. Upstream: `fixed_view::<2, 2>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix2x6FixedViewMatrix2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix2x6<T>, Matrix2<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix2x6<T>, irow: usize, icol: usize) -> Matrix2<T> {
        match icol {
            0 => match irow {
                0 => Matrix2 { m11: self.m11, m21: self.m21, m12: self.m12, m22: self.m22 },
                _ => core::panic_with_felt252(errors::INDEX_OUT_OF_BOUNDS),
    ...
    #[inline(always)]
    fn view(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix2x6<T>, irow: usize, icol: usize) -> Matrix2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x3 blocks of a `Matrix2x6` as a `Matrix2x3`. Upstream: `fixed_view::<2, 3>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix2x6FixedViewMatrix2x3<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix2x6<T>, Matrix2x3<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix2x6<T>, irow: usize, icol: usize) -> Matrix2x3<T> {
        match icol {
            0 => match irow {
                0 => Matrix2x3 {
                    m11: self.m11,
    ...
    #[inline(always)]
    fn view(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix2x6<T>, irow: usize, icol: usize) -> Matrix2x3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x3<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x4 blocks of a `Matrix2x6` as a `Matrix2x4`. Upstream: `fixed_view::<2, 4>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix2x6FixedViewMatrix2x4<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix2x6<T>, Matrix2x4<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix2x6<T>, irow: usize, icol: usize) -> Matrix2x4<T> {
        match icol {
            0 => match irow {
                0 => Matrix2x4 {
                    m11: self.m11,
    ...
    #[inline(always)]
    fn view(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix2x6<T>, irow: usize, icol: usize) -> Matrix2x4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x4<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x5 blocks of a `Matrix2x6` as a `Matrix2x5`. Upstream: `fixed_view::<2, 5>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix2x6FixedViewMatrix2x5<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix2x6<T>, Matrix2x5<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix2x6<T>, irow: usize, icol: usize) -> Matrix2x5<T> {
        match icol {
            0 => match irow {
                0 => Matrix2x5 {
                    m11: self.m11,
    ...
    #[inline(always)]
    fn view(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x5<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix2x6<T>, irow: usize, icol: usize) -> Matrix2x5<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x5<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x6 blocks of a `Matrix2x6` as a `Matrix2x6`. Upstream: `fixed_view::<2, 6>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix2x6FixedViewMatrix2x6<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix2x6<T>, Matrix2x6<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix2x6<T>, irow: usize, icol: usize) -> Matrix2x6<T> {
        match icol {
            0 => match irow {
                0 => Matrix2x6 {
                    m11: self.m11,
    ...
    #[inline(always)]
    fn view(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x6<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 6 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix2x6<T>, irow: usize, icol: usize) -> Matrix2x6<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix2x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x6<T> {
        Self::view(self, start, shape)
    }
}

/// The first 1 components of a row of a `Matrix2x6` as a `Matrix1`. Upstream: `row_part` (`n = 1`).
pub impl Matrix2x6RowPartMatrix1<T, +Copy<T>, +Drop<T>> of RowPart<Matrix2x6<T>, Matrix1<T>> {
    #[inline(always)]
    fn row_part(self: Matrix2x6<T>, i: usize, n: usize) -> Matrix1<T> {
        if n != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The first 2 components of a row of a `Matrix2x6` as a `RowVector2`. Upstream: `row_part` (`n =
/// 2`).
pub impl Matrix2x6RowPartRowVector2<T, +Copy<T>, +Drop<T>> of RowPart<Matrix2x6<T>, RowVector2<T>> {
    #[inline(always)]
    fn row_part(self: Matrix2x6<T>, i: usize, n: usize) -> RowVector2<T> {
        if n != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 3 components of a row of a `Matrix2x6` as a `RowVector3`. Upstream: `row_part` (`n =
/// 3`).
pub impl Matrix2x6RowPartRowVector3<T, +Copy<T>, +Drop<T>> of RowPart<Matrix2x6<T>, RowVector3<T>> {
    #[inline(always)]
    fn row_part(self: Matrix2x6<T>, i: usize, n: usize) -> RowVector3<T> {
        if n != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 4 components of a row of a `Matrix2x6` as a `RowVector4`. Upstream: `row_part` (`n =
/// 4`).
pub impl Matrix2x6RowPartRowVector4<T, +Copy<T>, +Drop<T>> of RowPart<Matrix2x6<T>, RowVector4<T>> {
    #[inline(always)]
    fn row_part(self: Matrix2x6<T>, i: usize, n: usize) -> RowVector4<T> {
        if n != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 5 components of a row of a `Matrix2x6` as a `RowVector5`. Upstream: `row_part` (`n =
/// 5`).
pub impl Matrix2x6RowPartRowVector5<T, +Copy<T>, +Drop<T>> of RowPart<Matrix2x6<T>, RowVector5<T>> {
    #[inline(always)]
    fn row_part(self: Matrix2x6<T>, i: usize, n: usize) -> RowVector5<T> {
        if n != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 6 components of a row of a `Matrix2x6` as a `RowVector6`. Upstream: `row_part` (`n =
/// 6`).
pub impl Matrix2x6RowPartRowVector6<T, +Copy<T>, +Drop<T>> of RowPart<Matrix2x6<T>, RowVector6<T>> {
    #[inline(always)]
    fn row_part(self: Matrix2x6<T>, i: usize, n: usize) -> RowVector6<T> {
        if n != 6 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 1 components of a column of a `Matrix2x6` as a `Matrix1`. Upstream: `column_part` (`n
/// = 1`).
pub impl Matrix2x6ColumnPartMatrix1<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix2x6<T>, Matrix1<T>> {
    #[inline(always)]
    fn column_part(self: Matrix2x6<T>, i: usize, n: usize) -> Matrix1<T> {
        if n != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The first 2 components of a column of a `Matrix2x6` as a `Vector2`. Upstream: `column_part` (`n
/// = 2`).
pub impl Matrix2x6ColumnPartVector2<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix2x6<T>, Vector2<T>> {
    #[inline(always)]
    fn column_part(self: Matrix2x6<T>, i: usize, n: usize) -> Vector2<T> {
        if n != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, 0, i)
...
/// The Kronecker product of a `Matrix2x6` and a `Matrix1`, a `Matrix2x6`: one floored product per
/// component. Panics on overflow. Upstream: `kronecker`.
pub impl Matrix2x6KroneckerMatrix1<
    T, +Mul<T>, +Copy<T>, +Drop<T>,
> of MatrixKronecker<Matrix2x6<T>, Matrix1<T>> {
    type Output = Matrix2x6<T>;
    #[inline(always)]
    fn kronecker(self: Matrix2x6<T>, rhs: Matrix1<T>) -> Matrix2x6<T> {
        Matrix2x6 {
            m11: self.m11 * rhs.x,
            m21: self.m21 * rhs.x,
            m12: self.m12 * rhs.x,
...
/// The Kronecker product of a `Matrix2x6` and a `Vector2`, a `Matrix4x6`: one floored product per
/// component. Panics on overflow. Upstream: `kronecker`.
pub impl Matrix2x6KroneckerVector2<
    T, +Mul<T>, +Copy<T>, +Drop<T>,
> of MatrixKronecker<Matrix2x6<T>, Vector2<T>> {
    type Output = Matrix4x6<T>;
    fn kronecker(self: Matrix2x6<T>, rhs: Vector2<T>) -> Matrix4x6<T> {
        Matrix4x6 {
            m11: self.m11 * rhs.x,
            m21: self.m11 * rhs.y,
            m31: self.m21 * rhs.x,
...
/// The Kronecker product of a `Matrix2x6` and a `Vector3`, a `Matrix6`: one floored product per
/// component. Panics on overflow. Upstream: `kronecker`.
pub impl Matrix2x6KroneckerVector3<
    T, +Mul<T>, +Copy<T>, +Drop<T>,
> of MatrixKronecker<Matrix2x6<T>, Vector3<T>> {
    type Output = Matrix6<T>;
    fn kronecker(self: Matrix2x6<T>, rhs: Vector3<T>) -> Matrix6<T> {
        Matrix6 {
            m11: self.m11 * rhs.x,
            m21: self.m11 * rhs.y,
            m31: self.m11 * rhs.z,
...
pub(crate) impl Matrix2x6PadTo6<T, +Copy<T>, +Drop<T>> of PadTo6<Matrix2x6<T>, T> {
    #[inline(always)]
...
pub(crate) impl Matrix2x6CropFrom6<T, +Copy<T>, +Drop<T>> of CropFrom6<Matrix2x6<T>, T> {
    #[inline(always)]
