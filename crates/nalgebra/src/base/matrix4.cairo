...
/// The 1 consecutive rows of a `Matrix4` as a `RowVector4`. Upstream: `fixed_rows::<1>`, `rows(i,
...
pub impl Matrix4FixedRowsRowVector4<T, +Copy<T>, +Drop<T>> of FixedRows<Matrix4<T>, RowVector4<T>> {
    ...
    #[inline(always)]
    fn rows(self: Matrix4<T>, first_row: usize, nrows: usize) -> RowVector4<T> {
        if nrows != 1 {
    ...
    #[inline(always)]
    fn rows_range(self: Matrix4<T>, rows: Range<usize>) -> RowVector4<T> {
...
/// The 2 consecutive rows of a `Matrix4` as a `Matrix2x4`. Upstream: `fixed_rows::<2>`, `rows(i,
...
pub impl Matrix4FixedRowsMatrix2x4<T, +Copy<T>, +Drop<T>> of FixedRows<Matrix4<T>, Matrix2x4<T>> {
    ...
    #[inline(always)]
    fn rows(self: Matrix4<T>, first_row: usize, nrows: usize) -> Matrix2x4<T> {
        if nrows != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_rows(self, first_row)
    ...
    #[inline(always)]
    fn rows_range(self: Matrix4<T>, rows: Range<usize>) -> Matrix2x4<T> {
        let Range { start, end } = rows;
        if end != start + 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 3 consecutive rows of a `Matrix4` as a `Matrix3x4`. Upstream: `fixed_rows::<3>`, `rows(i,
...
pub impl Matrix4FixedRowsMatrix3x4<T, +Copy<T>, +Drop<T>> of FixedRows<Matrix4<T>, Matrix3x4<T>> {
    ...
    #[inline(always)]
    fn rows(self: Matrix4<T>, first_row: usize, nrows: usize) -> Matrix3x4<T> {
        if nrows != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_rows(self, first_row)
    ...
    #[inline(always)]
    fn rows_range(self: Matrix4<T>, rows: Range<usize>) -> Matrix3x4<T> {
        let Range { start, end } = rows;
        if end != start + 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 4 consecutive rows of a `Matrix4` as a `Matrix4`. Upstream: `fixed_rows::<4>`, `rows(i, 4)`,
...
pub impl Matrix4FixedRowsMatrix4<T, +Copy<T>, +Drop<T>> of FixedRows<Matrix4<T>, Matrix4<T>> {
    ...
    #[inline(always)]
    fn rows(self: Matrix4<T>, first_row: usize, nrows: usize) -> Matrix4<T> {
        if nrows != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_rows(self, first_row)
    ...
    #[inline(always)]
    fn rows_range(self: Matrix4<T>, rows: Range<usize>) -> Matrix4<T> {
        let Range { start, end } = rows;
        if end != start + 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 1 consecutive columns of a `Matrix4` as a `Vector4`. Upstream: `fixed_columns::<1>`,
...
pub impl Matrix4FixedColumnsVector4<T, +Copy<T>, +Drop<T>> of FixedColumns<Matrix4<T>, Vector4<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix4<T>, first_col: usize, ncols: usize) -> Vector4<T> {
        if ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn columns_range(self: Matrix4<T>, cols: Range<usize>) -> Vector4<T> {
...
/// The 2 consecutive columns of a `Matrix4` as a `Matrix4x2`. Upstream: `fixed_columns::<2>`,
...
> of FixedColumns<Matrix4<T>, Matrix4x2<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix4<T>, first_col: usize, ncols: usize) -> Matrix4x2<T> {
        if ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix4<T>, cols: Range<usize>) -> Matrix4x2<T> {
        let Range { start, end } = cols;
        if end != start + 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 3 consecutive columns of a `Matrix4` as a `Matrix4x3`. Upstream: `fixed_columns::<3>`,
...
> of FixedColumns<Matrix4<T>, Matrix4x3<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix4<T>, first_col: usize, ncols: usize) -> Matrix4x3<T> {
        if ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix4<T>, cols: Range<usize>) -> Matrix4x3<T> {
        let Range { start, end } = cols;
        if end != start + 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 4 consecutive columns of a `Matrix4` as a `Matrix4`. Upstream: `fixed_columns::<4>`,
...
pub impl Matrix4FixedColumnsMatrix4<T, +Copy<T>, +Drop<T>> of FixedColumns<Matrix4<T>, Matrix4<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix4<T>, first_col: usize, ncols: usize) -> Matrix4<T> {
        if ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix4<T>, cols: Range<usize>) -> Matrix4<T> {
        let Range { start, end } = cols;
        if end != start + 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 1x1 blocks of a `Matrix4` as a `Matrix1`. Upstream: `fixed_view::<1, 1>`, `view`, and the
...
pub impl Matrix4FixedViewMatrix1<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4<T>, Matrix1<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix1<T> {
        let (irow, icol) = start;
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4<T>, irow: usize, icol: usize) -> Matrix1<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix1<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x2 blocks of a `Matrix4` as a `RowVector2`. Upstream: `fixed_view::<1, 2>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix4FixedViewRowVector2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4<T>, RowVector2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4<T>, irow: usize, icol: usize) -> RowVector2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector2<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x3 blocks of a `Matrix4` as a `RowVector3`. Upstream: `fixed_view::<1, 3>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix4FixedViewRowVector3<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4<T>, RowVector3<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4<T>, irow: usize, icol: usize) -> RowVector3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector3<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x4 blocks of a `Matrix4` as a `RowVector4`. Upstream: `fixed_view::<1, 4>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix4FixedViewRowVector4<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4<T>, RowVector4<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4<T>, irow: usize, icol: usize) -> RowVector4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector4<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x1 blocks of a `Matrix4` as a `Vector2`. Upstream: `fixed_view::<2, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix4FixedViewVector2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4<T>, Vector2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Vector2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4<T>, irow: usize, icol: usize) -> Vector2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Vector2<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x2 blocks of a `Matrix4` as a `Matrix2`. Upstream: `fixed_view::<2, 2>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix4FixedViewMatrix2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4<T>, Matrix2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4<T>, irow: usize, icol: usize) -> Matrix2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x3 blocks of a `Matrix4` as a `Matrix2x3`. Upstream: `fixed_view::<2, 3>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix4FixedViewMatrix2x3<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4<T>, Matrix2x3<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4<T>, irow: usize, icol: usize) -> Matrix2x3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x3<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x4 blocks of a `Matrix4` as a `Matrix2x4`. Upstream: `fixed_view::<2, 4>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix4FixedViewMatrix2x4<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4<T>, Matrix2x4<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4<T>, irow: usize, icol: usize) -> Matrix2x4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x4<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x1 blocks of a `Matrix4` as a `Vector3`. Upstream: `fixed_view::<3, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix4FixedViewVector3<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4<T>, Vector3<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Vector3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4<T>, irow: usize, icol: usize) -> Vector3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Vector3<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x2 blocks of a `Matrix4` as a `Matrix3x2`. Upstream: `fixed_view::<3, 2>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix4FixedViewMatrix3x2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4<T>, Matrix3x2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4<T>, irow: usize, icol: usize) -> Matrix3x2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x2<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x3 blocks of a `Matrix4` as a `Matrix3`. Upstream: `fixed_view::<3, 3>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix4FixedViewMatrix3<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4<T>, Matrix3<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4<T>, irow: usize, icol: usize) -> Matrix3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x4 blocks of a `Matrix4` as a `Matrix3x4`. Upstream: `fixed_view::<3, 4>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix4FixedViewMatrix3x4<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4<T>, Matrix3x4<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4<T>, irow: usize, icol: usize) -> Matrix3x4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x4<T> {
        Self::view(self, start, shape)
    }
}

/// The 4x1 blocks of a `Matrix4` as a `Vector4`. Upstream: `fixed_view::<4, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix4FixedViewVector4<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4<T>, Vector4<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Vector4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 4 || ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4<T>, irow: usize, icol: usize) -> Vector4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Vector4<T> {
        Self::view(self, start, shape)
    }
}

/// The 4x2 blocks of a `Matrix4` as a `Matrix4x2`. Upstream: `fixed_view::<4, 2>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix4FixedViewMatrix4x2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4<T>, Matrix4x2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4x2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 4 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4<T>, irow: usize, icol: usize) -> Matrix4x2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4x2<T> {
        Self::view(self, start, shape)
    }
}

/// The 4x3 blocks of a `Matrix4` as a `Matrix4x3`. Upstream: `fixed_view::<4, 3>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix4FixedViewMatrix4x3<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4<T>, Matrix4x3<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4x3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 4 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4<T>, irow: usize, icol: usize) -> Matrix4x3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4x3<T> {
        Self::view(self, start, shape)
    }
}

/// The 4x4 blocks of a `Matrix4` as a `Matrix4`. Upstream: `fixed_view::<4, 4>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix4FixedViewMatrix4<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4<T>, Matrix4<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 4 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4<T>, irow: usize, icol: usize) -> Matrix4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4<T> {
        Self::view(self, start, shape)
    }
}

/// The first 1 components of a row of a `Matrix4` as a `Matrix1`. Upstream: `row_part` (`n = 1`).
pub impl Matrix4RowPartMatrix1<T, +Copy<T>, +Drop<T>> of RowPart<Matrix4<T>, Matrix1<T>> {
    #[inline(always)]
    fn row_part(self: Matrix4<T>, i: usize, n: usize) -> Matrix1<T> {
        if n != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The first 2 components of a row of a `Matrix4` as a `RowVector2`. Upstream: `row_part` (`n =
/// 2`).
pub impl Matrix4RowPartRowVector2<T, +Copy<T>, +Drop<T>> of RowPart<Matrix4<T>, RowVector2<T>> {
    #[inline(always)]
    fn row_part(self: Matrix4<T>, i: usize, n: usize) -> RowVector2<T> {
        if n != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 3 components of a row of a `Matrix4` as a `RowVector3`. Upstream: `row_part` (`n =
/// 3`).
pub impl Matrix4RowPartRowVector3<T, +Copy<T>, +Drop<T>> of RowPart<Matrix4<T>, RowVector3<T>> {
    #[inline(always)]
    fn row_part(self: Matrix4<T>, i: usize, n: usize) -> RowVector3<T> {
        if n != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 4 components of a row of a `Matrix4` as a `RowVector4`. Upstream: `row_part` (`n =
/// 4`).
pub impl Matrix4RowPartRowVector4<T, +Copy<T>, +Drop<T>> of RowPart<Matrix4<T>, RowVector4<T>> {
    #[inline(always)]
    fn row_part(self: Matrix4<T>, i: usize, n: usize) -> RowVector4<T> {
        if n != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 1 components of a column of a `Matrix4` as a `Matrix1`. Upstream: `column_part` (`n =
/// 1`).
pub impl Matrix4ColumnPartMatrix1<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix4<T>, Matrix1<T>> {
    #[inline(always)]
    fn column_part(self: Matrix4<T>, i: usize, n: usize) -> Matrix1<T> {
        if n != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The first 2 components of a column of a `Matrix4` as a `Vector2`. Upstream: `column_part` (`n =
/// 2`).
pub impl Matrix4ColumnPartVector2<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix4<T>, Vector2<T>> {
    #[inline(always)]
    fn column_part(self: Matrix4<T>, i: usize, n: usize) -> Vector2<T> {
        if n != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, 0, i)
...
/// The first 3 components of a column of a `Matrix4` as a `Vector3`. Upstream: `column_part` (`n =
/// 3`).
pub impl Matrix4ColumnPartVector3<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix4<T>, Vector3<T>> {
    #[inline(always)]
    fn column_part(self: Matrix4<T>, i: usize, n: usize) -> Vector3<T> {
        if n != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, 0, i)
...
/// The first 4 components of a column of a `Matrix4` as a `Vector4`. Upstream: `column_part` (`n =
/// 4`).
pub impl Matrix4ColumnPartVector4<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix4<T>, Vector4<T>> {
    #[inline(always)]
    fn column_part(self: Matrix4<T>, i: usize, n: usize) -> Vector4<T> {
        if n != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, 0, i)
