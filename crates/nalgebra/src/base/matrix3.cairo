...
/// `selfᵀ * rhs` without forming the transpose, a `Vector3`: one `sum_prod3` per component (one
...
/// `selfᵀ * rhs` without forming the transpose, a `Matrix3x2`: one `sum_prod3` per component (one
/// rounding each). Bit-identical to `self.transpose().mul_mat(rhs)`. Panics on overflow. Upstream:
/// `tr_mul`.
...
/// `selfᵀ * rhs` without forming the transpose, a `Matrix3`: one `sum_prod3` per component (one
/// rounding each). Bit-identical to `self.transpose().mul_mat(rhs)`. Panics on overflow. Upstream:
/// `tr_mul`.
...
/// `selfᵀ * rhs` without forming the transpose, a `Matrix3x4`: one `sum_prod3` per component (one
/// rounding each). Bit-identical to `self.transpose().mul_mat(rhs)`. Panics on overflow. Upstream:
/// `tr_mul`.
...
/// `selfᵀ * rhs` without forming the transpose, a `Matrix3x5`: one `sum_prod3` per component (one
/// rounding each). Bit-identical to `self.transpose().mul_mat(rhs)`. Panics on overflow. Upstream:
/// `tr_mul`.
...
/// `selfᵀ * rhs` without forming the transpose, a `Matrix3x6`: one `sum_prod3` per component (one
/// rounding each). Bit-identical to `self.transpose().mul_mat(rhs)`. Panics on overflow. Upstream:
/// `tr_mul`.
