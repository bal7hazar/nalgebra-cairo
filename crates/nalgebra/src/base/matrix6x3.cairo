...
> of Matrix6x3Trait<T> {
    ...
    #[inline(always)]
    fn nrows(self: Matrix6x3<T>) -> usize {
        6
    }

    /// The number of columns, 3. Upstream: `ncols`.
    #[inline(always)]
    fn ncols(self: Matrix6x3<T>) -> usize {
        3
    }

    /// `(nrows, ncols)`, `(6, 3)`. Upstream: `shape`.
    #[inline(always)]
    fn shape(self: Matrix6x3<T>) -> (usize, usize) {
        (6, 3)
    }

    /// Whether the shape is square: `false`. Upstream: `is_square`.
    #[inline(always)]
    fn is_square(self: Matrix6x3<T>) -> bool {
        false
    }
    ...
    #[inline(always)]
    fn vector_to_matrix_index(self: Matrix6x3<T>, i: usize) -> (usize, usize) {
    ...
    #[inline(always)]
    fn row(self: Matrix6x3<T>, i: usize) -> RowVector3<T> {
        Matrix6x3EditTrait::row_at(self, i)
    }
    ...
    #[inline(always)]
    fn column(self: Matrix6x3<T>, j: usize) -> Vector6<T> {
        Matrix6x3EditTrait::column_at(self, j)
    }
    ...
    #[inline(always)]
    fn upper_triangle(self: Matrix6x3<T>) -> Matrix6x3<T> {
        Matrix6x3 {
            m11: self.m11,
            m21: R::zero(),
            m31: R::zero(),
    ...
    #[inline(always)]
    fn lower_triangle(self: Matrix6x3<T>) -> Matrix6x3<T> {
        Matrix6x3 {
            m11: self.m11,
            m21: self.m21,
            m31: self.m31,
    ...
    /// The 6x3 matrix whose 6 rows are the given vectors, each a `Vector3` of the row's 3
    ...
    /// The 6x3 matrix whose 3 columns are the given `Vector6`s. Exact. Upstream: `from_columns` (a
    /// slice of column vectors).
    #[inline(always)]
    fn from_columns(c1: Vector6<T>, c2: Vector6<T>, c3: Vector6<T>) -> Matrix6x3<T> {
        Matrix6x3 {
            m11: c1.x,
            m21: c1.y,
            m31: c1.z,
    ...
    #[inline(always)]
    fn is_orthogonal(self: Matrix6x3<T>, ulps: u64) -> bool {
        Matrix3Trait::is_identity(MatrixTrMul::tr_mul(self, self), ulps)
    }

    /// `self` with a column of `val` inserted at index `i` (`i <= 3`), a `Matrix6x4`. Panics with
    /// `nalgebra: index out of bounds` for `i > 3`. Upstream: `insert_column`.
    fn insert_column(self: Matrix6x3<T>, i: usize, val: T) -> Matrix6x4<T> {
        match i {
            0 => Matrix6x4 {
                m11: val,
                m21: val,
    ...
    /// `self` without its row `i`, a `Matrix5x3`. Panics with `nalgebra: index out of bounds` for
    /// `i >= 6`. Upstream: `remove_row`.
    #[inline(always)]
    fn remove_row(self: Matrix6x3<T>, i: usize) -> Matrix5x3<T> {
        match i {
            0 => Matrix5x3 {
                m11: self.m21,
                m21: self.m31,
    ...
    /// `self` without its column `i`, a `Matrix6x2`. Panics with `nalgebra: index out of bounds`
    /// for `i >= 3`. Upstream: `remove_column`.
    #[inline(always)]
    fn remove_column(self: Matrix6x3<T>, i: usize) -> Matrix6x2<T> {
        match i {
            0 => Matrix6x2 {
                m11: self.m12,
                m21: self.m22,
...
/// The 1 consecutive rows of a `Matrix6x3` as a `RowVector3`. Upstream: `fixed_rows::<1>`, `rows(i,
/// 1)`, `rows_range`, `select_rows`.
pub impl Matrix6x3FixedRowsRowVector3<
    T, +Copy<T>, +Drop<T>,
> of FixedRows<Matrix6x3<T>, RowVector3<T>> {
    #[inline(always)]
    fn fixed_rows(self: Matrix6x3<T>, i: usize) -> RowVector3<T> {
        match i {
            0 => RowVector3 { x: self.m11, y: self.m12, z: self.m13 },
            1 => RowVector3 { x: self.m21, y: self.m22, z: self.m23 },
            2 => RowVector3 { x: self.m31, y: self.m32, z: self.m33 },
    ...
    #[inline(always)]
    fn rows(self: Matrix6x3<T>, first_row: usize, nrows: usize) -> RowVector3<T> {
        if nrows != 1 {
    ...
    #[inline(always)]
    fn rows_range(self: Matrix6x3<T>, rows: Range<usize>) -> RowVector3<T> {
    ...
    #[inline(always)]
    fn select_rows(self: Matrix6x3<T>, irows: Span<usize>) -> RowVector3<T> {
        if irows.len() != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let r0 = Matrix6x3EditTrait::row_at(self, *irows[0]);
        RowVector3 { x: r0.x, y: r0.y, z: r0.z }
    }
}

/// The 2 consecutive rows of a `Matrix6x3` as a `Matrix2x3`. Upstream: `fixed_rows::<2>`, `rows(i,
/// 2)`, `rows_range`, `select_rows`.
pub impl Matrix6x3FixedRowsMatrix2x3<
    T, +Copy<T>, +Drop<T>,
> of FixedRows<Matrix6x3<T>, Matrix2x3<T>> {
    #[inline(always)]
    fn fixed_rows(self: Matrix6x3<T>, i: usize) -> Matrix2x3<T> {
        match i {
            0 => Matrix2x3 {
                m11: self.m11,
                m21: self.m21,
    ...
    #[inline(always)]
    fn rows(self: Matrix6x3<T>, first_row: usize, nrows: usize) -> Matrix2x3<T> {
        if nrows != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_rows(self, first_row)
    ...
    #[inline(always)]
    fn rows_range(self: Matrix6x3<T>, rows: Range<usize>) -> Matrix2x3<T> {
        let Range { start, end } = rows;
        if end != start + 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn select_rows(self: Matrix6x3<T>, irows: Span<usize>) -> Matrix2x3<T> {
        if irows.len() != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let r0 = Matrix6x3EditTrait::row_at(self, *irows[0]);
        let r1 = Matrix6x3EditTrait::row_at(self, *irows[1]);
        Matrix2x3 { m11: r0.x, m21: r1.x, m12: r0.y, m22: r1.y, m13: r0.z, m23: r1.z }
    }
}

/// The 3 consecutive rows of a `Matrix6x3` as a `Matrix3`. Upstream: `fixed_rows::<3>`, `rows(i,
/// 3)`, `rows_range`, `select_rows`.
pub impl Matrix6x3FixedRowsMatrix3<T, +Copy<T>, +Drop<T>> of FixedRows<Matrix6x3<T>, Matrix3<T>> {
    #[inline(always)]
    fn fixed_rows(self: Matrix6x3<T>, i: usize) -> Matrix3<T> {
        match i {
            0 => Matrix3 {
                m11: self.m11,
                m21: self.m21,
    ...
    #[inline(always)]
    fn rows(self: Matrix6x3<T>, first_row: usize, nrows: usize) -> Matrix3<T> {
        if nrows != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_rows(self, first_row)
    ...
    #[inline(always)]
    fn rows_range(self: Matrix6x3<T>, rows: Range<usize>) -> Matrix3<T> {
        let Range { start, end } = rows;
        if end != start + 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn select_rows(self: Matrix6x3<T>, irows: Span<usize>) -> Matrix3<T> {
        if irows.len() != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let r0 = Matrix6x3EditTrait::row_at(self, *irows[0]);
        let r1 = Matrix6x3EditTrait::row_at(self, *irows[1]);
        let r2 = Matrix6x3EditTrait::row_at(self, *irows[2]);
        Matrix3 {
            m11: r0.x,
            m21: r1.x,
            m31: r2.x,
...
/// The 4 consecutive rows of a `Matrix6x3` as a `Matrix4x3`. Upstream: `fixed_rows::<4>`, `rows(i,
/// 4)`, `rows_range`, `select_rows`.
pub impl Matrix6x3FixedRowsMatrix4x3<
    T, +Copy<T>, +Drop<T>,
> of FixedRows<Matrix6x3<T>, Matrix4x3<T>> {
    #[inline(always)]
    fn fixed_rows(self: Matrix6x3<T>, i: usize) -> Matrix4x3<T> {
        match i {
            0 => Matrix4x3 {
                m11: self.m11,
                m21: self.m21,
    ...
    #[inline(always)]
    fn rows(self: Matrix6x3<T>, first_row: usize, nrows: usize) -> Matrix4x3<T> {
        if nrows != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_rows(self, first_row)
    ...
    #[inline(always)]
    fn rows_range(self: Matrix6x3<T>, rows: Range<usize>) -> Matrix4x3<T> {
        let Range { start, end } = rows;
        if end != start + 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn select_rows(self: Matrix6x3<T>, irows: Span<usize>) -> Matrix4x3<T> {
        if irows.len() != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let r0 = Matrix6x3EditTrait::row_at(self, *irows[0]);
        let r1 = Matrix6x3EditTrait::row_at(self, *irows[1]);
        let r2 = Matrix6x3EditTrait::row_at(self, *irows[2]);
        let r3 = Matrix6x3EditTrait::row_at(self, *irows[3]);
        Matrix4x3 {
            m11: r0.x,
            m21: r1.x,
            m31: r2.x,
...
/// The 5 consecutive rows of a `Matrix6x3` as a `Matrix5x3`. Upstream: `fixed_rows::<5>`, `rows(i,
/// 5)`, `rows_range`, `select_rows`.
pub impl Matrix6x3FixedRowsMatrix5x3<
    T, +Copy<T>, +Drop<T>,
> of FixedRows<Matrix6x3<T>, Matrix5x3<T>> {
    #[inline(always)]
    fn fixed_rows(self: Matrix6x3<T>, i: usize) -> Matrix5x3<T> {
        match i {
            0 => Matrix5x3 {
                m11: self.m11,
                m21: self.m21,
    ...
    #[inline(always)]
    fn rows(self: Matrix6x3<T>, first_row: usize, nrows: usize) -> Matrix5x3<T> {
        if nrows != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_rows(self, first_row)
    ...
    #[inline(always)]
    fn rows_range(self: Matrix6x3<T>, rows: Range<usize>) -> Matrix5x3<T> {
        let Range { start, end } = rows;
        if end != start + 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn select_rows(self: Matrix6x3<T>, irows: Span<usize>) -> Matrix5x3<T> {
        if irows.len() != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let r0 = Matrix6x3EditTrait::row_at(self, *irows[0]);
        ...
        let r2 = Matrix6x3EditTrait::row_at(self, *irows[2]);
        let r3 = Matrix6x3EditTrait::row_at(self, *irows[3]);
        let r4 = Matrix6x3EditTrait::row_at(self, *irows[4]);
        Matrix5x3 {
            m11: r0.x,
            m21: r1.x,
            m31: r2.x,
...
/// The 6 consecutive rows of a `Matrix6x3` as a `Matrix6x3`. Upstream: `fixed_rows::<6>`, `rows(i,
/// 6)`, `rows_range`, `select_rows`.
pub impl Matrix6x3FixedRowsMatrix6x3<
    T, +Copy<T>, +Drop<T>,
> of FixedRows<Matrix6x3<T>, Matrix6x3<T>> {
    fn fixed_rows(self: Matrix6x3<T>, i: usize) -> Matrix6x3<T> {
        match i {
            0 => Matrix6x3 {
                m11: self.m11,
                m21: self.m21,
    ...
    #[inline(always)]
    fn rows(self: Matrix6x3<T>, first_row: usize, nrows: usize) -> Matrix6x3<T> {
        if nrows != 6 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_rows(self, first_row)
    ...
    #[inline(always)]
    fn rows_range(self: Matrix6x3<T>, rows: Range<usize>) -> Matrix6x3<T> {
        let Range { start, end } = rows;
        if end != start + 6 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    fn select_rows(self: Matrix6x3<T>, irows: Span<usize>) -> Matrix6x3<T> {
        if irows.len() != 6 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let r0 = Matrix6x3EditTrait::row_at(self, *irows[0]);
        ...
        let r3 = Matrix6x3EditTrait::row_at(self, *irows[3]);
        let r4 = Matrix6x3EditTrait::row_at(self, *irows[4]);
        let r5 = Matrix6x3EditTrait::row_at(self, *irows[5]);
        Matrix6x3 {
            m11: r0.x,
            m21: r1.x,
            m31: r2.x,
...
/// The 1 consecutive columns of a `Matrix6x3` as a `Vector6`. Upstream: `fixed_columns::<1>`,
/// `columns(j, 1)`, `columns_range`, `select_columns`.
pub impl Matrix6x3FixedColumnsVector6<
    T, +Copy<T>, +Drop<T>,
> of FixedColumns<Matrix6x3<T>, Vector6<T>> {
    #[inline(always)]
    fn fixed_columns(self: Matrix6x3<T>, i: usize) -> Vector6<T> {
        match i {
            0 => Vector6 {
                x: self.m11, y: self.m21, z: self.m31, w: self.m41, a: self.m51, b: self.m61,
            },
    ...
    #[inline(always)]
    fn columns(self: Matrix6x3<T>, first_col: usize, ncols: usize) -> Vector6<T> {
        if ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn columns_range(self: Matrix6x3<T>, cols: Range<usize>) -> Vector6<T> {
    ...
    #[inline(always)]
    fn select_columns(self: Matrix6x3<T>, icols: Span<usize>) -> Vector6<T> {
        if icols.len() != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let c0 = Matrix6x3EditTrait::column_at(self, *icols[0]);
        Vector6 { x: c0.x, y: c0.y, z: c0.z, w: c0.w, a: c0.a, b: c0.b }
    }
}

/// The 2 consecutive columns of a `Matrix6x3` as a `Matrix6x2`. Upstream: `fixed_columns::<2>`,
/// `columns(j, 2)`, `columns_range`, `select_columns`.
pub impl Matrix6x3FixedColumnsMatrix6x2<
    T, +Copy<T>, +Drop<T>,
> of FixedColumns<Matrix6x3<T>, Matrix6x2<T>> {
    #[inline(always)]
    fn fixed_columns(self: Matrix6x3<T>, i: usize) -> Matrix6x2<T> {
        match i {
            0 => Matrix6x2 {
                m11: self.m11,
                m21: self.m21,
    ...
    #[inline(always)]
    fn columns(self: Matrix6x3<T>, first_col: usize, ncols: usize) -> Matrix6x2<T> {
        if ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix6x3<T>, cols: Range<usize>) -> Matrix6x2<T> {
        let Range { start, end } = cols;
        if end != start + 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn select_columns(self: Matrix6x3<T>, icols: Span<usize>) -> Matrix6x2<T> {
        if icols.len() != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let c0 = Matrix6x3EditTrait::column_at(self, *icols[0]);
        let c1 = Matrix6x3EditTrait::column_at(self, *icols[1]);
        Matrix6x2 {
            m11: c0.x,
            m21: c0.y,
            m31: c0.z,
...
/// The 3 consecutive columns of a `Matrix6x3` as a `Matrix6x3`. Upstream: `fixed_columns::<3>`,
/// `columns(j, 3)`, `columns_range`, `select_columns`.
pub impl Matrix6x3FixedColumnsMatrix6x3<
    T, +Copy<T>, +Drop<T>,
> of FixedColumns<Matrix6x3<T>, Matrix6x3<T>> {
    fn fixed_columns(self: Matrix6x3<T>, i: usize) -> Matrix6x3<T> {
        match i {
            0 => Matrix6x3 {
                m11: self.m11,
                m21: self.m21,
    ...
    #[inline(always)]
    fn columns(self: Matrix6x3<T>, first_col: usize, ncols: usize) -> Matrix6x3<T> {
        if ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix6x3<T>, cols: Range<usize>) -> Matrix6x3<T> {
        let Range { start, end } = cols;
        if end != start + 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    fn select_columns(self: Matrix6x3<T>, icols: Span<usize>) -> Matrix6x3<T> {
        if icols.len() != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let c0 = Matrix6x3EditTrait::column_at(self, *icols[0]);
        let c1 = Matrix6x3EditTrait::column_at(self, *icols[1]);
        let c2 = Matrix6x3EditTrait::column_at(self, *icols[2]);
        Matrix6x3 {
            m11: c0.x,
            m21: c0.y,
            m31: c0.z,
...
/// The 1x1 blocks of a `Matrix6x3` as a `Matrix1`. Upstream: `fixed_view::<1, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix6x3FixedViewMatrix1<T, +Copy<T>, +Drop<T>> of FixedView<Matrix6x3<T>, Matrix1<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix6x3<T>, irow: usize, icol: usize) -> Matrix1<T> {
        match icol {
            0 => match irow {
    ...
    #[inline(always)]
    fn view(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix1<T> {
        let (irow, icol) = start;
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix6x3<T>, irow: usize, icol: usize) -> Matrix1<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix1<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x2 blocks of a `Matrix6x3` as a `RowVector2`. Upstream: `fixed_view::<1, 2>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix6x3FixedViewRowVector2<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix6x3<T>, RowVector2<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix6x3<T>, irow: usize, icol: usize) -> RowVector2<T> {
        match icol {
            0 => match irow {
    ...
    #[inline(always)]
    fn view(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix6x3<T>, irow: usize, icol: usize) -> RowVector2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector2<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x3 blocks of a `Matrix6x3` as a `RowVector3`. Upstream: `fixed_view::<1, 3>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix6x3FixedViewRowVector3<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix6x3<T>, RowVector3<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix6x3<T>, irow: usize, icol: usize) -> RowVector3<T> {
        match icol {
            0 => match irow {
                0 => RowVector3 { x: self.m11, y: self.m12, z: self.m13 },
                1 => RowVector3 { x: self.m21, y: self.m22, z: self.m23 },
    ...
    #[inline(always)]
    fn view(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix6x3<T>, irow: usize, icol: usize) -> RowVector3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector3<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x1 blocks of a `Matrix6x3` as a `Vector2`. Upstream: `fixed_view::<2, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix6x3FixedViewVector2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix6x3<T>, Vector2<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix6x3<T>, irow: usize, icol: usize) -> Vector2<T> {
        match icol {
            0 => match irow {
    ...
    #[inline(always)]
    fn view(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Vector2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix6x3<T>, irow: usize, icol: usize) -> Vector2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Vector2<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x2 blocks of a `Matrix6x3` as a `Matrix2`. Upstream: `fixed_view::<2, 2>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix6x3FixedViewMatrix2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix6x3<T>, Matrix2<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix6x3<T>, irow: usize, icol: usize) -> Matrix2<T> {
        match icol {
            0 => match irow {
    ...
    #[inline(always)]
    fn view(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix6x3<T>, irow: usize, icol: usize) -> Matrix2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x3 blocks of a `Matrix6x3` as a `Matrix2x3`. Upstream: `fixed_view::<2, 3>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix6x3FixedViewMatrix2x3<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix6x3<T>, Matrix2x3<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix6x3<T>, irow: usize, icol: usize) -> Matrix2x3<T> {
        match icol {
            0 => match irow {
                0 => Matrix2x3 {
                    m11: self.m11,
    ...
    #[inline(always)]
    fn view(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix6x3<T>, irow: usize, icol: usize) -> Matrix2x3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x3<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x1 blocks of a `Matrix6x3` as a `Vector3`. Upstream: `fixed_view::<3, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix6x3FixedViewVector3<T, +Copy<T>, +Drop<T>> of FixedView<Matrix6x3<T>, Vector3<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix6x3<T>, irow: usize, icol: usize) -> Vector3<T> {
        match icol {
            0 => match irow {
    ...
    #[inline(always)]
    fn view(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Vector3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix6x3<T>, irow: usize, icol: usize) -> Vector3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Vector3<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x2 blocks of a `Matrix6x3` as a `Matrix3x2`. Upstream: `fixed_view::<3, 2>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix6x3FixedViewMatrix3x2<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix6x3<T>, Matrix3x2<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix6x3<T>, irow: usize, icol: usize) -> Matrix3x2<T> {
        match icol {
            0 => match irow {
                0 => Matrix3x2 {
                    m11: self.m11,
    ...
    #[inline(always)]
    fn view(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix6x3<T>, irow: usize, icol: usize) -> Matrix3x2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x2<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x3 blocks of a `Matrix6x3` as a `Matrix3`. Upstream: `fixed_view::<3, 3>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix6x3FixedViewMatrix3<T, +Copy<T>, +Drop<T>> of FixedView<Matrix6x3<T>, Matrix3<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix6x3<T>, irow: usize, icol: usize) -> Matrix3<T> {
        match icol {
            0 => match irow {
                0 => Matrix3 {
                    m11: self.m11,
    ...
    #[inline(always)]
    fn view(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix6x3<T>, irow: usize, icol: usize) -> Matrix3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3<T> {
        Self::view(self, start, shape)
    }
}

/// The 4x1 blocks of a `Matrix6x3` as a `Vector4`. Upstream: `fixed_view::<4, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix6x3FixedViewVector4<T, +Copy<T>, +Drop<T>> of FixedView<Matrix6x3<T>, Vector4<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix6x3<T>, irow: usize, icol: usize) -> Vector4<T> {
        match icol {
            0 => match irow {
    ...
    #[inline(always)]
    fn view(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Vector4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 4 || ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix6x3<T>, irow: usize, icol: usize) -> Vector4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Vector4<T> {
        Self::view(self, start, shape)
    }
}

/// The 4x2 blocks of a `Matrix6x3` as a `Matrix4x2`. Upstream: `fixed_view::<4, 2>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix6x3FixedViewMatrix4x2<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix6x3<T>, Matrix4x2<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix6x3<T>, irow: usize, icol: usize) -> Matrix4x2<T> {
        match icol {
            0 => match irow {
                0 => Matrix4x2 {
                    m11: self.m11,
    ...
    #[inline(always)]
    fn view(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4x2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 4 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix6x3<T>, irow: usize, icol: usize) -> Matrix4x2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4x2<T> {
        Self::view(self, start, shape)
    }
}

/// The 4x3 blocks of a `Matrix6x3` as a `Matrix4x3`. Upstream: `fixed_view::<4, 3>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix6x3FixedViewMatrix4x3<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix6x3<T>, Matrix4x3<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix6x3<T>, irow: usize, icol: usize) -> Matrix4x3<T> {
        match icol {
            0 => match irow {
                0 => Matrix4x3 {
                    m11: self.m11,
    ...
    #[inline(always)]
    fn view(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4x3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 4 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix6x3<T>, irow: usize, icol: usize) -> Matrix4x3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4x3<T> {
        Self::view(self, start, shape)
    }
}

/// The 5x1 blocks of a `Matrix6x3` as a `Vector5`. Upstream: `fixed_view::<5, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix6x3FixedViewVector5<T, +Copy<T>, +Drop<T>> of FixedView<Matrix6x3<T>, Vector5<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix6x3<T>, irow: usize, icol: usize) -> Vector5<T> {
        match icol {
            0 => match irow {
    ...
    #[inline(always)]
    fn view(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Vector5<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 5 || ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix6x3<T>, irow: usize, icol: usize) -> Vector5<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Vector5<T> {
        Self::view(self, start, shape)
    }
}

/// The 5x2 blocks of a `Matrix6x3` as a `Matrix5x2`. Upstream: `fixed_view::<5, 2>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix6x3FixedViewMatrix5x2<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix6x3<T>, Matrix5x2<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix6x3<T>, irow: usize, icol: usize) -> Matrix5x2<T> {
        match icol {
            0 => match irow {
                0 => Matrix5x2 {
                    m11: self.m11,
    ...
    #[inline(always)]
    fn view(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix5x2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 5 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix6x3<T>, irow: usize, icol: usize) -> Matrix5x2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix5x2<T> {
        Self::view(self, start, shape)
    }
}

/// The 5x3 blocks of a `Matrix6x3` as a `Matrix5x3`. Upstream: `fixed_view::<5, 3>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix6x3FixedViewMatrix5x3<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix6x3<T>, Matrix5x3<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix6x3<T>, irow: usize, icol: usize) -> Matrix5x3<T> {
        match icol {
            0 => match irow {
                0 => Matrix5x3 {
                    m11: self.m11,
    ...
    #[inline(always)]
    fn view(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix5x3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 5 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix6x3<T>, irow: usize, icol: usize) -> Matrix5x3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix5x3<T> {
        Self::view(self, start, shape)
    }
}

/// The 6x1 blocks of a `Matrix6x3` as a `Vector6`. Upstream: `fixed_view::<6, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix6x3FixedViewVector6<T, +Copy<T>, +Drop<T>> of FixedView<Matrix6x3<T>, Vector6<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix6x3<T>, irow: usize, icol: usize) -> Vector6<T> {
        match icol {
            0 => match irow {
                0 => Vector6 {
                    x: self.m11, y: self.m21, z: self.m31, w: self.m41, a: self.m51, b: self.m61,
    ...
    #[inline(always)]
    fn view(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Vector6<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 6 || ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix6x3<T>, irow: usize, icol: usize) -> Vector6<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Vector6<T> {
        Self::view(self, start, shape)
    }
}

/// The 6x2 blocks of a `Matrix6x3` as a `Matrix6x2`. Upstream: `fixed_view::<6, 2>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix6x3FixedViewMatrix6x2<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix6x3<T>, Matrix6x2<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix6x3<T>, irow: usize, icol: usize) -> Matrix6x2<T> {
        match icol {
            0 => match irow {
                0 => Matrix6x2 {
                    m11: self.m11,
    ...
    #[inline(always)]
    fn view(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix6x2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 6 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix6x3<T>, irow: usize, icol: usize) -> Matrix6x2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix6x2<T> {
        Self::view(self, start, shape)
    }
}

/// The 6x3 blocks of a `Matrix6x3` as a `Matrix6x3`. Upstream: `fixed_view::<6, 3>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix6x3FixedViewMatrix6x3<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix6x3<T>, Matrix6x3<T>> {
    fn fixed_view(self: Matrix6x3<T>, irow: usize, icol: usize) -> Matrix6x3<T> {
        match icol {
            0 => match irow {
                0 => Matrix6x3 {
                    m11: self.m11,
    ...
    #[inline(always)]
    fn view(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix6x3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 6 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix6x3<T>, irow: usize, icol: usize) -> Matrix6x3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix6x3<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix6x3<T> {
        Self::view(self, start, shape)
    }
}

/// The first 1 components of a row of a `Matrix6x3` as a `Matrix1`. Upstream: `row_part` (`n = 1`).
pub impl Matrix6x3RowPartMatrix1<T, +Copy<T>, +Drop<T>> of RowPart<Matrix6x3<T>, Matrix1<T>> {
    #[inline(always)]
    fn row_part(self: Matrix6x3<T>, i: usize, n: usize) -> Matrix1<T> {
        if n != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The first 2 components of a row of a `Matrix6x3` as a `RowVector2`. Upstream: `row_part` (`n =
/// 2`).
pub impl Matrix6x3RowPartRowVector2<T, +Copy<T>, +Drop<T>> of RowPart<Matrix6x3<T>, RowVector2<T>> {
    #[inline(always)]
    fn row_part(self: Matrix6x3<T>, i: usize, n: usize) -> RowVector2<T> {
        if n != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 3 components of a row of a `Matrix6x3` as a `RowVector3`. Upstream: `row_part` (`n =
/// 3`).
pub impl Matrix6x3RowPartRowVector3<T, +Copy<T>, +Drop<T>> of RowPart<Matrix6x3<T>, RowVector3<T>> {
    #[inline(always)]
    fn row_part(self: Matrix6x3<T>, i: usize, n: usize) -> RowVector3<T> {
        if n != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 1 components of a column of a `Matrix6x3` as a `Matrix1`. Upstream: `column_part` (`n
/// = 1`).
pub impl Matrix6x3ColumnPartMatrix1<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix6x3<T>, Matrix1<T>> {
    #[inline(always)]
    fn column_part(self: Matrix6x3<T>, i: usize, n: usize) -> Matrix1<T> {
        if n != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The first 2 components of a column of a `Matrix6x3` as a `Vector2`. Upstream: `column_part` (`n
/// = 2`).
pub impl Matrix6x3ColumnPartVector2<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix6x3<T>, Vector2<T>> {
    #[inline(always)]
    fn column_part(self: Matrix6x3<T>, i: usize, n: usize) -> Vector2<T> {
        if n != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, 0, i)
...
/// The first 3 components of a column of a `Matrix6x3` as a `Vector3`. Upstream: `column_part` (`n
/// = 3`).
pub impl Matrix6x3ColumnPartVector3<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix6x3<T>, Vector3<T>> {
    #[inline(always)]
    fn column_part(self: Matrix6x3<T>, i: usize, n: usize) -> Vector3<T> {
        if n != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, 0, i)
...
/// The first 4 components of a column of a `Matrix6x3` as a `Vector4`. Upstream: `column_part` (`n
/// = 4`).
pub impl Matrix6x3ColumnPartVector4<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix6x3<T>, Vector4<T>> {
    #[inline(always)]
    fn column_part(self: Matrix6x3<T>, i: usize, n: usize) -> Vector4<T> {
        if n != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, 0, i)
...
/// The first 5 components of a column of a `Matrix6x3` as a `Vector5`. Upstream: `column_part` (`n
/// = 5`).
pub impl Matrix6x3ColumnPartVector5<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix6x3<T>, Vector5<T>> {
    #[inline(always)]
    fn column_part(self: Matrix6x3<T>, i: usize, n: usize) -> Vector5<T> {
        if n != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, 0, i)
...
/// The first 6 components of a column of a `Matrix6x3` as a `Vector6`. Upstream: `column_part` (`n
/// = 6`).
pub impl Matrix6x3ColumnPartVector6<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix6x3<T>, Vector6<T>> {
    #[inline(always)]
    fn column_part(self: Matrix6x3<T>, i: usize, n: usize) -> Vector6<T> {
        if n != 6 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, 0, i)
...
/// The Kronecker product of a `Matrix6x3` and a `Matrix1`, a `Matrix6x3`: one floored product per
/// component. Panics on overflow. Upstream: `kronecker`.
pub impl Matrix6x3KroneckerMatrix1<
    T, +Mul<T>, +Copy<T>, +Drop<T>,
> of MatrixKronecker<Matrix6x3<T>, Matrix1<T>> {
    type Output = Matrix6x3<T>;
    fn kronecker(self: Matrix6x3<T>, rhs: Matrix1<T>) -> Matrix6x3<T> {
        Matrix6x3 {
            m11: self.m11 * rhs.x,
            m21: self.m21 * rhs.x,
            m31: self.m31 * rhs.x,
...
/// The Kronecker product of a `Matrix6x3` and a `RowVector2`, a `Matrix6`: one floored product per
/// component. Panics on overflow. Upstream: `kronecker`.
pub impl Matrix6x3KroneckerRowVector2<
    T, +Mul<T>, +Copy<T>, +Drop<T>,
> of MatrixKronecker<Matrix6x3<T>, RowVector2<T>> {
    type Output = Matrix6<T>;
    fn kronecker(self: Matrix6x3<T>, rhs: RowVector2<T>) -> Matrix6<T> {
        Matrix6 {
            m11: self.m11 * rhs.x,
            m21: self.m21 * rhs.x,
            m31: self.m31 * rhs.x,
...
pub(crate) impl Matrix6x3PadTo6<T, +Copy<T>, +Drop<T>> of PadTo6<Matrix6x3<T>, T> {
    #[inline(always)]
...
pub(crate) impl Matrix6x3CropFrom6<T, +Copy<T>, +Drop<T>> of CropFrom6<Matrix6x3<T>, T> {
    #[inline(always)]
