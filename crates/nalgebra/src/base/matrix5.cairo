...
//! `Matrix5`: a statically sized 5x5 matrix (upstream `nalgebra::Matrix5`).
//!
...
pub impl Matrix5Impl<
    ...
    +PartialOrd<T>,
> of Matrix5Trait<T> {
    /// The 5x5 matrix with the given components, in ROW-major order. Upstream: `Matrix5::new`.
    #[inline(always)]
    fn new(
        m11: T,
        m12: T,
    ...
    /// The zero 5x5 matrix. Upstream: `Matrix5::zeros`.
    #[inline(always)]
    fn zeros() -> Matrix5<T> {
        Matrix5 {
            m11: R::zero(),
    ...
    /// The identity matrix. Upstream: `Matrix5::identity`.
    #[inline(always)]
    fn identity() -> Matrix5<T> {
        Matrix5 {
            m11: R::one(),
            m21: R::zero(),
            m31: R::zero(),
    ...
    /// The diagonal matrix `diag(d)`. Upstream: `Matrix5::from_diagonal`.
    #[inline(always)]
    fn from_diagonal(d: Vector5<T>) -> Matrix5<T> {
        Matrix5 {
            m11: d.x,
            m21: R::zero(),
            m31: R::zero(),
    ...
    /// The matrix `e * I`. Upstream: `Matrix5::from_diagonal_element`.
    #[inline(always)]
    fn from_diagonal_element(e: T) -> Matrix5<T> {
        Matrix5 {
            m11: e,
            m21: R::zero(),
            m31: R::zero(),
    ...
    /// The diagonal, a `Vector5`. Upstream: `diagonal`.
    #[inline(always)]
    fn diagonal(self: Matrix5<T>) -> Vector5<T> {
        Vector5 { x: self.m11, y: self.m22, z: self.m33, w: self.m44, a: self.m55 }
    }

    /// The transpose, a `Matrix5`. Exact. Upstream: `transpose`.
    #[inline(always)]
    fn transpose(self: Matrix5<T>) -> Matrix5<T> {
        Matrix5 {
            m11: self.m11,
            m21: self.m12,
            m31: self.m13,
    ...
    #[inline(always)]
    fn trace(self: Matrix5<T>) -> T {
        self.m11 + self.m22 + self.m33 + self.m44 + self.m55
    }

    /// Component-wise absolute value. Exact; panics on overflow (`|MIN|`). Upstream: `abs`.
    #[inline(always)]
    fn abs(self: Matrix5<T>) -> Matrix5<T> {
        Matrix5 {
            m11: R::abs(self.m11),
            m21: R::abs(self.m21),
            m31: R::abs(self.m31),
    ...
    #[inline(always)]
    fn scale(self: Matrix5<T>, k: T) -> Matrix5<T> {
        Matrix5 {
            m11: self.m11 * k,
            m21: self.m21 * k,
            m31: self.m31 * k,
    ...
    #[inline(always)]
    fn is_identity(self: Matrix5<T>, ulps: u64) -> bool {
    ...
    #[inline(always)]
    fn abs_diff_eq(self: Matrix5<T>, other: Matrix5<T>, ulps: u64) -> bool {
    ...
    /// The 5x5 matrix whose components all equal `elem`. Upstream: `Matrix5::repeat`.
    #[inline(always)]
    fn repeat(elem: T) -> Matrix5<T> {
        Matrix5 {
            m11: elem,
            m21: elem,
            m31: elem,
    ...
    /// Alias of `repeat`. Upstream: `Matrix5::from_element`.
    #[inline(always)]
    fn from_element(elem: T) -> Matrix5<T> {
        Matrix5 {
            m11: elem,
            m21: elem,
            m31: elem,
    ...
    /// The 5x5 matrix whose component `(i, j)` (row, column, 0-based) is `f(i, j)`, `f` being
    ...
    /// The 5x5 matrix of the 25 values of `data`, in row-major order. Panics with `nalgebra: wrong
    /// slice length` unless `data.len() == 25`. Upstream: `Matrix5::from_row_slice` (`&[T]`).
    ///
    /// `data` is read as ONE fixed-size array (`Span -> @Box<[T; 25]>`, one length check): measured
    /// about 5 times cheaper than a bounds-checked `*data[k]` per component
    /// (`bench_matrix3_from_row_slice__alt_span_index`).
    fn from_row_slice(data: Span<T>) -> Matrix5<T> {
        let boxed: @Box<[T; 25]> = data.try_into().expect(errors::SLICE_LENGTH);
        let [
            v0,
            v1,
            v2,
    ...
    /// wrong slice length` unless `data.len() == 25`. Upstream: `Matrix5::from_column_slice`
    /// (`&[T]`).
    ///
    /// `data` is read as ONE fixed-size array (`Span -> @Box<[T; 25]>`, one length check): measured
    /// about 5 times cheaper than a bounds-checked `*data[k]` per component
    /// (`bench_matrix3_from_row_slice__alt_span_index`).
    fn from_column_slice(data: Span<T>) -> Matrix5<T> {
        let boxed: @Box<[T; 25]> = data.try_into().expect(errors::SLICE_LENGTH);
        let [
            v0,
            v1,
    ...
    /// The 5x5 matrix whose first `data.len()` diagonal components are `data`, every other
    /// component zero. Panics with `nalgebra: diagonal too long` when `data.len() > 5`. Upstream:
    /// `Matrix5::from_partial_diagonal` (`&[T]`).
    fn from_partial_diagonal(data: Span<T>) -> Matrix5<T> {
        let len = data.len();
        if len > 5 {
    ...
    fn is_zero(self: Matrix5<T>) -> bool {
        self.m11 == R::zero()
            && self.m21 == R::zero()
            && self.m31 == R::zero()
            && self.m41 == R::zero()
    ...
    fn component_mul(self: Matrix5<T>, rhs: Matrix5<T>) -> Matrix5<T> {
        Matrix5 {
    ...
    fn component_mul_assign(ref self: Matrix5<T>, rhs: Matrix5<T>) {
        self =
            Matrix5 {
                m11: self.m11 * rhs.m11,
                m21: self.m21 * rhs.m21,
    ...
    fn component_div(self: Matrix5<T>, rhs: Matrix5<T>) -> Matrix5<T> {
        Matrix5 {
    ...
    fn component_div_assign(ref self: Matrix5<T>, rhs: Matrix5<T>) {
        self =
            Matrix5 {
                m11: R::div(self.m11, rhs.m11),
                m21: R::div(self.m21, rhs.m21),
    ...
    fn inf(self: Matrix5<T>, other: Matrix5<T>) -> Matrix5<T> {
        Matrix5 {
    ...
    fn sup(self: Matrix5<T>, other: Matrix5<T>) -> Matrix5<T> {
        Matrix5 {
    ...
    fn cmpy(ref self: Matrix5<T>, alpha: T, a: Matrix5<T>, b: Matrix5<T>, beta: T) {
        self =
            Matrix5 {
    ...
    fn cdpy(ref self: Matrix5<T>, alpha: T, a: Matrix5<T>, b: Matrix5<T>, beta: T) {
        self =
            Matrix5 {
    ...
    fn iamax_full(self: Matrix5<T>) -> (usize, usize) {
    ...
    #[inline(always)]
    fn icamax_full(self: Matrix5<T>) -> (usize, usize) {
    ...
    fn dot(self: Matrix5<T>, rhs: Matrix5<T>) -> T {
    ...
    /// `norm_squared`.
    fn norm_squared(self: Matrix5<T>) -> T {
        ...
        R::wide_rescale(R::wide_add_prod(w, self.m55, self.m55))
    }
    ...
    fn norm(self: Matrix5<T>) -> T {
        ...
        let w = R::wide_add_prod(w, self.m35, self.m35);
        let w = R::wide_add_prod(w, self.m45, self.m45);
        R::wide_sqrt(R::wide_add_prod(w, self.m55, self.m55))
    }

    /// Alias of `norm_squared`. Upstream: `magnitude_squared`.
    #[inline(always)]
    fn magnitude_squared(self: Matrix5<T>) -> T {
        Self::norm_squared(self)
    }

    /// Alias of `norm`. Upstream: `magnitude`.
    #[inline(always)]
    fn magnitude(self: Matrix5<T>) -> T {
        Self::norm(self)
    }
    ...
    fn metric_distance(self: Matrix5<T>, rhs: Matrix5<T>) -> T {
        ...
        R::wide_sqrt(R::wide_add_prod(w, self.m55 - rhs.m55, self.m55 - rhs.m55))
    }
    ...
    fn unscale(self: Matrix5<T>, k: T) -> Matrix5<T> {
    ...
    fn normalize(self: Matrix5<T>) -> Matrix5<T> {
    ...
    fn try_normalize(self: Matrix5<T>, min_norm: T) -> Option<Matrix5<T>> {
    ...
    /// `cap_magnitude`.
    fn cap_magnitude(self: Matrix5<T>, max: T) -> Matrix5<T> {
        let n = Self::norm(self);
        if n <= max {
            self
        } else {
    ...
    fn try_set_magnitude(ref self: Matrix5<T>, magnitude: T, min_magnitude: T) {
        let n = Self::norm(self);
        if n > min_magnitude {
    ...
    /// The conjugate transpose, a `Matrix5`: the transpose for a real scalar. Exact. Upstream:
    /// `adjoint`.
    #[inline(always)]
    fn adjoint(self: Matrix5<T>) -> Matrix5<T> {
        Self::transpose(self)
    }

    /// Alias of `adjoint` (deprecated upstream). Upstream: `conjugate_transpose`.
    #[inline(always)]
    fn conjugate_transpose(self: Matrix5<T>) -> Matrix5<T> {
        Self::transpose(self)
    }

    /// The component-wise conjugate: `self` for a real scalar. Upstream: `conjugate`.
    #[inline(always)]
    fn conjugate(self: Matrix5<T>) -> Matrix5<T> {
        self
    }
    ...
    fn symmetric_part(self: Matrix5<T>) -> Matrix5<T> {
    ...
    #[inline(always)]
    fn hermitian_part(self: Matrix5<T>) -> Matrix5<T> {
        Self::symmetric_part(self)
    }

    /// The `Matrix6` `[[self, 0], [0, 1]]`: `self` as the linear part of a homogeneous
    /// transformation. Exact. Upstream: `to_homogeneous`.
    fn to_homogeneous(self: Matrix5<T>) -> Matrix6<T> {
        Matrix6 {
            m11: self.m11,
            m21: self.m21,
            m31: self.m31,
    ...
    fn cast<U, +Into<T, U>, +Drop<U>>(self: Matrix5<T>) -> Matrix5<U> {
        Matrix5 {
            m11: self.m11.into(),
            m21: self.m21.into(),
            m31: self.m31.into(),
    ...
    fn try_cast<U, +TryInto<T, U>, +Drop<U>>(self: Matrix5<T>) -> Option<Matrix5<U>> {
    ...
    /// (DESIGN D3).
    fn relative_eq(self: Matrix5<T>, other: Matrix5<T>, epsilon: u64, max_relative: T) -> bool {
    ...
    fn ulps_eq(self: Matrix5<T>, other: Matrix5<T>, epsilon: u64, max_ulps: u32) -> bool {
    ...
    /// The 5x5 matrix of `f(x)` for every component `x` (called in column-major order). `f` is any
    ...
    /// The 5x5 matrix of `f(i, j, x)` for every component `x` at row `i`, column `j` (0-based,
    ...
    /// The 5x5 matrix of `f(a, b)` for the components `a` of `self` and `b` of `rhs` at the same
    ...
    fn zip_map<
        ...
        +Drop<Func::Output>,
    >(
        self: Matrix5<T>, rhs: Matrix5<T2>, f: F,
    ) -> Matrix5<Func::Output> {
        Matrix5 {
    ...
    /// The 5x5 matrix of `f(a, b, c)` for the components of `self`, `b` and `c` at the same
    /// position. `f` is any closure or `Fn` value; Cairo closures take their arguments by value and
    ...
    fn zip_zip_map<
        ...
        +Drop<Func::Output>,
    >(
        self: Matrix5<T>, b: Matrix5<T2>, c: Matrix5<T3>, f: F,
    ) -> Matrix5<Func::Output> {
        Matrix5 {
    ...
    /// `f(.. f(f(init, x0), x1) .., x24)` over the components in column-major order. The closure's
    ...
    >(
        self: Matrix5<T>, rhs: Matrix5<T2>, init: Acc, f: F,
    ) -> Acc {
    ...
    fn zip_apply<
        ...
        +Into<Func::Output, T>,
    >(
        ref self: Matrix5<T>, rhs: Matrix5<T2>, f: F,
    ) {
        self =
            Matrix5 {
    ...
    fn zip_zip_apply<
        ...
        +Into<Func::Output, T>,
    >(
        ref self: Matrix5<T>, b: Matrix5<T2>, c: Matrix5<T3>, f: F,
    ) {
        self =
            Matrix5 {
    ...
    /// The `Vector5` of `f(d)` for every diagonal component `d` (top to bottom):
    ...
    fn map_diagonal<F, +Drop<F>, impl Func: core::ops::Fn<F, (T,)>, +Drop<Func::Output>>(
        ...
        Vector5 { x: f(self.m11), y: f(self.m22), z: f(self.m33), w: f(self.m44), a: f(self.m55) }
    }

    /// Sets every component to `val`. Upstream: `fill`.
    fn fill(ref self: Matrix5<T>, val: T) {
        self =
            Matrix5 {
                m11: val,
                m21: val,
    ...
    fn fill_diagonal(ref self: Matrix5<T>, val: T) {
        self =
            Matrix5 {
                m11: val,
                m21: self.m21,
    ...
    fn fill_with_identity(ref self: Matrix5<T>) {
        self =
            Matrix5 {
                m11: R::one(),
                m21: R::zero(),
    ...
    fn fill_row(ref self: Matrix5<T>, i: usize, val: T) {
        self = match i {
            0 => Matrix5 {
                m11: val,
                m21: self.m21,
    ...
    fn fill_column(ref self: Matrix5<T>, j: usize, val: T) {
        self = match j {
            0 => Matrix5 {
                m11: val,
                m21: val,
    ...
    fn fill_lower_triangle(ref self: Matrix5<T>, val: T, shift: usize) {
        self = match shift {
            0 => Matrix5 {
                m11: val,
                m21: val,
    ...
    fn fill_upper_triangle(ref self: Matrix5<T>, val: T, shift: usize) {
        self = match shift {
            0 => Matrix5 {
                m11: val,
                m21: self.m21,
    ...
    fn fill_lower_triangle_with_upper_triangle(ref self: Matrix5<T>) {
        self =
            Matrix5 {
                m11: self.m11,
                m21: self.m12,
    ...
    fn fill_upper_triangle_with_lower_triangle(ref self: Matrix5<T>) {
        self =
            Matrix5 {
                m11: self.m11,
                m21: self.m21,
    ...
    fn set_row(ref self: Matrix5<T>, i: usize, row: RowVector5<T>) {
        self = match i {
            0 => Matrix5 {
                m11: row.x,
                m21: self.m21,
    ...
    fn set_column(ref self: Matrix5<T>, j: usize, column: Vector5<T>) {
        self = match j {
            0 => Matrix5 {
                m11: column.x,
                m21: column.y,
    ...
    fn set_diagonal(ref self: Matrix5<T>, diag: Vector5<T>) {
        self =
            Matrix5 {
                m11: diag.x,
                m21: self.m21,
    ...
    fn set_partial_diagonal(ref self: Matrix5<T>, diag: Span<T>) {
        let len = diag.len();
        self =
            Matrix5 {
                m11: if len > 0 {
    ...
    /// Sets `self` to `other` (a `Matrix5`: upstream requires the same shape). Upstream:
    /// `copy_from`.
    #[inline(always)]
    fn copy_from(ref self: Matrix5<T>, other: Matrix5<T>) {
        self = other;
    }

    /// Sets `self` to the 25 values of `slice` in column-major order (`from_column_slice`). Panics
    /// with `nalgebra: wrong slice length` unless `slice.len() == 25`. Upstream: `copy_from_slice`
    /// (`&[T]`).
    #[inline(always)]
    fn copy_from_slice(ref self: Matrix5<T>, slice: Span<T>) {
        self = Self::from_column_slice(slice);
    }

    /// Sets `self` to the transpose of `other`, a `Matrix5`. Exact. Upstream: `tr_copy_from`.
    fn tr_copy_from(ref self: Matrix5<T>, other: Matrix5<T>) {
        self =
            Matrix5 {
                m11: other.m11,
                m21: other.m12,
    ...
    #[inline]
    fn swap(ref self: Matrix5<T>, row_cols1: (usize, usize), row_cols2: (usize, usize)) {
        let a = MatrixIndex::index(self, row_cols1);
        let b = MatrixIndex::index(self, row_cols2);
        self =
            Matrix5EditTrait::replace(Matrix5EditTrait::replace(self, row_cols1, b), row_cols2, a);
    }
    ...
    /// benchmark.
    #[inline]
    fn swap_rows(ref self: Matrix5<T>, irow1: usize, irow2: usize) {
        let a = Matrix5EditTrait::row_at(self, irow1);
        let b = Matrix5EditTrait::row_at(self, irow2);
    ...
    #[inline]
    fn swap_columns(ref self: Matrix5<T>, icol1: usize, icol2: usize) {
        let a = Matrix5EditTrait::column_at(self, icol1);
        let b = Matrix5EditTrait::column_at(self, icol2);
    ...
    /// `scale_mut`.
    #[inline(always)]
    fn scale_mut(ref self: Matrix5<T>, k: T) {
    ...
    #[inline(always)]
    fn unscale_mut(ref self: Matrix5<T>, k: T) {
    ...
    #[inline(always)]
    fn normalize_mut(ref self: Matrix5<T>) -> T {
        let n = Self::norm(self);
    ...
    #[inline(always)]
    fn try_normalize_mut(ref self: Matrix5<T>, min_norm: T) -> Option<T> {
        let n = Self::norm(self);
        if n <= min_norm {
            return None;
        }
    ...
    #[inline(always)]
    fn set_magnitude(ref self: Matrix5<T>, magnitude: T) {
        let n = Self::norm(self);
        self = Self::scale(self, R::div(magnitude, n));
    }
    ...
    #[inline(always)]
    fn conjugate_transform_mut(ref self: Matrix5<T>) {
        self = Self::transpose(self);
    }

    /// Writes the transpose of `self` into `out`, a `Matrix5`. Exact. Upstream: `transpose_to`.
    #[inline(always)]
    fn transpose_to(self: Matrix5<T>, ref out: Matrix5<T>) {
        out = Self::transpose(self);
    }

    /// Writes the adjoint of `self` (its transpose for a real scalar) into `out`, a `Matrix5`.
    /// Exact. Upstream: `adjoint_to`.
    #[inline(always)]
    fn adjoint_to(self: Matrix5<T>, ref out: Matrix5<T>) {
        out = Self::transpose(self);
    }

    /// Writes the adjoint of `self` (deprecated upstream alias of `adjoint_to`) into `out`, a
    /// `Matrix5`. Exact. Upstream: `conjugate_transpose_to`.
    #[inline(always)]
    fn conjugate_transpose_to(self: Matrix5<T>, ref out: Matrix5<T>) {
        out = Self::transpose(self);
    }

    /// Writes the sum `self + rhs` into `out`. Exact; panics on overflow. Upstream: `add_to`.
    #[inline(always)]
    fn add_to(self: Matrix5<T>, rhs: Matrix5<T>, ref out: Matrix5<T>) {
        out = self + rhs;
    }
    ...
    /// `sub_to`.
    #[inline(always)]
    fn sub_to(self: Matrix5<T>, rhs: Matrix5<T>, ref out: Matrix5<T>) {
        out = self - rhs;
    }
    ...
    #[inline]
    fn apply_norm<N, +Drop<N>, impl Nm: Norm<N, Matrix5<T>, T>>(self: Matrix5<T>, norm: N) -> T {
        Nm::norm(@norm, self)
    }
    ...
    #[inline]
    fn apply_metric_distance<N, +Drop<N>, impl Nm: Norm<N, Matrix5<T>, T>>(
        self: Matrix5<T>, rhs: Matrix5<T>, norm: N,
    ) -> T {
    ...
    #[inline(always)]
    fn nrows(self: Matrix5<T>) -> usize {
        5
    }

    /// The number of columns, 5. Upstream: `ncols`.
    #[inline(always)]
    fn ncols(self: Matrix5<T>) -> usize {
        5
    }

    /// `(nrows, ncols)`, `(5, 5)`. Upstream: `shape`.
    #[inline(always)]
    fn shape(self: Matrix5<T>) -> (usize, usize) {
        (5, 5)
    }

    /// Whether the shape is square: `true`. Upstream: `is_square`.
    #[inline(always)]
    fn is_square(self: Matrix5<T>) -> bool {
        true
    }
    ...
    #[inline(always)]
    fn vector_to_matrix_index(self: Matrix5<T>, i: usize) -> (usize, usize) {
    ...
    #[inline(always)]
    fn row(self: Matrix5<T>, i: usize) -> RowVector5<T> {
        Matrix5EditTrait::row_at(self, i)
    }
    ...
    #[inline(always)]
    fn column(self: Matrix5<T>, j: usize) -> Vector5<T> {
        Matrix5EditTrait::column_at(self, j)
    }
    ...
    #[inline(always)]
    fn upper_triangle(self: Matrix5<T>) -> Matrix5<T> {
        Matrix5 {
            m11: self.m11,
            m21: R::zero(),
            m31: R::zero(),
    ...
    #[inline(always)]
    fn lower_triangle(self: Matrix5<T>) -> Matrix5<T> {
        Matrix5 {
            m11: self.m11,
            m21: self.m21,
            m31: self.m31,
    ...
    /// The 5x5 matrix whose 5 rows are the given vectors, each a `Vector5` of the row's 5
    ...
    #[inline(always)]
    fn from_rows(
        r1: Vector5<T>, r2: Vector5<T>, r3: Vector5<T>, r4: Vector5<T>, r5: Vector5<T>,
    ) -> Matrix5<T> {
        Matrix5 {
            m11: r1.x,
            m21: r2.x,
    ...
    /// The 5x5 matrix whose 5 columns are the given `Vector5`s. Exact. Upstream: `from_columns` (a
    /// slice of column vectors).
    #[inline(always)]
    fn from_columns(
        c1: Vector5<T>, c2: Vector5<T>, c3: Vector5<T>, c4: Vector5<T>, c5: Vector5<T>,
    ) -> Matrix5<T> {
        Matrix5 {
            m11: c1.x,
            m21: c1.y,
    ...
    #[inline(always)]
    fn is_orthogonal(self: Matrix5<T>, ulps: u64) -> bool {
        Self::is_identity(MatrixTrMul::tr_mul(self, self), ulps)
    }

    /// `self` with a row of `val` inserted at index `i` (`i <= 5`), a `Matrix6x5`. Panics with
    /// `nalgebra: index out of bounds` for `i > 5`. Upstream: `insert_row`.
    fn insert_row(self: Matrix5<T>, i: usize, val: T) -> Matrix6x5<T> {
        match i {
            0 => Matrix6x5 {
                m11: val,
                m21: self.m11,
    ...
    /// `self` with a column of `val` inserted at index `i` (`i <= 5`), a `Matrix5x6`. Panics with
    /// `nalgebra: index out of bounds` for `i > 5`. Upstream: `insert_column`.
    fn insert_column(self: Matrix5<T>, i: usize, val: T) -> Matrix5x6<T> {
        match i {
            0 => Matrix5x6 {
                m11: val,
                m21: val,
    ...
    /// `self` without its row `i`, a `Matrix4x5`. Panics with `nalgebra: index out of bounds` for
    /// `i >= 5`. Upstream: `remove_row`.
    fn remove_row(self: Matrix5<T>, i: usize) -> Matrix4x5<T> {
        match i {
            0 => Matrix4x5 {
                m11: self.m21,
                m21: self.m31,
    ...
    /// `self` without its column `i`, a `Matrix5x4`. Panics with `nalgebra: index out of bounds`
    /// for `i >= 5`. Upstream: `remove_column`.
    fn remove_column(self: Matrix5<T>, i: usize) -> Matrix5x4<T> {
        match i {
            0 => Matrix5x4 {
                m11: self.m12,
                m21: self.m22,
...
/// The operations of `Matrix5<T>` that need `Transcendental` (inverse trigonometry, `exp`, `ln`):
/// a scalar may implement `Real` only.
#[generate_trait]
pub impl Matrix5AngleImpl<
    T,
    impl R: Real<T>,
...
> of Matrix5AngleTrait<T> {
    ...
    fn angle(self: Matrix5<T>, other: Matrix5<T>) -> T {
        let n1 = Matrix5Trait::norm(self);
        let n2 = Matrix5Trait::norm(other);
        if n1 == R::zero() || n2 == R::zero() {
            return R::zero();
        }
        let u = Matrix5Trait::unscale(self, n1);
        let v = Matrix5Trait::unscale(other, n2);
        let half = Tr::atan2(Matrix5Trait::metric_distance(u, v), Matrix5Trait::norm(u + v));
        half + half
    }
    ...
    /// Upstream: `lp_norm`.
    fn lp_norm(self: Matrix5<T>, p: i32) -> T {
        if p < 1 {
        ...
        if p == 2 {
            return Matrix5Trait::norm(self);
        }
        let m = Matrix5Trait::amax(self);
        if m == R::zero() {
            return R::zero();
        }
        let r = Matrix5Trait::unscale(Matrix5Trait::abs(self), m);
...
pub impl Matrix5Add<T, +Add<T>, +Copy<T>, +Drop<T>> of Add<Matrix5<T>> {
    #[inline(always)]
    fn add(lhs: Matrix5<T>, rhs: Matrix5<T>) -> Matrix5<T> {
        Matrix5 {
            m11: lhs.m11 + rhs.m11,
            m21: lhs.m21 + rhs.m21,
            m31: lhs.m31 + rhs.m31,
...
pub impl Matrix5Sub<T, +Sub<T>, +Copy<T>, +Drop<T>> of Sub<Matrix5<T>> {
    #[inline(always)]
    fn sub(lhs: Matrix5<T>, rhs: Matrix5<T>) -> Matrix5<T> {
        Matrix5 {
            m11: lhs.m11 - rhs.m11,
            m21: lhs.m21 - rhs.m21,
            m31: lhs.m31 - rhs.m31,
...
pub impl Matrix5Neg<T, +Neg<T>, +Copy<T>, +Drop<T>> of Neg<Matrix5<T>> {
    #[inline(always)]
    fn neg(a: Matrix5<T>) -> Matrix5<T> {
        Matrix5 {
            m11: -a.m11,
            m21: -a.m21,
            m31: -a.m31,
...
pub impl Matrix5AddAssign<T, +Add<T>, +Copy<T>, +Drop<T>> of AddAssign<Matrix5<T>, Matrix5<T>> {
    #[inline(always)]
    fn add_assign(ref self: Matrix5<T>, rhs: Matrix5<T>) {
        self = self + rhs;
    }
}

/// `self -= rhs`. Exact; panics on overflow. Upstream: `SubAssign`.
pub impl Matrix5SubAssign<T, +Sub<T>, +Copy<T>, +Drop<T>> of SubAssign<Matrix5<T>, Matrix5<T>> {
    #[inline(always)]
    fn sub_assign(ref self: Matrix5<T>, rhs: Matrix5<T>) {
        self = self - rhs;
    }
}

/// `a * b` (matrix product): the 5 products of each component accumulated exactly in `Real::Wide`
/// and rescaled once. Panics on overflow. Upstream: `Mul`.
pub impl Matrix5Mul<
    T, impl R: Real<T>, +Mul<T>, +Copy<T>, +Drop<T>, +Drop<R::Wide>,
> of Mul<Matrix5<T>> {
    fn mul(lhs: Matrix5<T>, rhs: Matrix5<T>) -> Matrix5<T> {
        Matrix5 {
            m11: Fused::sum_prod5(
                lhs.m11,
                rhs.m11,
...
pub impl Matrix5MulAssign<
    T, impl R: Real<T>, +Mul<T>, +Copy<T>, +Drop<T>, +Drop<R::Wide>,
> of MulAssign<Matrix5<T>, Matrix5<T>> {
    #[inline(always)]
    fn mul_assign(ref self: Matrix5<T>, rhs: Matrix5<T>) {
        self = self * rhs;
    }
}
...
/// `self * rhs`, a `Vector5`: the 5 products of each component accumulated exactly in `Real::Wide`
/// and rescaled once. Panics on overflow. Upstream: `Matrix5 * Vector5` (`Mul`).
pub impl Matrix5MulVector5<
    T, impl R: Real<T>, +Mul<T>, +Copy<T>, +Drop<T>, +Drop<R::Wide>,
...
/// `self * rhs`, a `Matrix5x2`: the 5 products of each component accumulated exactly in
/// `Real::Wide` and rescaled once. Panics on overflow. Upstream: `Matrix5 * Matrix5x2` (`Mul`).
pub impl Matrix5MulMatrix5x2<
    T, impl R: Real<T>, +Mul<T>, +Copy<T>, +Drop<T>, +Drop<R::Wide>,
> of MatrixMul<Matrix5<T>, Matrix5x2<T>> {
    type Output = Matrix5x2<T>;
    fn mul_mat(self: Matrix5<T>, rhs: Matrix5x2<T>) -> Matrix5x2<T> {
        Matrix5x2 {
            m11: Fused::sum_prod5(
                self.m11,
                rhs.m11,
...
/// `self * rhs`, a `Matrix5x3`: the 5 products of each component accumulated exactly in
/// `Real::Wide` and rescaled once. Panics on overflow. Upstream: `Matrix5 * Matrix5x3` (`Mul`).
pub impl Matrix5MulMatrix5x3<
    T, impl R: Real<T>, +Mul<T>, +Copy<T>, +Drop<T>, +Drop<R::Wide>,
> of MatrixMul<Matrix5<T>, Matrix5x3<T>> {
    type Output = Matrix5x3<T>;
    fn mul_mat(self: Matrix5<T>, rhs: Matrix5x3<T>) -> Matrix5x3<T> {
        Matrix5x3 {
            m11: Fused::sum_prod5(
                self.m11,
                rhs.m11,
...
/// `self * rhs`, a `Matrix5x4`: the 5 products of each component accumulated exactly in
/// `Real::Wide` and rescaled once. Panics on overflow. Upstream: `Matrix5 * Matrix5x4` (`Mul`).
pub impl Matrix5MulMatrix5x4<
    T, impl R: Real<T>, +Mul<T>, +Copy<T>, +Drop<T>, +Drop<R::Wide>,
> of MatrixMul<Matrix5<T>, Matrix5x4<T>> {
    type Output = Matrix5x4<T>;
    fn mul_mat(self: Matrix5<T>, rhs: Matrix5x4<T>) -> Matrix5x4<T> {
        Matrix5x4 {
            m11: Fused::sum_prod5(
                self.m11,
                rhs.m11,
...
pub impl Matrix5MulMatrix5<
    T, impl R: Real<T>, +Mul<T>, +Copy<T>, +Drop<T>, +Drop<R::Wide>,
> of MatrixMul<Matrix5<T>, Matrix5<T>> {
    type Output = Matrix5<T>;
    #[inline(always)]
    fn mul_mat(self: Matrix5<T>, rhs: Matrix5<T>) -> Matrix5<T> {
        self * rhs
    }
}
...
/// `selfᵀ * rhs`, a `Vector5`: `mul_mat` of the transposed components, so the 5 products of each
...
pub impl Matrix5TrMulVector5<
    T, impl R: Real<T>, +Mul<T>, +Copy<T>, +Drop<T>, +Drop<R::Wide>,
> of MatrixTrMul<Matrix5<T>, Vector5<T>> {
    type Output = Vector5<T>;
    #[inline(always)]
    fn tr_mul(self: Matrix5<T>, rhs: Vector5<T>) -> Vector5<T> {
        MatrixMul::mul_mat(
            Matrix5 {
                m11: self.m11,
                m21: self.m12,
...
/// `selfᵀ * rhs`, a `Matrix5x2`: `mul_mat` of the transposed components, so the 5 products of
/// each component accumulated exactly in `Real::Wide` and rescaled once; bit-identical to
/// `self.transpose().mul_mat(rhs)`, at the same gas (the transpose only relabels values). Panics on
/// overflow. Upstream: `tr_mul`.
pub impl Matrix5TrMulMatrix5x2<
    T, impl R: Real<T>, +Mul<T>, +Copy<T>, +Drop<T>, +Drop<R::Wide>,
> of MatrixTrMul<Matrix5<T>, Matrix5x2<T>> {
    type Output = Matrix5x2<T>;
    #[inline(always)]
    fn tr_mul(self: Matrix5<T>, rhs: Matrix5x2<T>) -> Matrix5x2<T> {
        MatrixMul::mul_mat(
            Matrix5 {
                m11: self.m11,
                m21: self.m12,
...
/// `selfᵀ * rhs`, a `Matrix5x3`: `mul_mat` of the transposed components, so the 5 products of
/// each component accumulated exactly in `Real::Wide` and rescaled once; bit-identical to
/// `self.transpose().mul_mat(rhs)`, at the same gas (the transpose only relabels values). Panics on
/// overflow. Upstream: `tr_mul`.
pub impl Matrix5TrMulMatrix5x3<
    T, impl R: Real<T>, +Mul<T>, +Copy<T>, +Drop<T>, +Drop<R::Wide>,
> of MatrixTrMul<Matrix5<T>, Matrix5x3<T>> {
    type Output = Matrix5x3<T>;
    #[inline(always)]
    fn tr_mul(self: Matrix5<T>, rhs: Matrix5x3<T>) -> Matrix5x3<T> {
        MatrixMul::mul_mat(
            Matrix5 {
                m11: self.m11,
                m21: self.m12,
...
/// `selfᵀ * rhs`, a `Matrix5x4`: `mul_mat` of the transposed components, so the 5 products of
/// each component accumulated exactly in `Real::Wide` and rescaled once; bit-identical to
/// `self.transpose().mul_mat(rhs)`, at the same gas (the transpose only relabels values). Panics on
/// overflow. Upstream: `tr_mul`.
pub impl Matrix5TrMulMatrix5x4<
    T, impl R: Real<T>, +Mul<T>, +Copy<T>, +Drop<T>, +Drop<R::Wide>,
> of MatrixTrMul<Matrix5<T>, Matrix5x4<T>> {
    type Output = Matrix5x4<T>;
    #[inline(always)]
    fn tr_mul(self: Matrix5<T>, rhs: Matrix5x4<T>) -> Matrix5x4<T> {
        MatrixMul::mul_mat(
            Matrix5 {
                m11: self.m11,
                m21: self.m12,
...
/// `selfᵀ * rhs`, a `Matrix5`: `mul_mat` of the transposed components, so the 5 products of each
/// component accumulated exactly in `Real::Wide` and rescaled once; bit-identical to
/// `self.transpose().mul_mat(rhs)`, at the same gas (the transpose only relabels values). Panics on
/// overflow. Upstream: `tr_mul`.
pub impl Matrix5TrMulMatrix5<
    T, impl R: Real<T>, +Mul<T>, +Copy<T>, +Drop<T>, +Drop<R::Wide>,
> of MatrixTrMul<Matrix5<T>, Matrix5<T>> {
    type Output = Matrix5<T>;
    #[inline(always)]
    fn tr_mul(self: Matrix5<T>, rhs: Matrix5<T>) -> Matrix5<T> {
        MatrixMul::mul_mat(
            Matrix5 {
                m11: self.m11,
                m21: self.m12,
...
pub impl Matrix5MatrixIndexLinear<T, +Copy<T>, +Drop<T>> of MatrixIndex<Matrix5<T>, usize> {
    type Output = T;
    #[inline(always)]
    fn get(self: Matrix5<T>, index: usize) -> Option<T> {
        match index {
    ...
    #[inline(always)]
    fn index(self: Matrix5<T>, index: usize) -> T {
        match index {
            0 => self.m11,
            1 => self.m21,
            2 => self.m31,
...
pub impl Matrix5IndexLinear<T, +Copy<T>, +Drop<T>> of IndexView<Matrix5<T>, usize> {
    type Target = T;
    #[inline(always)]
    fn index(self: @Matrix5<T>, index: usize) -> T {
        match index {
...
pub impl Matrix5MatrixIndexPair<T, +Copy<T>, +Drop<T>> of MatrixIndex<Matrix5<T>, (usize, usize)> {
    type Output = T;
    #[inline(always)]
    fn get(self: Matrix5<T>, index: (usize, usize)) -> Option<T> {
        let (i, j) = index;
        match j {
            0 => match i {
                0 => Option::Some(self.m11),
    ...
    #[inline(always)]
    fn index(self: Matrix5<T>, index: (usize, usize)) -> T {
        let (i, j) = index;
        match j {
            0 => match i {
                0 => self.m11,
...
pub impl Matrix5IndexPair<T, +Copy<T>, +Drop<T>> of IndexView<Matrix5<T>, (usize, usize)> {
    type Target = T;
    #[inline(always)]
    fn index(self: @Matrix5<T>, index: (usize, usize)) -> T {
        let (i, j) = index;
...
pub impl Matrix5PartialOrd<T, +PartialOrd<T>, +Copy<T>, +Drop<T>> of PartialOrd<Matrix5<T>> {
    #[inline(always)]
    fn lt(lhs: Matrix5<T>, rhs: Matrix5<T>) -> bool {
        lhs.m11 < rhs.m11
            && lhs.m21 < rhs.m21
            && lhs.m31 < rhs.m31
            && lhs.m41 < rhs.m41
    ...
    #[inline(always)]
    fn le(lhs: Matrix5<T>, rhs: Matrix5<T>) -> bool {
        lhs.m11 <= rhs.m11
            && lhs.m21 <= rhs.m21
            && lhs.m31 <= rhs.m31
            && lhs.m41 <= rhs.m41
    ...
    #[inline(always)]
    fn gt(lhs: Matrix5<T>, rhs: Matrix5<T>) -> bool {
        lhs.m11 > rhs.m11
            && lhs.m21 > rhs.m21
            && lhs.m31 > rhs.m31
            && lhs.m41 > rhs.m41
    ...
    #[inline(always)]
    fn ge(lhs: Matrix5<T>, rhs: Matrix5<T>) -> bool {
        lhs.m11 >= rhs.m11
            && lhs.m21 >= rhs.m21
            && lhs.m31 >= rhs.m31
            && lhs.m41 >= rhs.m41
...
/// for Matrix`.
pub impl Matrix5Bounded<T, +Bounded<T>, +Drop<T>> of Bounded<Matrix5<T>> {
    const MIN: Matrix5<T> = Matrix5 {
        m11: Bounded::<T>::MIN,
        m21: Bounded::<T>::MIN,
        m31: Bounded::<T>::MIN,
        m41: Bounded::<T>::MIN,
...
pub impl Matrix5One<T, impl R: Real<T>, +PartialEq<T>, +Copy<T>, +Drop<T>> of One<Matrix5<T>> {
    #[inline(always)]
    fn one() -> Matrix5<T> {
        Matrix5 {
            m11: R::one(),
    ...
    #[inline(always)]
    fn is_non_one(self: @Matrix5<T>) -> bool {
        !Self::is_one(self)
    }
}

/// The 5x5 matrix of the given COLUMNS (`[[m11, m21, ..], [m12, ..], ..]`). Upstream: `From<[[T;
/// R]; C]>`.
pub impl Matrix5FromColumnArrays<T, +Drop<T>> of Into<[[T; 5]; 5], Matrix5<T>> {
    #[inline(always)]
    fn into(self: [[T; 5]; 5]) -> Matrix5<T> {
...
/// The columns of the 5x5 matrix as nested arrays (`[[m11, m21, ..], [m12, ..], ..]`). Upstream:
/// `Into<[[T; R]; C]>`.
pub impl Matrix5IntoColumnArrays<T, +Drop<T>> of Into<Matrix5<T>, [[T; 5]; 5]> {
    #[inline(always)]
    fn into(self: Matrix5<T>) -> [[T; 5]; 5] {
        let Matrix5 {
            m11,
            m21,
            m31,
        ...
        [
            ...
            [m14, m24, m34, m44, m54], [m15, m25, m35, m45, m55],
        ]
    }
}
...
pub impl Matrix5MulAssignScalar<T, +Mul<T>, +Copy<T>, +Drop<T>> of MulAssign<Matrix5<T>, T> {
    #[inline(always)]
    fn mul_assign(ref self: Matrix5<T>, rhs: T) {
        self =
            Matrix5 {
                m11: self.m11 * rhs,
                m21: self.m21 * rhs,
...
pub impl Matrix5DivAssignScalar<
    T, impl R: Real<T>, +Copy<T>, +Drop<T>,
> of DivAssign<Matrix5<T>, T> {
    fn div_assign(ref self: Matrix5<T>, rhs: T) {
        let (m11, m21, m31, m41, m51, m12, m22, m32, m42, m52, m13, m23, m33, m43, m53, m14) =
            R::div16(
            self.m11,
            self.m21,
...
/// `translation4.into()`: the homogeneous matrix. Exact (no arithmetic). Upstream:
/// `From<Translation4> for Matrix5`.
pub impl Matrix5FromTranslation4<
    T,
    impl R: Real<T>,
    +Copy<T>,
    +Drop<T>,
...
#[generate_trait]
impl Matrix5EditImpl<T, +Copy<T>, +Drop<T>> of Matrix5EditTrait<T> {
    /// `self` with the component at `index` (`(row, column)`) replaced by `v`; panics out of
    /// bounds.
    fn replace(self: Matrix5<T>, index: (usize, usize), v: T) -> Matrix5<T> {
        let (i, j) = index;
        match j {
            0 => match i {
                0 => Matrix5 {
    ...
    fn row_at(self: Matrix5<T>, i: usize) -> RowVector5<T> {
        match i {
    ...
    fn column_at(self: Matrix5<T>, j: usize) -> Vector5<T> {
        match j {
...
/// The 1 consecutive rows of a `Matrix5` as a `RowVector5`. Upstream: `fixed_rows::<1>`, `rows(i,
...
pub impl Matrix5FixedRowsRowVector5<T, +Copy<T>, +Drop<T>> of FixedRows<Matrix5<T>, RowVector5<T>> {
    ...
    #[inline(always)]
    fn rows(self: Matrix5<T>, first_row: usize, nrows: usize) -> RowVector5<T> {
        if nrows != 1 {
    ...
    #[inline(always)]
    fn rows_range(self: Matrix5<T>, rows: Range<usize>) -> RowVector5<T> {
...
/// The 2 consecutive rows of a `Matrix5` as a `Matrix2x5`. Upstream: `fixed_rows::<2>`, `rows(i,
...
pub impl Matrix5FixedRowsMatrix2x5<T, +Copy<T>, +Drop<T>> of FixedRows<Matrix5<T>, Matrix2x5<T>> {
    ...
    #[inline(always)]
    fn rows(self: Matrix5<T>, first_row: usize, nrows: usize) -> Matrix2x5<T> {
        if nrows != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_rows(self, first_row)
    ...
    #[inline(always)]
    fn rows_range(self: Matrix5<T>, rows: Range<usize>) -> Matrix2x5<T> {
        let Range { start, end } = rows;
        if end != start + 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 3 consecutive rows of a `Matrix5` as a `Matrix3x5`. Upstream: `fixed_rows::<3>`, `rows(i,
...
pub impl Matrix5FixedRowsMatrix3x5<T, +Copy<T>, +Drop<T>> of FixedRows<Matrix5<T>, Matrix3x5<T>> {
    ...
    #[inline(always)]
    fn rows(self: Matrix5<T>, first_row: usize, nrows: usize) -> Matrix3x5<T> {
        if nrows != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_rows(self, first_row)
    ...
    #[inline(always)]
    fn rows_range(self: Matrix5<T>, rows: Range<usize>) -> Matrix3x5<T> {
        let Range { start, end } = rows;
        if end != start + 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 4 consecutive rows of a `Matrix5` as a `Matrix4x5`. Upstream: `fixed_rows::<4>`, `rows(i,
...
pub impl Matrix5FixedRowsMatrix4x5<T, +Copy<T>, +Drop<T>> of FixedRows<Matrix5<T>, Matrix4x5<T>> {
    ...
    #[inline(always)]
    fn rows(self: Matrix5<T>, first_row: usize, nrows: usize) -> Matrix4x5<T> {
        if nrows != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_rows(self, first_row)
    ...
    #[inline(always)]
    fn rows_range(self: Matrix5<T>, rows: Range<usize>) -> Matrix4x5<T> {
        let Range { start, end } = rows;
        if end != start + 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 5 consecutive rows of a `Matrix5` as a `Matrix5`. Upstream: `fixed_rows::<5>`, `rows(i, 5)`,
...
pub impl Matrix5FixedRowsMatrix5<T, +Copy<T>, +Drop<T>> of FixedRows<Matrix5<T>, Matrix5<T>> {
    ...
    #[inline(always)]
    fn rows(self: Matrix5<T>, first_row: usize, nrows: usize) -> Matrix5<T> {
        if nrows != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_rows(self, first_row)
    ...
    #[inline(always)]
    fn rows_range(self: Matrix5<T>, rows: Range<usize>) -> Matrix5<T> {
        let Range { start, end } = rows;
        if end != start + 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 1 consecutive columns of a `Matrix5` as a `Vector5`. Upstream: `fixed_columns::<1>`,
...
pub impl Matrix5FixedColumnsVector5<T, +Copy<T>, +Drop<T>> of FixedColumns<Matrix5<T>, Vector5<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix5<T>, first_col: usize, ncols: usize) -> Vector5<T> {
        if ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn columns_range(self: Matrix5<T>, cols: Range<usize>) -> Vector5<T> {
...
/// The 2 consecutive columns of a `Matrix5` as a `Matrix5x2`. Upstream: `fixed_columns::<2>`,
...
> of FixedColumns<Matrix5<T>, Matrix5x2<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix5<T>, first_col: usize, ncols: usize) -> Matrix5x2<T> {
        if ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix5<T>, cols: Range<usize>) -> Matrix5x2<T> {
        let Range { start, end } = cols;
        if end != start + 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 3 consecutive columns of a `Matrix5` as a `Matrix5x3`. Upstream: `fixed_columns::<3>`,
...
> of FixedColumns<Matrix5<T>, Matrix5x3<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix5<T>, first_col: usize, ncols: usize) -> Matrix5x3<T> {
        if ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix5<T>, cols: Range<usize>) -> Matrix5x3<T> {
        let Range { start, end } = cols;
        if end != start + 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 4 consecutive columns of a `Matrix5` as a `Matrix5x4`. Upstream: `fixed_columns::<4>`,
...
> of FixedColumns<Matrix5<T>, Matrix5x4<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix5<T>, first_col: usize, ncols: usize) -> Matrix5x4<T> {
        if ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix5<T>, cols: Range<usize>) -> Matrix5x4<T> {
        let Range { start, end } = cols;
        if end != start + 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 5 consecutive columns of a `Matrix5` as a `Matrix5`. Upstream: `fixed_columns::<5>`,
...
pub impl Matrix5FixedColumnsMatrix5<T, +Copy<T>, +Drop<T>> of FixedColumns<Matrix5<T>, Matrix5<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix5<T>, first_col: usize, ncols: usize) -> Matrix5<T> {
        if ncols != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix5<T>, cols: Range<usize>) -> Matrix5<T> {
        let Range { start, end } = cols;
        if end != start + 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 1x1 blocks of a `Matrix5` as a `Matrix1`. Upstream: `fixed_view::<1, 1>`, `view`, and the
...
pub impl Matrix5FixedViewMatrix1<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, Matrix1<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix1<T> {
        let (irow, icol) = start;
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> Matrix1<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix1<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x2 blocks of a `Matrix5` as a `RowVector2`. Upstream: `fixed_view::<1, 2>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewRowVector2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, RowVector2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> RowVector2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector2<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x3 blocks of a `Matrix5` as a `RowVector3`. Upstream: `fixed_view::<1, 3>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewRowVector3<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, RowVector3<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> RowVector3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector3<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x4 blocks of a `Matrix5` as a `RowVector4`. Upstream: `fixed_view::<1, 4>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewRowVector4<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, RowVector4<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> RowVector4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector4<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x5 blocks of a `Matrix5` as a `RowVector5`. Upstream: `fixed_view::<1, 5>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewRowVector5<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, RowVector5<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector5<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> RowVector5<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector5<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x1 blocks of a `Matrix5` as a `Vector2`. Upstream: `fixed_view::<2, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewVector2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, Vector2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Vector2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> Vector2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Vector2<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x2 blocks of a `Matrix5` as a `Matrix2`. Upstream: `fixed_view::<2, 2>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewMatrix2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, Matrix2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> Matrix2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x3 blocks of a `Matrix5` as a `Matrix2x3`. Upstream: `fixed_view::<2, 3>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewMatrix2x3<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, Matrix2x3<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> Matrix2x3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x3<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x4 blocks of a `Matrix5` as a `Matrix2x4`. Upstream: `fixed_view::<2, 4>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewMatrix2x4<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, Matrix2x4<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> Matrix2x4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x4<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x5 blocks of a `Matrix5` as a `Matrix2x5`. Upstream: `fixed_view::<2, 5>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewMatrix2x5<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, Matrix2x5<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x5<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> Matrix2x5<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x5<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x1 blocks of a `Matrix5` as a `Vector3`. Upstream: `fixed_view::<3, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewVector3<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, Vector3<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Vector3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> Vector3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Vector3<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x2 blocks of a `Matrix5` as a `Matrix3x2`. Upstream: `fixed_view::<3, 2>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewMatrix3x2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, Matrix3x2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> Matrix3x2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x2<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x3 blocks of a `Matrix5` as a `Matrix3`. Upstream: `fixed_view::<3, 3>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewMatrix3<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, Matrix3<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> Matrix3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x4 blocks of a `Matrix5` as a `Matrix3x4`. Upstream: `fixed_view::<3, 4>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewMatrix3x4<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, Matrix3x4<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> Matrix3x4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x4<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x5 blocks of a `Matrix5` as a `Matrix3x5`. Upstream: `fixed_view::<3, 5>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewMatrix3x5<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, Matrix3x5<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x5<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> Matrix3x5<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x5<T> {
        Self::view(self, start, shape)
    }
}

/// The 4x1 blocks of a `Matrix5` as a `Vector4`. Upstream: `fixed_view::<4, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewVector4<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, Vector4<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Vector4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 4 || ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> Vector4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Vector4<T> {
        Self::view(self, start, shape)
    }
}

/// The 4x2 blocks of a `Matrix5` as a `Matrix4x2`. Upstream: `fixed_view::<4, 2>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewMatrix4x2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, Matrix4x2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4x2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 4 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> Matrix4x2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4x2<T> {
        Self::view(self, start, shape)
    }
}

/// The 4x3 blocks of a `Matrix5` as a `Matrix4x3`. Upstream: `fixed_view::<4, 3>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewMatrix4x3<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, Matrix4x3<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4x3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 4 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> Matrix4x3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4x3<T> {
        Self::view(self, start, shape)
    }
}

/// The 4x4 blocks of a `Matrix5` as a `Matrix4`. Upstream: `fixed_view::<4, 4>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewMatrix4<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, Matrix4<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 4 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> Matrix4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4<T> {
        Self::view(self, start, shape)
    }
}

/// The 4x5 blocks of a `Matrix5` as a `Matrix4x5`. Upstream: `fixed_view::<4, 5>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewMatrix4x5<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, Matrix4x5<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4x5<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 4 || ncols != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> Matrix4x5<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4x5<T> {
        Self::view(self, start, shape)
    }
}

/// The 5x1 blocks of a `Matrix5` as a `Vector5`. Upstream: `fixed_view::<5, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewVector5<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, Vector5<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Vector5<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 5 || ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> Vector5<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Vector5<T> {
        Self::view(self, start, shape)
    }
}

/// The 5x2 blocks of a `Matrix5` as a `Matrix5x2`. Upstream: `fixed_view::<5, 2>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewMatrix5x2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, Matrix5x2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix5x2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 5 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> Matrix5x2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix5x2<T> {
        Self::view(self, start, shape)
    }
}

/// The 5x3 blocks of a `Matrix5` as a `Matrix5x3`. Upstream: `fixed_view::<5, 3>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewMatrix5x3<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, Matrix5x3<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix5x3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 5 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> Matrix5x3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix5x3<T> {
        Self::view(self, start, shape)
    }
}

/// The 5x4 blocks of a `Matrix5` as a `Matrix5x4`. Upstream: `fixed_view::<5, 4>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewMatrix5x4<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, Matrix5x4<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix5x4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 5 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> Matrix5x4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix5x4<T> {
        Self::view(self, start, shape)
    }
}

/// The 5x5 blocks of a `Matrix5` as a `Matrix5`. Upstream: `fixed_view::<5, 5>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix5FixedViewMatrix5<T, +Copy<T>, +Drop<T>> of FixedView<Matrix5<T>, Matrix5<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix5<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 5 || ncols != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix5<T>, irow: usize, icol: usize) -> Matrix5<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix5<T> {
        Self::view(self, start, shape)
    }
}

/// The first 1 components of a row of a `Matrix5` as a `Matrix1`. Upstream: `row_part` (`n = 1`).
pub impl Matrix5RowPartMatrix1<T, +Copy<T>, +Drop<T>> of RowPart<Matrix5<T>, Matrix1<T>> {
    #[inline(always)]
    fn row_part(self: Matrix5<T>, i: usize, n: usize) -> Matrix1<T> {
        if n != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The first 2 components of a row of a `Matrix5` as a `RowVector2`. Upstream: `row_part` (`n =
/// 2`).
pub impl Matrix5RowPartRowVector2<T, +Copy<T>, +Drop<T>> of RowPart<Matrix5<T>, RowVector2<T>> {
    #[inline(always)]
    fn row_part(self: Matrix5<T>, i: usize, n: usize) -> RowVector2<T> {
        if n != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 3 components of a row of a `Matrix5` as a `RowVector3`. Upstream: `row_part` (`n =
/// 3`).
pub impl Matrix5RowPartRowVector3<T, +Copy<T>, +Drop<T>> of RowPart<Matrix5<T>, RowVector3<T>> {
    #[inline(always)]
    fn row_part(self: Matrix5<T>, i: usize, n: usize) -> RowVector3<T> {
        if n != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 4 components of a row of a `Matrix5` as a `RowVector4`. Upstream: `row_part` (`n =
/// 4`).
pub impl Matrix5RowPartRowVector4<T, +Copy<T>, +Drop<T>> of RowPart<Matrix5<T>, RowVector4<T>> {
    #[inline(always)]
    fn row_part(self: Matrix5<T>, i: usize, n: usize) -> RowVector4<T> {
        if n != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 5 components of a row of a `Matrix5` as a `RowVector5`. Upstream: `row_part` (`n =
/// 5`).
pub impl Matrix5RowPartRowVector5<T, +Copy<T>, +Drop<T>> of RowPart<Matrix5<T>, RowVector5<T>> {
    #[inline(always)]
    fn row_part(self: Matrix5<T>, i: usize, n: usize) -> RowVector5<T> {
        if n != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 1 components of a column of a `Matrix5` as a `Matrix1`. Upstream: `column_part` (`n =
/// 1`).
pub impl Matrix5ColumnPartMatrix1<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix5<T>, Matrix1<T>> {
    #[inline(always)]
    fn column_part(self: Matrix5<T>, i: usize, n: usize) -> Matrix1<T> {
        if n != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The first 2 components of a column of a `Matrix5` as a `Vector2`. Upstream: `column_part` (`n =
/// 2`).
pub impl Matrix5ColumnPartVector2<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix5<T>, Vector2<T>> {
    #[inline(always)]
    fn column_part(self: Matrix5<T>, i: usize, n: usize) -> Vector2<T> {
        if n != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, 0, i)
...
/// The first 3 components of a column of a `Matrix5` as a `Vector3`. Upstream: `column_part` (`n =
/// 3`).
pub impl Matrix5ColumnPartVector3<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix5<T>, Vector3<T>> {
    #[inline(always)]
    fn column_part(self: Matrix5<T>, i: usize, n: usize) -> Vector3<T> {
        if n != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, 0, i)
...
/// The first 4 components of a column of a `Matrix5` as a `Vector4`. Upstream: `column_part` (`n =
/// 4`).
pub impl Matrix5ColumnPartVector4<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix5<T>, Vector4<T>> {
    #[inline(always)]
    fn column_part(self: Matrix5<T>, i: usize, n: usize) -> Vector4<T> {
        if n != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, 0, i)
...
/// The first 5 components of a column of a `Matrix5` as a `Vector5`. Upstream: `column_part` (`n =
/// 5`).
pub impl Matrix5ColumnPartVector5<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix5<T>, Vector5<T>> {
    #[inline(always)]
    fn column_part(self: Matrix5<T>, i: usize, n: usize) -> Vector5<T> {
        if n != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, 0, i)
