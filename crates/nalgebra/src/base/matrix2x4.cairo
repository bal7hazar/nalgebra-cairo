...
> of Matrix2x4Trait<T> {
    ...
    /// The 2x4 matrix whose components all equal `elem`. Upstream: `Matrix2x4::repeat`.
    #[inline(always)]
    fn repeat(elem: T) -> Matrix2x4<T> {
        Matrix2x4 {
            m11: elem, m21: elem, m12: elem, m22: elem, m13: elem, m23: elem, m14: elem, m24: elem,
        }
    }

    /// Alias of `repeat`. Upstream: `Matrix2x4::from_element`.
    #[inline(always)]
    fn from_element(elem: T) -> Matrix2x4<T> {
        Matrix2x4 {
            m11: elem, m21: elem, m12: elem, m22: elem, m13: elem, m23: elem, m14: elem, m24: elem,
        }
    }

    /// The 2x4 matrix whose component `(i, j)` (row, column, 0-based) is `f(i, j)`, `f` being
    ...
    /// The 2x4 matrix of the 8 values of `data`, in row-major order. Panics with `nalgebra: wrong
    /// slice length` unless `data.len() == 8`. Upstream: `Matrix2x4::from_row_slice` (`&[T]`).
    ///
    ...
    #[inline(always)]
    fn from_row_slice(data: Span<T>) -> Matrix2x4<T> {
        let boxed: @Box<[T; 8]> = data.try_into().expect(errors::SLICE_LENGTH);
        let [v0, v1, v2, v3, v4, v5, v6, v7] = boxed.unbox();
        Matrix2x4 { m11: v0, m21: v4, m12: v1, m22: v5, m13: v2, m23: v6, m14: v3, m24: v7 }
    }

    /// The 2x4 matrix of the 8 values of `data`, in column-major order. Panics with `nalgebra:
    /// wrong slice length` unless `data.len() == 8`. Upstream: `Matrix2x4::from_column_slice`
    /// (`&[T]`).
    ///
    /// `data` is read as ONE fixed-size array (`Span -> @Box<[T; 8]>`, one length check): measured
    /// about 5 times cheaper than a bounds-checked `*data[k]` per component
    /// (`bench_matrix3_from_row_slice__alt_span_index`).
    #[inline(always)]
    fn from_column_slice(data: Span<T>) -> Matrix2x4<T> {
        let boxed: @Box<[T; 8]> = data.try_into().expect(errors::SLICE_LENGTH);
        let [v0, v1, v2, v3, v4, v5, v6, v7] = boxed.unbox();
        Matrix2x4 { m11: v0, m21: v1, m12: v2, m22: v3, m13: v4, m23: v5, m14: v6, m24: v7 }
    }

    /// The 2x4 matrix whose first `data.len()` diagonal components are `data`, every other
    /// component zero. Panics with `nalgebra: diagonal too long` when `data.len() > 2`. Upstream:
    /// `Matrix2x4::from_partial_diagonal` (`&[T]`).
    #[inline(always)]
    fn from_partial_diagonal(data: Span<T>) -> Matrix2x4<T> {
        let len = data.len();
        if len > 2 {
    ...
    #[inline(always)]
    fn is_zero(self: Matrix2x4<T>) -> bool {
        self.m11 == R::zero()
            && self.m21 == R::zero()
            && self.m12 == R::zero()
            && self.m22 == R::zero()
    ...
    #[inline(always)]
    fn component_mul(self: Matrix2x4<T>, rhs: Matrix2x4<T>) -> Matrix2x4<T> {
        Matrix2x4 {
    ...
    #[inline(always)]
    fn component_mul_assign(ref self: Matrix2x4<T>, rhs: Matrix2x4<T>) {
        self =
            Matrix2x4 {
                m11: self.m11 * rhs.m11,
                m21: self.m21 * rhs.m21,
    ...
    #[inline(always)]
    fn component_div(self: Matrix2x4<T>, rhs: Matrix2x4<T>) -> Matrix2x4<T> {
        Matrix2x4 {
    ...
    #[inline(always)]
    fn component_div_assign(ref self: Matrix2x4<T>, rhs: Matrix2x4<T>) {
        self =
            Matrix2x4 {
                m11: R::div(self.m11, rhs.m11),
                m21: R::div(self.m21, rhs.m21),
    ...
    #[inline(always)]
    fn inf(self: Matrix2x4<T>, other: Matrix2x4<T>) -> Matrix2x4<T> {
        Matrix2x4 {
    ...
    #[inline(always)]
    fn sup(self: Matrix2x4<T>, other: Matrix2x4<T>) -> Matrix2x4<T> {
        Matrix2x4 {
    ...
    #[inline(always)]
    fn inf_sup(self: Matrix2x4<T>, other: Matrix2x4<T>) -> (Matrix2x4<T>, Matrix2x4<T>) {
        (Self::inf(self, other), Self::sup(self, other))
    }

    /// `self + k` added to every component. Exact; panics on overflow. Upstream: `add_scalar`.
    #[inline(always)]
    fn add_scalar(self: Matrix2x4<T>, k: T) -> Matrix2x4<T> {
        Matrix2x4 {
            m11: self.m11 + k,
            m21: self.m21 + k,
            m12: self.m12 + k,
    ...
    #[inline(always)]
    fn cmpy(ref self: Matrix2x4<T>, alpha: T, a: Matrix2x4<T>, b: Matrix2x4<T>, beta: T) {
        self =
            Matrix2x4 {
    ...
    #[inline(always)]
    fn cdpy(ref self: Matrix2x4<T>, alpha: T, a: Matrix2x4<T>, b: Matrix2x4<T>, beta: T) {
        self =
            Matrix2x4 {
    ...
    #[inline(always)]
    fn min(self: Matrix2x4<T>) -> T {
        R::min(
            R::min(
                R::min(
                    R::min(
                        R::min(R::min(R::min(self.m11, self.m21), self.m12), self.m22), self.m13,
                    ),
                    self.m23,
                ),
                self.m14,
    ...
    #[inline(always)]
    fn max(self: Matrix2x4<T>) -> T {
        R::max(
            R::max(
                R::max(
                    R::max(
                        R::max(R::max(R::max(self.m11, self.m21), self.m12), self.m22), self.m13,
                    ),
                    self.m23,
                ),
                self.m14,
    ...
    fn iamax_full(self: Matrix2x4<T>) -> (usize, usize) {
    ...
    #[inline(always)]
    fn icamax_full(self: Matrix2x4<T>) -> (usize, usize) {
    ...
    fn dot(self: Matrix2x4<T>, rhs: Matrix2x4<T>) -> T {
    ...
    /// `norm_squared`.
    fn norm_squared(self: Matrix2x4<T>) -> T {
        ...
        R::wide_rescale(R::wide_add_prod(w, self.m24, self.m24))
    }
    ...
    fn norm(self: Matrix2x4<T>) -> T {
        ...
        let w = R::wide_add_prod(w, self.m23, self.m23);
        let w = R::wide_add_prod(w, self.m14, self.m14);
        R::wide_sqrt(R::wide_add_prod(w, self.m24, self.m24))
    }

    /// Alias of `norm_squared`. Upstream: `magnitude_squared`.
    #[inline(always)]
    fn magnitude_squared(self: Matrix2x4<T>) -> T {
        Self::norm_squared(self)
    }

    /// Alias of `norm`. Upstream: `magnitude`.
    #[inline(always)]
    fn magnitude(self: Matrix2x4<T>) -> T {
        Self::norm(self)
    }
    ...
    fn metric_distance(self: Matrix2x4<T>, rhs: Matrix2x4<T>) -> T {
        ...
        R::wide_sqrt(R::wide_add_prod(w, self.m24 - rhs.m24, self.m24 - rhs.m24))
    }
    ...
    fn unscale(self: Matrix2x4<T>, k: T) -> Matrix2x4<T> {
        let (m11, m21, m12, m22, m13) = R::div5(
            self.m11, self.m21, self.m12, self.m22, self.m13, k,
        );
        let (m23, m14, m24) = R::div3(self.m23, self.m14, self.m24, k);
        Matrix2x4 { m11, m21, m12, m22, m13, m23, m14, m24 }
    }
    ...
    fn normalize(self: Matrix2x4<T>) -> Matrix2x4<T> {
    ...
    fn try_normalize(self: Matrix2x4<T>, min_norm: T) -> Option<Matrix2x4<T>> {
    ...
    /// `cap_magnitude`.
    fn cap_magnitude(self: Matrix2x4<T>, max: T) -> Matrix2x4<T> {
        let n = Self::norm(self);
        if n <= max {
            self
        } else {
    ...
    fn try_set_magnitude(ref self: Matrix2x4<T>, magnitude: T, min_magnitude: T) {
        let n = Self::norm(self);
        if n > min_magnitude {
    ...
    #[inline(always)]
    fn one_norm(self: Matrix2x4<T>) -> T {
        R::max(
            R::max(
    ...
    /// The conjugate transpose, a `Matrix4x2`: the transpose for a real scalar. Exact. Upstream:
    /// `adjoint`.
    #[inline(always)]
    fn adjoint(self: Matrix2x4<T>) -> Matrix4x2<T> {
        Self::transpose(self)
    }

    /// Alias of `adjoint` (deprecated upstream). Upstream: `conjugate_transpose`.
    #[inline(always)]
    fn conjugate_transpose(self: Matrix2x4<T>) -> Matrix4x2<T> {
        Self::transpose(self)
    }

    /// The component-wise conjugate: `self` for a real scalar. Upstream: `conjugate`.
    #[inline(always)]
    fn conjugate(self: Matrix2x4<T>) -> Matrix2x4<T> {
        self
    }
    ...
    fn cast<U, +Into<T, U>, +Drop<U>>(self: Matrix2x4<T>) -> Matrix2x4<U> {
        Matrix2x4 {
            m11: self.m11.into(),
            m21: self.m21.into(),
            m12: self.m12.into(),
    ...
    fn try_cast<U, +TryInto<T, U>, +Drop<U>>(self: Matrix2x4<T>) -> Option<Matrix2x4<U>> {
        ...
        Option::Some(Matrix2x4 { m11, m21, m12, m22, m13, m23, m14, m24 })
    }
    ...
    /// (DESIGN D3).
    #[inline(always)]
    fn relative_eq(self: Matrix2x4<T>, other: Matrix2x4<T>, epsilon: u64, max_relative: T) -> bool {
    ...
    #[inline(always)]
    fn ulps_eq(self: Matrix2x4<T>, other: Matrix2x4<T>, epsilon: u64, max_ulps: u32) -> bool {
    ...
    /// The 2x4 matrix of `f(x)` for every component `x` (called in column-major order). `f` is any
    ...
    /// The 2x4 matrix of `f(i, j, x)` for every component `x` at row `i`, column `j` (0-based,
    ...
    /// The 2x4 matrix of `f(a, b)` for the components `a` of `self` and `b` of `rhs` at the same
    ...
    fn zip_map<
        ...
        +Drop<Func::Output>,
    >(
        self: Matrix2x4<T>, rhs: Matrix2x4<T2>, f: F,
    ) -> Matrix2x4<Func::Output> {
        Matrix2x4 {
    ...
    /// The 2x4 matrix of `f(a, b, c)` for the components of `self`, `b` and `c` at the same
    /// position. `f` is any closure or `Fn` value; Cairo closures take their arguments by value and
    ...
    fn zip_zip_map<
        ...
        +Drop<Func::Output>,
    >(
        self: Matrix2x4<T>, b: Matrix2x4<T2>, c: Matrix2x4<T3>, f: F,
    ) -> Matrix2x4<Func::Output> {
        Matrix2x4 {
    ...
    /// `f(.. f(f(init, x0), x1) .., x7)` over the components in column-major order. The closure's
    ...
    >(
        self: Matrix2x4<T>, rhs: Matrix2x4<T2>, init: Acc, f: F,
    ) -> Acc {
    ...
    fn zip_apply<
        ...
        +Into<Func::Output, T>,
    >(
        ref self: Matrix2x4<T>, rhs: Matrix2x4<T2>, f: F,
    ) {
        self =
            Matrix2x4 {
    ...
    fn zip_zip_apply<
        ...
        +Into<Func::Output, T>,
    >(
        ref self: Matrix2x4<T>, b: Matrix2x4<T2>, c: Matrix2x4<T3>, f: F,
    ) {
        self =
            Matrix2x4 {
    ...
    #[inline(always)]
    fn fill(ref self: Matrix2x4<T>, val: T) {
        self =
            Matrix2x4 {
                m11: val, m21: val, m12: val, m22: val, m13: val, m23: val, m14: val, m24: val,
            };
    }

    /// Sets the 2 diagonal components to `val`, the others unchanged. Upstream: `fill_diagonal`.
    #[inline(always)]
    fn fill_diagonal(ref self: Matrix2x4<T>, val: T) {
        self =
            Matrix2x4 {
                m11: val,
                m21: self.m21,
    ...
    #[inline(always)]
    fn fill_with_identity(ref self: Matrix2x4<T>) {
        self =
            Matrix2x4 {
                m11: R::one(),
                m21: R::zero(),
    ...
    #[inline(always)]
    fn fill_row(ref self: Matrix2x4<T>, i: usize, val: T) {
        self = match i {
            0 => Matrix2x4 {
                m11: val,
                m21: self.m21,
    ...
    #[inline(always)]
    fn fill_column(ref self: Matrix2x4<T>, j: usize, val: T) {
        self = match j {
            0 => Matrix2x4 {
                m11: val,
                m21: val,
    ...
    #[inline(always)]
    fn fill_lower_triangle(ref self: Matrix2x4<T>, val: T, shift: usize) {
        self = match shift {
            0 => Matrix2x4 {
                m11: val,
                m21: val,
    ...
    #[inline(always)]
    fn fill_upper_triangle(ref self: Matrix2x4<T>, val: T, shift: usize) {
        self = match shift {
            0 => Matrix2x4 {
                m11: val, m21: self.m21, m12: val, m22: val, m13: val, m23: val, m14: val, m24: val,
            },
            1 => Matrix2x4 {
                m11: self.m11,
                m21: self.m21,
    ...
    #[inline(always)]
    fn set_row(ref self: Matrix2x4<T>, i: usize, row: RowVector4<T>) {
        self = match i {
            0 => Matrix2x4 {
                m11: row.x,
                m21: self.m21,
    ...
    #[inline(always)]
    fn set_column(ref self: Matrix2x4<T>, j: usize, column: Vector2<T>) {
        self = match j {
            0 => Matrix2x4 {
                m11: column.x,
                m21: column.y,
    ...
    #[inline(always)]
    fn set_diagonal(ref self: Matrix2x4<T>, diag: Vector2<T>) {
        self =
            Matrix2x4 {
                m11: diag.x,
                m21: self.m21,
    ...
    #[inline(always)]
    fn set_partial_diagonal(ref self: Matrix2x4<T>, diag: Span<T>) {
        let len = diag.len();
        self =
            Matrix2x4 {
                m11: if len > 0 {
    ...
    /// Sets `self` to `other` (a `Matrix2x4`: upstream requires the same shape). Upstream:
    /// `copy_from`.
    #[inline(always)]
    fn copy_from(ref self: Matrix2x4<T>, other: Matrix2x4<T>) {
        self = other;
    }

    /// Sets `self` to the 8 values of `slice` in column-major order (`from_column_slice`). Panics
    /// with `nalgebra: wrong slice length` unless `slice.len() == 8`. Upstream: `copy_from_slice`
    /// (`&[T]`).
    #[inline(always)]
    fn copy_from_slice(ref self: Matrix2x4<T>, slice: Span<T>) {
        self = Self::from_column_slice(slice);
    }

    /// Sets `self` to the transpose of `other`, a `Matrix4x2`. Exact. Upstream: `tr_copy_from`.
    #[inline(always)]
    fn tr_copy_from(ref self: Matrix2x4<T>, other: Matrix4x2<T>) {
        self =
            Matrix2x4 {
                m11: other.m11,
                m21: other.m12,
    ...
    #[inline]
    fn swap(ref self: Matrix2x4<T>, row_cols1: (usize, usize), row_cols2: (usize, usize)) {
        let a = MatrixIndex::index(self, row_cols1);
        let b = MatrixIndex::index(self, row_cols2);
        self =
            Matrix2x4EditTrait::replace(
                Matrix2x4EditTrait::replace(self, row_cols1, b), row_cols2, a,
            );
    }
    ...
    /// benchmark.
    #[inline]
    fn swap_rows(ref self: Matrix2x4<T>, irow1: usize, irow2: usize) {
        let a = Matrix2x4EditTrait::row_at(self, irow1);
        let b = Matrix2x4EditTrait::row_at(self, irow2);
    ...
    #[inline]
    fn swap_columns(ref self: Matrix2x4<T>, icol1: usize, icol2: usize) {
        let a = Matrix2x4EditTrait::column_at(self, icol1);
        let b = Matrix2x4EditTrait::column_at(self, icol2);
    ...
    #[inline(always)]
    fn neg_mut(ref self: Matrix2x4<T>) {
        self = -self;
    }
    ...
    /// `scale_mut`.
    #[inline(always)]
    fn scale_mut(ref self: Matrix2x4<T>, k: T) {
    ...
    #[inline(always)]
    fn unscale_mut(ref self: Matrix2x4<T>, k: T) {
    ...
    #[inline(always)]
    fn normalize_mut(ref self: Matrix2x4<T>) -> T {
        let n = Self::norm(self);
    ...
    #[inline(always)]
    fn try_normalize_mut(ref self: Matrix2x4<T>, min_norm: T) -> Option<T> {
        let n = Self::norm(self);
        if n <= min_norm {
            return None;
        }
    ...
    #[inline(always)]
    fn set_magnitude(ref self: Matrix2x4<T>, magnitude: T) {
        let n = Self::norm(self);
        self = Self::scale(self, R::div(magnitude, n));
    }
    ...
    /// `conjugate_mut`.
    #[inline(always)]
    fn conjugate_mut(ref self: Matrix2x4<T>) {
        self = Self::conjugate(self);
    }

    /// Writes the transpose of `self` into `out`, a `Matrix4x2`. Exact. Upstream: `transpose_to`.
    #[inline(always)]
    fn transpose_to(self: Matrix2x4<T>, ref out: Matrix4x2<T>) {
        out = Self::transpose(self);
    }

    /// Writes the adjoint of `self` (its transpose for a real scalar) into `out`, a `Matrix4x2`.
    /// Exact. Upstream: `adjoint_to`.
    #[inline(always)]
    fn adjoint_to(self: Matrix2x4<T>, ref out: Matrix4x2<T>) {
        out = Self::transpose(self);
    }

    /// Writes the adjoint of `self` (deprecated upstream alias of `adjoint_to`) into `out`, a
    /// `Matrix4x2`. Exact. Upstream: `conjugate_transpose_to`.
    #[inline(always)]
    fn conjugate_transpose_to(self: Matrix2x4<T>, ref out: Matrix4x2<T>) {
        out = Self::transpose(self);
    }

    /// Writes the sum `self + rhs` into `out`. Exact; panics on overflow. Upstream: `add_to`.
    #[inline(always)]
    fn add_to(self: Matrix2x4<T>, rhs: Matrix2x4<T>, ref out: Matrix2x4<T>) {
        out = self + rhs;
    }
    ...
    /// `sub_to`.
    #[inline(always)]
    fn sub_to(self: Matrix2x4<T>, rhs: Matrix2x4<T>, ref out: Matrix2x4<T>) {
        out = self - rhs;
    }
    ...
    #[inline]
    fn apply_norm<N, +Drop<N>, impl Nm: Norm<N, Matrix2x4<T>, T>>(
        self: Matrix2x4<T>, norm: N,
    ) -> T {
        Nm::norm(@norm, self)
    }
    ...
    #[inline]
    fn apply_metric_distance<N, +Drop<N>, impl Nm: Norm<N, Matrix2x4<T>, T>>(
        self: Matrix2x4<T>, rhs: Matrix2x4<T>, norm: N,
    ) -> T {
    ...
    #[inline(always)]
    fn nrows(self: Matrix2x4<T>) -> usize {
        2
    }

    /// The number of columns, 4. Upstream: `ncols`.
    #[inline(always)]
    fn ncols(self: Matrix2x4<T>) -> usize {
        4
    }

    /// `(nrows, ncols)`, `(2, 4)`. Upstream: `shape`.
    #[inline(always)]
    fn shape(self: Matrix2x4<T>) -> (usize, usize) {
        (2, 4)
    }

    /// Whether the shape is square: `false`. Upstream: `is_square`.
    #[inline(always)]
    fn is_square(self: Matrix2x4<T>) -> bool {
        false
    }
    ...
    #[inline(always)]
    fn vector_to_matrix_index(self: Matrix2x4<T>, i: usize) -> (usize, usize) {
    ...
    #[inline(always)]
    fn row(self: Matrix2x4<T>, i: usize) -> RowVector4<T> {
        Matrix2x4EditTrait::row_at(self, i)
    }
    ...
    #[inline(always)]
    fn column(self: Matrix2x4<T>, j: usize) -> Vector2<T> {
        Matrix2x4EditTrait::column_at(self, j)
    }
    ...
    #[inline(always)]
    fn upper_triangle(self: Matrix2x4<T>) -> Matrix2x4<T> {
        Matrix2x4 {
            m11: self.m11,
            m21: R::zero(),
            m12: self.m12,
    ...
    #[inline(always)]
    fn lower_triangle(self: Matrix2x4<T>) -> Matrix2x4<T> {
        Matrix2x4 {
            m11: self.m11,
            m21: self.m21,
            m12: R::zero(),
    ...
    /// The 2x4 matrix whose 2 rows are the given vectors, each a `Vector4` of the row's 4
    ...
    #[inline(always)]
    fn from_rows(r1: Vector4<T>, r2: Vector4<T>) -> Matrix2x4<T> {
        Matrix2x4 {
            m11: r1.x, m21: r2.x, m12: r1.y, m22: r2.y, m13: r1.z, m23: r2.z, m14: r1.w, m24: r2.w,
        }
    }

    /// The 2x4 matrix whose 4 columns are the given `Vector2`s. Exact. Upstream: `from_columns` (a
    /// slice of column vectors).
    #[inline(always)]
    fn from_columns(
        c1: Vector2<T>, c2: Vector2<T>, c3: Vector2<T>, c4: Vector2<T>,
    ) -> Matrix2x4<T> {
        Matrix2x4 {
            m11: c1.x, m21: c1.y, m12: c2.x, m22: c2.y, m13: c3.x, m23: c3.y, m14: c4.x, m24: c4.y,
        }
    }
    ...
    #[inline(always)]
    fn is_orthogonal(self: Matrix2x4<T>, ulps: u64) -> bool {
        Matrix4Trait::is_identity(MatrixTrMul::tr_mul(self, self), ulps)
    }

    /// `self` with a row of `val` inserted at index `i` (`i <= 2`), a `Matrix3x4`. Panics with
    /// `nalgebra: index out of bounds` for `i > 2`. Upstream: `insert_row`.
    #[inline(always)]
    fn insert_row(self: Matrix2x4<T>, i: usize, val: T) -> Matrix3x4<T> {
        match i {
            0 => Matrix3x4 {
                m11: val,
                m21: self.m11,
    ...
    /// `self` with a column of `val` inserted at index `i` (`i <= 4`), a `Matrix2x5`. Panics with
    /// `nalgebra: index out of bounds` for `i > 4`. Upstream: `insert_column`.
    #[inline(always)]
    fn insert_column(self: Matrix2x4<T>, i: usize, val: T) -> Matrix2x5<T> {
        match i {
            0 => Matrix2x5 {
                m11: val,
                m21: val,
    ...
    /// `self` without its row `i`, a `RowVector4`. Panics with `nalgebra: index out of bounds` for
    /// `i >= 2`. Upstream: `remove_row`.
    #[inline(always)]
    fn remove_row(self: Matrix2x4<T>, i: usize) -> RowVector4<T> {
        match i {
            0 => RowVector4 { x: self.m21, y: self.m22, z: self.m23, w: self.m24 },
            1 => RowVector4 { x: self.m11, y: self.m12, z: self.m13, w: self.m14 },
            _ => core::panic_with_felt252(errors::INDEX_OUT_OF_BOUNDS),
        }
    }

    /// `self` without its column `i`, a `Matrix2x3`. Panics with `nalgebra: index out of bounds`
    /// for `i >= 4`. Upstream: `remove_column`.
    #[inline(always)]
    fn remove_column(self: Matrix2x4<T>, i: usize) -> Matrix2x3<T> {
        match i {
            0 => Matrix2x3 {
                m11: self.m12,
                m21: self.m22,
...
/// The operations of `Matrix2x4<T>` that need `Transcendental` (inverse trigonometry, `exp`, `ln`):
/// a scalar may implement `Real` only.
#[generate_trait]
pub impl Matrix2x4AngleImpl<
    T,
    impl R: Real<T>,
...
> of Matrix2x4AngleTrait<T> {
    ...
    fn angle(self: Matrix2x4<T>, other: Matrix2x4<T>) -> T {
        let n1 = Matrix2x4Trait::norm(self);
        let n2 = Matrix2x4Trait::norm(other);
        if n1 == R::zero() || n2 == R::zero() {
            return R::zero();
        }
        let u = Matrix2x4Trait::unscale(self, n1);
        let v = Matrix2x4Trait::unscale(other, n2);
        let half = Tr::atan2(Matrix2x4Trait::metric_distance(u, v), Matrix2x4Trait::norm(u + v));
        half + half
    }
    ...
    /// Upstream: `lp_norm`.
    fn lp_norm(self: Matrix2x4<T>, p: i32) -> T {
        if p < 1 {
        ...
        if p == 2 {
            return Matrix2x4Trait::norm(self);
        }
        let m = Matrix2x4Trait::amax(self);
        if m == R::zero() {
            return R::zero();
        }
        let r = Matrix2x4Trait::unscale(Matrix2x4Trait::abs(self), m);
...
/// `selfᵀ * rhs`, a `Vector4`: `mul_mat` of the transposed components, so one `sum_prod2` per
...
/// `selfᵀ * rhs`, a `Matrix4x2`: `mul_mat` of the transposed components, so one `sum_prod2` per
/// component (one rounding each); bit-identical to `self.transpose().mul_mat(rhs)`, at the same gas
/// (the transpose only relabels values). Panics on overflow. Upstream: `tr_mul`.
...
/// `selfᵀ * rhs`, a `Matrix4x3`: `mul_mat` of the transposed components, so one `sum_prod2` per
/// component (one rounding each); bit-identical to `self.transpose().mul_mat(rhs)`, at the same gas
/// (the transpose only relabels values). Panics on overflow. Upstream: `tr_mul`.
...
/// `selfᵀ * rhs`, a `Matrix4`: `mul_mat` of the transposed components, so one `sum_prod2` per
/// component (one rounding each); bit-identical to `self.transpose().mul_mat(rhs)`, at the same gas
/// (the transpose only relabels values). Panics on overflow. Upstream: `tr_mul`.
...
/// `selfᵀ * rhs`, a `Matrix4x5`: `mul_mat` of the transposed components, so one `sum_prod2` per
/// component (one rounding each); bit-identical to `self.transpose().mul_mat(rhs)`, at the same gas
/// (the transpose only relabels values). Panics on overflow. Upstream: `tr_mul`.
...
/// `selfᵀ * rhs`, a `Matrix4x6`: `mul_mat` of the transposed components, so one `sum_prod2` per
/// component (one rounding each); bit-identical to `self.transpose().mul_mat(rhs)`, at the same gas
/// (the transpose only relabels values). Panics on overflow. Upstream: `tr_mul`.
...
pub impl Matrix2x4MatrixIndexLinear<T, +Copy<T>, +Drop<T>> of MatrixIndex<Matrix2x4<T>, usize> {
    type Output = T;
    #[inline(always)]
    fn get(self: Matrix2x4<T>, index: usize) -> Option<T> {
        match index {
    ...
    #[inline(always)]
    fn index(self: Matrix2x4<T>, index: usize) -> T {
        match index {
            0 => self.m11,
            1 => self.m21,
            2 => self.m12,
...
pub impl Matrix2x4IndexLinear<T, +Copy<T>, +Drop<T>> of IndexView<Matrix2x4<T>, usize> {
    type Target = T;
    #[inline(always)]
    fn index(self: @Matrix2x4<T>, index: usize) -> T {
        match index {
...
pub impl Matrix2x4MatrixIndexPair<
    T, +Copy<T>, +Drop<T>,
> of MatrixIndex<Matrix2x4<T>, (usize, usize)> {
    type Output = T;
    #[inline(always)]
    fn get(self: Matrix2x4<T>, index: (usize, usize)) -> Option<T> {
        let (i, j) = index;
        match j {
            0 => match i {
                0 => Option::Some(self.m11),
    ...
    #[inline(always)]
    fn index(self: Matrix2x4<T>, index: (usize, usize)) -> T {
        let (i, j) = index;
        match j {
            0 => match i {
                0 => self.m11,
...
pub impl Matrix2x4IndexPair<T, +Copy<T>, +Drop<T>> of IndexView<Matrix2x4<T>, (usize, usize)> {
    type Target = T;
    #[inline(always)]
    fn index(self: @Matrix2x4<T>, index: (usize, usize)) -> T {
        let (i, j) = index;
...
pub impl Matrix2x4PartialOrd<T, +PartialOrd<T>, +Copy<T>, +Drop<T>> of PartialOrd<Matrix2x4<T>> {
    #[inline(always)]
    fn lt(lhs: Matrix2x4<T>, rhs: Matrix2x4<T>) -> bool {
        lhs.m11 < rhs.m11
            && lhs.m21 < rhs.m21
            && lhs.m12 < rhs.m12
            && lhs.m22 < rhs.m22
    ...
    #[inline(always)]
    fn le(lhs: Matrix2x4<T>, rhs: Matrix2x4<T>) -> bool {
        lhs.m11 <= rhs.m11
            && lhs.m21 <= rhs.m21
            && lhs.m12 <= rhs.m12
            && lhs.m22 <= rhs.m22
    ...
    #[inline(always)]
    fn gt(lhs: Matrix2x4<T>, rhs: Matrix2x4<T>) -> bool {
        lhs.m11 > rhs.m11
            && lhs.m21 > rhs.m21
            && lhs.m12 > rhs.m12
            && lhs.m22 > rhs.m22
    ...
    #[inline(always)]
    fn ge(lhs: Matrix2x4<T>, rhs: Matrix2x4<T>) -> bool {
        lhs.m11 >= rhs.m11
            && lhs.m21 >= rhs.m21
            && lhs.m12 >= rhs.m12
            && lhs.m22 >= rhs.m22
...
/// for Matrix`.
pub impl Matrix2x4Bounded<T, +Bounded<T>, +Drop<T>> of Bounded<Matrix2x4<T>> {
    const MIN: Matrix2x4<T> = Matrix2x4 {
        m11: Bounded::<T>::MIN,
        m21: Bounded::<T>::MIN,
        m12: Bounded::<T>::MIN,
        m22: Bounded::<T>::MIN,
    ...
    const MAX: Matrix2x4<T> = Matrix2x4 {
        m11: Bounded::<T>::MAX,
        m21: Bounded::<T>::MAX,
        m12: Bounded::<T>::MAX,
        m22: Bounded::<T>::MAX,
...
/// The 2x4 matrix of the given COLUMNS (`[[m11, m21, ..], [m12, ..], ..]`). Upstream: `From<[[T;
/// R]; C]>`.
pub impl Matrix2x4FromColumnArrays<T, +Drop<T>> of Into<[[T; 2]; 4], Matrix2x4<T>> {
    #[inline(always)]
    fn into(self: [[T; 2]; 4]) -> Matrix2x4<T> {
...
/// The columns of the 2x4 matrix as nested arrays (`[[m11, m21, ..], [m12, ..], ..]`). Upstream:
/// `Into<[[T; R]; C]>`.
pub impl Matrix2x4IntoColumnArrays<T, +Drop<T>> of Into<Matrix2x4<T>, [[T; 2]; 4]> {
    #[inline(always)]
...
#[generate_trait]
impl Matrix2x4EditImpl<T, +Copy<T>, +Drop<T>> of Matrix2x4EditTrait<T> {
...
/// The 1 consecutive rows of a `Matrix2x4` as a `RowVector4`. Upstream: `fixed_rows::<1>`, `rows(i,
...
> of FixedRows<Matrix2x4<T>, RowVector4<T>> {
    ...
    #[inline(always)]
    fn rows(self: Matrix2x4<T>, first_row: usize, nrows: usize) -> RowVector4<T> {
        if nrows != 1 {
    ...
    #[inline(always)]
    fn rows_range(self: Matrix2x4<T>, rows: Range<usize>) -> RowVector4<T> {
...
/// The 2 consecutive rows of a `Matrix2x4` as a `Matrix2x4`. Upstream: `fixed_rows::<2>`, `rows(i,
...
> of FixedRows<Matrix2x4<T>, Matrix2x4<T>> {
    ...
    #[inline(always)]
    fn rows(self: Matrix2x4<T>, first_row: usize, nrows: usize) -> Matrix2x4<T> {
        if nrows != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_rows(self, first_row)
    ...
    #[inline(always)]
    fn rows_range(self: Matrix2x4<T>, rows: Range<usize>) -> Matrix2x4<T> {
        let Range { start, end } = rows;
        if end != start + 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 1 consecutive columns of a `Matrix2x4` as a `Vector2`. Upstream: `fixed_columns::<1>`,
...
> of FixedColumns<Matrix2x4<T>, Vector2<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix2x4<T>, first_col: usize, ncols: usize) -> Vector2<T> {
        if ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn columns_range(self: Matrix2x4<T>, cols: Range<usize>) -> Vector2<T> {
...
/// The 2 consecutive columns of a `Matrix2x4` as a `Matrix2`. Upstream: `fixed_columns::<2>`,
...
> of FixedColumns<Matrix2x4<T>, Matrix2<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix2x4<T>, first_col: usize, ncols: usize) -> Matrix2<T> {
        if ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix2x4<T>, cols: Range<usize>) -> Matrix2<T> {
        let Range { start, end } = cols;
        if end != start + 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 3 consecutive columns of a `Matrix2x4` as a `Matrix2x3`. Upstream: `fixed_columns::<3>`,
...
> of FixedColumns<Matrix2x4<T>, Matrix2x3<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix2x4<T>, first_col: usize, ncols: usize) -> Matrix2x3<T> {
        if ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix2x4<T>, cols: Range<usize>) -> Matrix2x3<T> {
        let Range { start, end } = cols;
        if end != start + 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 4 consecutive columns of a `Matrix2x4` as a `Matrix2x4`. Upstream: `fixed_columns::<4>`,
...
> of FixedColumns<Matrix2x4<T>, Matrix2x4<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix2x4<T>, first_col: usize, ncols: usize) -> Matrix2x4<T> {
        if ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix2x4<T>, cols: Range<usize>) -> Matrix2x4<T> {
        let Range { start, end } = cols;
        if end != start + 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 1x1 blocks of a `Matrix2x4` as a `Matrix1`. Upstream: `fixed_view::<1, 1>`, `view`, and the
...
pub impl Matrix2x4FixedViewMatrix1<T, +Copy<T>, +Drop<T>> of FixedView<Matrix2x4<T>, Matrix1<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix2x4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix1<T> {
        let (irow, icol) = start;
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix2x4<T>, irow: usize, icol: usize) -> Matrix1<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix2x4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix1<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x2 blocks of a `Matrix2x4` as a `RowVector2`. Upstream: `fixed_view::<1, 2>`, `view`, and
...
> of FixedView<Matrix2x4<T>, RowVector2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix2x4<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix2x4<T>, irow: usize, icol: usize) -> RowVector2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix2x4<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector2<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x3 blocks of a `Matrix2x4` as a `RowVector3`. Upstream: `fixed_view::<1, 3>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix2x4<T>, RowVector3<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix2x4<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix2x4<T>, irow: usize, icol: usize) -> RowVector3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix2x4<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector3<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x4 blocks of a `Matrix2x4` as a `RowVector4`. Upstream: `fixed_view::<1, 4>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix2x4<T>, RowVector4<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix2x4<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix2x4<T>, irow: usize, icol: usize) -> RowVector4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix2x4<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector4<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x1 blocks of a `Matrix2x4` as a `Vector2`. Upstream: `fixed_view::<2, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix2x4FixedViewVector2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix2x4<T>, Vector2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix2x4<T>, start: (usize, usize), shape: (usize, usize)) -> Vector2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix2x4<T>, irow: usize, icol: usize) -> Vector2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix2x4<T>, start: (usize, usize), shape: (usize, usize)) -> Vector2<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x2 blocks of a `Matrix2x4` as a `Matrix2`. Upstream: `fixed_view::<2, 2>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix2x4FixedViewMatrix2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix2x4<T>, Matrix2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix2x4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix2x4<T>, irow: usize, icol: usize) -> Matrix2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix2x4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x3 blocks of a `Matrix2x4` as a `Matrix2x3`. Upstream: `fixed_view::<2, 3>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix2x4<T>, Matrix2x3<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix2x4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix2x4<T>, irow: usize, icol: usize) -> Matrix2x3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix2x4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x3<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x4 blocks of a `Matrix2x4` as a `Matrix2x4`. Upstream: `fixed_view::<2, 4>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix2x4<T>, Matrix2x4<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix2x4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix2x4<T>, irow: usize, icol: usize) -> Matrix2x4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix2x4<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x4<T> {
        Self::view(self, start, shape)
    }
}

/// The first 1 components of a row of a `Matrix2x4` as a `Matrix1`. Upstream: `row_part` (`n = 1`).
pub impl Matrix2x4RowPartMatrix1<T, +Copy<T>, +Drop<T>> of RowPart<Matrix2x4<T>, Matrix1<T>> {
    #[inline(always)]
    fn row_part(self: Matrix2x4<T>, i: usize, n: usize) -> Matrix1<T> {
        if n != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The first 2 components of a row of a `Matrix2x4` as a `RowVector2`. Upstream: `row_part` (`n =
/// 2`).
pub impl Matrix2x4RowPartRowVector2<T, +Copy<T>, +Drop<T>> of RowPart<Matrix2x4<T>, RowVector2<T>> {
    #[inline(always)]
    fn row_part(self: Matrix2x4<T>, i: usize, n: usize) -> RowVector2<T> {
        if n != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 3 components of a row of a `Matrix2x4` as a `RowVector3`. Upstream: `row_part` (`n =
/// 3`).
pub impl Matrix2x4RowPartRowVector3<T, +Copy<T>, +Drop<T>> of RowPart<Matrix2x4<T>, RowVector3<T>> {
    #[inline(always)]
    fn row_part(self: Matrix2x4<T>, i: usize, n: usize) -> RowVector3<T> {
        if n != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 4 components of a row of a `Matrix2x4` as a `RowVector4`. Upstream: `row_part` (`n =
/// 4`).
pub impl Matrix2x4RowPartRowVector4<T, +Copy<T>, +Drop<T>> of RowPart<Matrix2x4<T>, RowVector4<T>> {
    #[inline(always)]
    fn row_part(self: Matrix2x4<T>, i: usize, n: usize) -> RowVector4<T> {
        if n != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 1 components of a column of a `Matrix2x4` as a `Matrix1`. Upstream: `column_part` (`n
/// = 1`).
pub impl Matrix2x4ColumnPartMatrix1<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix2x4<T>, Matrix1<T>> {
    #[inline(always)]
    fn column_part(self: Matrix2x4<T>, i: usize, n: usize) -> Matrix1<T> {
        if n != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The first 2 components of a column of a `Matrix2x4` as a `Vector2`. Upstream: `column_part` (`n
/// = 2`).
pub impl Matrix2x4ColumnPartVector2<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix2x4<T>, Vector2<T>> {
    #[inline(always)]
    fn column_part(self: Matrix2x4<T>, i: usize, n: usize) -> Vector2<T> {
        if n != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, 0, i)
