...
/// The 1 consecutive rows of a `Matrix3x5` as a `RowVector5`. Upstream: `fixed_rows::<1>`, `rows(i,
...
> of FixedRows<Matrix3x5<T>, RowVector5<T>> {
    ...
    #[inline(always)]
    fn rows(self: Matrix3x5<T>, first_row: usize, nrows: usize) -> RowVector5<T> {
        if nrows != 1 {
    ...
    #[inline(always)]
    fn rows_range(self: Matrix3x5<T>, rows: Range<usize>) -> RowVector5<T> {
...
/// The 2 consecutive rows of a `Matrix3x5` as a `Matrix2x5`. Upstream: `fixed_rows::<2>`, `rows(i,
...
> of FixedRows<Matrix3x5<T>, Matrix2x5<T>> {
    ...
    #[inline(always)]
    fn rows(self: Matrix3x5<T>, first_row: usize, nrows: usize) -> Matrix2x5<T> {
        if nrows != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_rows(self, first_row)
    ...
    #[inline(always)]
    fn rows_range(self: Matrix3x5<T>, rows: Range<usize>) -> Matrix2x5<T> {
        let Range { start, end } = rows;
        if end != start + 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 3 consecutive rows of a `Matrix3x5` as a `Matrix3x5`. Upstream: `fixed_rows::<3>`, `rows(i,
...
> of FixedRows<Matrix3x5<T>, Matrix3x5<T>> {
    ...
    #[inline(always)]
    fn rows(self: Matrix3x5<T>, first_row: usize, nrows: usize) -> Matrix3x5<T> {
        if nrows != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_rows(self, first_row)
    ...
    #[inline(always)]
    fn rows_range(self: Matrix3x5<T>, rows: Range<usize>) -> Matrix3x5<T> {
        let Range { start, end } = rows;
        if end != start + 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 1 consecutive columns of a `Matrix3x5` as a `Vector3`. Upstream: `fixed_columns::<1>`,
...
> of FixedColumns<Matrix3x5<T>, Vector3<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix3x5<T>, first_col: usize, ncols: usize) -> Vector3<T> {
        if ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn columns_range(self: Matrix3x5<T>, cols: Range<usize>) -> Vector3<T> {
...
/// The 2 consecutive columns of a `Matrix3x5` as a `Matrix3x2`. Upstream: `fixed_columns::<2>`,
...
> of FixedColumns<Matrix3x5<T>, Matrix3x2<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix3x5<T>, first_col: usize, ncols: usize) -> Matrix3x2<T> {
        if ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix3x5<T>, cols: Range<usize>) -> Matrix3x2<T> {
        let Range { start, end } = cols;
        if end != start + 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 3 consecutive columns of a `Matrix3x5` as a `Matrix3`. Upstream: `fixed_columns::<3>`,
...
> of FixedColumns<Matrix3x5<T>, Matrix3<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix3x5<T>, first_col: usize, ncols: usize) -> Matrix3<T> {
        if ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix3x5<T>, cols: Range<usize>) -> Matrix3<T> {
        let Range { start, end } = cols;
        if end != start + 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 4 consecutive columns of a `Matrix3x5` as a `Matrix3x4`. Upstream: `fixed_columns::<4>`,
...
> of FixedColumns<Matrix3x5<T>, Matrix3x4<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix3x5<T>, first_col: usize, ncols: usize) -> Matrix3x4<T> {
        if ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix3x5<T>, cols: Range<usize>) -> Matrix3x4<T> {
        let Range { start, end } = cols;
        if end != start + 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 5 consecutive columns of a `Matrix3x5` as a `Matrix3x5`. Upstream: `fixed_columns::<5>`,
...
> of FixedColumns<Matrix3x5<T>, Matrix3x5<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix3x5<T>, first_col: usize, ncols: usize) -> Matrix3x5<T> {
        if ncols != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix3x5<T>, cols: Range<usize>) -> Matrix3x5<T> {
        let Range { start, end } = cols;
        if end != start + 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 1x1 blocks of a `Matrix3x5` as a `Matrix1`. Upstream: `fixed_view::<1, 1>`, `view`, and the
...
pub impl Matrix3x5FixedViewMatrix1<T, +Copy<T>, +Drop<T>> of FixedView<Matrix3x5<T>, Matrix1<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix1<T> {
        let (irow, icol) = start;
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x5<T>, irow: usize, icol: usize) -> Matrix1<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix1<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x2 blocks of a `Matrix3x5` as a `RowVector2`. Upstream: `fixed_view::<1, 2>`, `view`, and
...
> of FixedView<Matrix3x5<T>, RowVector2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x5<T>, irow: usize, icol: usize) -> RowVector2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector2<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x3 blocks of a `Matrix3x5` as a `RowVector3`. Upstream: `fixed_view::<1, 3>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix3x5<T>, RowVector3<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x5<T>, irow: usize, icol: usize) -> RowVector3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector3<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x4 blocks of a `Matrix3x5` as a `RowVector4`. Upstream: `fixed_view::<1, 4>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix3x5<T>, RowVector4<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x5<T>, irow: usize, icol: usize) -> RowVector4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector4<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x5 blocks of a `Matrix3x5` as a `RowVector5`. Upstream: `fixed_view::<1, 5>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix3x5<T>, RowVector5<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector5<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x5<T>, irow: usize, icol: usize) -> RowVector5<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector5<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x1 blocks of a `Matrix3x5` as a `Vector2`. Upstream: `fixed_view::<2, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix3x5FixedViewVector2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix3x5<T>, Vector2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> Vector2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x5<T>, irow: usize, icol: usize) -> Vector2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> Vector2<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x2 blocks of a `Matrix3x5` as a `Matrix2`. Upstream: `fixed_view::<2, 2>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix3x5FixedViewMatrix2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix3x5<T>, Matrix2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x5<T>, irow: usize, icol: usize) -> Matrix2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x3 blocks of a `Matrix3x5` as a `Matrix2x3`. Upstream: `fixed_view::<2, 3>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix3x5<T>, Matrix2x3<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x5<T>, irow: usize, icol: usize) -> Matrix2x3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x3<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x4 blocks of a `Matrix3x5` as a `Matrix2x4`. Upstream: `fixed_view::<2, 4>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix3x5<T>, Matrix2x4<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x5<T>, irow: usize, icol: usize) -> Matrix2x4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x4<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x5 blocks of a `Matrix3x5` as a `Matrix2x5`. Upstream: `fixed_view::<2, 5>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix3x5<T>, Matrix2x5<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x5<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x5<T>, irow: usize, icol: usize) -> Matrix2x5<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x5<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x1 blocks of a `Matrix3x5` as a `Vector3`. Upstream: `fixed_view::<3, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix3x5FixedViewVector3<T, +Copy<T>, +Drop<T>> of FixedView<Matrix3x5<T>, Vector3<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> Vector3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x5<T>, irow: usize, icol: usize) -> Vector3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> Vector3<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x2 blocks of a `Matrix3x5` as a `Matrix3x2`. Upstream: `fixed_view::<3, 2>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix3x5<T>, Matrix3x2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x5<T>, irow: usize, icol: usize) -> Matrix3x2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x2<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x3 blocks of a `Matrix3x5` as a `Matrix3`. Upstream: `fixed_view::<3, 3>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix3x5FixedViewMatrix3<T, +Copy<T>, +Drop<T>> of FixedView<Matrix3x5<T>, Matrix3<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x5<T>, irow: usize, icol: usize) -> Matrix3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x4 blocks of a `Matrix3x5` as a `Matrix3x4`. Upstream: `fixed_view::<3, 4>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix3x5<T>, Matrix3x4<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x5<T>, irow: usize, icol: usize) -> Matrix3x4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x4<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x5 blocks of a `Matrix3x5` as a `Matrix3x5`. Upstream: `fixed_view::<3, 5>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix3x5<T>, Matrix3x5<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x5<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x5<T>, irow: usize, icol: usize) -> Matrix3x5<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x5<T> {
        Self::view(self, start, shape)
    }
}

/// The first 1 components of a row of a `Matrix3x5` as a `Matrix1`. Upstream: `row_part` (`n = 1`).
pub impl Matrix3x5RowPartMatrix1<T, +Copy<T>, +Drop<T>> of RowPart<Matrix3x5<T>, Matrix1<T>> {
    #[inline(always)]
    fn row_part(self: Matrix3x5<T>, i: usize, n: usize) -> Matrix1<T> {
        if n != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The first 2 components of a row of a `Matrix3x5` as a `RowVector2`. Upstream: `row_part` (`n =
/// 2`).
pub impl Matrix3x5RowPartRowVector2<T, +Copy<T>, +Drop<T>> of RowPart<Matrix3x5<T>, RowVector2<T>> {
    #[inline(always)]
    fn row_part(self: Matrix3x5<T>, i: usize, n: usize) -> RowVector2<T> {
        if n != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 3 components of a row of a `Matrix3x5` as a `RowVector3`. Upstream: `row_part` (`n =
/// 3`).
pub impl Matrix3x5RowPartRowVector3<T, +Copy<T>, +Drop<T>> of RowPart<Matrix3x5<T>, RowVector3<T>> {
    #[inline(always)]
    fn row_part(self: Matrix3x5<T>, i: usize, n: usize) -> RowVector3<T> {
        if n != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 4 components of a row of a `Matrix3x5` as a `RowVector4`. Upstream: `row_part` (`n =
/// 4`).
pub impl Matrix3x5RowPartRowVector4<T, +Copy<T>, +Drop<T>> of RowPart<Matrix3x5<T>, RowVector4<T>> {
    #[inline(always)]
    fn row_part(self: Matrix3x5<T>, i: usize, n: usize) -> RowVector4<T> {
        if n != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 5 components of a row of a `Matrix3x5` as a `RowVector5`. Upstream: `row_part` (`n =
/// 5`).
pub impl Matrix3x5RowPartRowVector5<T, +Copy<T>, +Drop<T>> of RowPart<Matrix3x5<T>, RowVector5<T>> {
    #[inline(always)]
    fn row_part(self: Matrix3x5<T>, i: usize, n: usize) -> RowVector5<T> {
        if n != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 1 components of a column of a `Matrix3x5` as a `Matrix1`. Upstream: `column_part` (`n
/// = 1`).
pub impl Matrix3x5ColumnPartMatrix1<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix3x5<T>, Matrix1<T>> {
    #[inline(always)]
    fn column_part(self: Matrix3x5<T>, i: usize, n: usize) -> Matrix1<T> {
        if n != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The first 2 components of a column of a `Matrix3x5` as a `Vector2`. Upstream: `column_part` (`n
/// = 2`).
pub impl Matrix3x5ColumnPartVector2<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix3x5<T>, Vector2<T>> {
    #[inline(always)]
    fn column_part(self: Matrix3x5<T>, i: usize, n: usize) -> Vector2<T> {
        if n != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, 0, i)
...
/// The first 3 components of a column of a `Matrix3x5` as a `Vector3`. Upstream: `column_part` (`n
/// = 3`).
pub impl Matrix3x5ColumnPartVector3<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix3x5<T>, Vector3<T>> {
    #[inline(always)]
    fn column_part(self: Matrix3x5<T>, i: usize, n: usize) -> Vector3<T> {
        if n != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, 0, i)
