...
> of Matrix3x6Trait<T> {
    ...
    #[inline(always)]
    fn nrows(self: Matrix3x6<T>) -> usize {
        3
    }

    /// The number of columns, 6. Upstream: `ncols`.
    #[inline(always)]
    fn ncols(self: Matrix3x6<T>) -> usize {
        6
    }

    /// `(nrows, ncols)`, `(3, 6)`. Upstream: `shape`.
    #[inline(always)]
    fn shape(self: Matrix3x6<T>) -> (usize, usize) {
        (3, 6)
    }

    /// Whether the shape is square: `false`. Upstream: `is_square`.
    #[inline(always)]
    fn is_square(self: Matrix3x6<T>) -> bool {
        false
    }
    ...
    #[inline(always)]
    fn vector_to_matrix_index(self: Matrix3x6<T>, i: usize) -> (usize, usize) {
    ...
    #[inline(always)]
    fn row(self: Matrix3x6<T>, i: usize) -> RowVector6<T> {
        Matrix3x6EditTrait::row_at(self, i)
    }
    ...
    #[inline(always)]
    fn column(self: Matrix3x6<T>, j: usize) -> Vector3<T> {
        Matrix3x6EditTrait::column_at(self, j)
    }
    ...
    #[inline(always)]
    fn upper_triangle(self: Matrix3x6<T>) -> Matrix3x6<T> {
        Matrix3x6 {
            m11: self.m11,
            m21: R::zero(),
            m31: R::zero(),
    ...
    #[inline(always)]
    fn lower_triangle(self: Matrix3x6<T>) -> Matrix3x6<T> {
        Matrix3x6 {
            m11: self.m11,
            m21: self.m21,
            m31: self.m31,
    ...
    /// The 3x6 matrix whose 3 rows are the given vectors, each a `Vector6` of the row's 6
    ...
    #[inline(always)]
    fn from_rows(r1: Vector6<T>, r2: Vector6<T>, r3: Vector6<T>) -> Matrix3x6<T> {
        Matrix3x6 {
            m11: r1.x,
            m21: r2.x,
            m31: r3.x,
    ...
    /// The 3x6 matrix whose 6 columns are the given `Vector3`s. Exact. Upstream: `from_columns` (a
    ...
    #[inline(always)]
    fn is_orthogonal(self: Matrix3x6<T>, ulps: u64) -> bool {
        Matrix6Trait::is_identity(MatrixTrMul::tr_mul(self, self), ulps)
    }

    /// `self` with a row of `val` inserted at index `i` (`i <= 3`), a `Matrix4x6`. Panics with
    /// `nalgebra: index out of bounds` for `i > 3`. Upstream: `insert_row`.
    fn insert_row(self: Matrix3x6<T>, i: usize, val: T) -> Matrix4x6<T> {
        match i {
            0 => Matrix4x6 {
                m11: val,
                m21: self.m11,
    ...
    /// `self` without its row `i`, a `Matrix2x6`. Panics with `nalgebra: index out of bounds` for
    /// `i >= 3`. Upstream: `remove_row`.
    #[inline(always)]
    fn remove_row(self: Matrix3x6<T>, i: usize) -> Matrix2x6<T> {
        match i {
            0 => Matrix2x6 {
                m11: self.m21,
                m21: self.m31,
    ...
    /// `self` without its column `i`, a `Matrix3x5`. Panics with `nalgebra: index out of bounds`
    /// for `i >= 6`. Upstream: `remove_column`.
    #[inline(always)]
    fn remove_column(self: Matrix3x6<T>, i: usize) -> Matrix3x5<T> {
        match i {
            0 => Matrix3x5 {
                m11: self.m12,
                m21: self.m22,
...
/// The 1 consecutive rows of a `Matrix3x6` as a `RowVector6`. Upstream: `fixed_rows::<1>`, `rows(i,
/// 1)`, `rows_range`, `select_rows`.
pub impl Matrix3x6FixedRowsRowVector6<
    T, +Copy<T>, +Drop<T>,
> of FixedRows<Matrix3x6<T>, RowVector6<T>> {
    #[inline(always)]
    fn fixed_rows(self: Matrix3x6<T>, i: usize) -> RowVector6<T> {
        match i {
            0 => RowVector6 {
                x: self.m11, y: self.m12, z: self.m13, w: self.m14, a: self.m15, b: self.m16,
            },
    ...
    #[inline(always)]
    fn rows(self: Matrix3x6<T>, first_row: usize, nrows: usize) -> RowVector6<T> {
        if nrows != 1 {
    ...
    #[inline(always)]
    fn rows_range(self: Matrix3x6<T>, rows: Range<usize>) -> RowVector6<T> {
    ...
    #[inline(always)]
    fn select_rows(self: Matrix3x6<T>, irows: Span<usize>) -> RowVector6<T> {
        if irows.len() != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let r0 = Matrix3x6EditTrait::row_at(self, *irows[0]);
        RowVector6 { x: r0.x, y: r0.y, z: r0.z, w: r0.w, a: r0.a, b: r0.b }
    }
}

/// The 2 consecutive rows of a `Matrix3x6` as a `Matrix2x6`. Upstream: `fixed_rows::<2>`, `rows(i,
/// 2)`, `rows_range`, `select_rows`.
pub impl Matrix3x6FixedRowsMatrix2x6<
    T, +Copy<T>, +Drop<T>,
> of FixedRows<Matrix3x6<T>, Matrix2x6<T>> {
    #[inline(always)]
    fn fixed_rows(self: Matrix3x6<T>, i: usize) -> Matrix2x6<T> {
        match i {
            0 => Matrix2x6 {
                m11: self.m11,
                m21: self.m21,
    ...
    #[inline(always)]
    fn rows(self: Matrix3x6<T>, first_row: usize, nrows: usize) -> Matrix2x6<T> {
        if nrows != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_rows(self, first_row)
    ...
    #[inline(always)]
    fn rows_range(self: Matrix3x6<T>, rows: Range<usize>) -> Matrix2x6<T> {
        let Range { start, end } = rows;
        if end != start + 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn select_rows(self: Matrix3x6<T>, irows: Span<usize>) -> Matrix2x6<T> {
        if irows.len() != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let r0 = Matrix3x6EditTrait::row_at(self, *irows[0]);
        let r1 = Matrix3x6EditTrait::row_at(self, *irows[1]);
        Matrix2x6 {
            m11: r0.x,
            m21: r1.x,
            m12: r0.y,
...
/// The 3 consecutive rows of a `Matrix3x6` as a `Matrix3x6`. Upstream: `fixed_rows::<3>`, `rows(i,
/// 3)`, `rows_range`, `select_rows`.
pub impl Matrix3x6FixedRowsMatrix3x6<
    T, +Copy<T>, +Drop<T>,
> of FixedRows<Matrix3x6<T>, Matrix3x6<T>> {
    fn fixed_rows(self: Matrix3x6<T>, i: usize) -> Matrix3x6<T> {
        match i {
            0 => Matrix3x6 {
                m11: self.m11,
                m21: self.m21,
    ...
    #[inline(always)]
    fn rows(self: Matrix3x6<T>, first_row: usize, nrows: usize) -> Matrix3x6<T> {
        if nrows != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_rows(self, first_row)
    ...
    #[inline(always)]
    fn rows_range(self: Matrix3x6<T>, rows: Range<usize>) -> Matrix3x6<T> {
        let Range { start, end } = rows;
        if end != start + 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    fn select_rows(self: Matrix3x6<T>, irows: Span<usize>) -> Matrix3x6<T> {
        if irows.len() != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let r0 = Matrix3x6EditTrait::row_at(self, *irows[0]);
        let r1 = Matrix3x6EditTrait::row_at(self, *irows[1]);
        let r2 = Matrix3x6EditTrait::row_at(self, *irows[2]);
        Matrix3x6 {
            m11: r0.x,
            m21: r1.x,
            m31: r2.x,
...
/// The 1 consecutive columns of a `Matrix3x6` as a `Vector3`. Upstream: `fixed_columns::<1>`,
/// `columns(j, 1)`, `columns_range`, `select_columns`.
pub impl Matrix3x6FixedColumnsVector3<
    T, +Copy<T>, +Drop<T>,
> of FixedColumns<Matrix3x6<T>, Vector3<T>> {
    #[inline(always)]
    fn fixed_columns(self: Matrix3x6<T>, i: usize) -> Vector3<T> {
        match i {
            0 => Vector3 { x: self.m11, y: self.m21, z: self.m31 },
            1 => Vector3 { x: self.m12, y: self.m22, z: self.m32 },
            2 => Vector3 { x: self.m13, y: self.m23, z: self.m33 },
    ...
    #[inline(always)]
    fn columns(self: Matrix3x6<T>, first_col: usize, ncols: usize) -> Vector3<T> {
        if ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn columns_range(self: Matrix3x6<T>, cols: Range<usize>) -> Vector3<T> {
    ...
    #[inline(always)]
    fn select_columns(self: Matrix3x6<T>, icols: Span<usize>) -> Vector3<T> {
        if icols.len() != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let c0 = Matrix3x6EditTrait::column_at(self, *icols[0]);
        Vector3 { x: c0.x, y: c0.y, z: c0.z }
    }
}

/// The 2 consecutive columns of a `Matrix3x6` as a `Matrix3x2`. Upstream: `fixed_columns::<2>`,
/// `columns(j, 2)`, `columns_range`, `select_columns`.
pub impl Matrix3x6FixedColumnsMatrix3x2<
    T, +Copy<T>, +Drop<T>,
> of FixedColumns<Matrix3x6<T>, Matrix3x2<T>> {
    #[inline(always)]
    fn fixed_columns(self: Matrix3x6<T>, i: usize) -> Matrix3x2<T> {
        match i {
            0 => Matrix3x2 {
                m11: self.m11,
                m21: self.m21,
    ...
    #[inline(always)]
    fn columns(self: Matrix3x6<T>, first_col: usize, ncols: usize) -> Matrix3x2<T> {
        if ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix3x6<T>, cols: Range<usize>) -> Matrix3x2<T> {
        let Range { start, end } = cols;
        if end != start + 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn select_columns(self: Matrix3x6<T>, icols: Span<usize>) -> Matrix3x2<T> {
        if icols.len() != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let c0 = Matrix3x6EditTrait::column_at(self, *icols[0]);
        let c1 = Matrix3x6EditTrait::column_at(self, *icols[1]);
        Matrix3x2 { m11: c0.x, m21: c0.y, m31: c0.z, m12: c1.x, m22: c1.y, m32: c1.z }
    }
}

/// The 3 consecutive columns of a `Matrix3x6` as a `Matrix3`. Upstream: `fixed_columns::<3>`,
/// `columns(j, 3)`, `columns_range`, `select_columns`.
pub impl Matrix3x6FixedColumnsMatrix3<
    T, +Copy<T>, +Drop<T>,
> of FixedColumns<Matrix3x6<T>, Matrix3<T>> {
    #[inline(always)]
    fn fixed_columns(self: Matrix3x6<T>, i: usize) -> Matrix3<T> {
        match i {
            0 => Matrix3 {
                m11: self.m11,
                m21: self.m21,
    ...
    #[inline(always)]
    fn columns(self: Matrix3x6<T>, first_col: usize, ncols: usize) -> Matrix3<T> {
        if ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix3x6<T>, cols: Range<usize>) -> Matrix3<T> {
        let Range { start, end } = cols;
        if end != start + 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn select_columns(self: Matrix3x6<T>, icols: Span<usize>) -> Matrix3<T> {
        if icols.len() != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let c0 = Matrix3x6EditTrait::column_at(self, *icols[0]);
        let c1 = Matrix3x6EditTrait::column_at(self, *icols[1]);
        let c2 = Matrix3x6EditTrait::column_at(self, *icols[2]);
        Matrix3 {
            m11: c0.x,
            m21: c0.y,
            m31: c0.z,
...
/// The 4 consecutive columns of a `Matrix3x6` as a `Matrix3x4`. Upstream: `fixed_columns::<4>`,
/// `columns(j, 4)`, `columns_range`, `select_columns`.
pub impl Matrix3x6FixedColumnsMatrix3x4<
    T, +Copy<T>, +Drop<T>,
> of FixedColumns<Matrix3x6<T>, Matrix3x4<T>> {
    #[inline(always)]
    fn fixed_columns(self: Matrix3x6<T>, i: usize) -> Matrix3x4<T> {
        match i {
            0 => Matrix3x4 {
                m11: self.m11,
                m21: self.m21,
    ...
    #[inline(always)]
    fn columns(self: Matrix3x6<T>, first_col: usize, ncols: usize) -> Matrix3x4<T> {
        if ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix3x6<T>, cols: Range<usize>) -> Matrix3x4<T> {
        let Range { start, end } = cols;
        if end != start + 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn select_columns(self: Matrix3x6<T>, icols: Span<usize>) -> Matrix3x4<T> {
        if icols.len() != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let c0 = Matrix3x6EditTrait::column_at(self, *icols[0]);
        let c1 = Matrix3x6EditTrait::column_at(self, *icols[1]);
        let c2 = Matrix3x6EditTrait::column_at(self, *icols[2]);
        let c3 = Matrix3x6EditTrait::column_at(self, *icols[3]);
        Matrix3x4 {
            m11: c0.x,
            m21: c0.y,
            m31: c0.z,
...
/// The 5 consecutive columns of a `Matrix3x6` as a `Matrix3x5`. Upstream: `fixed_columns::<5>`,
/// `columns(j, 5)`, `columns_range`, `select_columns`.
pub impl Matrix3x6FixedColumnsMatrix3x5<
    T, +Copy<T>, +Drop<T>,
> of FixedColumns<Matrix3x6<T>, Matrix3x5<T>> {
    #[inline(always)]
    fn fixed_columns(self: Matrix3x6<T>, i: usize) -> Matrix3x5<T> {
        match i {
            0 => Matrix3x5 {
                m11: self.m11,
                m21: self.m21,
    ...
    #[inline(always)]
    fn columns(self: Matrix3x6<T>, first_col: usize, ncols: usize) -> Matrix3x5<T> {
        if ncols != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix3x6<T>, cols: Range<usize>) -> Matrix3x5<T> {
        let Range { start, end } = cols;
        if end != start + 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn select_columns(self: Matrix3x6<T>, icols: Span<usize>) -> Matrix3x5<T> {
        if icols.len() != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let c0 = Matrix3x6EditTrait::column_at(self, *icols[0]);
        ...
        let c2 = Matrix3x6EditTrait::column_at(self, *icols[2]);
        let c3 = Matrix3x6EditTrait::column_at(self, *icols[3]);
        let c4 = Matrix3x6EditTrait::column_at(self, *icols[4]);
        Matrix3x5 {
            m11: c0.x,
            m21: c0.y,
            m31: c0.z,
...
/// The 6 consecutive columns of a `Matrix3x6` as a `Matrix3x6`. Upstream: `fixed_columns::<6>`,
/// `columns(j, 6)`, `columns_range`, `select_columns`.
pub impl Matrix3x6FixedColumnsMatrix3x6<
    T, +Copy<T>, +Drop<T>,
> of FixedColumns<Matrix3x6<T>, Matrix3x6<T>> {
    fn fixed_columns(self: Matrix3x6<T>, i: usize) -> Matrix3x6<T> {
        match i {
            0 => Matrix3x6 {
                m11: self.m11,
                m21: self.m21,
    ...
    #[inline(always)]
    fn columns(self: Matrix3x6<T>, first_col: usize, ncols: usize) -> Matrix3x6<T> {
        if ncols != 6 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix3x6<T>, cols: Range<usize>) -> Matrix3x6<T> {
        let Range { start, end } = cols;
        if end != start + 6 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    fn select_columns(self: Matrix3x6<T>, icols: Span<usize>) -> Matrix3x6<T> {
        if icols.len() != 6 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        let c0 = Matrix3x6EditTrait::column_at(self, *icols[0]);
        ...
        let c3 = Matrix3x6EditTrait::column_at(self, *icols[3]);
        let c4 = Matrix3x6EditTrait::column_at(self, *icols[4]);
        let c5 = Matrix3x6EditTrait::column_at(self, *icols[5]);
        Matrix3x6 {
            m11: c0.x,
            m21: c0.y,
            m31: c0.z,
...
/// The 1x1 blocks of a `Matrix3x6` as a `Matrix1`. Upstream: `fixed_view::<1, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix3x6FixedViewMatrix1<T, +Copy<T>, +Drop<T>> of FixedView<Matrix3x6<T>, Matrix1<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix3x6<T>, irow: usize, icol: usize) -> Matrix1<T> {
        match icol {
            0 => match irow {
    ...
    #[inline(always)]
    fn view(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix1<T> {
        let (irow, icol) = start;
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x6<T>, irow: usize, icol: usize) -> Matrix1<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix1<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x2 blocks of a `Matrix3x6` as a `RowVector2`. Upstream: `fixed_view::<1, 2>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix3x6FixedViewRowVector2<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix3x6<T>, RowVector2<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix3x6<T>, irow: usize, icol: usize) -> RowVector2<T> {
        match icol {
            0 => match irow {
    ...
    #[inline(always)]
    fn view(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x6<T>, irow: usize, icol: usize) -> RowVector2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector2<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x3 blocks of a `Matrix3x6` as a `RowVector3`. Upstream: `fixed_view::<1, 3>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix3x6FixedViewRowVector3<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix3x6<T>, RowVector3<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix3x6<T>, irow: usize, icol: usize) -> RowVector3<T> {
        match icol {
            0 => match irow {
    ...
    #[inline(always)]
    fn view(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x6<T>, irow: usize, icol: usize) -> RowVector3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector3<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x4 blocks of a `Matrix3x6` as a `RowVector4`. Upstream: `fixed_view::<1, 4>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix3x6FixedViewRowVector4<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix3x6<T>, RowVector4<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix3x6<T>, irow: usize, icol: usize) -> RowVector4<T> {
        match icol {
            0 => match irow {
    ...
    #[inline(always)]
    fn view(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x6<T>, irow: usize, icol: usize) -> RowVector4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector4<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x5 blocks of a `Matrix3x6` as a `RowVector5`. Upstream: `fixed_view::<1, 5>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix3x6FixedViewRowVector5<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix3x6<T>, RowVector5<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix3x6<T>, irow: usize, icol: usize) -> RowVector5<T> {
        match icol {
            0 => match irow {
    ...
    #[inline(always)]
    fn view(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector5<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x6<T>, irow: usize, icol: usize) -> RowVector5<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector5<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x6 blocks of a `Matrix3x6` as a `RowVector6`. Upstream: `fixed_view::<1, 6>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix3x6FixedViewRowVector6<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix3x6<T>, RowVector6<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix3x6<T>, irow: usize, icol: usize) -> RowVector6<T> {
        match icol {
            0 => match irow {
                0 => RowVector6 {
                    x: self.m11, y: self.m12, z: self.m13, w: self.m14, a: self.m15, b: self.m16,
    ...
    #[inline(always)]
    fn view(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector6<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 6 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x6<T>, irow: usize, icol: usize) -> RowVector6<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector6<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x1 blocks of a `Matrix3x6` as a `Vector2`. Upstream: `fixed_view::<2, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix3x6FixedViewVector2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix3x6<T>, Vector2<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix3x6<T>, irow: usize, icol: usize) -> Vector2<T> {
        match icol {
            0 => match irow {
    ...
    #[inline(always)]
    fn view(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Vector2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x6<T>, irow: usize, icol: usize) -> Vector2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Vector2<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x2 blocks of a `Matrix3x6` as a `Matrix2`. Upstream: `fixed_view::<2, 2>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix3x6FixedViewMatrix2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix3x6<T>, Matrix2<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix3x6<T>, irow: usize, icol: usize) -> Matrix2<T> {
        match icol {
            0 => match irow {
    ...
    #[inline(always)]
    fn view(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x6<T>, irow: usize, icol: usize) -> Matrix2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x3 blocks of a `Matrix3x6` as a `Matrix2x3`. Upstream: `fixed_view::<2, 3>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix3x6FixedViewMatrix2x3<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix3x6<T>, Matrix2x3<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix3x6<T>, irow: usize, icol: usize) -> Matrix2x3<T> {
        match icol {
            0 => match irow {
                0 => Matrix2x3 {
                    m11: self.m11,
    ...
    #[inline(always)]
    fn view(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x6<T>, irow: usize, icol: usize) -> Matrix2x3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x3<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x4 blocks of a `Matrix3x6` as a `Matrix2x4`. Upstream: `fixed_view::<2, 4>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix3x6FixedViewMatrix2x4<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix3x6<T>, Matrix2x4<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix3x6<T>, irow: usize, icol: usize) -> Matrix2x4<T> {
        match icol {
            0 => match irow {
                0 => Matrix2x4 {
                    m11: self.m11,
    ...
    #[inline(always)]
    fn view(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x6<T>, irow: usize, icol: usize) -> Matrix2x4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x4<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x5 blocks of a `Matrix3x6` as a `Matrix2x5`. Upstream: `fixed_view::<2, 5>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix3x6FixedViewMatrix2x5<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix3x6<T>, Matrix2x5<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix3x6<T>, irow: usize, icol: usize) -> Matrix2x5<T> {
        match icol {
            0 => match irow {
                0 => Matrix2x5 {
                    m11: self.m11,
    ...
    #[inline(always)]
    fn view(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x5<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x6<T>, irow: usize, icol: usize) -> Matrix2x5<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x5<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x6 blocks of a `Matrix3x6` as a `Matrix2x6`. Upstream: `fixed_view::<2, 6>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix3x6FixedViewMatrix2x6<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix3x6<T>, Matrix2x6<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix3x6<T>, irow: usize, icol: usize) -> Matrix2x6<T> {
        match icol {
            0 => match irow {
                0 => Matrix2x6 {
                    m11: self.m11,
    ...
    #[inline(always)]
    fn view(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x6<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 6 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x6<T>, irow: usize, icol: usize) -> Matrix2x6<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x6<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x1 blocks of a `Matrix3x6` as a `Vector3`. Upstream: `fixed_view::<3, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix3x6FixedViewVector3<T, +Copy<T>, +Drop<T>> of FixedView<Matrix3x6<T>, Vector3<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix3x6<T>, irow: usize, icol: usize) -> Vector3<T> {
        match icol {
            0 => match irow {
                0 => Vector3 { x: self.m11, y: self.m21, z: self.m31 },
                _ => core::panic_with_felt252(errors::INDEX_OUT_OF_BOUNDS),
    ...
    #[inline(always)]
    fn view(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Vector3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x6<T>, irow: usize, icol: usize) -> Vector3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Vector3<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x2 blocks of a `Matrix3x6` as a `Matrix3x2`. Upstream: `fixed_view::<3, 2>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix3x6FixedViewMatrix3x2<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix3x6<T>, Matrix3x2<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix3x6<T>, irow: usize, icol: usize) -> Matrix3x2<T> {
        match icol {
            0 => match irow {
                0 => Matrix3x2 {
                    m11: self.m11,
    ...
    #[inline(always)]
    fn view(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x6<T>, irow: usize, icol: usize) -> Matrix3x2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x2<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x3 blocks of a `Matrix3x6` as a `Matrix3`. Upstream: `fixed_view::<3, 3>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix3x6FixedViewMatrix3<T, +Copy<T>, +Drop<T>> of FixedView<Matrix3x6<T>, Matrix3<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix3x6<T>, irow: usize, icol: usize) -> Matrix3<T> {
        match icol {
            0 => match irow {
                0 => Matrix3 {
                    m11: self.m11,
    ...
    #[inline(always)]
    fn view(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x6<T>, irow: usize, icol: usize) -> Matrix3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x4 blocks of a `Matrix3x6` as a `Matrix3x4`. Upstream: `fixed_view::<3, 4>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix3x6FixedViewMatrix3x4<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix3x6<T>, Matrix3x4<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix3x6<T>, irow: usize, icol: usize) -> Matrix3x4<T> {
        match icol {
            0 => match irow {
                0 => Matrix3x4 {
                    m11: self.m11,
    ...
    #[inline(always)]
    fn view(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x6<T>, irow: usize, icol: usize) -> Matrix3x4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x4<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x5 blocks of a `Matrix3x6` as a `Matrix3x5`. Upstream: `fixed_view::<3, 5>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix3x6FixedViewMatrix3x5<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix3x6<T>, Matrix3x5<T>> {
    #[inline(always)]
    fn fixed_view(self: Matrix3x6<T>, irow: usize, icol: usize) -> Matrix3x5<T> {
        match icol {
            0 => match irow {
                0 => Matrix3x5 {
                    m11: self.m11,
    ...
    #[inline(always)]
    fn view(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x5<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x6<T>, irow: usize, icol: usize) -> Matrix3x5<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x5<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x6 blocks of a `Matrix3x6` as a `Matrix3x6`. Upstream: `fixed_view::<3, 6>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
pub impl Matrix3x6FixedViewMatrix3x6<
    T, +Copy<T>, +Drop<T>,
> of FixedView<Matrix3x6<T>, Matrix3x6<T>> {
    fn fixed_view(self: Matrix3x6<T>, irow: usize, icol: usize) -> Matrix3x6<T> {
        match icol {
            0 => match irow {
                0 => Matrix3x6 {
                    m11: self.m11,
    ...
    #[inline(always)]
    fn view(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x6<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 6 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix3x6<T>, irow: usize, icol: usize) -> Matrix3x6<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix3x6<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x6<T> {
        Self::view(self, start, shape)
    }
}

/// The first 1 components of a row of a `Matrix3x6` as a `Matrix1`. Upstream: `row_part` (`n = 1`).
pub impl Matrix3x6RowPartMatrix1<T, +Copy<T>, +Drop<T>> of RowPart<Matrix3x6<T>, Matrix1<T>> {
    #[inline(always)]
    fn row_part(self: Matrix3x6<T>, i: usize, n: usize) -> Matrix1<T> {
        if n != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The first 2 components of a row of a `Matrix3x6` as a `RowVector2`. Upstream: `row_part` (`n =
/// 2`).
pub impl Matrix3x6RowPartRowVector2<T, +Copy<T>, +Drop<T>> of RowPart<Matrix3x6<T>, RowVector2<T>> {
    #[inline(always)]
    fn row_part(self: Matrix3x6<T>, i: usize, n: usize) -> RowVector2<T> {
        if n != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 3 components of a row of a `Matrix3x6` as a `RowVector3`. Upstream: `row_part` (`n =
/// 3`).
pub impl Matrix3x6RowPartRowVector3<T, +Copy<T>, +Drop<T>> of RowPart<Matrix3x6<T>, RowVector3<T>> {
    #[inline(always)]
    fn row_part(self: Matrix3x6<T>, i: usize, n: usize) -> RowVector3<T> {
        if n != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 4 components of a row of a `Matrix3x6` as a `RowVector4`. Upstream: `row_part` (`n =
/// 4`).
pub impl Matrix3x6RowPartRowVector4<T, +Copy<T>, +Drop<T>> of RowPart<Matrix3x6<T>, RowVector4<T>> {
    #[inline(always)]
    fn row_part(self: Matrix3x6<T>, i: usize, n: usize) -> RowVector4<T> {
        if n != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 5 components of a row of a `Matrix3x6` as a `RowVector5`. Upstream: `row_part` (`n =
/// 5`).
pub impl Matrix3x6RowPartRowVector5<T, +Copy<T>, +Drop<T>> of RowPart<Matrix3x6<T>, RowVector5<T>> {
    #[inline(always)]
    fn row_part(self: Matrix3x6<T>, i: usize, n: usize) -> RowVector5<T> {
        if n != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 6 components of a row of a `Matrix3x6` as a `RowVector6`. Upstream: `row_part` (`n =
/// 6`).
pub impl Matrix3x6RowPartRowVector6<T, +Copy<T>, +Drop<T>> of RowPart<Matrix3x6<T>, RowVector6<T>> {
    #[inline(always)]
    fn row_part(self: Matrix3x6<T>, i: usize, n: usize) -> RowVector6<T> {
        if n != 6 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 1 components of a column of a `Matrix3x6` as a `Matrix1`. Upstream: `column_part` (`n
/// = 1`).
pub impl Matrix3x6ColumnPartMatrix1<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix3x6<T>, Matrix1<T>> {
    #[inline(always)]
    fn column_part(self: Matrix3x6<T>, i: usize, n: usize) -> Matrix1<T> {
        if n != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The first 2 components of a column of a `Matrix3x6` as a `Vector2`. Upstream: `column_part` (`n
/// = 2`).
pub impl Matrix3x6ColumnPartVector2<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix3x6<T>, Vector2<T>> {
    #[inline(always)]
    fn column_part(self: Matrix3x6<T>, i: usize, n: usize) -> Vector2<T> {
        if n != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, 0, i)
...
/// The first 3 components of a column of a `Matrix3x6` as a `Vector3`. Upstream: `column_part` (`n
/// = 3`).
pub impl Matrix3x6ColumnPartVector3<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix3x6<T>, Vector3<T>> {
    #[inline(always)]
    fn column_part(self: Matrix3x6<T>, i: usize, n: usize) -> Vector3<T> {
        if n != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, 0, i)
...
/// The Kronecker product of a `Matrix3x6` and a `Matrix1`, a `Matrix3x6`: one floored product per
/// component. Panics on overflow. Upstream: `kronecker`.
pub impl Matrix3x6KroneckerMatrix1<
    T, +Mul<T>, +Copy<T>, +Drop<T>,
> of MatrixKronecker<Matrix3x6<T>, Matrix1<T>> {
    type Output = Matrix3x6<T>;
    fn kronecker(self: Matrix3x6<T>, rhs: Matrix1<T>) -> Matrix3x6<T> {
        Matrix3x6 {
            m11: self.m11 * rhs.x,
            m21: self.m21 * rhs.x,
            m31: self.m31 * rhs.x,
...
/// The Kronecker product of a `Matrix3x6` and a `Vector2`, a `Matrix6`: one floored product per
/// component. Panics on overflow. Upstream: `kronecker`.
pub impl Matrix3x6KroneckerVector2<
    T, +Mul<T>, +Copy<T>, +Drop<T>,
> of MatrixKronecker<Matrix3x6<T>, Vector2<T>> {
    type Output = Matrix6<T>;
    fn kronecker(self: Matrix3x6<T>, rhs: Vector2<T>) -> Matrix6<T> {
        Matrix6 {
            m11: self.m11 * rhs.x,
            m21: self.m11 * rhs.y,
            m31: self.m21 * rhs.x,
...
pub(crate) impl Matrix3x6PadTo6<T, +Copy<T>, +Drop<T>> of PadTo6<Matrix3x6<T>, T> {
    #[inline(always)]
...
pub(crate) impl Matrix3x6CropFrom6<T, +Copy<T>, +Drop<T>> of CropFrom6<Matrix3x6<T>, T> {
    #[inline(always)]
