...
/// `iter.sum()` of an iterator of `RowVector2`s: the first item plus the others, in order (exact;
/// panics on overflow); the zero 2-dimensional row vector when empty. Upstream: `Sum for Matrix` (a
/// fold from `zero()`: the same result, one addition more).
pub impl RowVector2Sum<
    T, impl R: Real<T>, +Add<T>, +Copy<T>, +Drop<T>,
> of core::iter::Sum<RowVector2<T>> {
    fn sum<I, +Iterator<I>[Item: RowVector2<T>], +Destruct<I>, +Destruct<RowVector2<T>>>(
        mut iter: I,
    ) -> RowVector2<T> {
        let Option::Some(mut acc) = iter.next() else {
            return RowVector2 { x: R::zero(), y: R::zero() };
        };
...
/// `*iter.sum()` of an iterator of snapshots `@RowVector2` (`span.into_iter()`): a snapshot of the
/// sum of the items, like `Sum<RowVector2>`. Upstream: `Sum<&Matrix> for Matrix` (references).
pub impl RowVector2SumSnapshot<
    T, impl R: Real<T>, +Add<T>, +Copy<T>, +Drop<T>,
> of core::iter::Sum<@RowVector2<T>> {
    fn sum<I, +Iterator<I>[Item: @RowVector2<T>], +Destruct<I>, +Destruct<@RowVector2<T>>>(
        mut iter: I,
