...
> of Matrix4x5Trait<T> {
    ...
    /// The 4x5 matrix whose components all equal `elem`. Upstream: `Matrix4x5::repeat`.
    #[inline(always)]
    fn repeat(elem: T) -> Matrix4x5<T> {
        Matrix4x5 {
            m11: elem,
            m21: elem,
            m31: elem,
    ...
    /// Alias of `repeat`. Upstream: `Matrix4x5::from_element`.
    #[inline(always)]
    fn from_element(elem: T) -> Matrix4x5<T> {
        Matrix4x5 {
            m11: elem,
            m21: elem,
            m31: elem,
    ...
    /// The 4x5 matrix whose component `(i, j)` (row, column, 0-based) is `f(i, j)`, `f` being
    ...
    /// The 4x5 matrix of the 20 values of `data`, in row-major order. Panics with `nalgebra: wrong
    /// slice length` unless `data.len() == 20`. Upstream: `Matrix4x5::from_row_slice` (`&[T]`).
    ///
    ...
    fn from_row_slice(data: Span<T>) -> Matrix4x5<T> {
    ...
    /// wrong slice length` unless `data.len() == 20`. Upstream: `Matrix4x5::from_column_slice`
    /// (`&[T]`).
    ///
    /// `data` is read as ONE fixed-size array (`Span -> @Box<[T; 20]>`, one length check): measured
    /// about 5 times cheaper than a bounds-checked `*data[k]` per component
    /// (`bench_matrix3_from_row_slice__alt_span_index`).
    fn from_column_slice(data: Span<T>) -> Matrix4x5<T> {
        let boxed: @Box<[T; 20]> = data.try_into().expect(errors::SLICE_LENGTH);
        let [
            v0,
            v1,
    ...
    /// The 4x5 matrix whose first `data.len()` diagonal components are `data`, every other
    /// component zero. Panics with `nalgebra: diagonal too long` when `data.len() > 4`. Upstream:
    /// `Matrix4x5::from_partial_diagonal` (`&[T]`).
    fn from_partial_diagonal(data: Span<T>) -> Matrix4x5<T> {
        let len = data.len();
        if len > 4 {
    ...
    fn is_zero(self: Matrix4x5<T>) -> bool {
        self.m11 == R::zero()
            && self.m21 == R::zero()
            && self.m31 == R::zero()
            && self.m41 == R::zero()
    ...
    fn component_mul(self: Matrix4x5<T>, rhs: Matrix4x5<T>) -> Matrix4x5<T> {
        Matrix4x5 {
    ...
    fn component_mul_assign(ref self: Matrix4x5<T>, rhs: Matrix4x5<T>) {
        self =
            Matrix4x5 {
                m11: self.m11 * rhs.m11,
                m21: self.m21 * rhs.m21,
    ...
    fn component_div(self: Matrix4x5<T>, rhs: Matrix4x5<T>) -> Matrix4x5<T> {
        Matrix4x5 {
    ...
    fn component_div_assign(ref self: Matrix4x5<T>, rhs: Matrix4x5<T>) {
        self =
            Matrix4x5 {
                m11: R::div(self.m11, rhs.m11),
                m21: R::div(self.m21, rhs.m21),
    ...
    fn inf(self: Matrix4x5<T>, other: Matrix4x5<T>) -> Matrix4x5<T> {
        Matrix4x5 {
    ...
    fn sup(self: Matrix4x5<T>, other: Matrix4x5<T>) -> Matrix4x5<T> {
        Matrix4x5 {
    ...
    fn inf_sup(self: Matrix4x5<T>, other: Matrix4x5<T>) -> (Matrix4x5<T>, Matrix4x5<T>) {
        (Self::inf(self, other), Self::sup(self, other))
    }

    /// `self + k` added to every component. Exact; panics on overflow. Upstream: `add_scalar`.
    fn add_scalar(self: Matrix4x5<T>, k: T) -> Matrix4x5<T> {
        Matrix4x5 {
            m11: self.m11 + k,
            m21: self.m21 + k,
            m31: self.m31 + k,
    ...
    fn cmpy(ref self: Matrix4x5<T>, alpha: T, a: Matrix4x5<T>, b: Matrix4x5<T>, beta: T) {
        self =
            Matrix4x5 {
    ...
    fn cdpy(ref self: Matrix4x5<T>, alpha: T, a: Matrix4x5<T>, b: Matrix4x5<T>, beta: T) {
        self =
            Matrix4x5 {
    ...
    fn iamax_full(self: Matrix4x5<T>) -> (usize, usize) {
    ...
    #[inline(always)]
    fn icamax_full(self: Matrix4x5<T>) -> (usize, usize) {
    ...
    fn dot(self: Matrix4x5<T>, rhs: Matrix4x5<T>) -> T {
    ...
    /// `norm_squared`.
    fn norm_squared(self: Matrix4x5<T>) -> T {
        ...
        R::wide_rescale(R::wide_add_prod(w, self.m45, self.m45))
    }
    ...
    fn norm(self: Matrix4x5<T>) -> T {
        ...
        let w = R::wide_add_prod(w, self.m25, self.m25);
        let w = R::wide_add_prod(w, self.m35, self.m35);
        R::wide_sqrt(R::wide_add_prod(w, self.m45, self.m45))
    }

    /// Alias of `norm_squared`. Upstream: `magnitude_squared`.
    #[inline(always)]
    fn magnitude_squared(self: Matrix4x5<T>) -> T {
        Self::norm_squared(self)
    }

    /// Alias of `norm`. Upstream: `magnitude`.
    #[inline(always)]
    fn magnitude(self: Matrix4x5<T>) -> T {
        Self::norm(self)
    }
    ...
    fn metric_distance(self: Matrix4x5<T>, rhs: Matrix4x5<T>) -> T {
        ...
        R::wide_sqrt(R::wide_add_prod(w, self.m45 - rhs.m45, self.m45 - rhs.m45))
    }
    ...
    fn unscale(self: Matrix4x5<T>, k: T) -> Matrix4x5<T> {
        ...
        let (m15, m25, m35, m45) = R::div4(self.m15, self.m25, self.m35, self.m45, k);
        Matrix4x5 {
            m11,
            m21,
            m31,
    ...
    fn normalize(self: Matrix4x5<T>) -> Matrix4x5<T> {
    ...
    fn try_normalize(self: Matrix4x5<T>, min_norm: T) -> Option<Matrix4x5<T>> {
    ...
    /// `cap_magnitude`.
    fn cap_magnitude(self: Matrix4x5<T>, max: T) -> Matrix4x5<T> {
        let n = Self::norm(self);
        if n <= max {
            self
        } else {
    ...
    fn try_set_magnitude(ref self: Matrix4x5<T>, magnitude: T, min_magnitude: T) {
        let n = Self::norm(self);
        if n > min_magnitude {
    ...
    fn one_norm(self: Matrix4x5<T>) -> T {
        R::max(
            R::max(
                R::max(
                    R::max(
    ...
    /// The conjugate transpose, a `Matrix5x4`: the transpose for a real scalar. Exact. Upstream:
    /// `adjoint`.
    #[inline(always)]
    fn adjoint(self: Matrix4x5<T>) -> Matrix5x4<T> {
        Self::transpose(self)
    }

    /// Alias of `adjoint` (deprecated upstream). Upstream: `conjugate_transpose`.
    #[inline(always)]
    fn conjugate_transpose(self: Matrix4x5<T>) -> Matrix5x4<T> {
        Self::transpose(self)
    }

    /// The component-wise conjugate: `self` for a real scalar. Upstream: `conjugate`.
    #[inline(always)]
    fn conjugate(self: Matrix4x5<T>) -> Matrix4x5<T> {
        self
    }
    ...
    fn cast<U, +Into<T, U>, +Drop<U>>(self: Matrix4x5<T>) -> Matrix4x5<U> {
        Matrix4x5 {
            m11: self.m11.into(),
            m21: self.m21.into(),
            m31: self.m31.into(),
    ...
    fn try_cast<U, +TryInto<T, U>, +Drop<U>>(self: Matrix4x5<T>) -> Option<Matrix4x5<U>> {
    ...
    /// (DESIGN D3).
    fn relative_eq(self: Matrix4x5<T>, other: Matrix4x5<T>, epsilon: u64, max_relative: T) -> bool {
    ...
    fn ulps_eq(self: Matrix4x5<T>, other: Matrix4x5<T>, epsilon: u64, max_ulps: u32) -> bool {
    ...
    /// The 4x5 matrix of `f(x)` for every component `x` (called in column-major order). `f` is any
    ...
    /// The 4x5 matrix of `f(i, j, x)` for every component `x` at row `i`, column `j` (0-based,
    ...
    /// The 4x5 matrix of `f(a, b)` for the components `a` of `self` and `b` of `rhs` at the same
    ...
    fn zip_map<
        ...
        +Drop<Func::Output>,
    >(
        self: Matrix4x5<T>, rhs: Matrix4x5<T2>, f: F,
    ) -> Matrix4x5<Func::Output> {
        Matrix4x5 {
    ...
    /// The 4x5 matrix of `f(a, b, c)` for the components of `self`, `b` and `c` at the same
    /// position. `f` is any closure or `Fn` value; Cairo closures take their arguments by value and
    ...
    fn zip_zip_map<
        ...
        +Drop<Func::Output>,
    >(
        self: Matrix4x5<T>, b: Matrix4x5<T2>, c: Matrix4x5<T3>, f: F,
    ) -> Matrix4x5<Func::Output> {
        Matrix4x5 {
    ...
    /// `f(.. f(f(init, x0), x1) .., x19)` over the components in column-major order. The closure's
    ...
    >(
        self: Matrix4x5<T>, rhs: Matrix4x5<T2>, init: Acc, f: F,
    ) -> Acc {
    ...
    fn zip_apply<
        ...
        +Into<Func::Output, T>,
    >(
        ref self: Matrix4x5<T>, rhs: Matrix4x5<T2>, f: F,
    ) {
        self =
            Matrix4x5 {
    ...
    fn zip_zip_apply<
        ...
        +Into<Func::Output, T>,
    >(
        ref self: Matrix4x5<T>, b: Matrix4x5<T2>, c: Matrix4x5<T3>, f: F,
    ) {
        self =
            Matrix4x5 {
    ...
    fn fill(ref self: Matrix4x5<T>, val: T) {
        self =
            Matrix4x5 {
                m11: val,
                m21: val,
    ...
    fn fill_diagonal(ref self: Matrix4x5<T>, val: T) {
        self =
            Matrix4x5 {
                m11: val,
                m21: self.m21,
    ...
    fn fill_with_identity(ref self: Matrix4x5<T>) {
        self =
            Matrix4x5 {
                m11: R::one(),
                m21: R::zero(),
    ...
    fn fill_row(ref self: Matrix4x5<T>, i: usize, val: T) {
        self = match i {
            0 => Matrix4x5 {
                m11: val,
                m21: self.m21,
    ...
    fn fill_column(ref self: Matrix4x5<T>, j: usize, val: T) {
        self = match j {
            0 => Matrix4x5 {
                m11: val,
                m21: val,
    ...
    fn fill_lower_triangle(ref self: Matrix4x5<T>, val: T, shift: usize) {
        self = match shift {
            0 => Matrix4x5 {
                m11: val,
                m21: val,
    ...
    fn fill_upper_triangle(ref self: Matrix4x5<T>, val: T, shift: usize) {
        self = match shift {
            0 => Matrix4x5 {
                m11: val,
                m21: self.m21,
    ...
    fn set_row(ref self: Matrix4x5<T>, i: usize, row: RowVector5<T>) {
        self = match i {
            0 => Matrix4x5 {
                m11: row.x,
                m21: self.m21,
    ...
    fn set_column(ref self: Matrix4x5<T>, j: usize, column: Vector4<T>) {
        self = match j {
            0 => Matrix4x5 {
                m11: column.x,
                m21: column.y,
    ...
    fn set_diagonal(ref self: Matrix4x5<T>, diag: Vector4<T>) {
        self =
            Matrix4x5 {
                m11: diag.x,
                m21: self.m21,
    ...
    fn set_partial_diagonal(ref self: Matrix4x5<T>, diag: Span<T>) {
        let len = diag.len();
        self =
            Matrix4x5 {
                m11: if len > 0 {
    ...
    /// Sets `self` to `other` (a `Matrix4x5`: upstream requires the same shape). Upstream:
    /// `copy_from`.
    #[inline(always)]
    fn copy_from(ref self: Matrix4x5<T>, other: Matrix4x5<T>) {
        self = other;
    }

    /// Sets `self` to the 20 values of `slice` in column-major order (`from_column_slice`). Panics
    /// with `nalgebra: wrong slice length` unless `slice.len() == 20`. Upstream: `copy_from_slice`
    /// (`&[T]`).
    #[inline(always)]
    fn copy_from_slice(ref self: Matrix4x5<T>, slice: Span<T>) {
        self = Self::from_column_slice(slice);
    }

    /// Sets `self` to the transpose of `other`, a `Matrix5x4`. Exact. Upstream: `tr_copy_from`.
    fn tr_copy_from(ref self: Matrix4x5<T>, other: Matrix5x4<T>) {
        self =
            Matrix4x5 {
                m11: other.m11,
                m21: other.m12,
    ...
    #[inline]
    fn swap(ref self: Matrix4x5<T>, row_cols1: (usize, usize), row_cols2: (usize, usize)) {
        let a = MatrixIndex::index(self, row_cols1);
        let b = MatrixIndex::index(self, row_cols2);
        self =
            Matrix4x5EditTrait::replace(
                Matrix4x5EditTrait::replace(self, row_cols1, b), row_cols2, a,
            );
    }
    ...
    /// benchmark.
    #[inline]
    fn swap_rows(ref self: Matrix4x5<T>, irow1: usize, irow2: usize) {
        let a = Matrix4x5EditTrait::row_at(self, irow1);
        let b = Matrix4x5EditTrait::row_at(self, irow2);
    ...
    #[inline]
    fn swap_columns(ref self: Matrix4x5<T>, icol1: usize, icol2: usize) {
        let a = Matrix4x5EditTrait::column_at(self, icol1);
        let b = Matrix4x5EditTrait::column_at(self, icol2);
    ...
    #[inline(always)]
    fn neg_mut(ref self: Matrix4x5<T>) {
        self = -self;
    }
    ...
    /// `scale_mut`.
    #[inline(always)]
    fn scale_mut(ref self: Matrix4x5<T>, k: T) {
    ...
    #[inline(always)]
    fn unscale_mut(ref self: Matrix4x5<T>, k: T) {
    ...
    #[inline(always)]
    fn normalize_mut(ref self: Matrix4x5<T>) -> T {
        let n = Self::norm(self);
    ...
    #[inline(always)]
    fn try_normalize_mut(ref self: Matrix4x5<T>, min_norm: T) -> Option<T> {
        let n = Self::norm(self);
        if n <= min_norm {
            return None;
        }
    ...
    #[inline(always)]
    fn set_magnitude(ref self: Matrix4x5<T>, magnitude: T) {
        let n = Self::norm(self);
        self = Self::scale(self, R::div(magnitude, n));
    }
    ...
    /// `conjugate_mut`.
    #[inline(always)]
    fn conjugate_mut(ref self: Matrix4x5<T>) {
        self = Self::conjugate(self);
    }

    /// Writes the transpose of `self` into `out`, a `Matrix5x4`. Exact. Upstream: `transpose_to`.
    #[inline(always)]
    fn transpose_to(self: Matrix4x5<T>, ref out: Matrix5x4<T>) {
        out = Self::transpose(self);
    }

    /// Writes the adjoint of `self` (its transpose for a real scalar) into `out`, a `Matrix5x4`.
    /// Exact. Upstream: `adjoint_to`.
    #[inline(always)]
    fn adjoint_to(self: Matrix4x5<T>, ref out: Matrix5x4<T>) {
        out = Self::transpose(self);
    }

    /// Writes the adjoint of `self` (deprecated upstream alias of `adjoint_to`) into `out`, a
    /// `Matrix5x4`. Exact. Upstream: `conjugate_transpose_to`.
    #[inline(always)]
    fn conjugate_transpose_to(self: Matrix4x5<T>, ref out: Matrix5x4<T>) {
        out = Self::transpose(self);
    }

    /// Writes the sum `self + rhs` into `out`. Exact; panics on overflow. Upstream: `add_to`.
    #[inline(always)]
    fn add_to(self: Matrix4x5<T>, rhs: Matrix4x5<T>, ref out: Matrix4x5<T>) {
        out = self + rhs;
    }
    ...
    /// `sub_to`.
    #[inline(always)]
    fn sub_to(self: Matrix4x5<T>, rhs: Matrix4x5<T>, ref out: Matrix4x5<T>) {
        out = self - rhs;
    }
    ...
    #[inline]
    fn apply_norm<N, +Drop<N>, impl Nm: Norm<N, Matrix4x5<T>, T>>(
        self: Matrix4x5<T>, norm: N,
    ) -> T {
        Nm::norm(@norm, self)
    }
    ...
    #[inline]
    fn apply_metric_distance<N, +Drop<N>, impl Nm: Norm<N, Matrix4x5<T>, T>>(
        self: Matrix4x5<T>, rhs: Matrix4x5<T>, norm: N,
    ) -> T {
    ...
    #[inline(always)]
    fn nrows(self: Matrix4x5<T>) -> usize {
        4
    }

    /// The number of columns, 5. Upstream: `ncols`.
    #[inline(always)]
    fn ncols(self: Matrix4x5<T>) -> usize {
        5
    }

    /// `(nrows, ncols)`, `(4, 5)`. Upstream: `shape`.
    #[inline(always)]
    fn shape(self: Matrix4x5<T>) -> (usize, usize) {
        (4, 5)
    }

    /// Whether the shape is square: `false`. Upstream: `is_square`.
    #[inline(always)]
    fn is_square(self: Matrix4x5<T>) -> bool {
        false
    }
    ...
    #[inline(always)]
    fn vector_to_matrix_index(self: Matrix4x5<T>, i: usize) -> (usize, usize) {
    ...
    #[inline(always)]
    fn row(self: Matrix4x5<T>, i: usize) -> RowVector5<T> {
        Matrix4x5EditTrait::row_at(self, i)
    }
    ...
    #[inline(always)]
    fn column(self: Matrix4x5<T>, j: usize) -> Vector4<T> {
        Matrix4x5EditTrait::column_at(self, j)
    }
    ...
    #[inline(always)]
    fn upper_triangle(self: Matrix4x5<T>) -> Matrix4x5<T> {
        Matrix4x5 {
            m11: self.m11,
            m21: R::zero(),
            m31: R::zero(),
    ...
    #[inline(always)]
    fn lower_triangle(self: Matrix4x5<T>) -> Matrix4x5<T> {
        Matrix4x5 {
            m11: self.m11,
            m21: self.m21,
            m31: self.m31,
    ...
    /// The 4x5 matrix whose 4 rows are the given vectors, each a `Vector5` of the row's 5
    ...
    #[inline(always)]
    fn from_rows(r1: Vector5<T>, r2: Vector5<T>, r3: Vector5<T>, r4: Vector5<T>) -> Matrix4x5<T> {
        Matrix4x5 {
            m11: r1.x,
            m21: r2.x,
            m31: r3.x,
    ...
    /// The 4x5 matrix whose 5 columns are the given `Vector4`s. Exact. Upstream: `from_columns` (a
    /// slice of column vectors).
    #[inline(always)]
    fn from_columns(
        c1: Vector4<T>, c2: Vector4<T>, c3: Vector4<T>, c4: Vector4<T>, c5: Vector4<T>,
    ) -> Matrix4x5<T> {
        Matrix4x5 {
            m11: c1.x,
            m21: c1.y,
    ...
    #[inline(always)]
    fn is_orthogonal(self: Matrix4x5<T>, ulps: u64) -> bool {
        Matrix5Trait::is_identity(MatrixTrMul::tr_mul(self, self), ulps)
    }

    /// `self` with a row of `val` inserted at index `i` (`i <= 4`), a `Matrix5`. Panics with
    /// `nalgebra: index out of bounds` for `i > 4`. Upstream: `insert_row`.
    fn insert_row(self: Matrix4x5<T>, i: usize, val: T) -> Matrix5<T> {
        match i {
            0 => Matrix5 {
                m11: val,
                m21: self.m11,
    ...
    /// `self` with a column of `val` inserted at index `i` (`i <= 5`), a `Matrix4x6`. Panics with
    /// `nalgebra: index out of bounds` for `i > 5`. Upstream: `insert_column`.
    fn insert_column(self: Matrix4x5<T>, i: usize, val: T) -> Matrix4x6<T> {
        match i {
            0 => Matrix4x6 {
                m11: val,
                m21: val,
    ...
    /// `self` without its row `i`, a `Matrix3x5`. Panics with `nalgebra: index out of bounds` for
    /// `i >= 4`. Upstream: `remove_row`.
    #[inline(always)]
    fn remove_row(self: Matrix4x5<T>, i: usize) -> Matrix3x5<T> {
        match i {
            0 => Matrix3x5 {
                m11: self.m21,
                m21: self.m31,
    ...
    /// `self` without its column `i`, a `Matrix4`. Panics with `nalgebra: index out of bounds` for
    /// `i >= 5`. Upstream: `remove_column`.
    #[inline(always)]
    fn remove_column(self: Matrix4x5<T>, i: usize) -> Matrix4<T> {
        match i {
            0 => Matrix4 {
                m11: self.m12,
                m21: self.m22,
...
/// The operations of `Matrix4x5<T>` that need `Transcendental` (inverse trigonometry, `exp`, `ln`):
/// a scalar may implement `Real` only.
#[generate_trait]
pub impl Matrix4x5AngleImpl<
    T,
    impl R: Real<T>,
...
> of Matrix4x5AngleTrait<T> {
    ...
    fn angle(self: Matrix4x5<T>, other: Matrix4x5<T>) -> T {
        let n1 = Matrix4x5Trait::norm(self);
        let n2 = Matrix4x5Trait::norm(other);
        if n1 == R::zero() || n2 == R::zero() {
            return R::zero();
        }
        let u = Matrix4x5Trait::unscale(self, n1);
        let v = Matrix4x5Trait::unscale(other, n2);
        let half = Tr::atan2(Matrix4x5Trait::metric_distance(u, v), Matrix4x5Trait::norm(u + v));
        half + half
    }
    ...
    /// Upstream: `lp_norm`.
    fn lp_norm(self: Matrix4x5<T>, p: i32) -> T {
        if p < 1 {
        ...
        if p == 2 {
            return Matrix4x5Trait::norm(self);
        }
        let m = Matrix4x5Trait::amax(self);
        if m == R::zero() {
            return R::zero();
        }
        let r = Matrix4x5Trait::unscale(Matrix4x5Trait::abs(self), m);
...
/// `selfᵀ * rhs`, a `Vector5`: `mul_mat` of the transposed components, so one `sum_prod4` per
...
/// `selfᵀ * rhs`, a `Matrix5x2`: `mul_mat` of the transposed components, so one `sum_prod4` per
/// component (one rounding each); bit-identical to `self.transpose().mul_mat(rhs)`, at the same gas
/// (the transpose only relabels values). Panics on overflow. Upstream: `tr_mul`.
...
/// `selfᵀ * rhs`, a `Matrix5x3`: `mul_mat` of the transposed components, so one `sum_prod4` per
/// component (one rounding each); bit-identical to `self.transpose().mul_mat(rhs)`, at the same gas
/// (the transpose only relabels values). Panics on overflow. Upstream: `tr_mul`.
...
/// `selfᵀ * rhs`, a `Matrix5x4`: `mul_mat` of the transposed components, so one `sum_prod4` per
/// component (one rounding each); bit-identical to `self.transpose().mul_mat(rhs)`, at the same gas
/// (the transpose only relabels values). Panics on overflow. Upstream: `tr_mul`.
...
/// `selfᵀ * rhs`, a `Matrix5`: `mul_mat` of the transposed components, so one `sum_prod4` per
/// component (one rounding each); bit-identical to `self.transpose().mul_mat(rhs)`, at the same gas
/// (the transpose only relabels values). Panics on overflow. Upstream: `tr_mul`.
...
/// `selfᵀ * rhs`, a `Matrix5x6`: `mul_mat` of the transposed components, so one `sum_prod4` per
/// component (one rounding each); bit-identical to `self.transpose().mul_mat(rhs)`, at the same gas
/// (the transpose only relabels values). Panics on overflow. Upstream: `tr_mul`.
...
pub impl Matrix4x5MatrixIndexLinear<T, +Copy<T>, +Drop<T>> of MatrixIndex<Matrix4x5<T>, usize> {
    type Output = T;
    #[inline(always)]
    fn get(self: Matrix4x5<T>, index: usize) -> Option<T> {
        match index {
    ...
    #[inline(always)]
    fn index(self: Matrix4x5<T>, index: usize) -> T {
        match index {
            0 => self.m11,
            1 => self.m21,
            2 => self.m31,
...
pub impl Matrix4x5IndexLinear<T, +Copy<T>, +Drop<T>> of IndexView<Matrix4x5<T>, usize> {
    type Target = T;
    #[inline(always)]
    fn index(self: @Matrix4x5<T>, index: usize) -> T {
        match index {
...
pub impl Matrix4x5MatrixIndexPair<
    T, +Copy<T>, +Drop<T>,
> of MatrixIndex<Matrix4x5<T>, (usize, usize)> {
    type Output = T;
    #[inline(always)]
    fn get(self: Matrix4x5<T>, index: (usize, usize)) -> Option<T> {
        let (i, j) = index;
        match j {
            0 => match i {
                0 => Option::Some(self.m11),
    ...
    #[inline(always)]
    fn index(self: Matrix4x5<T>, index: (usize, usize)) -> T {
        let (i, j) = index;
        match j {
            0 => match i {
                0 => self.m11,
...
pub impl Matrix4x5IndexPair<T, +Copy<T>, +Drop<T>> of IndexView<Matrix4x5<T>, (usize, usize)> {
    type Target = T;
    #[inline(always)]
    fn index(self: @Matrix4x5<T>, index: (usize, usize)) -> T {
        let (i, j) = index;
...
pub impl Matrix4x5PartialOrd<T, +PartialOrd<T>, +Copy<T>, +Drop<T>> of PartialOrd<Matrix4x5<T>> {
    #[inline(always)]
    fn lt(lhs: Matrix4x5<T>, rhs: Matrix4x5<T>) -> bool {
        lhs.m11 < rhs.m11
            && lhs.m21 < rhs.m21
            && lhs.m31 < rhs.m31
            && lhs.m41 < rhs.m41
    ...
    #[inline(always)]
    fn le(lhs: Matrix4x5<T>, rhs: Matrix4x5<T>) -> bool {
        lhs.m11 <= rhs.m11
            && lhs.m21 <= rhs.m21
            && lhs.m31 <= rhs.m31
            && lhs.m41 <= rhs.m41
    ...
    #[inline(always)]
    fn gt(lhs: Matrix4x5<T>, rhs: Matrix4x5<T>) -> bool {
        lhs.m11 > rhs.m11
            && lhs.m21 > rhs.m21
            && lhs.m31 > rhs.m31
            && lhs.m41 > rhs.m41
    ...
    #[inline(always)]
    fn ge(lhs: Matrix4x5<T>, rhs: Matrix4x5<T>) -> bool {
        lhs.m11 >= rhs.m11
            && lhs.m21 >= rhs.m21
            && lhs.m31 >= rhs.m31
            && lhs.m41 >= rhs.m41
...
/// for Matrix`.
pub impl Matrix4x5Bounded<T, +Bounded<T>, +Drop<T>> of Bounded<Matrix4x5<T>> {
    const MIN: Matrix4x5<T> = Matrix4x5 {
        m11: Bounded::<T>::MIN,
        m21: Bounded::<T>::MIN,
        m31: Bounded::<T>::MIN,
        m41: Bounded::<T>::MIN,
    ...
    const MAX: Matrix4x5<T> = Matrix4x5 {
        m11: Bounded::<T>::MAX,
        m21: Bounded::<T>::MAX,
        m31: Bounded::<T>::MAX,
        m41: Bounded::<T>::MAX,
...
/// The 4x5 matrix of the given COLUMNS (`[[m11, m21, ..], [m12, ..], ..]`). Upstream: `From<[[T;
/// R]; C]>`.
pub impl Matrix4x5FromColumnArrays<T, +Drop<T>> of Into<[[T; 4]; 5], Matrix4x5<T>> {
    #[inline(always)]
    fn into(self: [[T; 4]; 5]) -> Matrix4x5<T> {
...
/// The columns of the 4x5 matrix as nested arrays (`[[m11, m21, ..], [m12, ..], ..]`). Upstream:
/// `Into<[[T; R]; C]>`.
pub impl Matrix4x5IntoColumnArrays<T, +Drop<T>> of Into<Matrix4x5<T>, [[T; 4]; 5]> {
    #[inline(always)]
    fn into(self: Matrix4x5<T>) -> [[T; 4]; 5] {
        let Matrix4x5 {
            m11,
            m21,
            m31,
...
#[generate_trait]
impl Matrix4x5EditImpl<T, +Copy<T>, +Drop<T>> of Matrix4x5EditTrait<T> {
    /// `self` with the component at `index` (`(row, column)`) replaced by `v`; panics out of
    /// bounds.
    fn replace(self: Matrix4x5<T>, index: (usize, usize), v: T) -> Matrix4x5<T> {
        let (i, j) = index;
        match j {
            0 => match i {
                0 => Matrix4x5 {
    ...
    fn row_at(self: Matrix4x5<T>, i: usize) -> RowVector5<T> {
        match i {
    ...
    fn column_at(self: Matrix4x5<T>, j: usize) -> Vector4<T> {
        match j {
...
/// The 1 consecutive rows of a `Matrix4x5` as a `RowVector5`. Upstream: `fixed_rows::<1>`, `rows(i,
...
> of FixedRows<Matrix4x5<T>, RowVector5<T>> {
    ...
    #[inline(always)]
    fn rows(self: Matrix4x5<T>, first_row: usize, nrows: usize) -> RowVector5<T> {
        if nrows != 1 {
    ...
    #[inline(always)]
    fn rows_range(self: Matrix4x5<T>, rows: Range<usize>) -> RowVector5<T> {
...
/// The 2 consecutive rows of a `Matrix4x5` as a `Matrix2x5`. Upstream: `fixed_rows::<2>`, `rows(i,
...
> of FixedRows<Matrix4x5<T>, Matrix2x5<T>> {
    ...
    #[inline(always)]
    fn rows(self: Matrix4x5<T>, first_row: usize, nrows: usize) -> Matrix2x5<T> {
        if nrows != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_rows(self, first_row)
    ...
    #[inline(always)]
    fn rows_range(self: Matrix4x5<T>, rows: Range<usize>) -> Matrix2x5<T> {
        let Range { start, end } = rows;
        if end != start + 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 3 consecutive rows of a `Matrix4x5` as a `Matrix3x5`. Upstream: `fixed_rows::<3>`, `rows(i,
...
> of FixedRows<Matrix4x5<T>, Matrix3x5<T>> {
    ...
    #[inline(always)]
    fn rows(self: Matrix4x5<T>, first_row: usize, nrows: usize) -> Matrix3x5<T> {
        if nrows != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_rows(self, first_row)
    ...
    #[inline(always)]
    fn rows_range(self: Matrix4x5<T>, rows: Range<usize>) -> Matrix3x5<T> {
        let Range { start, end } = rows;
        if end != start + 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 4 consecutive rows of a `Matrix4x5` as a `Matrix4x5`. Upstream: `fixed_rows::<4>`, `rows(i,
...
> of FixedRows<Matrix4x5<T>, Matrix4x5<T>> {
    ...
    #[inline(always)]
    fn rows(self: Matrix4x5<T>, first_row: usize, nrows: usize) -> Matrix4x5<T> {
        if nrows != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_rows(self, first_row)
    ...
    #[inline(always)]
    fn rows_range(self: Matrix4x5<T>, rows: Range<usize>) -> Matrix4x5<T> {
        let Range { start, end } = rows;
        if end != start + 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 1 consecutive columns of a `Matrix4x5` as a `Vector4`. Upstream: `fixed_columns::<1>`,
...
> of FixedColumns<Matrix4x5<T>, Vector4<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix4x5<T>, first_col: usize, ncols: usize) -> Vector4<T> {
        if ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
    ...
    #[inline(always)]
    fn columns_range(self: Matrix4x5<T>, cols: Range<usize>) -> Vector4<T> {
...
/// The 2 consecutive columns of a `Matrix4x5` as a `Matrix4x2`. Upstream: `fixed_columns::<2>`,
...
> of FixedColumns<Matrix4x5<T>, Matrix4x2<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix4x5<T>, first_col: usize, ncols: usize) -> Matrix4x2<T> {
        if ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix4x5<T>, cols: Range<usize>) -> Matrix4x2<T> {
        let Range { start, end } = cols;
        if end != start + 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 3 consecutive columns of a `Matrix4x5` as a `Matrix4x3`. Upstream: `fixed_columns::<3>`,
...
> of FixedColumns<Matrix4x5<T>, Matrix4x3<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix4x5<T>, first_col: usize, ncols: usize) -> Matrix4x3<T> {
        if ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix4x5<T>, cols: Range<usize>) -> Matrix4x3<T> {
        let Range { start, end } = cols;
        if end != start + 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 4 consecutive columns of a `Matrix4x5` as a `Matrix4`. Upstream: `fixed_columns::<4>`,
...
> of FixedColumns<Matrix4x5<T>, Matrix4<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix4x5<T>, first_col: usize, ncols: usize) -> Matrix4<T> {
        if ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix4x5<T>, cols: Range<usize>) -> Matrix4<T> {
        let Range { start, end } = cols;
        if end != start + 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 5 consecutive columns of a `Matrix4x5` as a `Matrix4x5`. Upstream: `fixed_columns::<5>`,
...
> of FixedColumns<Matrix4x5<T>, Matrix4x5<T>> {
    ...
    #[inline(always)]
    fn columns(self: Matrix4x5<T>, first_col: usize, ncols: usize) -> Matrix4x5<T> {
        if ncols != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        Self::fixed_columns(self, first_col)
    ...
    #[inline(always)]
    fn columns_range(self: Matrix4x5<T>, cols: Range<usize>) -> Matrix4x5<T> {
        let Range { start, end } = cols;
        if end != start + 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The 1x1 blocks of a `Matrix4x5` as a `Matrix1`. Upstream: `fixed_view::<1, 1>`, `view`, and the
...
pub impl Matrix4x5FixedViewMatrix1<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4x5<T>, Matrix1<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix1<T> {
        let (irow, icol) = start;
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4x5<T>, irow: usize, icol: usize) -> Matrix1<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix1<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x2 blocks of a `Matrix4x5` as a `RowVector2`. Upstream: `fixed_view::<1, 2>`, `view`, and
...
> of FixedView<Matrix4x5<T>, RowVector2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4x5<T>, irow: usize, icol: usize) -> RowVector2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector2<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x3 blocks of a `Matrix4x5` as a `RowVector3`. Upstream: `fixed_view::<1, 3>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix4x5<T>, RowVector3<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4x5<T>, irow: usize, icol: usize) -> RowVector3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector3<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x4 blocks of a `Matrix4x5` as a `RowVector4`. Upstream: `fixed_view::<1, 4>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix4x5<T>, RowVector4<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4x5<T>, irow: usize, icol: usize) -> RowVector4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector4<T> {
        Self::view(self, start, shape)
    }
}

/// The 1x5 blocks of a `Matrix4x5` as a `RowVector5`. Upstream: `fixed_view::<1, 5>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix4x5<T>, RowVector5<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector5<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 1 || ncols != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4x5<T>, irow: usize, icol: usize) -> RowVector5<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> RowVector5<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x1 blocks of a `Matrix4x5` as a `Vector2`. Upstream: `fixed_view::<2, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix4x5FixedViewVector2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4x5<T>, Vector2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Vector2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4x5<T>, irow: usize, icol: usize) -> Vector2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Vector2<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x2 blocks of a `Matrix4x5` as a `Matrix2`. Upstream: `fixed_view::<2, 2>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix4x5FixedViewMatrix2<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4x5<T>, Matrix2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4x5<T>, irow: usize, icol: usize) -> Matrix2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x3 blocks of a `Matrix4x5` as a `Matrix2x3`. Upstream: `fixed_view::<2, 3>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix4x5<T>, Matrix2x3<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4x5<T>, irow: usize, icol: usize) -> Matrix2x3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x3<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x4 blocks of a `Matrix4x5` as a `Matrix2x4`. Upstream: `fixed_view::<2, 4>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix4x5<T>, Matrix2x4<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4x5<T>, irow: usize, icol: usize) -> Matrix2x4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x4<T> {
        Self::view(self, start, shape)
    }
}

/// The 2x5 blocks of a `Matrix4x5` as a `Matrix2x5`. Upstream: `fixed_view::<2, 5>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix4x5<T>, Matrix2x5<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x5<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 2 || ncols != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4x5<T>, irow: usize, icol: usize) -> Matrix2x5<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix2x5<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x1 blocks of a `Matrix4x5` as a `Vector3`. Upstream: `fixed_view::<3, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix4x5FixedViewVector3<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4x5<T>, Vector3<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Vector3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4x5<T>, irow: usize, icol: usize) -> Vector3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Vector3<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x2 blocks of a `Matrix4x5` as a `Matrix3x2`. Upstream: `fixed_view::<3, 2>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix4x5<T>, Matrix3x2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4x5<T>, irow: usize, icol: usize) -> Matrix3x2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x2<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x3 blocks of a `Matrix4x5` as a `Matrix3`. Upstream: `fixed_view::<3, 3>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix4x5FixedViewMatrix3<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4x5<T>, Matrix3<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4x5<T>, irow: usize, icol: usize) -> Matrix3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x4 blocks of a `Matrix4x5` as a `Matrix3x4`. Upstream: `fixed_view::<3, 4>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix4x5<T>, Matrix3x4<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4x5<T>, irow: usize, icol: usize) -> Matrix3x4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x4<T> {
        Self::view(self, start, shape)
    }
}

/// The 3x5 blocks of a `Matrix4x5` as a `Matrix3x5`. Upstream: `fixed_view::<3, 5>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix4x5<T>, Matrix3x5<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x5<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 3 || ncols != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4x5<T>, irow: usize, icol: usize) -> Matrix3x5<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix3x5<T> {
        Self::view(self, start, shape)
    }
}

/// The 4x1 blocks of a `Matrix4x5` as a `Vector4`. Upstream: `fixed_view::<4, 1>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix4x5FixedViewVector4<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4x5<T>, Vector4<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Vector4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 4 || ncols != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4x5<T>, irow: usize, icol: usize) -> Vector4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Vector4<T> {
        Self::view(self, start, shape)
    }
}

/// The 4x2 blocks of a `Matrix4x5` as a `Matrix4x2`. Upstream: `fixed_view::<4, 2>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix4x5<T>, Matrix4x2<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4x2<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 4 || ncols != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4x5<T>, irow: usize, icol: usize) -> Matrix4x2<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4x2<T> {
        Self::view(self, start, shape)
    }
}

/// The 4x3 blocks of a `Matrix4x5` as a `Matrix4x3`. Upstream: `fixed_view::<4, 3>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix4x5<T>, Matrix4x3<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4x3<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 4 || ncols != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4x5<T>, irow: usize, icol: usize) -> Matrix4x3<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4x3<T> {
        Self::view(self, start, shape)
    }
}

/// The 4x4 blocks of a `Matrix4x5` as a `Matrix4`. Upstream: `fixed_view::<4, 4>`, `view`, and the
/// deprecated `fixed_slice` / `slice`.
pub impl Matrix4x5FixedViewMatrix4<T, +Copy<T>, +Drop<T>> of FixedView<Matrix4x5<T>, Matrix4<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 4 || ncols != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4x5<T>, irow: usize, icol: usize) -> Matrix4<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4<T> {
        Self::view(self, start, shape)
    }
}

/// The 4x5 blocks of a `Matrix4x5` as a `Matrix4x5`. Upstream: `fixed_view::<4, 5>`, `view`, and
/// the deprecated `fixed_slice` / `slice`.
...
> of FixedView<Matrix4x5<T>, Matrix4x5<T>> {
    ...
    #[inline(always)]
    fn view(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4x5<T> {
        let (irow, icol) = start;
        let (nrows, ncols) = shape;
        if nrows != 4 || ncols != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
    ...
    #[inline(always)]
    fn fixed_slice(self: Matrix4x5<T>, irow: usize, icol: usize) -> Matrix4x5<T> {
        Self::fixed_view(self, irow, icol)
    }

    #[inline(always)]
    fn slice(self: Matrix4x5<T>, start: (usize, usize), shape: (usize, usize)) -> Matrix4x5<T> {
        Self::view(self, start, shape)
    }
}

/// The first 1 components of a row of a `Matrix4x5` as a `Matrix1`. Upstream: `row_part` (`n = 1`).
pub impl Matrix4x5RowPartMatrix1<T, +Copy<T>, +Drop<T>> of RowPart<Matrix4x5<T>, Matrix1<T>> {
    #[inline(always)]
    fn row_part(self: Matrix4x5<T>, i: usize, n: usize) -> Matrix1<T> {
        if n != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The first 2 components of a row of a `Matrix4x5` as a `RowVector2`. Upstream: `row_part` (`n =
/// 2`).
pub impl Matrix4x5RowPartRowVector2<T, +Copy<T>, +Drop<T>> of RowPart<Matrix4x5<T>, RowVector2<T>> {
    #[inline(always)]
    fn row_part(self: Matrix4x5<T>, i: usize, n: usize) -> RowVector2<T> {
        if n != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 3 components of a row of a `Matrix4x5` as a `RowVector3`. Upstream: `row_part` (`n =
/// 3`).
pub impl Matrix4x5RowPartRowVector3<T, +Copy<T>, +Drop<T>> of RowPart<Matrix4x5<T>, RowVector3<T>> {
    #[inline(always)]
    fn row_part(self: Matrix4x5<T>, i: usize, n: usize) -> RowVector3<T> {
        if n != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 4 components of a row of a `Matrix4x5` as a `RowVector4`. Upstream: `row_part` (`n =
/// 4`).
pub impl Matrix4x5RowPartRowVector4<T, +Copy<T>, +Drop<T>> of RowPart<Matrix4x5<T>, RowVector4<T>> {
    #[inline(always)]
    fn row_part(self: Matrix4x5<T>, i: usize, n: usize) -> RowVector4<T> {
        if n != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 5 components of a row of a `Matrix4x5` as a `RowVector5`. Upstream: `row_part` (`n =
/// 5`).
pub impl Matrix4x5RowPartRowVector5<T, +Copy<T>, +Drop<T>> of RowPart<Matrix4x5<T>, RowVector5<T>> {
    #[inline(always)]
    fn row_part(self: Matrix4x5<T>, i: usize, n: usize) -> RowVector5<T> {
        if n != 5 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, i, 0)
...
/// The first 1 components of a column of a `Matrix4x5` as a `Matrix1`. Upstream: `column_part` (`n
/// = 1`).
pub impl Matrix4x5ColumnPartMatrix1<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix4x5<T>, Matrix1<T>> {
    #[inline(always)]
    fn column_part(self: Matrix4x5<T>, i: usize, n: usize) -> Matrix1<T> {
        if n != 1 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
...
/// The first 2 components of a column of a `Matrix4x5` as a `Vector2`. Upstream: `column_part` (`n
/// = 2`).
pub impl Matrix4x5ColumnPartVector2<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix4x5<T>, Vector2<T>> {
    #[inline(always)]
    fn column_part(self: Matrix4x5<T>, i: usize, n: usize) -> Vector2<T> {
        if n != 2 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, 0, i)
...
/// The first 3 components of a column of a `Matrix4x5` as a `Vector3`. Upstream: `column_part` (`n
/// = 3`).
pub impl Matrix4x5ColumnPartVector3<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix4x5<T>, Vector3<T>> {
    #[inline(always)]
    fn column_part(self: Matrix4x5<T>, i: usize, n: usize) -> Vector3<T> {
        if n != 3 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, 0, i)
...
/// The first 4 components of a column of a `Matrix4x5` as a `Vector4`. Upstream: `column_part` (`n
/// = 4`).
pub impl Matrix4x5ColumnPartVector4<T, +Copy<T>, +Drop<T>> of ColumnPart<Matrix4x5<T>, Vector4<T>> {
    #[inline(always)]
    fn column_part(self: Matrix4x5<T>, i: usize, n: usize) -> Vector4<T> {
        if n != 4 {
            core::panic_with_felt252(errors::DIMENSION_MISMATCH)
        }
        FixedView::fixed_view(self, 0, i)
