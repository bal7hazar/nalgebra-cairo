...
/// `iter.sum()` of an iterator of `Matrix3x2`s: the first item plus the others, in order (exact;
/// panics on overflow); the zero 3x2 matrix when empty. Upstream: `Sum for Matrix` (a fold
/// from `zero()`: the same result, one addition more).
pub impl Matrix3x2Sum<
    T, impl R: Real<T>, +Add<T>, +Copy<T>, +Drop<T>,
> of core::iter::Sum<Matrix3x2<T>> {
    fn sum<I, +Iterator<I>[Item: Matrix3x2<T>], +Destruct<I>, +Destruct<Matrix3x2<T>>>(
        mut iter: I,
    ) -> Matrix3x2<T> {
...
/// `*iter.sum()` of an iterator of snapshots `@Matrix3x2` (`span.into_iter()`): a snapshot of the
/// sum of the items, like `Sum<Matrix3x2>`. Upstream: `Sum<&Matrix> for Matrix` (references).
pub impl Matrix3x2SumSnapshot<
    T, impl R: Real<T>, +Add<T>, +Copy<T>, +Drop<T>,
> of core::iter::Sum<@Matrix3x2<T>> {
    fn sum<I, +Iterator<I>[Item: @Matrix3x2<T>], +Destruct<I>, +Destruct<@Matrix3x2<T>>>(
        mut iter: I,
