...
use crate::geometry::{
    ...
    Point6Trait, Projective2, Projective2Trait, Projective3, Projective3Trait, Transform2,
    Transform2Trait, Transform3, Transform3Trait,
};
...
/// `PointMetric` on `Point5`: the norms of `Vector5` of `p2 - p1` (upstream's formulation, measured
/// cheaper than a chain of this module: `bench_distance_point5`).
...
> of PointMetric<Point5<T>, T> {
    ...
    fn distance(p1: Point5<T>, p2: Point5<T>) -> T {
        Vector5Trait::norm(p2.coords() - p1.coords())
    }
    #[inline(always)]
    fn distance_squared(p1: Point5<T>, p2: Point5<T>) -> T {
        Vector5Trait::norm_squared(p2.coords() - p1.coords())
    }
}

/// `PointMetric` on `Point6`: the norms of `Vector6` of `p2 - p1` (upstream's formulation, measured
/// cheaper than a chain of this module: `bench_distance_point5`).
...
> of PointMetric<Point6<T>, T> {
    ...
    fn distance(p1: Point6<T>, p2: Point6<T>) -> T {
        Vector6Trait::norm(p2.coords() - p1.coords())
    }
    #[inline(always)]
    fn distance_squared(p1: Point6<T>, p2: Point6<T>) -> T {
        Vector6Trait::norm_squared(p2.coords() - p1.coords())
    }
}
