//! Transcendental functions of `Fixed` (DESIGN D6): trigonometry and its inverses.
//!
//! Free functions; the method syntax (`x.sin()`) comes from `simba::scalar::Transcendental`.
//! Angles are in radians. The implementation is the pair
//! `kernels::poly` (generated typed Horner kernels, `tools/polygen`) +
//! `kernels::transcendental` (hand-written signs, reflections and domain checks).
//!
//! # Numeric specification
//!
//! Every function is *deterministic* and *exact to the last bit forever*: the same raw input gives
//! the same raw output (AGENTS.md numeric rules). Internally the reduced angle carries 61
//! fractional bits and the polynomial accumulators 64, so the only rounding of a result is the
//! final one: **round to nearest, ties away from zero** for the magnitude, unlike the floor
//! rounding of the algebraic kernels (`mul`, `div`, `sum_prod*`), which is why the errors below
//! stay near half an ulp.
//!
...
//!
//! | function | polynomial | domain | max abs error (ulp) | exact at |
//! |---|---|---|---:|---|
//! | `sin`, `cos`, `sin_cos` | degree 11 / 10 on `[0, pi/4]` | `abs(x) <= 2^26` rad | **0.56** |
//! `sin(0) = 0`, `cos(0) = 1` |
//! | `sin`, `cos`, `sin_cos` | — | `abs(x) < 2^31` rad | **2.53** | — |
//! | `tan` | the two above | `abs(tan x) <= MAX` | 1 + `2 * tan^2` | `tan(0) = 0`, `tan(pi/4) = 1`
//! |
//! | `atan`, `atan2` | degree 25 on `[0, 1]` | all | **1.12** | see below |
//! | `asin` | degree 15 on `[0, 1/2]` | `[-1, 1]` | **0.96** | `asin(0) = 0`, `asin(±1) =
//! ±FRAC_PI_2` |
//! | `acos` | the same | `[-1, 1]` | **1.20** | `acos(0) = FRAC_PI_2`, `acos(1) = 0`, `acos(-1) =
//! PI` |
//!
//! The degrees were chosen on the measured accuracy / gas trade-off (`polygen.py --report` and
//! the `bench_*` groups below). Degree 11 / 10 costs **the same** as degree 9 / 8 (the extra
//! Horner step is absorbed by the lazy rescale: both programs use two `div_rem`s) while halving
//! the error, so it is the default; degree 7 / 6 saves 1,410 gas but is 466 ulp off. `atan`
//! stops at degree 25 (1.12 ulp): degree 29 costs 600 gas more for 0.09 ulp, degree 21 saves
//! 2,000 gas but is 6.4 ulp off. `asin` stops at degree 15 for the same reason.
//!
//! ## Symmetries (exact, not approximate)
//!
//! - `sin(-x) == -sin(x)`, `tan(-x) == -tan(x)`, `asin(-x) == -asin(x)`, `atan(-x) == -atan(x)`,
//!   `atan2(-y, x) == -atan2(y, x)` and `cos(-x) == cos(x)` hold **bit for bit**: the sign is
...
//!   whole domain: the two functions share one kernel value per branch, and the `Fixed` constant
//!   `PI` is exactly `2 * FRAC_PI_2`.
//! - `sin_cos(x) == (sin(x), cos(x))` bit for bit: the same reduction and the same polynomial
//!   programs, with `r^2` computed once.
...
//! (`kernels::poly::QUARTER_PI`), not with the Q32.32 `TAU`. The reduction is
//! `q * QUARTER_PI + r = x * 2^29 + OFF` exactly, so the only error is the quantisation of
//! `QUARTER_PI` itself (0.13 of a unit at scale 2^61): reducing the largest representable angle,
//! `abs(x) < 2^31` rad (2.7e9 octants), drifts by at most **2.9 raw Q32.32 units**, and the
//! measured worst case over the whole range is 2.53 ulp (against 0.53 for physics-scale
//! angles). A Q32.32 `TAU` would drift by
//! `abs(x) / 8` ulp instead, i.e. 2.7e8 ulp at the same argument — the tolerance the oracle
//! allows (`tools/oracle`, `4 + abs(x) / 8`) and that this implementation beats by eight orders
//! of magnitude. No argument is ever rejected for being large.
//!
//! ## Panics
//!
//! | function | condition | error |
//! |---|---|---|
//! | `asin`, `acos` | `abs(x) > 1` | `errors::DOMAIN` (`'simba: out of domain'`) |
//! | `tan` | `abs(tan x)` does not fit Q32.32 | `errors::OVERFLOW` |
//! | `tan` | the reduced cosine is exactly 0 (unreachable: `pi/2` is not representable) |
//! `errors::DIVISION_BY_ZERO` |
//!
//! `sin`, `cos`, `sin_cos`, `atan` and `atan2` are **total**: they cannot panic for any `Fixed`.
...
mod tests {
    ...
    use crate::fixed::types::{
        EPSILON, FRAC_1_SQRT_2, FRAC_PI_2, FRAC_PI_3, FRAC_PI_4, FRAC_PI_6, HALF, MAX, MIN, ONE, PI,
        TAU, ZERO,
    };
    use crate::fixed::{fused, oracle_scalar as oracle};
    use super::{acos, asin, atan, atan2, cos, sin, sin_cos, tan};
    ...
    fn bench_tan__q32_ratio() {
        let x = black_box(fx(BX));
        let e = black_box(fx(-0xbf3cda70));
        assert!(fx(alt::tan_from_q32(x.raw)) == fx(-0xbf3cda6f));
    }
