...
//! `Schur6`, `Matrix6SchurTrait`, `Eigen6` through the public API (WP 8.5-P16): oracle vectors
//! (`tools/oracle` suite `schur`), the factor identities, the iteration counts, gas benchmarks.

use core::cmp::max;
use fixed::Fixed;
use nalgebra::linalg::{Eigen6Trait, Matrix6SchurTrait, Schur6Trait};
use nalgebra::{Matrix6, MatrixMul, Vector6};
...
/// (equal) sorted and compared with upstream's; `Eigen6`: the same eigenvalues in the order of the
/// diagonal of `T`, unit eigenvectors with `|A v - λ v|` within the measured bound (per unit of
/// max |a_ij|).
#[test]
fn test_oracle_schur6_eigenvalues_real() {
    let mut cases = oracle::schur6_eigenvalues_real_cases();
    ...
    while let Some(case) = cases.pop_front() {
        ...
        let got = sorted(array![vals.x, vals.y, vals.z, vals.w, vals.a, vals.b]);
        let ev = {
            let (ev_0, ev_1, ev_2, ev_3, ev_4, ev_5) = ev;
            array![ev_0, ev_1, ev_2, ev_3, ev_4, ev_5]
        };
        let mut k = 0;
        while k < 6 {
            let e = fx(*ev[k]);
        ...
        let eig = Eigen6Trait::new(a).unwrap();
        assert!(eig.eigenvalues == svals, "Eigen order");
        let v = eig.eigenvectors;
        let av = a.mul_mat(v);
        res = max(res, ulp_diff(av.m11, v.m11 * svals.x) / amax_6x6(a));
        res = max(res, ulp_diff(av.m12, v.m12 * svals.y) / amax_6x6(a));
        res = max(res, ulp_diff(av.m13, v.m13 * svals.z) / amax_6x6(a));
        res = max(res, ulp_diff(av.m14, v.m14 * svals.w) / amax_6x6(a));
        res = max(res, ulp_diff(av.m15, v.m15 * svals.a) / amax_6x6(a));
        res = max(res, ulp_diff(av.m16, v.m16 * svals.b) / amax_6x6(a));
        res = max(res, ulp_diff(av.m21, v.m21 * svals.x) / amax_6x6(a));
        res = max(res, ulp_diff(av.m22, v.m22 * svals.y) / amax_6x6(a));
        res = max(res, ulp_diff(av.m23, v.m23 * svals.z) / amax_6x6(a));
        res = max(res, ulp_diff(av.m24, v.m24 * svals.w) / amax_6x6(a));
        res = max(res, ulp_diff(av.m25, v.m25 * svals.a) / amax_6x6(a));
        res = max(res, ulp_diff(av.m26, v.m26 * svals.b) / amax_6x6(a));
        res = max(res, ulp_diff(av.m31, v.m31 * svals.x) / amax_6x6(a));
        res = max(res, ulp_diff(av.m32, v.m32 * svals.y) / amax_6x6(a));
        res = max(res, ulp_diff(av.m33, v.m33 * svals.z) / amax_6x6(a));
        res = max(res, ulp_diff(av.m34, v.m34 * svals.w) / amax_6x6(a));
        res = max(res, ulp_diff(av.m35, v.m35 * svals.a) / amax_6x6(a));
        res = max(res, ulp_diff(av.m36, v.m36 * svals.b) / amax_6x6(a));
        res = max(res, ulp_diff(av.m41, v.m41 * svals.x) / amax_6x6(a));
        res = max(res, ulp_diff(av.m42, v.m42 * svals.y) / amax_6x6(a));
        res = max(res, ulp_diff(av.m43, v.m43 * svals.z) / amax_6x6(a));
        res = max(res, ulp_diff(av.m44, v.m44 * svals.w) / amax_6x6(a));
        res = max(res, ulp_diff(av.m45, v.m45 * svals.a) / amax_6x6(a));
        res = max(res, ulp_diff(av.m46, v.m46 * svals.b) / amax_6x6(a));
        res = max(res, ulp_diff(av.m51, v.m51 * svals.x) / amax_6x6(a));
        res = max(res, ulp_diff(av.m52, v.m52 * svals.y) / amax_6x6(a));
        res = max(res, ulp_diff(av.m53, v.m53 * svals.z) / amax_6x6(a));
        res = max(res, ulp_diff(av.m54, v.m54 * svals.w) / amax_6x6(a));
        res = max(res, ulp_diff(av.m55, v.m55 * svals.a) / amax_6x6(a));
        res = max(res, ulp_diff(av.m56, v.m56 * svals.b) / amax_6x6(a));
        res = max(res, ulp_diff(av.m61, v.m61 * svals.x) / amax_6x6(a));
        res = max(res, ulp_diff(av.m62, v.m62 * svals.y) / amax_6x6(a));
        res = max(res, ulp_diff(av.m63, v.m63 * svals.z) / amax_6x6(a));
        res = max(res, ulp_diff(av.m64, v.m64 * svals.w) / amax_6x6(a));
        res = max(res, ulp_diff(av.m65, v.m65 * svals.a) / amax_6x6(a));
        res = max(res, ulp_diff(av.m66, v.m66 * svals.b) / amax_6x6(a));
        unit =
            max(
                unit,
                ulp_diff(
...
fn test_schur6_iterations() {
    ...
    println!("schur6: {} cases, {} iterations in total, at most {}", count, total, worst);
}

/// The number of passes of the Schur loop on `a` (exponential then binary search on `max_niter`).
fn iterations6(a: Matrix6<Fixed>) -> usize {
    let mut hi: usize = 1;
...
fn test_schur6_trivial() {
    ...
    assert!(
        i
            .eigenvalues()
            .unwrap() == Vector6 {
                x: fx(ONE), y: fx(ONE), z: fx(ONE), w: fx(ONE), a: fx(ONE), b: fx(ONE),
            },
    );
    let z = black_box(
        Matrix6 {
    ...
    assert!(
        re == Vector6 { x: fx(0), y: fx(0), z: fx(0), w: fx(0), a: fx(0), b: fx(0) } && im == re,
    );
    let e = Eigen6Trait::new(i).unwrap();
    assert!(
        e.eigenvectors == i
            && e
                .eigenvalues == Vector6 {
...
fn test_schur6_rotation_block() {
    ...
    assert!(Eigen6Trait::new(a).is_none(), "Eigen of complex eigenvalues");
}
...
#[test]
#[inline(never)]
fn bench_schur6_new__baseline() {
    let (a, _, _, _) = *oracle::schur6_eigenvalues_cases().at(3);
    let a = black_box(mat6x6(a));
...
#[test]
#[inline(never)]
fn bench_schur6_complex_eigenvalues__baseline() {
    let (a, _, _, _) = *oracle::schur6_eigenvalues_cases().at(3);
    let a = black_box(mat6x6(a));
    let e = black_box(true);
    let _ = a;
...
#[test]
#[inline(never)]
fn bench_schur6_complex_eigenvalues__without_q() {
    let (a, _, _, _) = *oracle::schur6_eigenvalues_cases().at(3);
    let a = black_box(mat6x6(a));
    let e = black_box(true);
...
#[test]
#[inline(never)]
fn bench_schur6_eigenvalues__baseline() {
    let (a, _, _) = *oracle::schur6_eigenvalues_real_cases().at(3);
    let a = black_box(mat6x6(a));
    let e = black_box(true);
    let _ = a;
    assert!(e == e);
...
#[test]
#[inline(never)]
fn bench_schur6_eigenvalues__without_q() {
    let (a, _, _) = *oracle::schur6_eigenvalues_real_cases().at(3);
    let a = black_box(mat6x6(a));
    let e = black_box(true);
    let v = a.eigenvalues();
...
#[test]
#[inline(never)]
fn bench_eigen6_new__schur() {
    let (a, _, _) = *oracle::schur6_eigenvalues_real_cases().at(3);
    let a = black_box(mat6x6(a));
    let e = black_box(true);
    let v = Eigen6Trait::new(a);
    assert!((v.is_some()) == e);
}
