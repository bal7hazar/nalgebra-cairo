...
//!
//! Moved from `crates/nalgebra/src/geometry/translation3/tests.cairo` (WP 8.1c, test-only package).
...
use nalgebra_tests_utils::{ONE_RAW, fx, p3t, t3t, v3t};
use simba::scalar::Real;
use crate::translation3::oracle;
...
fn test_inverse_of_min_component_panics() {
    let v = Vector3 { x: Real::<Fixed>::min_value().unwrap(), y: Real::zero(), z: Real::zero() };
...
fn test_mul_overflow_panics() {
    let max = Vector3 { x: Real::<Fixed>::max_value().unwrap(), y: Real::zero(), z: Real::zero() };
...
fn test_to_homogeneous_oracle_consistency() {
    ...
    while let Some(case) = cases.pop_front() {
        ...
        let h = t3t(t).to_homogeneous().mul_mat(p3t(p).to_homogeneous());
