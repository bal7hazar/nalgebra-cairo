...
//!
//! Moved from `crates/nalgebra/src/geometry/unit_complex/tests.cairo` (WP 8.1c, test-only package):
...
use nalgebra_tests_utils::{ONE_RAW, fx, m2, max_ulp_diff_uc, p2t, r2, uct, ulp_diff, v2t};
use simba::scalar::Real;
use crate::unit_complex::oracle;
...
fn test_new_cardinal_angles() {
    ...
    assert!(
        max_ulp_diff_uc(UnitComplexAngleTrait::<Fixed>::new(Real::frac_pi_2()), quarter()) <= 1,
    );
    assert!(
...
fn test_inverse_of_min_imaginary_part_panics() {
    let _ = black_box(UnitComplex { re: Real::<Fixed>::zero(), im: Real::min_value().unwrap() })
        .inverse();
}
