...
//!
//! Moved from `crates/nalgebra/src/geometry/similarity3/tests.cairo` (WP 8.1c, test-only package).
...
use nalgebra_tests_utils::{ONE_RAW, fx, p3t, sim3t, uqt, v3t};
use simba::scalar::Real;
use crate::similarity3::oracle;
...
fn test_append_and_prepend_translation_rotation_match_composition() {
    ...
    assert!(x.mul_translation(t).abs_diff_eq(x * ti, 4));
    assert!({
        let mut m = x;
        m.append_rotation_mut(r);
        m
    } == ri * x);
    assert!(x.mul_unit_quaternion(r) == x * ri);
}
