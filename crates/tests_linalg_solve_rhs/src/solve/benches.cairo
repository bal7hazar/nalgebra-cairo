...
//! `baseline`): the square right-hand sides of the 2, 3, 4 and 6 squares, with the per-column
//! alternative (`alt_columns`).

use fixed::Fixed;
use nalgebra::{
    Matrix2, Matrix2Trait, Matrix3, Matrix3Trait, Matrix4, Matrix4Trait, Matrix6, Matrix6Trait,
    MatrixSolve, Vector3Trait, Vector4Trait, Vector6Trait,
};
...
/// The square right-hand side as 3 single-column solves (the vector kernels, inlined; the
/// `columns` form of `gemm`): bit-identical (`test_solve_alt_columns_is_bit_identical`). Against
/// the direct kernel, out of line with each row's 3 quotients through one prepared divisor
/// (`Real::div3`), the measured trade-off of `tools/shapegen/solve.py` (`small`). Kept as
/// evidence (AGENTS.md rule 8).
#[test]
#[inline(never)]
...
/// The square right-hand side as 4 single-column solves (the vector kernels, inlined; the
/// `columns` form of `gemm`): bit-identical (`test_solve_alt_columns_is_bit_identical`). Against
/// the direct kernel, out of line with each row's 4 quotients through one prepared divisor
/// (`Real::div4`), the measured trade-off of `tools/shapegen/solve.py` (`small`). Kept as
/// evidence (AGENTS.md rule 8).
#[test]
#[inline(never)]
...
/// The square right-hand side as 6 single-column solves (the vector kernels, inlined; the
/// `columns` form of `gemm`): bit-identical (`test_solve_alt_columns_is_bit_identical`). Against
/// the direct kernel, out of line with each row's 6 quotients through one prepared divisor
/// (`Real::div6`), the measured trade-off of `tools/shapegen/solve.py` (`small`). Kept as
/// evidence (AGENTS.md rule 8).
#[test]
#[inline(never)]
...
/// The direct 3x3 kernel (one prepared divisor per row) and the per-column solves agree bit for
/// bit on a right-hand side whose quotients are NOT exact.
#[test]
fn test_solve_alt_columns_is_bit_identical() {
    let l = black_box(
        Matrix3Trait::new(
            fx(12884901888),
            fx(0),
    ...
    assert!(x == l.solve_lower_triangular_unchecked(b));
}
