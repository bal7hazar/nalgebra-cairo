// Generated by crates/nalgebra/src/linalg/generate.py: do not edit by hand.
//! `SymmetricTridiagonal3` / `Matrix3SymmetricTridiagonalTrait` through the public API (WP
//! 8.5-P16): oracle vectors (`tools/oracle` suite `schur`), the factor identities, gas benchmarks.

use core::cmp::max;
use nalgebra::linalg::{Matrix3SymmetricTridiagonalTrait, SymmetricTridiagonal3Trait};
use nalgebra_testing::black_box;
use nalgebra_tests_utils::{abs_raw, excess, oracle_tol, ulp_diff};
use crate::builders::{amax_3x3, mat3x3, max_ulp_3x3, orth_3x3, vec2, vec3};
use crate::oracle_schur as oracle;

/// `symmetric_tridiagonal3` (oracle): `q`, the diagonal and the off-diagonal within the oracle
/// tolerance (upstream's signs), `recompose()` = `A` and `QᵀQ = I` within the measured bounds,
/// the accessors consistent.
#[test]
fn test_oracle_symmetric_tridiagonal3() {
    let mut cases = oracle::symmetric_tridiagonal3_cases();
    let (mut ex, mut rec, mut orth) = (0, 0, 0);
    while let Some(case) = cases.pop_front() {
        let (a, eq, ed, ee, tol) = *case;
        let a = black_box(mat3x3(a));
        let t = a.symmetric_tridiagonalize();
        let (q, d, e) = t.unpack();
        assert!(q == t.q() && d == t.diagonal() && t.internal_tri() == t.tri);
        assert!(t.unpack_tridiagonal() == (d, e) && t.off_diagonal() == e);
        let (eq, ed) = (mat3x3(eq), vec3(ed));
        ex = max(ex, excess(ulp_diff(q.m11, eq.m11), oracle_tol(abs_raw(eq.m11), tol)));
        ex = max(ex, excess(ulp_diff(q.m12, eq.m12), oracle_tol(abs_raw(eq.m12), tol)));
        ex = max(ex, excess(ulp_diff(q.m13, eq.m13), oracle_tol(abs_raw(eq.m13), tol)));
        ex = max(ex, excess(ulp_diff(q.m21, eq.m21), oracle_tol(abs_raw(eq.m21), tol)));
        ex = max(ex, excess(ulp_diff(q.m22, eq.m22), oracle_tol(abs_raw(eq.m22), tol)));
        ex = max(ex, excess(ulp_diff(q.m23, eq.m23), oracle_tol(abs_raw(eq.m23), tol)));
        ex = max(ex, excess(ulp_diff(q.m31, eq.m31), oracle_tol(abs_raw(eq.m31), tol)));
        ex = max(ex, excess(ulp_diff(q.m32, eq.m32), oracle_tol(abs_raw(eq.m32), tol)));
        ex = max(ex, excess(ulp_diff(q.m33, eq.m33), oracle_tol(abs_raw(eq.m33), tol)));
        ex = max(ex, excess(ulp_diff(d.x, ed.x), oracle_tol(abs_raw(ed.x), tol)));
        ex = max(ex, excess(ulp_diff(d.y, ed.y), oracle_tol(abs_raw(ed.y), tol)));
        ex = max(ex, excess(ulp_diff(d.z, ed.z), oracle_tol(abs_raw(ed.z), tol)));
        let ee = vec2(ee);
        ex = max(ex, excess(ulp_diff(e.x, ee.x), oracle_tol(abs_raw(ee.x), tol)));
        ex = max(ex, excess(ulp_diff(e.y, ee.y), oracle_tol(abs_raw(ee.y), tol)));
        rec = max(rec, max_ulp_3x3(t.recompose(), a) / amax_3x3(a));
        orth = max(orth, orth_3x3(q));
    }
    assert!(ex == 0, "oracle tolerance exceeded by {}", ex);
    assert!(rec <= 24 && orth <= 7, "measured {} {}", rec, orth);
}

#[test]
#[inline(never)]
fn bench_symmetric_tridiagonal3_new__baseline() {
    let (a, _, _, _, _) = *oracle::symmetric_tridiagonal3_cases().at(3);
    let a = black_box(mat3x3(a));
    let e = black_box(true);
    let _ = a;
    assert!(e == e);
}

/// Householder reflections on the lower triangle (`dsytd2` form), unrolled.
#[test]
#[inline(never)]
fn bench_symmetric_tridiagonal3_new__householder() {
    let (a, _, _, _, _) = *oracle::symmetric_tridiagonal3_cases().at(3);
    let a = black_box(mat3x3(a));
    let e = black_box(true);
    let t = a.symmetric_tridiagonalize();
    assert!((t.tri.m11 == t.tri.m11) == e);
}

#[test]
#[inline(never)]
fn bench_symmetric_tridiagonal3_q__baseline() {
    let (a, _, _, _, _) = *oracle::symmetric_tridiagonal3_cases().at(3);
    let a = black_box(black_box(mat3x3(a)).symmetric_tridiagonalize());
    let e = black_box(true);
    let _ = a;
    assert!(e == e);
}

/// `assemble_q` on symbolic identity entries.
#[test]
#[inline(never)]
fn bench_symmetric_tridiagonal3_q__assemble() {
    let (a, _, _, _, _) = *oracle::symmetric_tridiagonal3_cases().at(3);
    let a = black_box(black_box(mat3x3(a)).symmetric_tridiagonalize());
    let e = black_box(true);
    let q = a.q();
    assert!((q.m11 == q.m11) == e);
}

#[test]
#[inline(never)]
fn bench_symmetric_tridiagonal3_recompose__baseline() {
    let (a, _, _, _, _) = *oracle::symmetric_tridiagonal3_cases().at(3);
    let a = black_box(black_box(mat3x3(a)).symmetric_tridiagonalize());
    let e = black_box(true);
    let _ = a;
    assert!(e == e);
}

/// `Q T Qᵀ`: `Q T` (tridiagonal) then `(Q T) Qᵀ`, fused sums.
#[test]
#[inline(never)]
fn bench_symmetric_tridiagonal3_recompose__two_products() {
    let (a, _, _, _, _) = *oracle::symmetric_tridiagonal3_cases().at(3);
    let a = black_box(black_box(mat3x3(a)).symmetric_tridiagonalize());
    let e = black_box(true);
    let m = a.recompose();
    assert!((m.m11 == m.m11) == e);
}
