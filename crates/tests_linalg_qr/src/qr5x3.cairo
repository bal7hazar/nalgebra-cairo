...
//! `Qr5x3` / `Matrix5x3QrTrait` through the public API (WP 8.5-P14b): oracle vectors
//! (`tools/oracle` suite `spectral`), the factor identities, gas benchmarks.

use core::cmp::max;
use nalgebra::linalg::{Matrix5x3QrTrait, Qr5x3Trait};
use nalgebra::{Matrix5x3, MatrixMul, Vector5};
use nalgebra_testing::black_box;
use nalgebra_tests_utils::{abs_raw, excess, fx, oracle_tol, ulp_diff};
use crate::builders::{amax_5x3, mat3x3, mat5x3, max_ulp_5x1, max_ulp_5x3, orth_5x3, vec5};
use crate::oracle_qr as oracle;

const ONE: i64 = 0x100000000;

/// `qr5x3_q_r` (oracle): `q` and `r` entry by entry within the oracle tolerance (upstream's
/// unpacked factors: `diag(r) >= 0`), `q r = A` and the orthonormality of `q` within the
/// measured bounds.
#[test]
fn test_oracle_qr5x3_q_r() {
    let mut cases = oracle::qr5x3_q_r_cases();
    ...
    while let Some(case) = cases.pop_front() {
        ...
        rec = max(rec, max_ulp_5x3(q.mul_mat(r), a) / amax_5x3(a));
        orth = max(orth, orth_5x3(q));
    }
    assert!(ex == 0, "oracle tolerance exceeded by {}", ex);
    assert!(rec <= 2 && orth <= 8, "measured {} {}", rec, orth);
}

#[test]
#[inline(never)]
fn bench_qr5x3_new__baseline() {
    let (a, _, _, _) = *oracle::qr5x3_q_r_cases().at(3);
    let _a = black_box(mat5x3(a));
...
#[test]
#[inline(never)]
fn bench_qr5x3_new__mgs() {
    let (a, _, _, _) = *oracle::qr5x3_q_r_cases().at(3);
    let a = black_box(mat5x3(a));
    let e = black_box(true);
    let f = Qr5x3Trait::new(a);
