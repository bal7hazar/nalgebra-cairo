...
//! `Qr6x5` / `Matrix6x5QrTrait` through the public API (WP 8.5-P14b): oracle vectors
//! (`tools/oracle` suite `spectral`), the factor identities, gas benchmarks.

use core::cmp::max;
use nalgebra::linalg::{Matrix6x5QrTrait, Qr6x5Trait};
use nalgebra::{Matrix6x5, MatrixMul, Vector6};
use nalgebra_testing::black_box;
use nalgebra_tests_utils::{abs_raw, excess, fx, oracle_tol, ulp_diff};
use crate::builders::{amax_6x5, mat5x5, mat6x5, max_ulp_6x1, max_ulp_6x5, orth_6x5, vec6};
use crate::oracle_qr as oracle;

const ONE: i64 = 0x100000000;

/// `qr6x5_q_r` (oracle): `q` and `r` entry by entry within the oracle tolerance (upstream's
/// unpacked factors: `diag(r) >= 0`), `q r = A` and the orthonormality of `q` within the
/// measured bounds.
#[test]
fn test_oracle_qr6x5_q_r() {
    ...
    while let Some(case) = cases.pop_front() {
        ...
        rec = max(rec, max_ulp_6x5(q.mul_mat(r), a) / amax_6x5(a));
...
#[test]
#[inline(never)]
fn bench_qr6x5_new__baseline() {
    let (a, _, _, _) = *oracle::qr6x5_q_r_cases().at(3);
    let _a = black_box(mat6x5(a));
...
#[test]
#[inline(never)]
fn bench_qr6x5_new__mgs() {
    let (a, _, _, _) = *oracle::qr6x5_q_r_cases().at(3);
    let a = black_box(mat6x5(a));
    let e = black_box(true);
    let f = Qr6x5Trait::new(a);
