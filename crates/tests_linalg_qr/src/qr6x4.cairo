...
//! `Qr6x4` / `Matrix6x4QrTrait` through the public API (WP 8.5-P14b): oracle vectors
//! (`tools/oracle` suite `spectral`), the factor identities, gas benchmarks.

use core::cmp::max;
use nalgebra::linalg::{Matrix6x4QrTrait, Qr6x4Trait};
use nalgebra::{Matrix6x4, MatrixMul, Vector6};
use nalgebra_testing::black_box;
use nalgebra_tests_utils::{abs_raw, excess, fx, oracle_tol, ulp_diff};
use crate::builders::{amax_6x4, mat4x4, mat6x4, max_ulp_6x1, max_ulp_6x4, orth_6x4, vec6};
use crate::oracle_qr as oracle;

const ONE: i64 = 0x100000000;

/// `qr6x4_q_r` (oracle): `q` and `r` entry by entry within the oracle tolerance (upstream's
/// unpacked factors: `diag(r) >= 0`), `q r = A` and the orthonormality of `q` within the
/// measured bounds.
#[test]
fn test_oracle_qr6x4_q_r() {
    let mut cases = oracle::qr6x4_q_r_cases();
    ...
    while let Some(case) = cases.pop_front() {
        ...
        rec = max(rec, max_ulp_6x4(q.mul_mat(r), a) / amax_6x4(a));
        orth = max(orth, orth_6x4(q));
    }
    assert!(ex == 0, "oracle tolerance exceeded by {}", ex);
    assert!(rec <= 3 && orth <= 51, "measured {} {}", rec, orth);
}

#[test]
#[inline(never)]
fn bench_qr6x4_new__baseline() {
    let (a, _, _, _) = *oracle::qr6x4_q_r_cases().at(3);
    let _a = black_box(mat6x4(a));
...
#[test]
#[inline(never)]
fn bench_qr6x4_new__mgs() {
    let (a, _, _, _) = *oracle::qr6x4_q_r_cases().at(3);
    let a = black_box(mat6x4(a));
    let e = black_box(true);
    let f = Qr6x4Trait::new(a);
