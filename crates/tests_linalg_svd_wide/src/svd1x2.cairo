...
//! `Svd1x2` / `RowVector2SvdTrait` through the public API (WP 8.5-P14b): oracle vectors
//! (`tools/oracle` suite `spectral`), exact cases, the `try` / unordered forms, gas benchmarks.

use core::cmp::max;
use fixed::Fixed;
use nalgebra::linalg::{RowVector2SvdTrait, Svd1x2Trait};
use nalgebra::{Matrix1, RowVector2};
use nalgebra_testing::black_box;
use nalgebra_tests_utils::{abs_raw, excess, fx, oracle_tol, ulp_diff};
use crate::builders::{
    Svd1x2PartialEq, amax_1x2, mat1x1, mat1x2, mat2x1, max_ulp_1x2, orth_1x1, orth_1x2, vec1,
};
...
fn run(cases: Span<([[i64; 2]; 1], (i64,), u64)>) -> (u128, u128, u128) {
    let mut cases = cases;
    let (mut ex, mut rec, mut orth) = (0, 0, 0);
    while let Some(case) = cases.pop_front() {
        let (a, expected, tol) = *case;
        let (e, r, o) = measure(black_box(mat1x2(a)), vec1(expected), tol);
        ex = max(ex, e);
        rec = max(rec, r);
        orth = max(orth, o);
    }
...
/// `svd1x2_singular_values` (oracle, well-conditioned): singular values within the oracle
/// tolerance, `U Σ Vᵀ = M` and the orthonormality of `U` / `V` within the measured bounds.
#[test]
fn test_oracle_svd1x2_singular_values() {
    let (ex, rec, orth) = run(oracle::svd1x2_singular_values_cases());
    assert!(ex == 0, "oracle tolerance exceeded by {}", ex);
    assert!(rec <= 2 && orth <= 3, "measured {} {}", rec, orth);
}
/// `svd1x2_pseudo_inverse` (oracle): every entry within the oracle tolerance.
#[test]
fn test_oracle_svd1x2_pseudo_inverse() {
    let mut cases = oracle::svd1x2_pseudo_inverse_cases();
    let mut ex = 0;
    while let Some(case) = cases.pop_front() {
        let (a, e, tol) = *case;
        let p = black_box(mat1x2(a)).pseudo_inverse(fx(EPS)).unwrap();
        let e = mat2x1(e);
...
/// `svd1x2_polar` (oracle): `P` and `U` of the left polar decomposition, every entry within
/// the oracle tolerance (both factors are unique for a full-rank `M`).
#[test]
fn test_oracle_svd1x2_polar() {
    let mut cases = oracle::svd1x2_polar_cases();
    let mut ex = 0;
    while let Some(case) = cases.pop_front() {
        let (a, ep, eu, tol) = *case;
        let (p, u) = black_box(mat1x2(a)).polar();
        let (ep, eu) = (mat1x1(ep), mat1x2(eu));
...
#[test]
fn test_svd1x2_diagonal_and_zero_are_exact() {
...
fn a_bench() -> RowVector2<Fixed> {
    let (a, _, _) = *oracle::svd1x2_singular_values_cases().at(3);
    mat1x2(a)
}

#[test]
#[inline(never)]
fn bench_svd1x2_new__baseline() {
...
#[test]
#[inline(never)]
fn bench_svd1x2_singular_values__baseline() {
    let _a = black_box(a_bench());
    let e = black_box(true);
    assert!(e == e);
}
...
#[test]
#[inline(never)]
fn bench_svd1x2_singular_values__eigen_of_gram() {
    let a = black_box(a_bench());
    let e = black_box(true);
