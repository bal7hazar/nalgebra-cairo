...
//! `Bidiagonal2x3` / `Matrix2x3BidiagonalTrait` through the public API (WP 8.5-P16): oracle vectors
...
use fixed::Fixed;
use nalgebra::linalg::{
    Bidiagonal2x3Trait, Matrix2x3BidiagonalTrait, clear_column_unchecked, clear_row_unchecked,
};
use nalgebra::{Matrix1, Matrix2x3, MatrixMul, Vector2, Vector3};
use nalgebra_testing::black_box;
use nalgebra_tests_utils::{abs_raw, excess, fx, oracle_tol, ulp_diff};
use crate::builders::{amax_2x3, mat2x2, mat2x3, max_ulp_2x3, orth_2x2, orth_2x3};
use crate::oracle_schur as oracle;

/// `bidiagonal2x3` (oracle): `u`, `d` and `v_t` entry by entry within the oracle tolerance
...
#[test]
fn test_oracle_bidiagonal2x3() {
    let mut cases = oracle::bidiagonal2x3_cases();
    ...
    while let Some(case) = cases.pop_front() {
        ...
        let (eu, ed, evt) = (mat2x2(eu), mat2x2(ed), mat2x3(evt));
        ...
        rec = max(rec, max_ulp_2x3(u.mul_mat(d).mul_mat(vt), a) / amax_2x3(a));
        orth = max(orth, max(orth_2x2(u), orth_2x3(vt)));
        let mut m = a;
...
#[test]
#[inline(never)]
fn bench_bidiagonal2x3_new__baseline() {
    let (a, _, _, _, _) = *oracle::bidiagonal2x3_cases().at(3);
    let a = black_box(mat2x3(a));
...
#[test]
#[inline(never)]
fn bench_bidiagonal2x3_new__householder() {
    let (a, _, _, _, _) = *oracle::bidiagonal2x3_cases().at(3);
    let a = black_box(mat2x3(a));
    let e = black_box(true);
    let bd = a.bidiagonalize();
...
#[test]
#[inline(never)]
fn bench_bidiagonal2x3_unpack__baseline() {
    let (a, _, _, _, _) = *oracle::bidiagonal2x3_cases().at(3);
    let a = black_box(black_box(mat2x3(a)).bidiagonalize());
    let e = black_box(true);
    let _ = a;
    assert!(e == e);
}
...
#[test]
#[inline(never)]
fn bench_bidiagonal2x3_unpack__symbolic() {
    let (a, _, _, _, _) = *oracle::bidiagonal2x3_cases().at(3);
    let a = black_box(black_box(mat2x3(a)).bidiagonalize());
    let e = black_box(true);
    let (u, _, vt) = a.unpack();
