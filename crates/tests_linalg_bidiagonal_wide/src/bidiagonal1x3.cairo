...
//! `Bidiagonal1x3` / `RowVector3BidiagonalTrait` through the public API (WP 8.5-P16): oracle
...
#[test]
fn test_oracle_bidiagonal1x3() {
    let mut cases = oracle::bidiagonal1x3_cases();
...
#[test]
#[inline(never)]
fn bench_bidiagonal1x3_new__baseline() {
    let (a, _, _, _, _) = *oracle::bidiagonal1x3_cases().at(3);
    let a = black_box(mat1x3(a));
...
#[test]
#[inline(never)]
fn bench_bidiagonal1x3_new__householder() {
    let (a, _, _, _, _) = *oracle::bidiagonal1x3_cases().at(3);
    let a = black_box(mat1x3(a));
    let e = black_box(true);
    let bd = a.bidiagonalize();
...
#[test]
#[inline(never)]
fn bench_bidiagonal1x3_unpack__baseline() {
    let (a, _, _, _, _) = *oracle::bidiagonal1x3_cases().at(3);
    let a = black_box(black_box(mat1x3(a)).bidiagonalize());
    let e = black_box(true);
    let _ = a;
    assert!(e == e);
}
...
#[test]
#[inline(never)]
fn bench_bidiagonal1x3_unpack__symbolic() {
    let (a, _, _, _, _) = *oracle::bidiagonal1x3_cases().at(3);
    let a = black_box(black_box(mat1x3(a)).bidiagonalize());
    let e = black_box(true);
    let (u, _, vt) = a.unpack();
