//!
//! Moved from `crates/nalgebra/src/base/matrix3/tests.cairo` (WP 8.1c, test-only package): the
//! tests of crate-internal items stay there.

use fixed::Fixed;
use nalgebra::base::matrix3::{Matrix3, Matrix3Trait};
use nalgebra::base::{MatrixMul, MatrixTrMul};
use nalgebra_testing::black_box;
use nalgebra_tests_utils::{fx, int, m3, m3i, max_ulp_diff3, ulp_diff, v3i, v3t};
use simba::scalar::Real;
use crate::{oracle_matrix3, oracle_matrix3_inverse};
...
fn test_from_diagonal() {
    assert!(Matrix3Trait::from_diagonal(v3i(2, -3, 4)) == m3i([[2, 0, 0], [0, -3, 0], [0, 0, 4]]));
    assert!(Matrix3Trait::from_diagonal_element(int(7)) == m3i([[7, 0, 0], [0, 7, 0], [0, 0, 7]]));
}
...
fn test_neg_min_panics() {
    let _ = -black_box(Matrix3Trait::from_diagonal_element(Real::<Fixed>::min_value().unwrap()));
}
...
fn test_determinant_oracle() {
    ...
    while let Some(case) = cases.pop_front() {
        ...
        worst_alt =
            core::cmp::max(worst_alt, ulp_diff(determinant_triple_products(m3(a)), fx(expected)));
    }
...
fn test_try_inverse_exact() {
    ...
    assert!(Matrix3Trait::<Fixed>::identity().try_inverse().unwrap() == Matrix3Trait::identity());
}
...
fn bench_matrix3_new__struct() {
    ...
    assert!(Matrix3Trait::new(a.m11, a.m12, a.m13, a.m21, a.m22, a.m23, a.m31, a.m32, a.m33) == e);
}
...
fn bench_matrix3_add__baseline() {
    ...
    let e = black_box(
        m3(
            [
                [-14736756803, -11784916900, -10803461533], [-1695724392, 11493453460, 12809574443],
                [-2339912399, -3710838010, 7822493936],
            ],
        ),
    );
    assert!(e == e);
...
fn bench_matrix3_scale__baseline() {
    ...
    let e = black_box(
        m3(
            [
                [14583481608, 6234065043, 12498009601], [10841618991, -14065125479, -9461464051],
...
fn bench_matrix3_try_inverse__prescaled_small() {
    ...
    let e = black_box(
        m3(
            [
                [16495394595, 3675684940, -11699880121], [-13341091189, 955040073, -17220417569],
                [93636196, -10763579368, -164828975],
            ],
        ),
    );
    assert!(a.try_inverse().unwrap() == e);
