//!
//! Moved from `crates/nalgebra/src/base/matrix2/tests.cairo` (WP 8.1c, test-only package): the
...
use nalgebra_tests_utils::{fx, int, m2, m2i, max_ulp_diff2, v2i, v2t};
use simba::scalar::Real;
use crate::{oracle_matrix2, oracle_matrix2_inverse};
...
fn test_mul_vec_oracle() {
    ...
    while let Some(case) = cases.pop_front() {
        ...
        assert!(m2(a).mul_mat(v2t(v)) == v2t(expected));
        assert!(m2(a).transpose().tr_mul(v2t(v)) == v2t(expected));
    }
}
...
fn test_mul_vec_tr_mul_vec_exact() {
    let a = m2i([[1, 2], [3, 4]]);
    assert!(a.mul_mat(v2i(1, 0)) == v2i(1, 3));
    assert!(a.tr_mul(v2i(1, 0)) == v2i(1, 2));
}
