...
//! <oracle.cairo>`.
//!
//! Moved from `crates/nalgebra/src/base/vector4/tests.cairo` (WP 8.1c, test-only package).

use fixed::Fixed;
use nalgebra::base::vector4::{Vector4, Vector4AngleTrait, Vector4Trait};
use nalgebra_testing::black_box;
use nalgebra_tests_utils::{fx, v2, v3, v4, v4t};
use simba::scalar::Real;
use crate::vector4::oracle;
...
fn test_component_div_exact() {
    ...
    assert!(a().component_div(b()) == v4(-1431655765, -0x900000000, 0x1e0000000, -0x10000000));
...
fn test_abs_diff_eq_counts_ulps() {
    ...
    assert!(
        v4(MAX, MAX, MAX, MAX)
            .abs_diff_eq(v4(MAX, MAX, MAX, MAX).scale(Real::zero()), 0xffffffffffffffff),
    );
}
...
fn test_try_normalize_none() {
    ...
    assert!(
        v4(3, -4, 0, 0).try_normalize(Real::default_epsilon()) == Some(v4(3, -4, 0, 0).normalize()),
    );
...
fn test_angle_oracle() {
    ...
    while let Some(case) = cases.pop_front() {
        ...
        assert!(Real::abs_diff_eq(v4t(a).angle(v4t(b)), fx(expected), tol));
    }
}
...
fn test_add_oracle() {
    ...
    while let Some(case) = cases.pop_front() {
        let (a, b, expected, tol) = *case;
        let (a, b) = (v4t(a), v4t(b));
        assert!((a + b).abs_diff_eq(v4t(expected), tol));
    }
}
...
fn test_neg_oracle() {
    ...
    while let Some(case) = cases.pop_front() {
        ...
        let a = v4t(a);
        assert!((-a).abs_diff_eq(v4t(expected), tol));
    }
}
...
fn test_scale_oracle() {
    ...
    while let Some(case) = cases.pop_front() {
        ...
        let a = v4t(a);
        assert!((a.scale(fx(k))).abs_diff_eq(v4t(expected), tol));
    }
}
...
fn test_normalize_oracle() {
    ...
    while let Some(case) = cases.pop_front() {
        let (a, expected, tol) = *case;
        let a = v4t(a);
        assert!((a.normalize()).abs_diff_eq(v4t(expected), tol));
    }
}
...
fn test_lerp_oracle() {
    ...
    while let Some(case) = cases.pop_front() {
        ...
        let (a, b) = (v4t(a), v4t(b));
        assert!((a.lerp(b, fx(t))).abs_diff_eq(v4t(expected), tol));
    }
}
...
fn test_component_mul_oracle() {
    ...
    while let Some(case) = cases.pop_front() {
        let (a, b, expected, tol) = *case;
        let (a, b) = (v4t(a), v4t(b));
        assert!((a.component_mul(b)).abs_diff_eq(v4t(expected), tol));
    }
}
...
fn test_component_div_oracle() {
    ...
    while let Some(case) = cases.pop_front() {
        let (a, b, expected, tol) = *case;
        let (a, b) = (v4t(a), v4t(b));
        assert!((a.component_div(b)).abs_diff_eq(v4t(expected), tol));
    }
}
...
fn test_abs_oracle() {
    ...
    while let Some(case) = cases.pop_front() {
        let (a, expected, tol) = *case;
        let a = v4t(a);
        assert!((a.abs()).abs_diff_eq(v4t(expected), tol));
    }
}
...
fn test_inf_oracle() {
    ...
    while let Some(case) = cases.pop_front() {
        let (a, b, expected, tol) = *case;
        let (a, b) = (v4t(a), v4t(b));
        assert!((a.inf(b)).abs_diff_eq(v4t(expected), tol));
    }
}
...
fn test_sup_oracle() {
    ...
    while let Some(case) = cases.pop_front() {
        let (a, b, expected, tol) = *case;
        let (a, b) = (v4t(a), v4t(b));
        assert!((a.sup(b)).abs_diff_eq(v4t(expected), tol));
    }
}
