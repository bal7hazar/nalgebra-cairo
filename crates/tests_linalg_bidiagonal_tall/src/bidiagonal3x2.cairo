...
//! `Bidiagonal3x2` / `Matrix3x2BidiagonalTrait` through the public API (WP 8.5-P16): oracle vectors
...
use fixed::Fixed;
use nalgebra::linalg::{
    Bidiagonal3x2Trait, Matrix3x2BidiagonalTrait, clear_column_unchecked, clear_row_unchecked,
};
use nalgebra::{Matrix1, Matrix3x2, MatrixMul, Vector2, Vector3};
use nalgebra_testing::black_box;
use nalgebra_tests_utils::{abs_raw, excess, fx, oracle_tol, ulp_diff};
use crate::builders::{amax_3x2, mat2x2, mat3x2, max_ulp_3x2, orth_2x2, orth_3x2};
use crate::oracle_schur as oracle;

/// `bidiagonal3x2` (oracle): `u`, `d` and `v_t` entry by entry within the oracle tolerance
...
#[test]
fn test_oracle_bidiagonal3x2() {
    let mut cases = oracle::bidiagonal3x2_cases();
    ...
    while let Some(case) = cases.pop_front() {
        ...
        let (eu, ed, evt) = (mat3x2(eu), mat2x2(ed), mat2x2(evt));
        ...
        rec = max(rec, max_ulp_3x2(u.mul_mat(d).mul_mat(vt), a) / amax_3x2(a));
        orth = max(orth, max(orth_3x2(u), orth_2x2(vt)));
        let mut m = a;
    ...
    assert!(rec <= 5 && orth <= 10, "measured {} {}", rec, orth);
}

#[test]
#[inline(never)]
fn bench_bidiagonal3x2_new__baseline() {
    let (a, _, _, _, _) = *oracle::bidiagonal3x2_cases().at(3);
    let a = black_box(mat3x2(a));
...
#[test]
#[inline(never)]
fn bench_bidiagonal3x2_new__householder() {
    let (a, _, _, _, _) = *oracle::bidiagonal3x2_cases().at(3);
    let a = black_box(mat3x2(a));
    let e = black_box(true);
    let bd = a.bidiagonalize();
...
#[test]
#[inline(never)]
fn bench_bidiagonal3x2_unpack__baseline() {
    let (a, _, _, _, _) = *oracle::bidiagonal3x2_cases().at(3);
    let a = black_box(black_box(mat3x2(a)).bidiagonalize());
    let e = black_box(true);
    let _ = a;
    assert!(e == e);
}
...
#[test]
#[inline(never)]
fn bench_bidiagonal3x2_unpack__symbolic() {
    let (a, _, _, _, _) = *oracle::bidiagonal3x2_cases().at(3);
    let a = black_box(black_box(mat3x2(a)).bidiagonalize());
    let e = black_box(true);
    let (u, _, vt) = a.unpack();
