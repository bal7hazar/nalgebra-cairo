...
//! `Bidiagonal5x2` / `Matrix5x2BidiagonalTrait` through the public API (WP 8.5-P16): oracle vectors
...
use fixed::Fixed;
use nalgebra::linalg::{
    Bidiagonal5x2Trait, Matrix5x2BidiagonalTrait, clear_column_unchecked, clear_row_unchecked,
};
use nalgebra::{Matrix1, Matrix5x2, MatrixMul, Vector2, Vector5};
use nalgebra_testing::black_box;
use nalgebra_tests_utils::{abs_raw, excess, fx, oracle_tol, ulp_diff};
use crate::builders::{amax_5x2, mat2x2, mat5x2, max_ulp_5x2, orth_2x2, orth_5x2};
use crate::oracle_schur as oracle;

/// `bidiagonal5x2` (oracle): `u`, `d` and `v_t` entry by entry within the oracle tolerance
...
#[test]
fn test_oracle_bidiagonal5x2() {
    let mut cases = oracle::bidiagonal5x2_cases();
    ...
    while let Some(case) = cases.pop_front() {
        ...
        let (eu, ed, evt) = (mat5x2(eu), mat2x2(ed), mat2x2(evt));
        ...
        rec = max(rec, max_ulp_5x2(u.mul_mat(d).mul_mat(vt), a) / amax_5x2(a));
        orth = max(orth, max(orth_5x2(u), orth_2x2(vt)));
        let mut m = a;
...
#[test]
#[inline(never)]
fn bench_bidiagonal5x2_new__baseline() {
    let (a, _, _, _, _) = *oracle::bidiagonal5x2_cases().at(3);
    let a = black_box(mat5x2(a));
...
#[test]
#[inline(never)]
fn bench_bidiagonal5x2_new__householder() {
    let (a, _, _, _, _) = *oracle::bidiagonal5x2_cases().at(3);
    let a = black_box(mat5x2(a));
    let e = black_box(true);
    let bd = a.bidiagonalize();
...
#[test]
#[inline(never)]
fn bench_bidiagonal5x2_unpack__baseline() {
    let (a, _, _, _, _) = *oracle::bidiagonal5x2_cases().at(3);
    let a = black_box(black_box(mat5x2(a)).bidiagonalize());
    let e = black_box(true);
    let _ = a;
    assert!(e == e);
}
...
#[test]
#[inline(never)]
fn bench_bidiagonal5x2_unpack__symbolic() {
    let (a, _, _, _, _) = *oracle::bidiagonal5x2_cases().at(3);
    let a = black_box(black_box(mat5x2(a)).bidiagonalize());
    let e = black_box(true);
    let (u, _, vt) = a.unpack();
