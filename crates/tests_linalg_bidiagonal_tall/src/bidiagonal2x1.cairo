...
//! `Bidiagonal2x1` / `Vector2BidiagonalTrait` through the public API (WP 8.5-P16): oracle vectors
...
use fixed::Fixed;
use nalgebra::linalg::{
    Bidiagonal2x1Trait, Vector2BidiagonalTrait, clear_column_unchecked, clear_row_unchecked,
};
use nalgebra::{Matrix1, MatrixMul, Vector2};
use nalgebra_testing::black_box;
use nalgebra_tests_utils::{abs_raw, excess, fx, oracle_tol, ulp_diff};
use crate::builders::{amax_2x1, mat1x1, mat2x1, max_ulp_2x1, orth_1x1, orth_2x1};
use crate::oracle_schur as oracle;

/// `bidiagonal2x1` (oracle): `u`, `d` and `v_t` entry by entry within the oracle tolerance
...
#[test]
fn test_oracle_bidiagonal2x1() {
    let mut cases = oracle::bidiagonal2x1_cases();
    ...
    while let Some(case) = cases.pop_front() {
        ...
        let (eu, ed, evt) = (mat2x1(eu), mat1x1(ed), mat1x1(evt));
        ...
        rec = max(rec, max_ulp_2x1(u.mul_mat(d).mul_mat(vt), a) / amax_2x1(a));
        orth = max(orth, max(orth_2x1(u), orth_1x1(vt)));
        let mut m = a;
    ...
    assert!(rec <= 3 && orth <= 6, "measured {} {}", rec, orth);
}

#[test]
#[inline(never)]
fn bench_bidiagonal2x1_new__baseline() {
    let (a, _, _, _, _) = *oracle::bidiagonal2x1_cases().at(3);
    let a = black_box(mat2x1(a));
...
#[test]
#[inline(never)]
fn bench_bidiagonal2x1_new__householder() {
    let (a, _, _, _, _) = *oracle::bidiagonal2x1_cases().at(3);
    let a = black_box(mat2x1(a));
    let e = black_box(true);
    let bd = a.bidiagonalize();
...
#[test]
#[inline(never)]
fn bench_bidiagonal2x1_unpack__baseline() {
    let (a, _, _, _, _) = *oracle::bidiagonal2x1_cases().at(3);
    let a = black_box(black_box(mat2x1(a)).bidiagonalize());
    let e = black_box(true);
    let _ = a;
    assert!(e == e);
}
...
#[test]
#[inline(never)]
fn bench_bidiagonal2x1_unpack__symbolic() {
    let (a, _, _, _, _) = *oracle::bidiagonal2x1_cases().at(3);
    let a = black_box(black_box(mat2x1(a)).bidiagonalize());
    let e = black_box(true);
    let (u, _, vt) = a.unpack();
