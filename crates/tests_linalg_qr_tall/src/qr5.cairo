...
//! `Qr5` / `Matrix5QrTrait` through the public API (WP 8.5-P14b): oracle vectors (`tools/oracle`
...
/// `qr5_q_r` (oracle): `q` and `r` entry by entry within the oracle tolerance (upstream's
/// unpacked factors: `diag(r) >= 0`), `q r = A` and the orthonormality of `q` within the
/// measured bounds.
#[test]
fn test_oracle_qr5_q_r() {
    let mut cases = oracle::qr5_q_r_cases();
    ...
    while let Some(case) = cases.pop_front() {
        ...
        rec = max(rec, max_ulp_5x5(q.mul_mat(r), a) / amax_5x5(a));
        orth = max(orth, orth_5x5(q));
    }
    assert!(ex == 0, "oracle tolerance exceeded by {}", ex);
    assert!(rec <= 4 && orth <= 22, "measured {} {}", rec, orth);
}

#[test]
fn test_qr5_q_tr_mul() {
    let (a, _, _, _) = *oracle::qr5_q_r_cases().at(0);
    let f = black_box(mat5x5(a)).qr();
    let mut b = Vector5 {
...
/// `qr5_solve` (oracle): every component within the oracle tolerance; `solve_mut` agrees.
#[test]
fn test_oracle_qr5_solve() {
    let mut cases = oracle::qr5_solve_cases();
    let mut ex = 0;
    while let Some(case) = cases.pop_front() {
        let (a, b, e, tol) = *case;
        let f = black_box(mat5x5(a)).qr();
        ...
        ex = max(ex, excess(max_ulp_5x1(y, x), 2));
...
fn test_qr5_singular() {
    ...
    assert!(
        f.solve(Vector5 { x: fx(ONE), y: fx(ONE), z: fx(ONE), w: fx(ONE), a: fx(ONE) }).is_none(),
    );
}

#[test]
#[inline(never)]
fn bench_qr5_new__baseline() {
    let (a, _, _, _) = *oracle::qr5_q_r_cases().at(3);
    let _a = black_box(mat5x5(a));
...
#[test]
#[inline(never)]
fn bench_qr5_new__mgs() {
    let (a, _, _, _) = *oracle::qr5_q_r_cases().at(3);
    let a = black_box(mat5x5(a));
    let e = black_box(true);
    let f = Qr5Trait::new(a);
